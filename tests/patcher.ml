let set_debug_level n sections =
  let debug_sections =
    List.fold_left
      (fun map elt -> OpamCoreConfig.StringMap.add elt None map)
      OpamCoreConfig.StringMap.empty sections
  in
...
let write_file ~dir ~name use_crlf ?(eol_at_eof = true) pattern =
  ...
  List.fold_left
    (fun a (n, s) ->
      for i = a to a + n - 1 do
        Printf.fprintf ch "Line %d%s" i eol
      done;
      a + n + s)
    1 pattern
  |> ignore;
  output_string ch "End of file";
  if eol_at_eof then output_string ch eol;
  close_out ch
...
let touch ~dir name = close_out (open_out_bin (Filename.concat dir name))
...
let pattern1 ?(test1 = false) ?(test2 = false) ?(test3 = false) ?(test4 = false)
    ?(eoleof_cr = false) dir =
...
let pattern2 dir =
  ...
  write_file ~dir ~name:"no-eol-at-eof" false ~eol_at_eof:false
    [ (3, 1); (1, 0) ];
...
let print_directory dir =
  List.iter
    (fun fn ->
      Printf.eprintf "  %s: %a\n%!" (OpamSystem.back_to_forward fn) eol_style fn)
    (OpamSystem.ls dir)

let generate_patch () =
  flush stderr;
  flush stdout;
  if Sys.command "diff -Naur a b > input.patch" <> 1 then (
    Printf.eprintf "patch generation failed\n%!";
    exit 2 );
  ...
  print_directory ".";
  flush stdout;
  if Sys.command "patch -p1 -i ../output.patch" <> 0 then (
    Printf.eprintf "patch application failed\n%!";
    exit 2 );
