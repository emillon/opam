...
let tool_arch = if Config.system = "mingw64" then "x86_64" else "i686"

let runtime =
  let c =
    Unix.open_process_in
    @@ Printf.sprintf "%s-w64-mingw32-gcc -print-sysroot | cygpath -f - -w"
         tool_arch
  in
  let sysroot = input_line c in
  if Unix.close_process_in c <> Unix.WEXITED 0 then exit 1
  else Filename.concat sysroot @@ Filename.concat "mingw" "bin"
...
let link_item item =
  ...
  if not (Sys.file_exists runtime) then (
    ...
    exit 1 );
  if Sys.file_exists item then Unix.unlink item;
  try Unix.symlink runtime item
  with Unix.Unix_error _ -> copy_file runtime item
