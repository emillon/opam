...
let env ~with_test ~with_doc ~dev nv v =
  match
    (OpamVariable.Full.scope v, OpamVariable.(to_string (Full.variable v)))
  with
  ...
  | OpamVariable.Full.Global, "with-test" -> Some (B with_test)
  | OpamVariable.Full.Global, "dev" -> Some (B dev)
  | OpamVariable.Full.Global, "with-doc" -> Some (B with_doc)
  | _ -> None
  ...
  let packages = OpamPackage.keys opams in
  {
    ...
    u_depends =
      OpamPackage.Map.mapi
        (fun nv o ->
          OpamFile.OPAM.depends o |> OpamFilter.partial_filter_formula (env nv))
        opams;
    u_depopts =
      OpamPackage.Map.mapi
        (fun nv o ->
          OpamFile.OPAM.depopts o |> OpamFilter.partial_filter_formula (env nv))
        opams;
    u_conflicts =
      OpamPackage.Map.mapi
        (fun nv o ->
...
let installability_check univ =
  ...
  let graph =
    ...
    @@ OpamSolver.load_cudf_universe ~depopts:false ~build:true ~post:true univ
         packages ()
  in
  let filter_roots g packages =
    let has_pkg p = OpamPackage.Set.mem (OpamCudf.cudf2opam p) packages in
    OpamCudf.Graph.fold_vertex
      (fun p acc ->
        if has_pkg p && not (List.exists has_pkg (OpamCudf.Graph.succ g p)) then
      ...
      Atom
        ( name,
          OpamFormula.formula_of_version_set
            ...
            (OpamPackage.versions_of_packages (OpamPackage.Set.of_list nvs)) )

let cycle_check univ =
  let cudf_univ =
    OpamSolver.load_cudf_universe ~depopts:true ~build:true ~post:false univ
      univ.u_packages ()
  in
  let graph = OpamCudf.Graph.of_universe cudf_univ |> OpamCudf.Graph.mirror in
  (* conflicts break cycles *)
  let conflicts = Algo.Defaultgraphs.PackageGraph.conflict_graph cudf_univ in
  ...
  let node_map, cy =
    List.fold_left
      (fun (node_map, acc) pkgs ->
        let univ = Cudf.load_universe pkgs in
        let g = OpamCudf.Graph.of_universe univ in
        let conflicts = Algo.Defaultgraphs.PackageGraph.conflict_graph univ in
        ...
        let node_map =
          Cudf.fold_packages_by_name
            (fun node_map _ pkgs ->
              let id p =
                let f pl =
                  List.sort compare @@ List.map (Cudf.uid_by_package univ) pl
                in
              ...
              let ids =
                List.fold_left
                  (fun acc p -> OpamCudf.Map.add p (id p) acc)
  ...
  let rec has_conflict = function
    | [] | [ _ ] -> false
    | p :: r -> List.exists (CGraph.mem_edge conflicts p) r || has_conflict r
  in
  let cy = List.rev cy |> List.filter (fun c -> not (has_conflict c)) in
  let cycle_packages =
    List.fold_left
      (List.fold_left (fun acc p ->
           List.fold_left
             (fun acc p -> OpamPackage.Set.add (OpamCudf.cudf2opam p) acc)
             acc
             (OpamCudf.Map.find p node_map)))
...
let print_cycles cy =
  let arrow =
    ...
    @@ if OpamConsole.utf8 () then " \xe2\x86\x92 " (* U+2192 *) else " -> "
  in
...
let pkg_deps univ package =
  let deps =
    try
      OpamFilter.filter_deps ~build:true ~post:true ~default:true
  ...
  let product ss1 ss2 =
    PkgSetSet.fold
      (fun s1 -> PkgSetSet.union (PkgSetSet.map (PkgSet.union s1) ss2))
      ss1 PkgSetSet.empty
  in
  let depsets =
    (* PkgSetSet-encoded CNF *)
    match
...
let aggregate packages deps revdeps =
  if OpamStd.Config.env_bool "NOAGGREGATE" = Some true then
    PkgSet.fold
      (fun nv -> PkgSetSet.add (PkgSet.singleton nv))
      packages PkgSetSet.empty
  else
    let friends p (deps, revdeps) =
      (* dependencies which have a 1-1 version relationship *)
      try
        PkgMap.find p deps |> OpamPackage.to_map
        |> OpamPackage.Name.Map.filter (fun _ vs ->
               OpamPackage.Version.Set.is_singleton vs)
        |> OpamPackage.of_map
        |> PkgSet.filter (fun d ->
    ...
    let rec all_friends acc p =
      ...
      PkgSet.fold
        (fun p acc -> all_friends acc p)
        (friends p (deps, revdeps) ++ friends p (revdeps, deps) -- acc)
        acc
    in
    let rec aux acc packages =
      if PkgSet.is_empty packages then acc
      else
...
let get_obsolete univ opams =
  let deps_map =
    (* pkg -> setset-encoded CNF *)
    PkgSet.fold
      (fun p -> PkgMap.add p (pkg_deps univ p))
      univ.u_packages PkgMap.empty
  in
  let simple_deps =
    (* pkg -> set *)
    PkgMap.map
      (fun deps -> PkgSetSet.fold PkgSet.union deps PkgSet.empty)
      deps_map
  in
  let revdeps_map =
    (* pkg -> set *)
    PkgMap.fold
      (fun pkg ->
        PkgSet.fold (fun d -> PkgMap.update d (PkgSet.add pkg) PkgSet.empty))
      simple_deps PkgMap.empty
  in
  let aggregates = aggregate univ.u_packages simple_deps revdeps_map in
  let aggregate_deps pkgs =
    PkgSet.fold
      (fun pkg -> PkgSetSet.union (PkgMap.find pkg deps_map))
  ...
  let aggregate_nextv pkgs =
    ...
    PkgSet.map
      (fun p ->
        match PkgSet.split p ps with
        | _, true, s1 ->
            let next = PkgSet.min_elt s1 in
            if next.name = p.name then next else raise Not_found
        | _ -> raise Not_found)
      pkgs
  in
  PkgSetSet.fold
        ...
        @@ PkgSet.exists
             (fun p ->
               OpamFile.OPAM.has_flag Pkgflag_Compiler
                 (OpamPackage.Map.find p opams))
             pkgs
        &&
        try
          let next = aggregate_nextv pkgs in
          more_restrictive_deps_than (aggregate_deps pkgs) (aggregate_deps next)
          &&
...
let check ~quiet ~installability ~cycles ~obsolete ~ignore_test repo_root =
    ...
    OpamPackage.Map.fold
      (fun nv prefix acc ->
        ...
        | None ->
            ...
            acc)
      pkg_prefixes OpamPackage.Map.empty
  in
  let univ =
    get_universe ~with_test:(not ignore_test) ~with_doc:(not ignore_test)
      ~dev:false opams
  in

  (* Installability check *)
  let unav_roots, uninstallable =
    if not installability then (PkgSet.empty, PkgSet.empty)
    else (
      if not quiet then
        OpamConsole.msg
          "Checking installability of every package. This may take a few \
           minutes...\n";
    ...
    if not (PkgSet.is_empty uninstallable) then
      OpamConsole.error "These packages are not installable (%d):\n%s%s"
        ...
        (let unav_others = uninstallable -- unav_roots in
         if PkgSet.is_empty unav_others then ""
         else
  ...
  let cycle_packages, cycle_formulas =
    if not cycles then (PkgSet.empty, []) else cycle_check univ
  in
         ...
         (fun (n, vs) ->
           Printf.sprintf "%s %s"
             ...
             (OpamStd.List.concat_map ", "
                (fun v ->
                  OpamConsole.colorise `magenta
                    (OpamPackage.Version.to_string v))
                (OpamPackage.Version.Set.elements vs)))
         (OpamPackage.Name.Map.bindings (OpamPackage.to_map obsolete_packages)))
    );
