...
let string_of_selector =
  ...
  | Atoms atoms ->
      OpamStd.List.concat_map ~left:"(" ~right:")" " | "
        (fun a -> OpamFormula.short_string_of_atom a % `bold)
        atoms
  | Flag fl ->
      Printf.sprintf "%s(%s)"
        ("has-flag" % `green)
        (OpamTypesBase.string_of_pkg_flag fl % `bold)
  | Tag t -> Printf.sprintf "%s(%s)" ("has-tag" % `green) (t % `bold)
  | From_repository r ->
      Printf.sprintf "%s(%s)"
        ("from-repository" % `magenta)
...
let string_of_formula = OpamFormula.string_of_formula string_of_selector

let get_opam st nv =
  match OpamSwitchState.opam_opt st nv with
  | Some o ->
      OpamFile.OPAM.(
        with_name nv.OpamPackage.name (with_version nv.OpamPackage.version o))
...
let package_dependencies st tog nv =
  get_opam st nv
  |> OpamPackageVar.all_depends ~build:tog.build ~post:tog.post ~test:tog.test
       ~doc:tog.doc ~dev:tog.dev ~depopts:tog.depopts st

let atom_dependencies st tog atoms =
  atoms |> OpamFormula.packages_of_atoms (st.packages ++ st.installed)
  |> fun pkgs ->
  OpamPackage.Set.fold
    (fun nv acc -> OpamFormula.ors [ acc; package_dependencies st tog nv ])
...
let get_universe st ?requested tog =
  ...
  in
  OpamSwitchState.universe st ~test:tog.test ~doc:tog.doc
    ~force_dev_deps:tog.dev ~requested Query
...
let rec value_strings value =
  ...
  | Relop (_, _, v1, v2) | Logop (_, _, v1, v2) | Env_binding (_, v1, _, v2) ->
      SS.union (value_strings v1) (value_strings v2)
  | Prefix_relop (_, _, v) | Pfxop (_, _, v) -> value_strings v
  | List (_, l) | Group (_, l) ->
      List.fold_left (fun acc v -> SS.union acc (value_strings v)) SS.empty l
  | Option (_, v, vl) ->
      List.fold_left
        (fun acc v -> SS.union acc (value_strings v))
        (value_strings v) vl
  ...
  OpamFormula.ors
    (List.map
       (fun patt ->
         ...
         | None -> Atom (Pattern (name_patt, patt))
         | Some (name, version) ->
             OpamFormula.ands
               [
...
let apply_selector ~base st = function
  ...
  | ( Required_by (({ recursive = true; _ } as tog), atoms)
    | Depends_on (({ recursive = true; _ } as tog), atoms) ) as direction ->
      ...
      deps_fun ~depopts:tog.depopts ~build:tog.build ~post:tog.post
        ~installed:false ~unavailable:true (get_universe st tog)
        (packages_of_atoms st atoms)
      |> OpamPackage.Set.of_list
  | Required_by (tog, atoms) ->
      atom_dependencies st tog atoms |> OpamFormula.packages base
  | Depends_on (tog, atoms) ->
      ...
      OpamPackage.Set.filter
        ...
        base
  | Conflicts_with packages ->
      OpamSwitchState.conflicts_with st
  ...
  | Solution (tog, atoms) -> (
      ...
      match
        OpamSolver.resolve universe ~orphans:OpamPackage.Set.empty
          (OpamSolver.request ~install:atoms ())
      with
      | Success s -> OpamSolver.new_packages s
      | Conflicts cs ->
          OpamConsole.error_and_exit `No_solution "No solution%s for %s: %s"
  ...
  | Pattern (psel, pat) ->
      let re =
        if psel.glob then Re.Glob.glob ~expand_braces:true pat else Re.str pat
      in
      ...
      let content_strings nv =
        ...
        if psel.fields = [] then
          ...
          try
            List.map
              (fun f ->
                match OpamFile.OPAM.print_field_as_syntax f opam with
      ...
      OpamPackage.Set.filter
        (fun nv ->
          List.exists
            (OpamStd.String.Set.exists (Re.execp re))
            (content_strings nv))
        base
  | Atoms atoms -> OpamFormula.packages_of_atoms base atoms
  | Flag f ->
      OpamPackage.Set.filter
        (fun nv -> get_opam st nv |> OpamFile.OPAM.has_flag f)
        base
  | Tag t ->
      OpamPackage.Set.filter
        (fun nv -> get_opam st nv |> List.mem t @* OpamFile.OPAM.tags)
        base
          ...
          List.filter
            (fun change_f ->
              ...
              OpamStd.String.Map.exists
                (fun f -> function OpamDirTrack.Removed -> false
                  | _ -> rel_name = f)
                changes)
...
let version_color st nv =
  ...
  in
  let is_available nv =
    (* Ignore unavailability due to pinning *)
    try
  ...
  in
  if OpamPackage.Set.mem nv st.installed then [ `bold; `magenta ]
  else
...
let mini_field_printer ?(prettify = false) ?(normalise = false) =
  if normalise then OpamPrinter.Normalise.value
  else function
    | String (_, s) when prettify -> s
    | List (_, l)
      when prettify && List.for_all (function String _ -> true | _ -> false) l
      ->
        OpamStd.List.concat_map ", "
          (function String (_, s) -> s | _ -> assert false)
          l
...
let detail_printer ?prettify ?normalise st nv =
  ...
  let root_sty =
    if OpamPackage.Set.mem nv st.installed_roots then [ `underline ] else []
  in
  function
  ...
  | Synopsis ->
      (get_opam st nv |> OpamFile.OPAM.descr >>| OpamFile.Descr.synopsis) +! ""
  | Synopsis_or_target -> (
      ...
      | None ->
          ...
          +! "" )
  | Description ->
      (get_opam st nv |> OpamFile.OPAM.descr >>| OpamFile.Descr.body) +! ""
  | Raw_field f | Field f -> (
      try
  ...
  | Installed_version -> (
      try
        OpamPackage.package_of_name st.installed nv.name |> fun inst_nv ->
        OpamPackage.version_to_string inst_nv |> fun s ->
        if OpamPackage.Set.mem inst_nv st.pinned then s % [ `blue ]
        else if OpamPackage.has_name st.pinned nv.name then s % [ `bold; `red ]
        else if
          nv <> inst_nv
  ...
  | Source_hash ->
      let hash_opt =
        ...
        OpamSwitchState.source_dir st nv |> OpamFilename.opt_dir
        >>= fun srcdir ->
  ...
  | All_installed_versions ->
      ...
      |> OpamPackage.Map.values |> String.concat "  "
  | Available_versions ->
      let available =
...
let display st format packages =
  let packages =
    if format.all_versions then packages
    else
      OpamPackage.Name.Set.fold
        (fun name ->
          ...
          let nv =
            ...
            try get (pkgs %% st.installed)
            with Not_found -> (
              try get (pkgs %% st.pinned)
              with Not_found -> (
                try get (pkgs %% Lazy.force st.available_packages)
                with Not_found -> get pkgs ) )
          in
  ...
  let packages =
    if format.order = `Dependency then
      ...
      in
      let deps_packages =
        OpamSolver.dependencies ~depopts:true ~installed:false ~unavailable:true
          ~build:true ~post:false universe packages
      in
      List.filter (fun nv -> OpamPackage.Set.mem nv packages) deps_packages
      |> List.rev
    else
      match format.order with
      | `Custom o -> List.sort o (OpamPackage.Set.elements packages)
      | _ -> OpamPackage.Set.elements packages
  in
  let add_head l =
    if format.header then
      List.map (fun f -> "# " ^ disp_header f) format.columns :: l
    else l
  in
  ...
  if packages = [] then (
    ...
    List.rev_map
      ...
      packages
    |> List.rev |> add_head |> OpamStd.Format.align_table
...
let get_depexts st packages =
  OpamPackage.Name.Set.fold
    (fun name acc ->
      ...
      let nv =
        if OpamPackage.Set.mem nv packages then nv
        else
          OpamPackage.Set.max_elt (OpamPackage.packages_of_name packages name)
      in
      OpamStd.String.Set.union acc (OpamSwitchState.depexts st nv))
    (OpamPackage.names_of_packages packages)
    OpamStd.String.Set.empty

let print_depexts = OpamStd.String.Set.iter (OpamConsole.msg "%s\n")
...
let info st ~fields ~raw ~where ?normalise ?(show_empty = false)
  ...
  let all_versions_fields = [ Name; All_installed_versions; All_versions ] in
  ...
  let output_table fields nv =
    let tbl =
      List.fold_left
        ...
        [] (List.rev fields)
    in
    let cut = match normalise with Some true -> Some `None | _ -> None in
    ...
    if where then
      OpamConsole.msg "%s\n"
        ...
        | Some (Some repo, rdir) ->
            ...
            if OpamFilename.exists tar && not (OpamFilename.exists_dir repo_dir)
            then
              Printf.sprintf "<%s>%s%s"
                (OpamFilename.to_string tar)
                Filename.dir_sep rdir
            else OpamFilename.Dir.to_string OpamFilename.Op.(repo_dir / rdir)
        | None -> "<nowhere>" )
    else if raw && fields = [] then OpamFile.OPAM.write_to_channel stdout opam
    else
      match fields with
      | [] ->
  ...
  |> OpamPackage.Name.Set.iter (fun name ->
         ...
         if all_versions then (
           ...
           let choose =
             try OpamPackage.Set.choose (nvs %% st.pinned)
             with Not_found -> (
               try OpamPackage.Set.choose (nvs %% st.installed)
               with Not_found -> (
                 try
                   OpamPackage.Set.max_elt
                     (nvs %% Lazy.force st.available_packages)
                 with Not_found -> OpamPackage.Set.max_elt nvs ) )
           in
           header choose;
