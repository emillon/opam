...
let switch_to_updated_self debug opamroot =
  ...
  if
    updated_self_str <> Sys.executable_name
  ...
  then
    ...
    if update_version = no_version then
      ...
      OpamConsole.warning
        "Obsolete opam self-upgrade package v.%s found, not using it (current \
         system version is %s)."
      ...
      if OpamVersion.git () <> None then
        OpamConsole.warning
          "Using opam self-upgrade to %s while the system opam is a \
      ...
      with e ->
        OpamStd.Exn.fatal e;
        OpamConsole.error
          "Couldn't run the upgraded opam %s found at %s. Continuing with %s \
           from the system."
          (OpamVersion.to_string update_version)
          updated_self_str
          (OpamVersion.to_string OpamVersion.current) )
...
let global_options =
    ...
    mk_flag ~section:global_option_section [ "no-self-upgrade" ]
      ( Printf.sprintf
          "Opam will replace itself with a newer binary found at \
           $(b,OPAMROOT%sopam) if present. This disables this behaviour."
          Filename.dir_sep
  ...
  let self_upgrade no_self_upgrade options =
    let self_upgrade_status =
      if
        OpamStd.Config.env_string "NOSELFUPGRADE"
...
let apply_global_options (options, self_upgrade) =
  ...
  try
    ...
    if
      self_upgrade <> `Running
      && OpamFilename.starts_with OpamStateConfig.(!r.root_dir) argv0
      && not !OpamCoreConfig.r.OpamCoreConfig.safe_mode
    then
      OpamConsole.warning
        "You should not be running opam from within %s. Copying %s to your \
         PATH first is advised."
...
let get_init_config ~no_sandboxing ~no_default_config_file ~add_config_file =
    ...
    @ List.map
        (fun url ->
          ...
          | None ->
              ...
              let hash =
                OpamHash.compute ~kind:`SHA256 (OpamFilename.to_string f)
              in
              if
                OpamConsole.confirm
                  "Using configuration file from %s. Please verify the \
                   following SHA256:\n\
                  \    %s\n\
                   Is this correct?" (OpamUrl.to_string url)
  ...
  try
    ...
    List.fold_left
      (fun acc f -> OpamFile.InitConfig.add acc (OpamFile.InitConfig.read f))
      builtin_config config_files
  with e ->
    OpamConsole.error
      "Error in configuration file, fix it, use '--no-opamrc', or check your \
...
let init =
  ...
  let man =
    [
      ...
      `P
        ( Printf.sprintf
            "The $(b,init) command initialises a local \"opam root\" (by \
             default, $(i,~%s.opam%s)) that holds opam's data and packages. \
             This is a necessary step for normal operation of opam. The \
             initial software repositories are fetched, and an initial \
             'switch' can also be installed, according to the configuration \
             and options. These can be afterwards configured using $(b,opam \
             switch) and $(b,opam repository)."
      ...
      `P
        "Additionally, this command allows one to customise some aspects of \
         opam's shell integration, when run initially (avoiding the \
         interactive dialog), but also at any later time.";
      `S "ARGUMENTS";
      `S "OPTIONS";
      `S "CONFIGURATION FILE";
      `P
        ( Printf.sprintf
            "Any field from the built-in initial configuration can be \
             overridden through $(i,~%s.opamrc), $(i,/etc/opamrc), or a file \
             supplied with $(i,--config). The default configuration for this \
             version of opam can be obtained using $(b,--show-default-opamrc)."
            Filename.dir_sep
        |> Cmdliner.Manpage.escape );
  ...
  let repo_name =
    let doc =
      Arg.info [] ~docv:"NAME"
        ~doc:"Name of the initial repository, when creating a new opam root."
    in
    Arg.(
      value & pos ~rev:true 1 repository_name OpamRepositoryName.default & doc)
  in
  let repo_url =
    let doc =
      Arg.info [] ~docv:"ADDRESS"
        ~doc:
      ...
      & vflag None
          [
            ( Some false,
              info [ "a"; "auto-setup" ]
                ~doc:
                  "Automatically do a full setup, including adding a line to \
                   your shell init files." );
            ( Some true,
              info [ "i"; "interactive" ]
                ~doc:
                  "Run the setup interactively (this is the default for an \
                   initial run, or when no more specific options are \
                   specified)" );
          ])
  in
  let update_config =
              ...
              info [ "shell-setup" ]
                ~doc:
                  "Automatically setup the user shell configuration for opam, \
                   e.g. adding a line to the `~/.profile' file." );
            ( Some false,
              info [ "n"; "no-setup" ]
                ~doc:
                  "Do not update the user shell configuration to setup opam. \
                   Also implies $(b,--disable-shell-hook), unless \
                   $(b,--interactive) or specified otherwise" );
          ])
  in
  let setup_completion =
    Arg.(
      ...
      & vflag None
          [
            ( Some true,
              info [ "enable-completion" ]
                ~doc:
                  "Setup shell completion in opam init scripts, for supported \
                   shells." );
            ( Some false,
              info [ "disable-completion" ]
                ~doc:"Disable shell completion in opam init scripts." );
          ])
  in
  let env_hook =
    Arg.(
      ...
      & vflag None
          [
            ( Some true,
              info [ "enable-shell-hook" ]
                ~doc:
                  "Setup opam init scripts to register a shell hook that will \
                   automatically keep the shell environment up-to-date at \
                   every prompt." );
            ( Some false,
              info [ "disable-shell-hook" ]
                ~doc:
  ...
  let config_file =
    mk_opt_all [ "config" ] "FILE"
      "Use the given init config file. If repeated, latest has the highest \
       ...
       last). Specifying a URL pointing to a config file instead is allowed."
      OpamArg.url
  in
  let no_config_file =
    mk_flag [ "no-opamrc" ]
      ( Printf.sprintf
          "Don't read `/etc/opamrc' or `~%s.opamrc': use the default settings \
           and the files specified through $(b,--config) only"
          Filename.dir_sep
      |> Cmdliner.Manpage.escape )
  in
  let reinit =
  ...
  in
  let init global_options build_options repo_kind repo_name repo_url interactive
      update_config completion env_hook no_sandboxing shell dot_profile_o
      compiler no_compiler config_file no_config_file reinit show_opamrc
      bypass_checks =
    ...
    let interactive, update_config, completion, env_hook =
      ...
      | None ->
          ( (not already_init)
            ...
            update_config,
            completion,
            OpamStd.Option.Op.(env_hook ++ update_config) )
      | Some true ->
    ...
    in
    let shell =
      match shell with Some s -> s | None -> OpamStd.Sys.guess_shell_compat ()
    in
    let dot_profile =
      match dot_profile_o with
      | Some n -> n
    ...
    if already_init then
      if reinit then
        ...
        in
        OpamClient.reinit ~init_config ~interactive ~dot_profile ?update_config
          ?env_hook ?completion
          (OpamFile.Config.safe_read config_f)
          shell
      else
      ...
      let gt, rt, default_compiler =
        OpamClient.init ~init_config ~interactive ?repo ~bypass_checks
          ~dot_profile ?update_config ?env_hook ?completion shell
      in
      OpamStd.Exn.finally (fun () -> OpamRepositoryState.drop rt) @@ fun () ->
      if no_compiler then ()
      else
        match compiler with
        | Some comp when String.length comp <> 0 ->
            ...
            OpamSwitchCommand.install gt ~rt ~packages ~update_config:true
        ...
        | _ as nocomp -> (
                ...
                Some
                  (List.find
                     (fun atoms ->
                       ...
                       let pkgs =
                         OpamFormula.packages_of_atoms all_packages atoms
                       in
            ...
            | None ->
                OpamConsole.note
                  "No compiler selected, and no available default switch \
                   found: no switch has been created.\n\
                   Use 'opam switch create <compiler>' to get started." )
  in
  ( Term.(
      const init $ global_options $ build_options $ repo_kind_flag $ repo_name
      $ repo_url $ interactive $ update_config $ setup_completion $ env_hook
      $ no_sandboxing $ shell_opt $ dot_profile_flag $ compiler $ no_compiler
      $ config_file $ no_config_file $ reinit $ show_default_opamrc
      $ bypass_checks),
...
let list ?(force_search = false) () =
  ...
  let man =
    [
      ...
      `P
        "Without argument, the command displays the list of currently \
         installed packages. With pattern arguments, lists all available \
         packages matching one of the patterns.";
      `P
        "Unless the $(b,--short) switch is used, the output format displays \
         one package per line, and each line contains the name of the package, \
         the installed version or `--' if the package is not installed, and a \
         short description. In color mode, manually installed packages (as \
         opposed to automatically installed ones because of dependencies) are \
         underlined.";
      `P
        ( "See section $(b," ^ selection_docs
        ^ ") for all the ways to select the packages to be displayed, and \
           section $(b," ^ display_docs ^ ") to customise the output format." );
      `P
        "For a more detailed description of packages, see $(b,opam show). For \
          ...
          [
            ...
            ( OpamListCommand.Root,
              info ~docs
                ...
                ~doc:
                  "List only packages that were explicitly installed, \
                   excluding the ones installed as dependencies" );
            ( OpamListCommand.Available,
              info ~docs [ "a"; "available" ]
                ~doc:
                  "List only packages that are available on the current system"
            );
            ( OpamListCommand.Installable,
              info ~docs [ "installable" ]
                ~doc:
                  "List only packages that can be installed on the current \
                   switch (this calls the solver and may be more costly; a \
                   package depending on an unavailable package may be \
            ...
            ( OpamListCommand.Pinned,
              info ~docs [ "pinned" ] ~doc:"List only the pinned packages" );
          ])
  in
  let search =
    if force_search then Term.const true
    else
  ...
  in
  let list global_options selection state_selector no_switch depexts vars repos
      owns_file disjunction search silent format packages =
    apply_global_options global_options;
    let no_switch = no_switch || OpamStateConfig.get_switch_opt () = None in
    let format =
      let force_all_versions =
        (not search) && state_selector = []
        &&
        match packages with
        | [ single ] -> (
            let nameglob =
            ...
            in
            try
              ignore (OpamPackage.Name.of_string nameglob);
              true
    ...
    in
    let join = if disjunction then OpamFormula.ors else OpamFormula.ands in
    let state_selector =
      if state_selector = [] then
        if no_switch || search || owns_file <> None then Empty
        else if packages = [] && selection = [] then
          Atom OpamListCommand.Installed
        else Or (Atom OpamListCommand.Installed, Atom OpamListCommand.Available)
        ...
        [
          ...
          join
            ...
            :: ( if no_switch then Empty
                 ...
                 match repos with
                 | None -> Empty
                 | Some repos -> Atom (OpamListCommand.From_repository repos) )
            :: OpamStd.Option.Op.(
    ...
    let results = OpamListCommand.filter ~base:all st filter in
    if not depexts then (
      if not silent then OpamListCommand.display st format results
      ...
      if not silent then OpamListCommand.print_depexts results_depexts
      else if OpamStd.String.Set.is_empty results_depexts then
        OpamStd.Sys.exit_because `False
  in
  ( Term.(
      const list $ global_options $ package_selection $ state_selector
...
let show =
    ...
    [
      ...
      `P
        "The information block consists of the name of the package, the \
         ...
         selected compiler, the list of available (installable) versions, and \
         a complete description.";
      `P
        "$(b,opam list) can be used to display the list of available packages \
         as well as a short description for each.";
      `P
        "Paths to package definition files or to directories containing \
         package definitions can also be specified, in which case the \
         corresponding metadata will be shown.";
    ]
  in
  let fields =
    let doc =
      Arg.info ~docv:"FIELDS"
        ~doc:
          ( "Only display the values of these fields. Fields can be selected \
             among "
          ^ OpamStd.List.concat_map ", "
              (Printf.sprintf "$(i,%s)" @* snd)
  ...
  let show_empty =
    mk_flag [ "empty-fields" ]
      "Show fields that are empty. This is implied when $(b,--field) is given."
  in
  let raw = mk_flag [ "raw" ] "Print the raw opam file for this package" in
  let where =
    mk_flag [ "where" ]
  ...
  let list_files =
    mk_flag [ "list-files" ]
      "List the files installed by the package. Equivalent to \
       $(b,--field=installed-files), and only available for installed packages"
  in
  let file =
    let doc =
      Arg.info ~docv:"FILE" ~doc:"DEPRECATED: see $(i,--just-file)" [ "file" ]
    in
    Arg.(value & opt (some existing_filename_or_dash) None & doc)
  in
  let normalise =
    mk_flag [ "normalise" ]
  ...
  let just_file =
    mk_flag [ "just-file" ]
      "Load and display information from the given files (allowed \
       ...
       the repositories or state of the package. This implies $(b,--raw) \
       unless $(b,--fields) is used. Only raw opam-file fields can be queried. \
       If no PACKAGES argument is given, read opam file from stdin."
  in
  let all_versions =
  ...
  in
  let pkg_info global_options fields show_empty raw where list_files file
      normalise no_lint just_file all_versions atom_locs =
    let print_just_file f =
      ...
      if where then
        ...
        let get_field f =
          try
            OpamListCommand.mini_field_printer ~prettify:true ~normalise
    ...
    | [], false -> `Error (true, "required argument PACKAGES is missing")
    | [], true ->
    ...
    | atom_locs, false ->
        ...
        let st, atoms =
          OpamAuxCommands.simulate_autopin ~quiet:no_lint ~for_view:true st
            atom_locs
        in
        OpamListCommand.info st ~fields ~raw ~where ~normalise ~show_empty
          ~all_versions atoms;
        `Ok ()
    | atom_locs, true ->
...
let config =
    ...
    [
      ( "env",
        ...
        "Returns the bindings for the environment variables set in the current \
         ...
         With $(i,-v), add comments documenting the reason or package of \
         origin for each binding. This is most usefully used as $(b,eval \
         \\$(opam config env\\)) to have further shell commands be evaluated \
         in the proper opam context. Can also be accessed through $(b,opam \
         env)." );
      ( "revert-env",
        `revert_env,
        [],
        "Reverts environment changes made by opam, e.g. $(b,eval \\$(opam \
         config revert-env)) undoes what $(b,eval \\$(opam config env\\)) did, \
         as much as possible." );
      ( "exec",
        `exec,
        [ "[--] COMMAND"; "[ARG]..." ],
        "Execute $(i,COMMAND) with the correct environment variables. This \
         ...
         $(i,OPAMSWITCH) is not set in $(i,COMMAND)'s environment. Can also be \
         accessed through $(b,opam exec)." );
      ( "var",
        `var,
        [ "VAR" ],
        "Return the value associated with variable $(i,VAR). Package variables \
         can be accessed with the syntax $(i,pkg:var). Can also be accessed \
         through $(b,opam var)" );
      ( "list",
        `list,
        [ "[PACKAGE]..." ],
        "Without argument, prints a documented list of all available \
         variables. With $(i,PACKAGE), lists all the variables available for \
         these packages. Use $(i,-) to include global configuration variables \
         for this switch." );
      ( "set",
        `set,
        [ "VAR"; "VALUE" ],
        "Set the given opam variable for the current switch. Warning: changing \
         a configured path will not move any files! This command does not \
    ...
    [
      ...
      `P
        "This command uses opam state to output information on how to use \
         installed libraries, update the $(b,PATH), and substitute variables \
         used in opam packages.";
      `P
        "Apart from $(b,opam config env), most of these commands are used by \
         opam internally, and are of limited interest for the casual user.";
    ]
    @ mk_subdoc commands @ [ `S "OPTIONS" ]
  in

  let command, params = mk_subcommands commands in
  let open Common_config_flags in
  let config global_options command shell sexp inplace_path set_opamroot
      set_opamswitch params =
    apply_global_options global_options;
    let shell =
      match shell with Some s -> s | None -> OpamStd.Sys.guess_shell_compat ()
    in
    ...
    | Some `env, [] -> (
        ...
        | Some sw ->
            `Ok
              (OpamConfigCommand.env gt sw ~set_opamroot ~set_opamswitch
    ...
    | Some `revert_env, [] -> (
        ...
        | Some sw ->
            `Ok
              ( OpamConfigCommand.ensure_env gt sw;
                OpamConfigCommand.print_eval_env ~csh:(shell = SH_csh) ~sexp
                  ~fish:(shell = SH_fish) (OpamEnv.add [] []) ) )
    | Some `exec, (_ :: _ as c) ->
        OpamGlobalState.with_ `Lock_none @@ fun gt ->
        `Ok
          (OpamConfigCommand.exec gt ~set_opamroot ~set_opamswitch ~inplace_path
             c)
    | Some `list, params -> (
        OpamGlobalState.with_ `Lock_none @@ fun gt ->
        try
          `Ok
            (OpamConfigCommand.list gt
               (List.map OpamPackage.Name.of_string params))
        with Failure msg -> `Error (false, msg) )
    | Some `set, [ var; value ] ->
        `Ok
          (OpamConfigCommand.set (OpamVariable.Full.of_string var) (Some value))
    | Some `unset, [ var ] ->
        `Ok (OpamConfigCommand.set (OpamVariable.Full.of_string var) None)
    | Some `set_global, [ var; value ] ->
        `Ok
          (OpamConfigCommand.set_global
             (OpamVariable.Full.of_string var)
             (Some value))
    | Some `unset_global, [ var ] ->
        `Ok
          (OpamConfigCommand.set_global (OpamVariable.Full.of_string var) None)
    | Some `expand, [ str ] ->
        OpamGlobalState.with_ `Lock_none @@ fun gt ->
    ...
    | Some `var, [ var ] -> (
        OpamGlobalState.with_ `Lock_none @@ fun gt ->
        try
          `Ok (OpamConfigCommand.variable gt (OpamVariable.Full.of_string var))
        with Failure msg -> `Error (false, msg) )
    | Some `subst, (_ :: _ as files) ->
        OpamGlobalState.with_ `Lock_none @@ fun gt ->
        `Ok
          (OpamConfigCommand.subst gt
    ...
    | Some `pef, params -> (
        ...
        match params with
        | [] | [ "-" ] ->
            OpamSwitchState.dump_pef_state st stdout;
            `Ok ()
        | [ file ] ->
    ...
    | Some `report, [] -> (
        ...
        try
          ...
          print "solver" "%s" (OpamCudfSolver.get_name (module Solver));
          print "install-criteria" "%s" (OpamSolverConfig.criteria `Default);
          print "upgrade-criteria" "%s" (OpamSolverConfig.criteria `Upgrade);
          let nprint label n =
            if n <> 0 then [ Printf.sprintf "%d (%s)" n label ] else []
          in
          ...
          print "repositories" "%s"
            (let repos = state.switch_repos.repositories in
             let default, nhttp, nlocal, nvcs =
               OpamRepositoryName.Map.fold
                 (fun _ repo (dft, nhttp, nlocal, nvcs) ->
                   let dft =
                     if
                       OpamUrl.root repo.repo_url
                       = OpamUrl.root OpamInitDefaults.repository_url
                     then
                       OpamRepositoryName.Map.find repo.repo_name
             ...
             in
             String.concat ", "
               ( nprint "http" nhttp @ nprint "local" nlocal
             ...
             | None -> "");
          print "pinned" "%s"
            ( if OpamPackage.Set.is_empty state.pinned then "0"
            else
              let pinnings =
          ...
          print "current-switch" "%s" (OpamSwitch.to_string state.switch);
          let process nv =
          ...
          `Ok ()
        with e ->
          print "read-state" "%s" (Printexc.to_string e);
          `Ok () )
  ...
  ( Term.ret
      Term.(
        const config $ global_options $ command $ shell_opt $ sexp
        $ inplace_path $ set_opamroot $ set_opamswitch $ params),
...
let var =
  ...
  let man =
    [
      `S "DESCRIPTION";
      `P
        "With a $(i,VAR) argument, prints the value associated with $(i,VAR). \
  ...
  in
  let varname = Arg.(value & pos 0 (some string) None & info ~docv:"VAR" []) in
  let package =
    Arg.(
      value
      & opt (some package_name) None
  ...
  let print_var global_options package var =
    ...
    | Some _, Some _ ->
        `Error
          ( true,
            "--package can't be specified with a var argument, use 'pkg:var' \
             instead." )
  in
  ( Term.ret Term.(const print_var $ global_options $ package $ varname),
...
let exec =
  ...
  let exec global_options inplace_path set_opamroot set_opamswitch cmd =
    apply_global_options global_options;
    OpamGlobalState.with_ `Lock_none @@ fun gt ->
    OpamConfigCommand.exec gt ~set_opamroot ~set_opamswitch ~inplace_path cmd
  in
  let open Common_config_flags in
  ( Term.(
      const exec $ global_options $ inplace_path $ set_opamroot $ set_opamswitch
      $ cmd),
...
let env =
    ...
    [
      ...
      `P
        "Returns the bindings for the environment variables set in the current \
         ...
         With $(i,-v), add comments documenting the reason or package of \
         origin for each binding. This is most usefully used as $(b,eval \
         \\$(opam env\\)) to have further shell commands be evaluated in the \
         proper opam context.";
  ...
  in
  let env global_options shell sexp inplace_path set_opamroot set_opamswitch
      revert =
    apply_global_options global_options;
    let shell =
      match shell with Some s -> s | None -> OpamStd.Sys.guess_shell_compat ()
  ...
  let open Common_config_flags in
  ( Term.(
      const env $ global_options $ shell_opt $ sexp $ inplace_path
...
let install =
  ...
  let man =
    [
      `S "DESCRIPTION";
      `P
        "This command installs one or more packages inside the currently \
         selected switch (see $(b,opam switch)). Once installed, you can \
         remove packages with $(b,opam remove), upgrade them with $(b,opam \
         upgrade), and list them with $(b,opam list). See $(b,opam pin) as \
         well to manage package versions, reroute existing packages or add \
         packages that are not defined in the repositories.";
      `P
        "All required dependencies of the selected packages will be installed \
         first. Any already installed packages having dependencies, or \
         optional dependencies to the changed packages will be recompiled. The \
         proposed solution may also imply removing incompatible or conflicting \
         packages.";
      `P
        "If paths are provided as argument instead of packages, they are \
  ...
  let add_to_roots =
    ...
    let unroot =
      ( Some false,
        Arg.info [ "unset-root" ]
          ~doc:"Mark given packages as \"installed automatically\"." )
    in
  ...
  let check =
    mk_flag [ "check" ]
      "Check if dependencies are installed. If some packages are missing, it \
       will ask if you want them installed and launch install of $(i,PACKAGES) \
       with option $(b,deps-only) enabled."
  in
  let install global_options build_options add_to_roots deps_only restore
    ...
    if atoms_or_locals = [] && not restore then
      ...
      let atoms_or_locals =
        if restore then (
          let to_restore =
            OpamPackage.Set.diff st.installed_roots st.installed
          in
          ...
          atoms_or_locals
          @ List.map
              (fun p -> `Atom (OpamSolution.atom_of_package p))
              (OpamPackage.Set.elements to_restore) )
        else atoms_or_locals
      in
      if atoms_or_locals = [] then `Ok ()
      else
        let st, atoms =
        ...
        ( if check then
          ...
          if OpamPackage.Map.is_empty missing then (
            ...
            OpamStd.Sys.exit_because `False ) );
        let st =
          OpamClient.install st atoms ~autoupdate:pure_atoms ?add_to_roots
  ...
  ( Term.ret
      Term.(
        const install $ global_options $ build_options $ add_to_roots
        $ deps_only $ restore $ destdir $ assume_built $ check
        $ atom_or_local_list),
...
let remove =
  ...
  let man =
    [
      `S "DESCRIPTION";
      `P
        "This command uninstalls one or more packages currently installed in \
         the currently selected compiler switch. To remove packages installed \
         in another compiler, you need to switch compilers using $(b,opam \
         switch) or use the $(b,--switch) flag. This command is the inverse of \
         $(b,opam-install).";
      `P
  ...
  let remove global_options build_options autoremove force destdir atom_locs =
    ...
    | Some d ->
        ...
        let uninst =
          List.filter
            (fun (name, _) -> not (OpamPackage.has_name packages name))
            atoms
        in
        if uninst <> [] then
          OpamConsole.warning
            ...
            (OpamStd.List.concat_map " " OpamFormula.short_string_of_atom
               uninst);
  ...
  in
  ( Term.(
      const remove $ global_options $ build_options $ autoremove $ force
      $ destdir $ atom_or_dir_list),
...
let reinstall =
      ...
      & vflag `Default
          [
            ...
            ( `List_pending,
              info [ "list-pending" ]
                ~doc:
                  "List packages that have been changed since installed and \
                   are marked for reinstallation" );
            ( `Forget_pending,
              info [ "forget-pending" ]
                ~doc:
                  "Forget about pending reinstallations of listed packages. \
                   This implies making opam assume that your packages were \
                   installed with a newer version of their metadata, so only \
                   use this if you know what you are doing, and the actual \
                   changes you are overriding." );
          ])
  in
  let reinstall global_options build_options assume_built atoms_locs cmd =
    ...
    | `Default, (_ :: _ as atom_locs) ->
        ...
        @@ OpamClient.reinstall st ~assume_built
    ...
    | `List_pending, [] ->
        ...
        OpamListCommand.display st
          {
            OpamListCommand.default_package_listing_format with
            OpamListCommand.columns = [ OpamListCommand.Package ];
            short = true;
            header = false;
            order = `Dependency;
          }
    ...
    | `Forget_pending, atom_locs ->
        ...
        OpamPackage.Set.iter
          (fun nv ->
            try
              ...
              if
                (not (OpamFile.OPAM.effectively_equal installed upstream))
                && OpamConsole.confirm
                     "Metadata of %s were updated. Force-update, without \
        ...
        OpamSwitchState.drop
        @@ OpamSwitchAction.update_switch_state ~reinstall st;
        `Ok ()
    | _, _ :: _ ->
  ...
  ( Term.(
      ret
        ( const reinstall $ global_options $ build_options $ assume_built
...
let update =
  ...
  let man =
    [
      `S "DESCRIPTION";
      `P
        "Update the package definitions. This fetches the newest version of \
         the repositories configured through $(b, opam repository), and the \
         sources of installed development packages and packages pinned in the \
         current switch. To use the updated sources and definitions, use \
         $(b,opam upgrade).";
    ]
  in
  let repos_only =
    mk_flag [ "R"; "repositories" ]
      "Update repositories (skipping development packages unless \
       $(b,--development) is also specified)."
  in
  let dev_only =
    mk_flag [ "development" ]
      "Update development packages (skipping repositories unless \
       $(b,--repositories) is also specified)."
  in
  let upgrade =
    mk_flag [ "u"; "upgrade" ]
      "Automatically run $(b,opam upgrade) after the update."
  in
  let name_list =
    arg_list "NAMES"
      "List of repository or development package names to update." Arg.string
  in
  let all =
    mk_flag [ "a"; "all" ]
  ...
  let check =
    mk_flag [ "check" ]
      "Do the update, then return with code 0 if there were any upstream \
       ...
       returns 0 only if there are currently availbale updates."
  in
  let update global_options jobs names repos_only dev_only all check upgrade =
    apply_global_options global_options;
    OpamStateConfig.update ?jobs:OpamStd.Option.Op.(jobs >>| fun j -> lazy j) ();
  ...
  in
  ( Term.(
      const update $ global_options $ jobs_flag $ name_list $ repos_only
      $ dev_only $ all $ check $ upgrade),
...
let upgrade =
    ...
    [
      ...
      `P
        "This command upgrades the installed packages to their latest \
         available versions. More precisely, this command calls the dependency \
         solver to find a consistent state where $(i,most) of the installed \
  ...
  ( Term.(
      ret
        ( const upgrade $ global_options $ build_options $ fixup $ check $ all
        $ atom_or_dir_list )),
...
let repository =
  ...
  let commands =
    [
      ( "add",
        ...
        "Adds under $(i,NAME) the repository at address $(i,ADDRESS) to the \
         list of configured repositories, if not already registered, and sets \
         this repository for use in the current switch (or the specified \
         scope). $(i,ADDRESS) is required if the repository name is not \
         already registered, and is otherwise an error if different from the \
      ...
      ( " ",
       ...
       of the previous labelled paragraph, which is what we want for our note. *)
        "$(b,Note:) By default, the repository is only added to the current \
         ...
         for $(b,opam switch create --repositories=REPOS)." );
      ( "remove",
        `remove,
        [ "NAME..." ],
      ...
      ( "list",
        `list,
        [],
        "Lists the currently selected repositories in priority order from rank \
         1. With $(b,--all), lists all configured repositories and the \
  ...
  let man =
    [
      `S "DESCRIPTION";
      `P
        "This command is used to manage package repositories. Repositories can \
      ...
      `P
        ( "Main commands $(b,add), $(b,remove) and $(b,set-repos) act only on \
           the current switch, unless differently specified using options \
           explained in $(b," ^ scope_section ^ ")." );
      `P
        "Without a subcommand, or with the subcommand $(b,list), lists \
         selected repositories, or all configured repositories with \
         $(b,--all).";
    ]
    ...
    @ [
        `S scope_section;
        `P
          "These flags allow one to choose which selections are changed by \
           $(b,add), $(b,remove), $(b,set-repos). If no flag in this section \
           is specified the updated selections default to the current switch. \
           Multiple scopes can be selected, e.g. $(b,--this-switch \
           --set-default).";
        `S "OPTIONS";
      ]
  in
  ...
  let scope =
    ...
    let flags =
      Arg.vflag_all []
        [
          ( `No_selection,
            scope_info [ "dont-select" ] "Don't update any selections" );
          ( `Current_switch,
            scope_info [ "this-switch" ]
              "Act on the selections for the current switch (this is the \
               default)" );
          ( `Default,
            scope_info [ "set-default" ]
              "Act on the default repository selection that is used for newly \
               created switches" );
          ( `All,
            scope_info [ "all-switches"; "a" ]
    ...
    let switches =
      Term.(
        const (List.map (fun s -> `Switch (OpamSwitch.of_string s)))
        $ Arg.value switches)
    in
    Term.(
      const (fun l1 l2 ->
          match l1 @ l2 with [] -> [ `Current_switch ] | l -> l)
  ...
  let repository global_options command kind short scope rank params =
    ...
    let command, params, rank =
      match (command, params, rank) with
    ...
    let update_repos new_repo repos =
      let rank = if rank < 0 then List.length repos + rank + 1 else rank - 1 in
      OpamStd.List.insert_at rank new_repo (List.filter (( <> ) new_repo) repos)
    in
    let check_for_repos rt names err =
      match
        List.filter
          (fun n -> not (OpamRepositoryName.Map.mem n rt.repositories))
          names
      with
      | [] -> ()
      | l -> err (OpamStd.List.concat_map " " OpamRepositoryName.to_string l)
    in
    OpamGlobalState.with_ `Lock_none @@ fun gt ->
    ...
    let switches =
      ...
      List.fold_left
        (fun acc -> function `Default | `No_selection -> acc
          | `All -> all_switches
          | `Switch sw ->
              if
                (not (OpamSwitch.Set.mem sw all))
                && not (OpamSwitch.is_external sw)
              then
                OpamConsole.error_and_exit `Not_found "No switch %s found"
          ...
          | `Current_switch -> (
              ...
              | None ->
                  OpamConsole.warning
                    "No switch is currently set, perhaps you meant \
                     '--set-default'?";
                  acc
              | Some sw -> if List.mem sw acc then acc else acc @ [ sw ] ))
        [] scope
    in
    match (command, params) with
    | Some `add, name :: url :: security ->
        ...
        let trust_anchors =
          ...
          | quorum :: fingerprints -> (
              try
                ...
                if quorum < 0 then failwith "neg"
                else Some { quorum; fingerprints }
        ...
        if scope = [ `Current_switch ] then
          OpamConsole.note
            "Repository %s has been added to the selections of switch %s only.\n\
             Run `opam repository add %s --all-switches|--set-default' to use \
             it in all existing switches, or in newly created switches, \
             respectively.\n"
    ...
    | Some `remove, names ->
        ...
        if full_wipe then (
          ...
          OpamConsole.msg
            "Repositories removed from the selections of switch %s. Use \
             '--all' to forget about them altogether.\n"
            (OpamSwitch.to_string (OpamStateConfig.get_switch ()));
        `Ok ()
    | Some `add, [ name ] ->
        let name = OpamRepositoryName.of_string name in
    ...
    | (None | Some `list), [] ->
        OpamRepositoryState.with_ `Lock_none gt @@ fun rt ->
        if List.mem `All scope then OpamRepositoryCommand.list_all rt ~short;
        let global = List.mem `Default scope in
        let switches =
          if
            scope = []
            || List.exists
                 (function `Current_switch | `Switch _ -> true | _ -> false)
                 scope
          then switches
          else []
        in
  ...
  ( Term.ret
      Term.(
        const repository $ global_options $ command $ repo_kind_flag
  ...
  let repos, rt =
    ...
    | Some repos ->
        ...
        let new_defs =
          OpamStd.List.filter_map
            (function
              | _, None -> None | n, Some u -> Some (n, OpamUrl.of_string u))
            repos
        in
        if
          List.for_all
            (fun (name, url) ->
              ...
              | None -> false)
            new_defs
        then (Some (List.map fst repos), rt)
        else
...
let switch =
    ...
    [
      ( "create",
        ...
        "Create a new switch, and install the given compiler there. \
         $(i,SWITCH) can be a plain name, or a directory, absolute or \
         relative, in which case a local switch is created below the given \
         directory. $(i,COMPILER), if omitted, defaults to $(i,SWITCH) if it \
         is a plain name, unless $(b,--packages) or $(b,--empty) is specified. \
         ...
         opam-init(1)). If the chosen directory contains package definitions, \
         a compatible compiler is searched within the default selection, and \
         the packages will automatically get installed." );
      ( "set",
        `set,
        [ "SWITCH" ],
      ...
      ( "export",
        `export,
        [ "FILE" ],
        "Save the current switch state to a file. If $(b,--full) is specified, \
         it includes the metadata of all installed packages" );
      ( "import",
        `import,
        [ "FILE" ],
        "Import a saved switch state. If $(b,--switch) is specified and \
      ...
      ("list", `list, [], "Lists installed switches.");
      ( "list-available",
        `list_available,
        [ "[PATTERN]" ],
      ...
      ("install", `install, [ "SWITCH" ], "Deprecated alias for 'create'.");
    ]
  in
  let man =
    [
      ...
      `P
        "Use $(b,opam switch create) to create a new switch, and $(b,opam \
      ...
      `P
        ( Printf.sprintf
            "Switch handles $(i,SWITCH) can be either a plain name, for \
             switches that will be held inside $(i,~%s.opam), or a directory \
             name, which in that case is the directory where the switch prefix \
             will be installed, as %s. Opam will automatically select a switch \
             by that name found in the current directory or its parents, \
             unless $(i,OPAMSWITCH) is set or $(b,--switch) is specified. When \
             creating a directory switch, if package definitions are found \
             locally, the user is automatically prompted to install them after \
             the switch is created unless $(b,--no-install) is specified."
      ...
      `P
        "$(b,opam switch set) sets the default switch globally, but it is also \
         ...
         environment. For that, use $(i,eval \\$(opam env --switch=SWITCH \
         --set-switch\\)).";
    ]
  ...
  in
  let empty =
    mk_flag [ "empty" ] "Allow creating an empty (without compiler) switch."
  in
  let repos =
  ...
  (* Deprecated options *)
  let d_alias_of =
    mk_opt [ "A"; "alias-of" ] "COMP" "This option is deprecated."
      Arg.(some string)
      None
  in
  let d_no_autoinstall =
    mk_flag [ "no-autoinstall" ] "This option is deprecated."
  in
  let switch global_options build_options command print_short no_switch packages
      empty descr full no_install deps_only repos d_alias_of d_no_autoinstall
      params =
    OpamArg.deprecated_option d_alias_of None "alias-of"
    ...
    let packages =
      ...
      | Some packages, true when packages <> [] ->
          OpamConsole.error_and_exit `Bad_arguments
            "Options --packages and --empty may not be specified at the same \
             time"
      | packages, _ -> packages
    in
    ...
    let param_compiler = function
      ...
      | [ comp ] -> Some comp
      | args ->
          OpamConsole.error_and_exit `Bad_arguments "Invalid extra arguments %s"
            (String.concat " " args)
    in
    match (command, params) with
    | None, [] | Some `list, [] ->
    ...
    | Some `list_available, pattlist ->
        ...
        in
        let order nv1 nv2 =
          if nv1.version = nv2.version then
            OpamPackage.Name.compare nv1.name nv2.name
          else OpamPackage.Version.compare nv1.version nv2.version
        in
        OpamListCommand.display st
          {
            OpamListCommand.default_package_listing_format with
            OpamListCommand.short = print_short;
    ...
    | Some `install, switch_arg :: params ->
        ...
        in
        let gt, st =
          OpamSwitchCommand.install gt ~rt ?synopsis:descr ?repos
            ~update_config:(not no_switch) ~packages ~local_compiler switch
        in
        OpamGlobalState.drop gt;
        let st =
          if
            (not no_install) && (not empty)
            && OpamSwitch.is_external switch
            && not local_compiler
          then
            let st, atoms =
            ...
            in
            OpamClient.install st atoms ~autoupdate:[] ~add_to_roots:true
              ~deps_only
          else st
        in
    ...
    | Some `import, [ filename ] ->
        ...
        let with_gt_rt f =
          if is_new_switch then
            ...
            OpamRepositoryState.with_ `Lock_none gt @@ fun rt -> f (gt, rt) )
        in
        ...
        let _gt =
          List.fold_left
            (fun gt switch ->
              let opam_dir =
                OpamFilename.Op.(
                  OpamFilename.Dir.of_string switch
    ...
    | Some `current, [] ->
        ...
        `Ok ()
    | Some `set, [ switch ] | Some (`default switch), [] ->
        OpamGlobalState.with_ `Lock_write @@ fun gt ->
    ...
    | Some `set_compiler, packages -> (
        try
          let parse_namev s =
            match fst OpamArg.package s with
    ...
    | Some `link, args -> (
        try
          let switch, dir =
            match args with
            | [ switch; dir ] ->
                (OpamSwitch.of_string switch, OpamFilename.Dir.of_string dir)
            | [ switch ] -> (OpamSwitch.of_string switch, OpamFilename.cwd ())
          ...
          if not (OpamGlobalState.switch_exists gt switch) then
            OpamConsole.error_and_exit `Not_found "The switch %s was not found"
              (OpamSwitch.to_string switch);
  ...
  ( Term.(
      ret
        ( const switch $ global_options $ build_options $ command
        $ print_short_flag $ no_switch $ packages $ empty $ descr $ full
        $ no_install $ deps_only $ repos $ d_alias_of $ d_no_autoinstall
        $ params )),
...
let pin ?(unpin_only = false) () =
  ...
  let commands =
    [
      ...
      ( "add",
        `add,
        [ "PACKAGE"; "TARGET" ],
        "Pins package $(i,PACKAGE) to $(i,TARGET), which may be a version, a \
         path, or a URL.\n\
         $(i,PACKAGE) can be omitted if $(i,TARGET) contains one or more \
         package descriptions. $(i,TARGET) can be replaced by $(b,--dev-repo) \
         if a package by that name is already known. If $(i,TARGET) is $(b,-), \
         the package is pinned as a virtual package, without any source. opam \
         will infer the kind of pinning from the format (and contents, if \
         local) of $(i,TARGET), Use $(b,--kind) or an explicit URL to disable \
         that behaviour.\n\
         Pins to version control systems may target a specific branch or \
         commit using $(b,#branch) e.g. $(b,git://host/me/pkg#testing).\n\
         If $(i,PACKAGE) is not a known package name, a new package by that \
         name will be locally created.\n\
         For source pinnings, the package version may be specified by using \
         the format $(i,NAME).$(i,VERSION) for $(i,PACKAGE), in the source \
         opam file, or with $(b,edit)." );
      ( "remove",
        `remove,
        [ "NAMES...|TARGET" ],
      ...
      ( "edit",
        ...
        "Opens an editor giving you the opportunity to change the package \
         definition that opam will locally use for package $(i,NAME), \
         including its version and source URL. Using the format \
         $(i,NAME.VERSION) will update the version in the opam file in advance \
         of editing, without changing the actual target. The chosen editor is \
         determined from environment variables $(b,OPAM_EDITOR), $(b,VISUAL) \
         or $(b,EDITOR), in order." );
    ]
  in
  let man =
    [
      ...
      `P
        "This command allows local customisation of the packages in a given \
         ...
         a local, editable version of the definition of the package. It is \
         also possible to create a new package just by pinning a non-existing \
         package name.";
      `P
        "Any customisation is available through the $(i,edit) subcommand, but \
         ...
         used locally.";
      `P
        "If (or $(i,-)) is specified, the package is pinned without a source \
         archive. The package name can be omitted if the target is a directory \
         containing one or more valid package definitions (this allows one to \
         do e.g. $(i,opam pin add .) from a source directory.";
      `P
        "If $(i,PACKAGE) has the form $(i,name.version), the pinned package \
         ...
         package.";
      `P
        "The default subcommand is $(i,list) if there are no further \
         arguments, and $(i,add) otherwise if unambiguous.";
    ]
    @ mk_subdoc ~defaults:[ ("", "list") ] commands
    @ [ `S "OPTIONS"; `S OpamArg.build_option_section ]
  in
  let command, params =
    if unpin_only then
      (Term.const (Some `remove), Arg.(value & pos_all string [] & Arg.info []))
    else mk_subcommands_with_default commands
  in
  let edit =
    mk_flag [ "e"; "edit" ]
  ...
  let kind =
    ...
    let help =
      Printf.sprintf
        "Sets the kind of pinning. Must be one of %s. If unset or $(i,auto), \
         is inferred from the format of the target, defaulting to the \
         appropriate version control if one is detected in the given \
         directory, or to $(i,path) otherwise. $(i,OPAMPINKINDAUTO) can be set \
         to \"0\" to disable automatic detection of version control.Use \
         $(i,none) to pin without a target (for virtual packages)."
        (Arg.doc_alts_enum main_kinds)
    in
    let doc = Arg.info ~docv:"KIND" ~doc:help [ "k"; "kind" ] in
    let kinds = main_kinds @ [ ("local", `rsync); ("rsync", `rsync) ] in
  ...
  let guess_names url k =
    ...
    let basename =
      ...
      | [] ->
          OpamConsole.error_and_exit `Bad_arguments
            "Can not retrieve a path from '%s'" (OpamUrl.to_string url)
      | b :: _ -> b
    in
    let found, cleanup =
      ...
      | None -> (
          ...
          try
            ...
            @@ OpamRepository.pull_tree
                 ~cache_dir:
                   (OpamRepositoryPath.download_cache
                      OpamStateConfig.(!r.root_dir))
                 basename pin_cache_dir [] [ url ]
            @@| function
            | Not_available (_, u) ->
                OpamConsole.error_and_exit `Sync_error "Could not retrieve %s" u
            | Result _ | Up_to_date _ ->
                (from_opam_files pin_cache_dir, Some cleanup)
    ...
    let names_found =
      ...
      | _ :: _ -> found
      | [] -> (
          try [ (OpamPackage.Name.of_string basename, None) ]
          with Failure _ ->
            OpamConsole.error_and_exit `Bad_arguments
              "Could not infer a package name from %s, please specify it on \
               the command-line, e.g. 'opam pin NAME TARGET'"
  ...
  let pin_target kind target =
    let looks_like_version_re =
      Re.(
        compile
        @@ seq [ bos; opt @@ char 'v'; digit; rep @@ diff any (set "/\\"); eos ])
    in
    let auto () =
      if target = "-" then `None
    ...
    | _ -> target
  in
  let pin global_options build_options kind edit no_act dev_repo print_short
      command params =
    apply_global_options global_options;
    apply_build_options build_options;
          ...
          List.fold_left
            (fun (err, acc) arg ->
                ...
                OpamPackage.Set.filter
                  (fun nv ->
                    ...
                    | Some u ->
                        ...
                        OpamUrl.(
                          u.transport = url.transport && u.path = url.path)
                    | None -> false)
                  st.pinned
              ...
              | [] -> (
                  ...
                  | `Error _ ->
                      OpamConsole.error
                        "No package pinned to this target found, or invalid \
                         package name: %s"
                        arg;
                      (true, acc) ))
            (false, []) arg
    ...
    | (Some `add, [ nv ] | Some (`default nv), []) when dev_repo -> (
        ...
        | `Ok (name, version) ->
            ...
            OpamSwitchState.drop
            @@ OpamClient.PIN.pin st name ~edit ?version ~action `Dev_upstream;
            `Ok ()
        | `Error e ->
    ...
    | Some `add, [ arg ] | Some (`default arg), [] -> (
        ...
        | `None | `Version _ ->
            let msg =
              Printf.sprintf
                "Ambiguous argument %S, if it is the pinning target, you must \
                 specify a package name first"
                arg
            in
            `Error (true, msg)
        | `Source url ->
              ...
              | _ :: _ :: _ ->
                  if
                    OpamConsole.confirm
                      ...
                      (OpamStd.List.concat_map ", "
                         (fst @> OpamPackage.Name.to_string)
                         names)
                  then names
                  else OpamStd.Sys.exit_because `Aborted
              | _ -> names
            ...
            let st =
              List.fold_left
                (fun st (name, opam_opt) ->
                  OpamStd.Option.iter
                    (fun opam ->
                      let opam_localf =
                        OpamPath.Switch.Overlay.tmp_opam st.switch_global.root
                          st.switch name
                      in
                      if
                        not
                          (OpamFilename.exists (OpamFile.filename opam_localf))
  ...
  ( Term.ret
      Term.(
        const pin $ global_options $ build_options $ kind $ edit $ no_act
        $ dev_repo $ print_short_flag $ command $ params),
...
let source =
  ...
  let source global_options atom dev_repo pin dir =
    ...
    let nv =
      ...
      with Not_found ->
        OpamConsole.error_and_exit `Not_found "No package matching %s found."
    ...
    ( if dev_repo then
      ...
      | None ->
          OpamConsole.error_and_exit `Not_found
            "Version-controlled repo for %s unknown (\"dev-repo\" field \
            ...
            OpamProcess.Job.run
              (OpamRepository.pull_tree
                 ~cache_dir:
                   (OpamRepositoryPath.download_cache
                      OpamStateConfig.(!r.root_dir))
                 (OpamPackage.to_string nv) dir [] [ url ])
          with
          | Not_available (_, u) ->
      ...
      if OpamPinned.find_opam_file_in_source nv.name dir = None then
        let f =
          if OpamFilename.exists_dir Op.(dir / "opam") then
            OpamFile.make Op.(dir / "opam" // "opam")
          else OpamFile.make Op.(dir // "opam")
        in
        OpamFile.OPAM.write f
          (OpamFile.OPAM.with_substs [] @@ OpamFile.OPAM.with_patches [] @@ opam)
    );
    if pin then
      let backend =
        if dev_repo then
          match OpamFile.OPAM.dev_repo opam with
          | Some { OpamUrl.backend = #OpamUrl.version_control as kind; _ } ->
              kind
          | _ -> `rsync
        else `rsync
      in
      let target =
        `Source
          (OpamUrl.parse ~backend ("file://" ^ OpamFilename.Dir.to_string dir))
      in
      OpamSwitchState.drop
        (OpamClient.PIN.pin t nv.name ~version:nv.version target)
  in
  ( Term.(const source $ global_options $ atom $ dev_repo $ pin $ dir),
...
let lint =
  ...
  let man =
    [
      `S "DESCRIPTION";
      `P
        "Given an $(i,opam) file, performs several quality checks on it and \
  ...
  let files =
    Arg.(
      value
      & pos_all existing_filename_dirname_or_dash []
      & info ~docv:"FILES" []
          ~doc:
  ...
  let warnings =
    mk_opt [ "warnings"; "W" ] "WARNS"
      "Select the warnings to show or hide. $(i,WARNS) should be a \
       ...
       respectively enable or disable warning or error number $(b,N) or all \
    ...
    let opam_files_in_dir d =
      ...
      | [] ->
          OpamConsole.warning "No opam files found in %s"
            (OpamFilename.Dir.to_string d);
          []
      | l -> List.map (fun (_name, f) -> Some f) l
    in
    let files =
      match (files, package) with
      | [], None ->
          (* Lookup in cwd if nothing was specified *)
          opam_files_in_dir (OpamFilename.cwd ())
      | files, None ->
          List.map
            (function
              | None -> [ None ] (* this means '-' was specified for stdin *)
              | Some (OpamFilename.D d) -> opam_files_in_dir d
      ...
      List.fold_left
        (fun (err, json) opam_f ->
          try
            ...
            let enabled =
              let default =
                match warnings_sel with (_, true) :: _ -> false | _ -> true
              in
              let map =
                List.fold_left
            ...
            let failed =
              List.exists
                (function _, `Error, _ -> true | _ -> false)
                warnings
            in
            if short then (
              if warnings <> [] then
  ...
  in
  ( Term.(
      const lint $ global_options $ files $ package $ normalise $ short
      $ warnings $ check_upstream),
...
let clean =
  ...
  in
  let dry_run =
    mk_flag [ "dry-run" ] "Print the removal commands, but don't execute them"
  in
  let download_cache =
    mk_flag [ "c"; "download-cache" ]
      ( Printf.sprintf
          "Clear the cache of downloaded files (\\$OPAMROOT%sdownload-cache), \
           as well as the obsolete \\$OPAMROOT%sarchives, if that exists."
          Filename.dir_sep Filename.dir_sep
      |> Cmdliner.Manpage.escape )
  in
  let repos =
  ...
  let repo_cache =
    mk_flag [ "r"; "repo-cache" ]
      "Clear the repository cache. It will be rebuilt by the next opam command \
       that needs it."
  in
  let logs = mk_flag [ "logs" ] "Clear the logs directory." in
  let switch =
  ...
  let all_switches =
    mk_flag [ "a"; "all-switches" ]
      "Run the switch cleanup commands in all switches. Implies \
       $(b,--switch-cleanup)"
  in
  let clean global_options dry_run download_cache repos repo_cache logs switch
      all_switches =
    apply_global_options global_options;
    ...
    let cleandir d =
      if dry_run then
        ...
        with OpamSystem.Internal_error msg ->
          OpamConsole.warning "Error ignored: %s" msg
    in
    let rmdir d =
      if dry_run then
        OpamConsole.msg "rm -rf \"%s\"\n" (OpamFilename.Dir.to_string d)
    ...
    if repos then (
      ...
      in
      let default_repos =
        OpamGlobalState.repos_list gt |> OpamRepositoryName.Set.of_list
      in
      let unused_repos =
        List.fold_left
          (fun repos sw ->
  ...
  in
  ( Term.(
      const clean $ global_options $ dry_run $ download_cache $ repos
...
let lock =
    ...
    [
      ...
      `P
        "Generates a lock file of a package: checks the current state of their \
         installed dependencies, and outputs modified versions of the opam \
         file with a $(i,.locked) suffix, where all the (transitive) \
      ...
      `P
        "If paths (filename or directory) are given, those opam files are \
         locked. If package is given, installed one is locked, otherwise its \
         latest version.";
      `P "Fails if all mandatory dependencies are not installed in the switch.";
      `S "What is changed in the locked file?";
      `P
        "- $(i,depends) are fixed to their specific versions, with all filters \
         removed (except for the exceptions below";
      `P
        "- $(i,depopts) that are installed in the current switch are turned \
         into depends, with their version set. Others are set in the \
         $(i,conflict) field";
      `P
        "- `{dev}`, `{with-test}, and `{with-doc}` filters are kept if all \
         packages of a specific filters are installed in the switch. Versions \
         are fixed and the same filter is on all dependencies that are added \
         from them";
      `P
        "- $(i,pin-depends) are kept and new ones are added if in the \
         dependencies some packages are pinned ";
      `P
        "- pins are resolved: if a package is locally pinned, opam tries to \
         get its remote url and branch, and sets this as the target URL";
      `S "ARGUMENTS";
      `S "OPTIONS";
    ]
  in
  ...
  in
  ( Term.(
      pure lock $ global_options $ only_direct_flag $ lock_suffix
      $ atom_or_local_list),
...
let default =
  ...
  let man =
    [
      ...
      `P
        "Use either $(b,opam <command> --help) or $(b,opam help <command>) for \
  ...
  let usage global_options =
    ...
    OpamConsole.formatted_msg
      ...
      init_doc list_doc show_doc install_doc remove_doc update_doc upgrade_doc
      config_doc repository_doc switch_doc pin_doc
...
let commands =
  [
    ...
    list ();
    make_command_alias
      (list ~force_search:true ())
      ~options:" --search" "search";
    show;
    make_command_alias show "info";
    install;
    remove;
    make_command_alias remove "uninstall";
    reinstall;
    update;
    upgrade;
    config;
    ...
    env;
    repository;
    make_command_alias repository "remote";
    switch;
    pin ();
    make_command_alias (pin ~unpin_only:true ()) ~options:" remove" "unpin";
    source;
    lint;
    clean;
    lock;
