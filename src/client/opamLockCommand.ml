...
let select_packages atom_locs st =
  ...
  let packages =
    OpamFormula.packages_of_atoms
      OpamPackage.Set.Op.(st.packages ++ st.installed)
      atoms
  in
  if OpamPackage.Set.is_empty packages then
    ...
    let missing =
      OpamStd.List.filter_map
        (fun (n, vc) ->
          if OpamPackage.Name.Set.mem n names then None else Some (n, vc))
        atoms
    in
    if missing <> [] then
      ...
      OpamPackage.Name.Set.fold
        (fun name acc ->
          ...
          let pkg =
            ...
            if OpamPackage.Set.is_empty pinned then pkgs %% st.installed
            else pinned
          in
          let nv =
            ...
            | _ ->
                ...
                OpamConsole.note
                  "Package %s is not installed nor pinned, generating lock \
          ...
          let atoms =
            OpamFormula.atoms
              (OpamPackageVar.all_depends ~depopts:false st
          ...
          if missing <> [] then (
            OpamConsole.error
              "Skipping %s, dependencies are not satisfied in this switch, not \
...
let get_git_url url nv dir =
  ...
  OpamProcess.Job.run
  @@ VCS.get_remote_url ?hash:url.OpamUrl.hash dir
  @@| function
  | Some u ->
      if u.OpamUrl.hash = None then
        OpamConsole.warning
          "Referenced git branch for %s is not available in remote: %s, use \
...
let lock_opam ?(only_direct = false) st opam =
  ...
  (* Depends *)
  let all_depends =
    OpamSolver.dependencies ~depopts:true ~build:true ~post:true ~installed:true
      univ
  ...
  let depends =
    if only_direct then
      let names =
        OpamFilter.filter_formula ~default:true
          (fun _ -> None)
  ...
  let map_of_set x set =
    OpamPackage.Map.of_list
      (List.map (fun nv -> (nv, x)) (OpamPackage.Set.elements set))
  in
  ...
  let select ?(build = false) ?(test = false) ?(doc = false) ?(dev = false)
    ...
    OpamFormula.packages st.packages
      (OpamFilter.filter_deps ~build ~test ~doc ~dev ~default ~post
         (OpamFile.OPAM.depends opam))
  in
  ...
  let select_depends typ selection =
    ...
    let uninstalled =
      OpamPackage.(
        Name.Set.diff (names_of_packages depends) (names_of_packages installed))
    in
  ...
  let depends =
    let f a b =
      match (a, b) with
      | _, (`other_dep _ as ot) | (`other_dep _ as ot), _ -> ot
      | _, `other | `other, _ -> `other
      | `version, `version -> `version
    in
    OpamPackage.Map.(
      depends_map |> union f dev_depends_map |> union f test_depends_map
                ...
                | `other_dep typ ->
                    And
                      ( cst_v,
                        Atom
                          (Filter
                             (FIdent ([], OpamVariable.of_string typ, None))) )
                | `other ->
                    ...
                    let new_formula =
                      OpamFormula.map
                        (function
                          | Constraint _ -> cst_v | Filter _ as f -> Atom f)
                        orig_deps_formula
                    in
  ...
  let all_depopts =
    OpamFormula.packages st.packages
      (OpamFilter.filter_deps ~build:true ~test:true ~doc:true ~dev:true
         ~default:true ~post:false
  ...
  let pin_depends =
    OpamStd.List.filter_map
      (fun nv ->
        if not (OpamSwitchState.is_pinned st nv.name) then None
        else
          ...
          | Some u -> (
              ...
              | Some d -> (
                  let local_warn () =
                    OpamConsole.warning
                      "Dependency %s is pinned to local target %s"
