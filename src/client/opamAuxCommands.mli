...
val remove_files_from_destdir :
  'a switch_state -> dirname -> package_set -> unit
...
val resolve_locals_pinned :
  'a switch_state -> [ `Atom of atom | `Dirname of dirname ] list -> atom list
(** Resolves the opam files and directories in the list to package name and
...
val get_compatible_compiler :
  ?repos:repository_name list -> 'a repos_state -> dirname -> atom option * bool
