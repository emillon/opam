      ...
      then (
        ...
        let env =
          if has_init then (
            ...
            | Some sw ->
                env_array
                  (OpamEnv.full_with_path ~force_path:false ~updates root_dir
                     sw) )
            ...
            | Some sw ->
                ...
                let plugins =
                  OpamPackage.Set.filter
                    (fun nv ->
                      OpamFile.OPAM.has_flag Pkgflag_Plugin
                ...
                if OpamPackage.Set.is_empty candidates then ()
                  ...
                  OpamConsole.error
                    "Plugin %s is already installed, but no %s command was \
                     found.\n\
                  ...
                  OpamConsole.error
                    "%s is not a known command or plugin (package %s does not \
                     have the 'plugin' flag set)."
                    name
                    (OpamPackage.to_string (OpamPackage.Set.max_elt candidates));
                  exit (OpamStd.Sys.get_exit_code `Bad_arguments) )
                else if
                  OpamConsole.confirm
                    "Opam plugin \"%s\" is not installed. Install it on the \
                     current switch?"
                    name
                then (
                  let nv =
                  ...
                  | None ->
                      OpamConsole.error_and_exit `Package_operation_error
                        ...
                        (OpamPackage.to_string nv) command
                  | Some command ->
...
let rec main_catch_all f =
  ...
  | OpamFormatUpgrade.Upgrade_done conf ->
      ...
      OpamConsole.msg "Update done, please now retry your command.\n";
      ...
      let exit_code =
        ...
        | OpamSystem.Process_error result ->
            OpamConsole.errmsg "%s Command %S failed:\n%s\n"
              ...
              ( try List.assoc "command" result.OpamProcess.r_info
                with Not_found -> "" )
              (Printexc.to_string e);
            OpamConsole.errmsg "%s" (OpamStd.Exn.pretty_backtrace e);
            OpamStd.Sys.get_exit_code `Internal_error
        | Sys.Break | OpamParallel.Errors (_, (_, Sys.Break) :: _, _) ->
    ...
    if Array.length Sys.argv > 1 && Sys.argv.(1) = "admin" then
      ( true,
        Array.init
          ...
          (function 0 -> Sys.argv.(0) | i -> Sys.argv.(i + 1)) )
    else (false, Sys.argv)
  in
  let eval () =
    if admin then
    ...
    else Term.eval_choice ~catch:false ~argv:argv1 default commands
  in
  match eval () with
  ...
  | Some s -> (
      let file_name () =
        ...
        | Some (pfx, sfx) ->
            let rec getname i =
              let f =
                OpamFilename.of_string (Printf.sprintf "%s%d%s" pfx i sfx)
              in
      ...
      try
        ...
        close_out f
      with e ->
        OpamConsole.warning "Couldn't write json log: %s" (Printexc.to_string e)
      )
