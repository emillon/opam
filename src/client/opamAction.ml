...
let process_dot_install st nv build_dir =
  ...
  let warning, had_windows_warnings =
    ...
    else (OpamSystem.default_install_warning, fun () -> false)
  in
    ...
    OpamFilename.in_dir build_dir (fun () ->
            ...
            (fun (base, dst) ->
              let base, append =
                if
                  ...
                  && not
                       (OpamFilename.exists
                          (OpamFilename.create build_dir base.c))
                then
                  let base' =
                    {
                      base with
                      c = OpamFilename.Base.add_extension base.c "exe";
                    }
                  in
                  if OpamFilename.exists (OpamFilename.create build_dir base'.c)
                  then (
                    OpamConsole.warning
              ...
              let dst_file =
                ...
                | Some d ->
                    if append && not (OpamFilename.Base.check_suffix d ".exe")
                    then
                      OpamFilename.create dst_dir
                        (OpamFilename.Base.add_extension d "exe")
                    else OpamFilename.create dst_dir d
              in
        ...
        List.iter
          (fun (src, dst) ->
            ...
            if
              ...
              && OpamConsole.confirm "Overwriting %s?"
                   (OpamFilename.to_string dst)
            then OpamFilename.install ~warning ~src:src_file ~dst ()
            else (
              OpamConsole.msg "Installing %s to %s.\n"
                (OpamFilename.Base.to_string src.c)
                (OpamFilename.to_string dst);
        ...
        if !warnings <> [] then (
          ...
          OpamConsole.error "Installation of %s failed"
            (OpamPackage.to_string nv);
          let msg =
            Printf.sprintf "Some files in %s couldn't be installed:\n%s"
...
let download_package st nv =
  ...
  if OpamStateConfig.(!r.dryrun) || OpamClientConfig.(!r.fake) then Done None
  else
    let dir = OpamSwitchState.source_dir st nv in
    if
      OpamPackage.Set.mem nv st.pinned
      && OpamFilename.exists_dir dir
      && OpamStd.Option.Op.(
           OpamPinned.find_opam_file_in_source nv.name dir
           >>= OpamFile.OPAM.read_opt >>= OpamFile.OPAM.version_opt)
         = Some nv.version
    then Done None
    else
      ...
      | _, (name, Not_available (s, l)) :: _ ->
          let msg = match s with None -> l | Some s -> s in
          OpamConsole.error "Failed to get extra source \"%s\" of %s: %s" name
            (OpamPackage.to_string nv) msg;
          Some (s, l)
      | Some (Result msg), _ ->
          print_action msg;
      ...
      | None, (e :: es as extras) ->
          ( if
            List.for_all
              (function _, Up_to_date _ -> true | _ -> false)
              extras
          then print_action "cached"
          else
            match (e, es) with
            | (_, Result msg), [] -> print_action msg
            | _, _ ->
                print_action
                  (Printf.sprintf "%d extra sources" (List.length extras)) );
          None
  ...
  let rec iter_patches f = function
    ...
    | (patchname, filter) :: rest ->
        ...
        then
          ...
          | Some err -> iter_patches f rest @@| fun e -> (patchname, err) :: e
...
let prepare_package_source st nv dir =
  log "prepare_package_source: %a at %a"
    ...
    dir;
  if OpamStateConfig.(!r.dryrun) then Done None
  else
    let opam = OpamSwitchState.opam st nv in
    let get_extra_sources_job =
      ...
      List.fold_left
        (fun job dl ->
          job @@+ function None -> dl_file_job dl | some_err -> Done some_err)
        (Done None)
      ...
      try
        List.iter
          (fun (src, base, hash) ->
            ...
            else OpamFilename.copy ~src ~dst:(OpamFilename.create dir base))
    ...
    get_extra_sources_job @@+ function
    | Some _ as err -> Done err
    | None -> (
        check_extra_files |> function
        | Some _ as err -> Done err
        | None -> prepare_package_build st nv dir )

let compilation_env t opam =
  OpamEnv.get_full ~force_path:true t
    ~updates:
      ( [
...
let removal_needs_download st nv =
  ...
  | Some opam ->
      not
        ( OpamFile.OPAM.has_flag Pkgflag_LightUninstall opam
        || OpamFilter.commands
             (OpamPackageVar.resolve ~opam st)
...
let get_wrapper t opam wrappers ?local getter =
  ...
  in
  OpamFilter.commands
    (OpamPackageVar.resolve ~local:local_env ~opam t)
    (getter wrappers)
...
let cmd_wrapper t opam wrappers getter cmd args =
  ...
  | (cmd, args) :: r ->
      (cmd, args @ List.concat (List.map (fun (c, a) -> c :: a) r))
  | [] -> assert false
...
let make_command st opam ?dir ?text_command (cmd, args) =
  ...
  let context =
    ...
    String.concat " | "
      [
        ...
        (OpamSysPoll.os () +! "unknown")
        ^ "/"
        ^ (OpamSysPoll.arch () +! "unknown");
        OpamStd.List.concat_map " " OpamPackage.to_string
          OpamPackage.Set.(
            elements @@ inter st.compiler_packages st.installed_roots);
        ( if OpamPackage.Set.mem nv st.pinned then
          ...
          | Some u ->
              ...
              Printf.sprintf "pinned(%s%s)" (OpamUrl.to_string u)
                (OpamStd.Option.to_string
                   (fun r -> "#" ^ OpamPackage.Version.to_string r)
                   rev)
        else
          match
  ...
  OpamSystem.make_command ~env ~name ?dir ~text
    ...
    ~metadata:[ ("context", context) ] ~verbose:(OpamConsole.verbose ()) cmd
    args
...
let noop_remove_package t nv =
  ...
  let has_tracked_files =
    ...
    | None ->
        ...
        in
        OpamFile.exists install_file
  in
  not (has_remove_commands || has_tracked_files)

(* Remove a given package *)
let remove_package_aux t ?(silent = false) ?changes ?force ?build_dir nv =
  ...
  *)
  let dot_install = OpamPath.Switch.install root t.switch name in
  let changes_file = OpamPath.Switch.changes root t.switch name in
  let opam =
    match installed_opam_opt t nv with
    | Some o -> o
  ...
  (* handle .install file *)
  let uninstall_files () =
    let install = OpamFile.Dot_install.safe_read dot_install in
      ...
      List.iter
        (fun (base, dst) ->
          let dst_file =
            ...
            | None ->
                dst_dir
                // Filename.basename (OpamFilename.Base.to_string base.c)
            | Some b -> OpamFilename.create dst_dir b
          in
    ...
    List.iter
      (fun (_, dst) ->
        if OpamFilename.exists dst then (
          ...
          if OpamConsole.confirm "Continue?" then OpamFilename.remove dst ))
  ...
  let build_dir =
    OpamStd.Option.default_map
      (OpamFilename.opt_dir (OpamPath.Switch.remove root t.switch nv))
      build_dir
  in
  ...
  if not OpamStateConfig.(!r.dryrun) then (
    OpamFilename.remove
      (OpamFile.filename (OpamPath.Switch.config root t.switch nv.name));
    uninstall_files ();
    OpamFilename.remove (OpamFile.filename dot_install) );

  (* Run the post-remove commands *)
  let local =
    opam_local_env_of_status OpamStd.Option.Op.(error_pre ++ error >>| snd)
  in
  OpamProcess.Job.of_fun_list ~keep_going:true
    (List.map
       (fun cmd () -> mk_cmd cmd)
    ...
    | None ->
        OpamConsole.msg "%s removed   %s.%s\n"
          ( if not (OpamConsole.utf8 ()) then "->"
            ...
            OpamActionGraph.(
              action_color (`Remove ()) (action_strings (`Remove ()))) )
          (OpamConsole.colorise `bold (OpamPackage.name_to_string nv))
          (OpamPackage.version_to_string nv);
        Done ()
...
let cleanup_package_artefacts t nv =
  ...
  if OpamPackage.Set.mem nv t.installed then (
    if not (OpamSwitchState.is_dev_package t nv) then OpamFilename.rmdir dev_dir
    )
  else (
    log "Removing the local metadata";
    OpamSwitchAction.remove_metadata t (OpamPackage.Set.singleton nv);
    if not (OpamPackage.Set.mem nv t.pinned) then OpamFilename.rmdir dev_dir )
...
let sources_needed st g =
  PackageActionGraph.fold_vertex
    (fun act acc ->
      match act with
      | `Remove nv ->
          if removal_needs_download st nv then OpamPackage.Set.add nv acc
          else acc
...
let remove_package t ?silent ?changes ?force ?build_dir nv =
  ...
  else remove_package_aux t ?silent ?changes ?force ?build_dir nv
...
let build_package t ?(test = false) ?(doc = false) build_dir nv =
  ...
  let jobs =
    ...
    List.map
      (fun cmd -> function
        | None ->
            let cmd = mk_cmd cmd in
            cmd @@> check_result cmd | some -> Done some)
      (get_wrapper t opam wrappers OpamFile.Wrappers.pre_build)
    @ List.map
        (fun ((cmd, args) as cmd_args) -> function
          | None ->
              let base_cmd = OpamProcess.command cmd args in
              ( mk_cmd ~text_command:cmd_args
              @@ cmd_wrapper t opam wrappers OpamFile.Wrappers.wrap_build cmd
                   args )
              @@> check_result base_cmd | some -> Done some)
        commands
  in
  OpamProcess.Job.seq jobs None @@+ fun result ->
  ...
  | Some (cmd, result), _ | _, Some (cmd, result) ->
      OpamConsole.error "The compilation of %s failed at %S." name
        (OpamProcess.string_of_command cmd);
...
let install_package t ?(test = false) ?(doc = false) ?build_dir nv =
  ...
  let rec run_commands = function
    | ((cmd, args) as ca) :: commands ->
        ...
        if OpamProcess.is_success result then run_commands commands
        else (
          OpamConsole.error "The installation of %s failed at %S." name
  ...
  let rel_meta_dir =
    OpamFilename.(
      Base.of_string
        (remove_prefix_dir switch_prefix (OpamPath.Switch.meta root t.switch)))
  in
  OpamDirTrack.track switch_prefix
    ~except:(OpamFilename.Base.Set.singleton rel_meta_dir)
    install_job
  @@+ fun (error, changes) ->
  post_install error changes @@+ function
  | Some e, changes ->
  ...
  | None, changes ->
      ...
      ( if OpamFile.OPAM.has_flag Pkgflag_Plugin opam then
        ...
        if OpamFilename.exists target then
          ...
          OpamConsole.warning
            "%s claims to be a plugin but no %s file was found" name
