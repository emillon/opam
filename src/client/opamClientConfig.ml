...
let setk k t ?print_stats ?pin_kind_auto ?autoremove ?editor ?keep_build_dir
    ?reuse_build_dir ?inplace_build ?working_dir ?drop_working_dir
    ?ignore_pin_depends ?show ?fake ?skip_dev_update ?json_out ?root_is_ok
    ?no_auto_upgrade =
...
let initk k =
  ...
  setk
    (setk (fun c ->
         r := c;
         k))
    !r ?print_stats:(env_bool "STATS") ?pin_kind_auto:(env_bool "PINKINDAUTO")
    ?autoremove:(env_bool "AUTOREMOVE") ?editor
    ...
    ?show:(env_bool "SHOW") ?fake:(env_bool "FAKE")
...
let opam_init ?root_dir ?strict =
  ...
  | Some conf ->
      ...
      OpamSolverConfig.update
        ?solver:
          ( OpamFile.Config.solver conf >>| fun s ->
            lazy (OpamCudfSolver.custom_solver s) )
        ?solver_preferences_default:
          (criteria `Default >>| fun s -> lazy (Some s))
        ?solver_preferences_upgrade:
          (criteria `Upgrade >>| fun s -> lazy (Some s))
  ...
  let log_dir =
    if log_dir = None && initialised then
      Some OpamFilename.(Dir.to_string (OpamPath.log root))
    else None
  in
  (fun () -> ())
  |> OpamStd.Config.initk ?log_dir
  |> OpamRepositoryConfig.initk |> OpamSolverConfig.initk
