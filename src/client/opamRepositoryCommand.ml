...
let add rt name url trust_anchors =
  ...
  let repo_exists =
    OpamStd.Option.of_Not_found
      (OpamRepositoryName.Map.find name)
      rt.repositories
  in
  ...
  | None ->
      let repo =
        { repo_name = name; repo_url = url; repo_trust = trust_anchors }
      in
      if
        OpamFilename.exists_dir (OpamRepositoryPath.root root name)
      ...
      if
        url.OpamUrl.backend = `rsync
        && OpamUrl.local_dir url <> None
        && OpamUrl.local_dir (OpamRepositoryPath.Remote.packages_url url) = None
        && not
             (OpamConsole.confirm
      ...
      (fun i name ->
        [
          ...
          ( try
              ...
              if r.repo_url = OpamUrl.empty then "-"
              else
...
let list rt ~global ~switches ~short =
  ...
  List.iter
    (fun sw ->
      if not short then
        OpamConsole.header_msg "Repository configuration for switch %s"
...
let list_all rt ~short =
  ...
  if short then
    OpamRepositoryName.Map.iter
      (fun r _ -> OpamConsole.msg "%s\n" (OpamRepositoryName.to_string r))
      rt.repositories
  else
    let repos_switches, _ =
      List.fold_left
        (fun (acc, i) repo ->
          (OpamRepositoryName.Map.add repo [ (None, i) ] acc, i + 1))
        ...
        (fun name repo ->
          [
            ...
            OpamStd.List.concat_map " "
              (fun (sw, i) ->
                OpamStd.Option.to_string ~none:"<default>" OpamSwitch.to_string
                  sw
                ^ (Printf.sprintf "(%d)" i |> OpamConsole.colorise `yellow))
              (List.rev
                 ( try OpamRepositoryName.Map.find name repos_switches
                   with Not_found -> [] ));
          ])
        rt.repositories
    in
...
let update_with_auto_upgrade rt repo_names =
  ...
  then (failed, rt)
    ...
    let rt, done_upgrade =
      List.fold_left
        (fun (rt, done_upgrade) r ->
          if List.mem r.repo_name failed then (rt, done_upgrade)
          else
            let def =
              OpamRepositoryName.Map.find r.repo_name rt.repos_definitions
            in
            let need_upgrade =
              match OpamFile.Repo.opam_version def with
              | None ->
                  OpamConsole.note
                    "Repository at %s doesn't define its version, assuming \
                     it's 1.2."
                    (OpamUrl.to_string r.repo_url);
                  true
              | Some v
                when OpamVersion.compare v
                       OpamAdminRepoUpgrade.upgradeto_version
                     < 0 ->
                  true
              | _ -> false
            in
              ...
              let rt =
                {
                  rt with
                  repos_definitions =
                    OpamRepositoryName.Map.add r.repo_name def
