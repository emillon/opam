  ...
  | `Build pkg, true | `Install pkg, _ | `Change (_, _, pkg), _ ->
      ...
      let local_variables =
        OpamVariable.Map.add
          (OpamVariable.of_string "success")
          (Some (B (not failed))) local_variables
      in
      let local_variables =
        OpamVariable.Map.add
          (OpamVariable.of_string "failure")
          (Some (B failed)) local_variables
      in
      let messages =
        let filter_env =
          OpamPackageVar.resolve ~opam ~local:local_variables st
        in
...
let check_solution ?(quiet = false) st = function
  ...
  | Success Nothing_to_do ->
      ...
      OpamEnv.check_and_print_env_warning st
  | Success Aborted ->
      if not OpamClientConfig.(!r.show) then OpamStd.Sys.exit_because `Aborted

let sum stats =
  stats.s_install + stats.s_reinstall + stats.s_remove + stats.s_upgrade
  + stats.s_downgrade

let eq_atom name version = (name, Some (`Eq, version))

let eq_atom_of_package nv = eq_atom nv.name nv.version

let eq_atoms_of_packages set =
  List.rev_map eq_atom_of_package (OpamPackage.Set.elements set)

let atom_of_package nv = (nv.name, None)
...
let check_availability ?permissive t set atoms =
  ...
  let check_atom ((name, cstr) as atom) =
    ...
    in
    if exists then None
    else if permissive = Some true then
      Some (OpamSwitchState.not_found_message t atom)
    else
      ...
      Some
        (Printf.sprintf "%s %s"
           ...
           (OpamSwitchState.unavailable_reason
              ~default:
                "unavailable for unknown reasons (this may be a bug in opam)" t
              f))
  in
...
let fuzzy_name t name =
  ...
  match OpamPackage.Name.Set.elements names with [ name ] -> name | _ -> name
...
let sanitize_atom_list ?(permissive = false) t atoms =
  ...
  if permissive then
    check_availability ~permissive t
      (OpamPackage.Set.union t.packages t.installed)
      atoms
  else
...
let display_error (n, error) =
  let f action nv =
    let disp =
      OpamConsole.header_error "while %s %s" action (OpamPackage.to_string nv)
    in
    match error with
  ...
  | `Fetch nv -> if OpamConsole.verbose () then f "fetching sources for" nv
...
module Json = struct
      ...
      | Success solution ->
          ...
          let to_proceed =
            PackageActionGraph.Topological.fold
              (fun a acc -> PackageAction.to_json a :: acc)
              action_graph []
          in
            ...
            `A
              (List.map
                 (fun c ->
                   `A
                     (List.map
                        (fun f -> `String (OpamFormula.to_string (Atom f)))
                        c))
                 chains)
          in
  ...
  let exc e =
    ...
    if OpamClientConfig.(!r.json_out = None) then `O []
          ...
          `O
            [
              ( "process-error",
                `O
                  ( [
                      ("code", `String (string_of_int r_code));
      ...
      | OpamSystem.Internal_error s -> `O [ ("internal-error", `String s) ]
...
let parallel_apply t ~requested ?add_roots ~assume_built action_graph =
  ...
  let remove_action_packages =
    PackageActionGraph.fold_vertex
      (function `Remove nv -> OpamPackage.Set.add nv | _ -> fun acc -> acc)
      action_graph OpamPackage.Set.empty
  in

  let install_action_packages =
    PackageActionGraph.fold_vertex
      (function `Install nv -> OpamPackage.Set.add nv | _ -> fun acc -> acc)
      action_graph OpamPackage.Set.empty
  in
  ...
  let root_installs =
    OpamPackage.Name.Set.union (OpamPackage.names_of_packages t.installed_roots)
    @@
  ...
  let inplace =
    if
      OpamClientConfig.(!r.inplace_build)
      || assume_built
      || OpamClientConfig.(!r.working_dir)
    then
      OpamPackage.Set.fold
        (fun nv acc ->
          match
            OpamStd.Option.Op.(
              OpamSwitchState.url t nv >>| OpamFile.URL.url
              >>= OpamUrl.local_dir)
          with
          | None -> acc
          | Some path -> OpamPackage.Map.add nv path acc)
        requested OpamPackage.Map.empty
  ...
  let action_graph =
    (* Add build actions *)
    let noop_remove nv = OpamAction.noop_remove_package t nv in
    PackageActionGraph.explicit ~noop_remove
  ...
  let job ~pred action =
    ...
    let has_nonfetch_failure =
      List.exists
        (function _, `Successful _ | `Fetch _, _ -> false | _ -> true)
        pred
    in
    if has_failure && ((not action_is_remove) || has_nonfetch_failure) then
      (* fatal error *)
      ...
      let not_yet_removed =
        match action with
        | `Remove _ ->
            PackageActionGraph.fold_descendants
              (function
                | `Remove nv -> OpamPackage.Set.add nv | _ -> fun acc -> acc)
            ...
            OpamPackage.Name.Map.filter
              (fun name _ ->
                OpamPackage.Set.exists
                  (fun pkg -> OpamPackage.Name.equal name pkg.name)
                  visible_installed)
              !t_ref.conf_files;
        }
      in
      ...
      if OpamClientConfig.(!r.fake) then
        ...
        | `Build nv -> (
            if assume_built && OpamPackage.Set.mem nv requested then (
              ...
              in
              let test =
                OpamStateConfig.(!r.build_test)
                && OpamPackage.Set.mem nv requested
              in
              let doc =
                OpamStateConfig.(!r.build_doc)
                && OpamPackage.Set.mem nv requested
              in
              if OpamFilename.exists_dir source_dir then (
              ...
              | Some exn ->
                  ...
                  Done (`Exception exn)
              | None -> (
                  OpamAction.build_package t ~test ~doc build_dir nv
                  @@+ function
                  | Some exn ->
                      store_time ();
                      Done (`Exception exn)
        ...
        | `Remove nv ->
            ...
            @@+ fun _ ->
            OpamProcess.Job.ignore_errors ~default:() (fun () ->
                OpamAction.remove_package t nv)
            @@| fun () ->
            remove_from_install
  ...
  let action_results =
    ...
    try
      let installs_removes, fetches =
        PackageActionGraph.fold_vertex
          (fun a ((installs_removes, fetches) as acc) ->
            match a with
             ...
             (fun excl ->
               match
                 OpamStd.List.filter_map
                   (fun nv ->
                     let act = `Build nv in
                     if PackageActionGraph.mem_vertex action_graph act then
                       Some act
                     else None)
                   excl
               with
          ...
          PackageActionGraph.Topological.fold
            (fun a acc ->
              ...
              | _ ->
                  let r =
                    ...
                    | `Error (`Aborted deps) ->
                        ...
                        `O
                          [
                            ( "aborted",
                              `A (List.map OpamSolver.Action.to_json deps) );
                          ]
                  in
                  let duration =
                  ...
                  in
                  `O
                    ( [ ("action", PackageAction.to_json a); ("result", r) ]
                    @ duration )
                  :: acc)
            action_graph []
        in
      ...
      let success, failure, aborted =
        List.fold_left
          (fun (success, failure, aborted) -> function
  ...
  let cleanup_artefacts graph =
    PackageActionGraph.iter_vertex
      (function
        | `Remove nv when not (OpamPackage.has_name t.pinned nv.name) ->
            OpamAction.cleanup_package_artefacts t nv
        (* if reinstalled, only removes build dir *)
        | `Install nv when not (OpamPackage.has_name t.pinned nv.name) ->
            let build_dir =
              OpamPath.Switch.build t.switch_global.root t.switch nv
            in
      ...
      | Partial_error solution_res ->
         ...
         doesn't add information *)
          let successful =
            List.filter
              (function
                | (`Fetch p | `Build p) when List.mem (`Install p) failed ->
                    false
                | _ -> true)
              successful
          in
          let remaining =
            List.filter
              (function
                | (`Remove p | `Install p) when List.mem (`Build p) failed ->
                    false
                | (`Remove p | `Install p | `Build p)
                  when List.mem (`Fetch p) failed ->
                    false
                | _ -> true)
              remaining
          in
            ...
            List.filter
              (function
                | `Fetch _ as a ->
                    ...
                    not
                      (List.for_all
                         (fun a -> List.mem a removes_missing_source)
                         succ)
                | _ -> true)
              failed
          in
          let filter_graph l =
            if l = [] then PackageActionGraph.create ()
            else
          ...
          let print_actions filter tint header ?empty actions =
            let actions =
              PackageActionGraph.fold_vertex
                (fun v acc -> if filter v then v :: acc else acc)
                actions []
            in
          ...
          print_actions
            ...
            failed;
          print_actions
            (function `Build _ | `Fetch _ -> false | _ -> true)
            `cyan
            ( "The following changes have been performed"
            ^ if remaining <> [] then " (the rest was aborted)" else "" )
            ~empty:"No changes have been performed" successful;
          (t, err)
      | _ -> assert false )
  ...
  let installed =
    OpamSolver.ActionGraph.Topological.fold
      (fun action installed ->
        ...
        | `Remove p -> OpamPackage.Set.remove p installed
...
let run_hook_job t name ?(local = []) w =
  ...
  let mk_cmd = function
    | cmd :: args ->
        ...
        Some
          (fun () ->
            OpamSystem.make_command ~verbose:(OpamConsole.verbose ())
  ...
  @@+ function
  | Some (cmd, _err) ->
      OpamConsole.error "The %s hook failed at %S" name
...
let apply ?ask t ~requested ?add_roots ?(assume_built = false) solution =
  ...
  if OpamSolver.solution_is_empty solution then
    ...
    if show_solution then (
      OpamConsole.msg "The following actions %s be %s:\n"
        (if OpamClientConfig.(!r.show) then "would" else "will")
        ( if OpamStateConfig.(!r.dryrun) then "simulated"
        else if OpamClientConfig.(!r.fake) then "faked"
        else "performed" );
      let messages p =
        ...
        OpamStd.List.filter_map
          (fun (s, f) ->
            if
              OpamFilter.opt_eval_to_bool
                (OpamPackageVar.resolve ~opam new_state)
                f
            then Some s
            else None)
          messages
      ...
      let append nv =
        ...
        if OpamPackage.Set.mem nv t.pinned then "*" else ""
      in
      OpamSolver.print_solution ~messages ~append ~requested
        ~reinstall:t.reinstall solution;
      let total_actions = sum stats in
      if total_actions >= 2 then
        OpamConsole.msg "===== %s =====\n" (OpamSolver.string_of_stats stats) );

    if
      (not OpamClientConfig.(!r.show))
    ...
    then (
      ...
      let run_job =
        if OpamStateConfig.(!r.dryrun) || OpamClientConfig.(!r.fake) then
          OpamProcess.Job.dry_run
        else OpamProcess.Job.run
      in
      let var_def name l = (OpamVariable.Full.of_string name, L l) in
      ...
      let depexts =
        OpamPackage.Set.fold
          (fun nv depexts ->
            OpamStd.String.Set.union depexts (OpamSwitchState.depexts t nv))
      ...
      in
      if not pre_session then OpamStd.Sys.exit_because `Configuration_error;
      let t0 = t in
      let t, r =
      ...
      let post_session =
        ...
        @@ run_hook_job t "post-session" ~local
             (OpamFile.Wrappers.post_session
                (OpamFile.Config.wrappers t.switch_global.config))
      in
      if not post_session then OpamStd.Sys.exit_because `Configuration_error;
      (t, r) )
    else (t, Aborted)
