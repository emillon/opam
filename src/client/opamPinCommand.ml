...
let read_opam_file_for_pinning ?(quiet = false) name f url =
  ...
  | None ->
      ...
      OpamConsole.error "Invalid opam file in %s source from %s:"
    ...
    | Some files ->
        OpamConsole.warning
          ...
          (OpamStd.List.to_string
             (fun (b, _) -> OpamFilename.Base.to_string b)
             files) );
  let destdir =
...
let edit_raw name temp_file =
  let rec edit () =
    ...
    let edited_ok =
      try
        ...
        = 0
        &&
        match OpamFilename.read (OpamFile.filename temp_file) with
        | "" | "\n" -> false
        | _ -> true
      with _ -> false
    in
    if not edited_ok then (
      ...
      try
        let warnings, opam_opt = OpamFileTools.lint_file temp_file in
        let opam =
          match opam_opt with
          | None ->
        ...
        | ws ->
            ...
            if OpamConsole.confirm "Proceed anyway ('no' will re-edit)?" then
              Some opam
            else edit ()
      with e ->
        OpamStd.Exn.fatal e;
  ...
  | Some new_opam ->
      OpamConsole.msg
        "You can edit this file again with \"opam pin edit %s\", export it \
         with \"opam show %s --raw\"\n"
        (OpamPackage.Name.to_string name)
        (OpamPackage.Name.to_string name);
      Some new_opam
...
let edit st ?version name =
  ...
  in
  let new_nv =
    match version with None -> nv | Some v -> OpamPackage.create name v
  in
            ...
            (fun f ->
              let base =
                ...
                @@ OpamFilename.remove_prefix
                     (path OpamPath.Switch.Overlay.files)
                     f
              in
...
let version_pin st name version =
  ...
  | Some pinned_nv ->
      ...
      if Some opam = OpamPackage.Map.find_opt pinned_nv st.repos_package_index
      then
        (* already version-pinned *)
        if pinned_nv <> nv then (
          OpamConsole.note "Package %s used to be pinned to version %s"
            (OpamPackage.Name.to_string name)
        ...
        OpamConsole.confirm "Package %s is already %s. Unpin and continue?"
          (OpamPackage.Name.to_string name)
          (string_of_pinned opam)
      then
        OpamFilename.rmdir (OpamPath.Switch.Overlay.package root st.switch name)
      else (
...
let rec handle_pin_depends st nv opam =
  ...
  let extra_pins =
    List.filter
      ...
      extra_pins
  in
  if extra_pins = [] then st
  else (
    OpamConsole.msg "The following additional pinnings are required by %s:\n%s"
    ...
    if OpamConsole.confirm "Pin and install them?" then
      ...
      OpamConsole.confirm "Try to install anyway, assuming `--ignore-depends`?"
    then st
    else OpamStd.Sys.exit_because `Aborted )

and source_pin st name ?version ?edit:(need_edit = false) ?opam:opam_opt
    ?(quiet = false) ?(force = false)
    ?(ignore_extra_pins = OpamClientConfig.(!r.ignore_pin_depends)) target_url =
  log "pin %a to %a %a"
    (slog OpamPackage.Name.to_string)
    name
    (slog (OpamStd.Option.to_string OpamPackage.Version.to_string))
    version
    (slog (OpamStd.Option.to_string ~none:"none" OpamUrl.to_string))
    target_url;
  ...
  let cur_version, cur_urlf =
    try
      ...
      if not (quiet && no_changes) then
        OpamConsole.note "Package %s is %s %s."
          (OpamPackage.Name.to_string name)
      ...
      if no_changes then ()
        ...
        OpamFilename.remove
          (OpamFile.filename
             (OpamPath.Switch.Overlay.tmp_opam st.switch_global.root st.switch
                name)) (* else raise Exns.Aborted *);
  ...
  if
    (not (OpamPackage.has_name st.packages name))
    && opam_opt = None
    && not
         (OpamConsole.confirm
  ...
  let nv =
    ...
    | None ->
        OpamPackage.create name
          OpamStd.Option.Op.(
            (opam_opt >>= OpamFile.OPAM.version_opt) +! cur_version)
  in
  ...
  let opam_opt =
    ...
    | Some local, None ->
        OpamConsole.warning
          "Couldn't retrieve opam file from versioned source, using the one \
           found locally.";
        Some local
    | Some local, Some vers
      when not
             OpamFile.(
               OPAM.effectively_equal
  ...
  let opam_opt =
    let opam_base =
      ...
      | Some opam -> opam
    in
    let opam_base = OpamFile.OPAM.with_url_opt urlf opam_base in
    if need_edit then (
      if not (OpamFile.exists temp_file) then
        OpamFile.OPAM.write_with_preserved_format
  ...
  match opam_opt with
  | None ->
      OpamConsole.error_and_exit `Not_found "No valid package definition found"
  | Some opam ->
      let opam =
      ...
      let st =
        if ignore_extra_pins then st else handle_pin_depends st nv opam
      in
      let opam =
        opam
      ...
      in
      OpamFilename.rmdir
        (OpamPath.Switch.Overlay.package st.switch_global.root st.switch
           nv.name);
...
(* pure *)
let unpin_one st nv =
  let st = { st with pinned = OpamPackage.Set.remove nv st.pinned } in
  (* Restore availability of other versions of this package from the repos *)
  let repo_package =
    OpamPackage.Map.filter
      (fun nv2 _ -> nv2.name = nv.name)
      st.repos_package_index
  in
  let available_packages =
    lazy
      ( OpamSwitchState.compute_available_packages st.switch_global st.switch
          st.switch_config ~pinned:OpamPackage.Set.empty ~opams:repo_package
  ...
  with
  | None, None -> OpamSwitchState.remove_package_metadata nv st
...
let unpin st names =
  log "unpin %a"
    (slog @@ OpamStd.List.concat_map " " OpamPackage.Name.to_string)
    names;
  List.fold_left
    (fun st name ->
      OpamFilename.rmdir
        (OpamPath.Switch.pinned_package st.switch_global.root st.switch name);
      OpamFilename.rmdir
        (OpamPath.Switch.Overlay.package st.switch_global.root st.switch name);
      match OpamPinned.package_opt st name with
      | Some nv ->
          let pin_str =
            OpamStd.Option.to_string ~none:"pinned" string_of_pinned
              (OpamSwitchState.opam_opt st nv)
          in
