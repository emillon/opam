...
let admin_command_doc = "Tools for repository administrators"
...
let admin_command_man =
  [
    ...
    `P
      ( Printf.sprintf
          "This command can perform various actions on repositories in the \
           opam format. It is expected to be run from the root of a \
           repository, i.e. a directory containing a 'repo' file and a \
           subdirectory 'packages%s' holding package definition within \
           subdirectories. A 'compilers%s' subdirectory (opam repository \
           format version < 2) will also be used by the $(b,upgrade-format) \
           subcommand."
...
let index_command =
    ...
    [
      ...
      `P
        "An opam repository can be served over HTTP or HTTPS using any web \
         ...
         when fetching from an HTTP remote, and should be run after any \
         changes are done to the contents of the repository.";
    ]
  in
  let urls_txt_arg =
    Arg.(
      value
      & vflag `minimal_urls_txt
          [
            ( `no_urls_txt,
              info [ "no-urls-txt" ]
                ~doc:
                  "Don't generate a 'urls.txt' file. That index file is no \
                   longer needed from opam 2.0 on, but is still used by older \
                   versions." );
            ( `full_urls_txt,
              info [ "full-urls-txt" ]
                ~doc:
                  "Generate an inclusive 'urls.txt', for a repository that \
                   will be used by opam versions earlier than 2.0." );
            ( `minimal_urls_txt,
              info [ "minimal-urls-txt" ]
                ~doc:
                  "Generate a minimal 'urls.txt' file, that only includes the \
                   'repo' file. This allows opam versions earlier than 2.0 to \
                   read that file, and be properly redirected to a repository \
                   dedicated to their version, assuming a suitable 'redirect:' \
                   field is defined, instead of failing. This is the default."
            );
          ])
  in
  let cmd global_options urls_txt =
    ...
    let repo_stamp =
      ...
      in
      match
        OpamUrl.guess_version_control (OpamFilename.Dir.to_string repo_root)
      with
      | None -> date ()
      | Some vcs -> (
...
let package_files_to_cache repo_root cache_dir ?link (nv, prefix) =
  match
    OpamFileTools.read_opam (OpamRepositoryPath.packages repo_root prefix nv)
  with
  | None -> Done OpamPackage.Map.empty
  | Some opam ->
      let add_to_cache ?name urlf errors =
        let label =
          OpamPackage.to_string nv ^ OpamStd.Option.to_string (( ^ ) "/") name
        in
        ...
        | first_checksum :: _ as checksums -> (
            OpamRepository.pull_file_to_cache label ~cache_dir checksums
              (OpamFile.URL.url urlf :: OpamFile.URL.mirrors urlf)
            @@| fun r ->
            match OpamRepository.report_fetch_result nv r with
            | Not_available (_, m) ->
            ...
            | Up_to_date () | Result () ->
                OpamStd.Option.iter
                  (fun link_dir ->
                    ...
                    let link =
                      OpamFilename.Op.(
                        link_dir / OpamPackage.to_string nv // name)
                    in
...
let cache_command =
  ...
  let man =
    [
      `S "DESCRIPTION";
      `P
        "Downloads the archives for all packages to fill a local cache, that \
  ...
  let cache_dir_arg =
    Arg.(
      value
      & pos 0 OpamArg.dirname (OpamFilename.Dir.of_string "./cache")
      & info [] ~docv:"DIR" ~doc:"Name of the cache directory to use.")
  in
  let no_repo_update_arg =
    Arg.(
      value & flag
      & info [ "no-repo-update"; "n" ]
          ~doc:
  ...
  let link_arg =
    Arg.(
      value
      & opt (some OpamArg.dirname) None
      & info [ "link" ] ~docv:"DIR"
          ~doc:
            ( Printf.sprintf
                "Create reverse symbolic links to the archives within \
                 $(i,DIR), in the form $(b,DIR%sPKG.VERSION%sFILENAME)."
                Filename.dir_sep Filename.dir_sep
            |> Cmdliner.Manpage.escape ))
  in
  let jobs_arg =
    Arg.(
      value
      & opt OpamArg.positive_integer 8
      & info [ "jobs"; "j" ] ~docv:"JOBS" ~doc:"Number of parallel downloads")
  in
  let cmd global_options cache_dir no_repo_update link jobs =
    ...
    let errors =
      OpamParallel.reduce ~jobs ~nil:OpamPackage.Map.empty
  ...
  in
  ( Term.(
      const cmd $ OpamArg.global_options $ cache_dir_arg $ no_repo_update_arg
      $ link_arg $ jobs_arg),
    OpamArg.term_info command ~doc ~man )

let add_hashes_command_doc = "Add archive hashes to an opam repository."
...
let add_hashes_command =
      ...
      `P
        (Printf.sprintf
           "This command scans through package definitions, and add hashes as \
            requested (fetching the archives if required). A cache is \
            generated in %s for subsequent runs."
  ...
  let hash_types_arg =
    let hash_kind_conv =
      Arg.enum (List.map (fun k -> (OpamHash.string_of_kind k, k)) hash_kinds)
    in
    Arg.(
      non_empty & pos_all hash_kind_conv []
      & info [] ~docv:"HASH_ALGO" ~doc:"The hash, or hashes to be added")
  in
  let packages =
    Arg.(
      value
      & opt (list OpamArg.package) []
      & info ~docv:"PACKAGES" ~doc:"Only add hashes for the given packages"
          [ "p"; "packages" ])
  in
  let replace_arg =
    Arg.(
      value & flag
      & info [ "replace" ]
          ~doc:"Replace the existing hashes rather than adding to them")
  in
  let hash_tables =
        ...
        List.iter
          (fun k2 ->
            if k1 <> k2 then (
              ...
              |> List.iter @@ function
                 | [ src; dst ] ->
                     Hashtbl.add t_mapping (OpamHash.of_string src)
                       (OpamHash.of_string dst)
                 | _ ->
                     failwith ("Bad cache at " ^ OpamFile.to_string cache_file)
              );
  ...
  let save_hashes () =
    Hashtbl.iter
      (fun _ (file, tbl) ->
        Hashtbl.fold
          (fun src dst l ->
            [ OpamHash.to_string src; OpamHash.to_string dst ] :: l)
          tbl []
        |> fun lines ->
        try OpamFile.Lines.write file lines
        with e ->
          OpamStd.Exn.fatal e;
          OpamConsole.log "ADMIN"
            "Could not write hash cache to %s, skipping (%s)"
            (OpamFile.to_string file) (Printexc.to_string e))
      hash_tables
  in
  ...
  let get_hash cache_urls kind known_hashes url =
    ...
    | None ->
        let h =
          OpamProcess.Job.run @@ OpamFilename.with_tmp_dir_job
          @@ fun dir ->
  ...
  let cmd global_options hash_types replace packages =
    ...
    let cache_urls =
      ...
      List.map
        (fun rel ->
          if OpamStd.String.contains ~sub:"://" rel then OpamUrl.of_string rel
          else
            OpamUrl.Op.(
              OpamUrl.of_string (OpamFilename.Dir.to_string repo_root) / rel))
    ...
    let pkg_prefixes =
      ...
      if packages = [] then pkgs_map
        ...
        let pkgs_map, missing_pkgs =
          List.fold_left
            (fun ((map : string option OpamPackage.Map.t), error) (n, vo) ->
              match vo with
              | Some v -> (
              ...
              | None ->
                  ...
                  if OpamPackage.Map.is_empty n_map then (map, (n, vo) :: error)
                  else
                    ( OpamPackage.Map.union
                        (fun _nv _nv' -> assert false)
                        n_map map,
                      error ))
        ...
        (fun nv prefix has_error ->
          ...
          let process_url has_error urlf =
            ...
            let hashes =
              if replace then
                List.filter
                  (fun h -> List.mem (OpamHash.kind h) hash_types)
                  hashes
              else hashes
            in
            let has_error, hashes =
              List.fold_left
                (fun (has_error, hashes) kind ->
                  if List.exists (fun h -> OpamHash.kind h = kind) hashes then
                    (has_error, hashes)
                  else
                    match
                      get_hash cache_urls kind hashes (OpamFile.URL.url urlf)
                    with
          ...
          let has_error, extra_sources =
            List.fold_right
              (fun (basename, urlf) (has_error, acc) ->
                let has_error, urlf = process_url has_error urlf in
  ...
  in
  ( Term.(
      const cmd $ OpamArg.global_options $ hash_types_arg $ replace_arg
      $ packages),
    OpamArg.term_info command ~doc ~man )

let upgrade_command_doc = "Upgrades repository from earlier opam versions."
...
let upgrade_command =
      ...
      `P
        ( Printf.sprintf
            "This command reads repositories from earlier opam versions, and \
             converts them to repositories suitable for the current opam \
             version. Packages might be created or renamed, and any compilers \
             defined in the old format ('compilers%s' directory) will be \
             turned into packages, using a pre-defined hierarchy that assumes \
             OCaml compilers."
            Filename.dir_sep
        |> Cmdliner.Manpage.escape );
    ]
    ...
    let doc =
      Printf.sprintf
        "Instead of running the upgrade, clear the cache of archive hashes \
         (held in ~%s.cache), that is used to avoid re-downloading files to \
  ...
  let cmd global_options clear_cache create_mirror =
    ...
    if clear_cache then OpamAdminRepoUpgrade.clear_cache ()
      ...
      | None ->
          ...
          if
            OpamFilename.exists (OpamFilename.of_string "index.tar.gz")
  ...
  in
  ( Term.(
      const cmd $ OpamArg.global_options $ clear_cache_arg $ create_mirror_arg),
    OpamArg.term_info command ~doc ~man )

let lint_command_doc = "Runs 'opam lint' and reports on a whole repository"
...
let lint_command =
  ...
  let man =
    [
      `S "DESCRIPTION";
      `P
        "This command gathers linting results on all files in a repository. \
         The warnings and errors to show or hide can be selected";
    ]
  in
  let short_arg =
  ...
  in
  let include_arg =
    OpamArg.arg_list "INT" "Show only these warnings" OpamArg.positive_integer
  in
  let exclude_arg =
    OpamArg.mk_opt_all [ "exclude"; "x" ] "INT"
      "Exclude the given warnings or errors" OpamArg.positive_integer
  in
  let ignore_arg =
    ...
    let ret =
      OpamPackage.Map.fold
        (fun nv prefix ret ->
          ...
          if List.exists (fun (n, _, _) -> List.mem n ign) w then ret
          else
            let w =
              List.filter
                (fun (n, _, _) ->
            ...
            in
            if w <> [] then
              if list then OpamConsole.msg "%s\n" (OpamPackage.to_string nv)
              else if short then
                OpamConsole.msg "%s %s\n" (OpamPackage.to_string nv)
  ...
  in
  ( Term.(
      const cmd $ OpamArg.global_options $ short_arg $ list_arg $ include_arg
      $ exclude_arg $ ignore_arg $ warn_error_arg),
    OpamArg.term_info command ~doc ~man )

let check_command_doc = "Runs some consistency checks on a repository"
...
let check_command =
  ...
  in
  let obsolete_arg =
    OpamArg.mk_flag [ "obsolete" ] "Analyse for obsolete packages"
  in
  let cmd global_options ignore_test print_short installability cycles obsolete
      =
    OpamArg.apply_global_options global_options;
    let repo_root = checked_repo_root () in
    let installability, cycles, obsolete =
      if installability || cycles || obsolete then
        (installability, cycles, obsolete)
      else (true, true, false)
    in
    let pkgs, unav_roots, uninstallable, cycle_packages, obsolete =
      OpamAdminCheck.check ~quiet:print_short ~installability ~cycles ~obsolete
        ~ignore_test repo_root
    in
    let all_ok =
    ...
    ( if print_short then
      ...
      in
      OpamConsole.msg
        "Summary: out of %d packages (%d distinct names)\n%s%s%s%s\n"
        (OpamPackage.Set.cardinal pkgs)
  ...
  in
  ( Term.(
      const cmd $ OpamArg.global_options $ ignore_test_arg $ print_short_arg
        ...
        ~doc:
          ( Printf.sprintf
              "Use the given opam environment, in the form of a list of \
               ...
               This is used e.g. when computing available packages: if \
               undefined, availability of packages will be assumed as soon as \
               it can not be resolved purely from globally defined variables. \
               Note that, unless overridden, variables like 'root' or \
               'opam-version' may be taken from the current opam installation. \
               What is defined in $(i,~%s.opam%sconfig) is always ignored."
              Filename.dir_sep Filename.dir_sep
          |> Cmdliner.Manpage.escape ))
...
let state_selection_arg =
  ...
  Arg.(
    value
    & vflag OpamListCommand.Available
        [
          ( OpamListCommand.Any,
            info ~docs [ "A"; "all" ]
          ...
          ( OpamListCommand.Available,
            info ~docs [ "a"; "available" ]
              ~doc:
                "List only packages that are available according to the \
                 defined $(b,environment). Without $(b,--environment), this \
                 will include any packages for which availability is not \
                 resolvable at this point." );
          ( OpamListCommand.Installable,
            info ~docs [ "installable" ]
              ~doc:
                "List only packages that are installable according to the \
                 defined $(b,environment) (this calls the solver and may be \
                 more costly; a package depending on an unavailable one may be \
...
let get_virtual_switch_state repo_root env =
  ...
  let gt =
    {
      ...
      config =
        OpamStd.Option.Op.(
          OpamStateConfig.(load !r.root_dir) +! OpamFile.Config.empty);
  ...
  let repos_tmp =
    ...
    Hashtbl.add t repo.repo_name (lazy repo_root);
    t
  in
  let rt =
    {
  ...
  in
  OpamSwitchState.load_virtual ~repos_list:[ repo.repo_name ]
    ~avail_default:(env = []) gt rt

let or_arg =
  Arg.(
    value & flag
    & info ~docs:OpamArg.package_selection_section [ "or" ]
        ~doc:
          "Instead of selecting packages that match $(i,all) the criteria, \
           select packages that match $(i,any) of them")
...
let list_command =
  ...
  in
  let cmd global_options package_selection disjunction state_selection
    ...
    let filter =
      OpamFormula.ands
        [
          ...
          join (pattern_selector :: List.map (fun x -> Atom x) package_selection);
        ]
    in
    ...
    let results = OpamListCommand.filter ~base:st.packages st filter in
    OpamListCommand.display st format results
  in
  ( Term.(
      const cmd $ OpamArg.global_options $ OpamArg.package_selection $ or_arg
...
let filter_command =
  ...
  let man =
    [
      `S "DESCRIPTION";
      `P
        "This command removes all package definitions that don't match the \
  ...
  in
  let cmd global_options package_selection disjunction state_selection env
...
let add_constraint_command =
  ...
  let man =
    [
      `S "DESCRIPTION";
      `P
        "This command searches to all dependencies towards a given package, \
         and adds a version constraint to them. It is particularly useful to \
         add upper bounds to existing dependencies when a new, incompatible \
         major version of a library is added to a repository. The new version \
  ...
  let atom_arg =
    Arg.(
      required
      & pos 0 (some OpamArg.atom) None
      & info [] ~docv:"PACKAGE"
          ~doc:
            "A package name with a version constraint, e.g. \
             $(b,name>=version). If no version constraint is specified, the \
             command will just simplify existing version constraints on \
             dependencies to the named package.")
  in
  let force_arg =
    Arg.(
      value & flag
          ...
          if force then f
            ...
            OpamConsole.warning
              ...
              (OpamConsole.colorise `bold
                 (OpamFilter.string_of_filtered_formula (Atom (n, f))));
            c )
    in
    OpamPackage.Map.iter
      (fun nv prefix ->
        ...
        let deps =
          OpamFormula.map
            (function
              | (n, c) as atom ->
                  if n = name then Atom (n, add_cstr nv n c) else Atom atom)
            deps0
        in
        if deps <> deps0 then
...
let admin_subcommands =
  [
    index_command;
    OpamArg.make_command_alias index_command "make";
    cache_command;
    upgrade_command;
    lint_command;
    check_command;
  ...
  let usage global_options =
    ...
    OpamConsole.formatted_msg
      ...
      \    upgrade-format %s\n\n\
       ...
       command.\n"
      index_command_doc cache_command_doc upgrade_command_doc
  in
  ( Term.(const usage $ OpamArg.global_options),
    Term.info "opam admin"
      ~version:(OpamVersion.to_string OpamVersion.current)
      ~sdocs:OpamArg.global_option_section ~doc:admin_command_doc ~man )
