...
let deprecated_option option absent name instead =
  if option <> absent then
    OpamConsole.warning "Option %s is deprecated, ignoring it.%s" name
      ( match instead with
      | None -> ""
      | Some instead -> Printf.sprintf " You can use %s instead." instead )

let create_global_options git_version debug debug_level verbose quiet color
    opt_switch yes strict opt_root external_solver use_internal_solver cudf_file
    solver_preferences best_effort safe_mode json no_auto_upgrade working_dir
    ignore_pin_depends d_no_aspcud =
  deprecated_option d_no_aspcud false "no-aspcud" None;
  let debug_level =
    OpamStd.Option.Op.(debug_level >>+ fun () -> if debug then Some 1 else None)
  in
...
let apply_global_options o =
  ...
  let solver =
    if o.use_internal_solver then
      Some
        ( lazy
          (OpamCudfSolver.get_solver ~internal:true
             OpamCudfSolver.default_solver_selection) )
    else o.external_solver >>| fun s -> lazy (OpamCudfSolver.solver_of_string s)
  in
  let solver_prefs = o.solver_preferences >>| fun p -> lazy (Some p) in
  OpamClientConfig.opam_init (* - format options - *)
    ?strict:
      (flag o.strict)
    ...
    ?verbose_level:
      ( if o.quiet then Some 0
      else if o.verbose = 0 then None
      else Some o.verbose )
    ?color:
      o.color
    ...
    ?safe_mode:
      ...
      (* - solver options *)
    ?cudf_file:(some o.cudf_file) ?solver ?best_effort:(flag o.best_effort)
    ...
    ?solver_preferences_fixup:
      ...
      (* - state options - *) ?root_dir:o.opt_root
    ?current_switch:(o.opt_switch >>| OpamSwitch.of_string)
    ?switch_from:
      ( o.opt_switch >>| fun _ -> `Command_line )
      (* ?jobs: int *)
      (* ?dl_jobs: int *)
      (* ?keep_build_dir:bool *)
      (* ?build_test:bool *)
    ...
    ?json_out:
      OpamStd.Option.Op.(o.json >>| function "" -> None | s -> Some s)
      (* ?root_is_ok:bool *)
    ?no_auto_upgrade:(flag o.no_auto_upgrade) (* - client options - *)
...
let create_build_options keep_build_dir reuse_build_dir inplace_build make
    no_checksums req_checksums build_test build_doc show dryrun skip_update fake
...
let apply_build_options b =
  ...
  (* ?retries:int *)
    ?force_checksums:
      ( if b.req_checksums then Some (Some true)
      else if b.no_checksums then Some (Some false)
      else None )
    ();
  OpamStateConfig.update (* ?root: -- handled globally *)
    ?jobs:
      OpamStd.Option.Op.(b.jobs >>| fun j -> lazy j)
      (* ?dl_jobs:int *)
      (* ?no_base_packages:(flag o.no_base_packages) -- handled globally *)
    ?build_test:(flag b.build_test) ?build_doc:(flag b.build_doc)
    ?dryrun:(flag b.dryrun)
    ?makecmd:OpamStd.Option.Op.(b.make >>| fun m -> lazy m)
    ?ignore_constraints_on:
      OpamStd.Option.Op.(
        b.ignore_constraints_on >>| OpamPackage.Name.Set.of_list)
    ?unlock_base:(flag b.unlock_base)
    ?locked:(if b.locked then Some (Some b.lock_suffix) else None)
    ();
  OpamClientConfig.update ?keep_build_dir:(flag b.keep_build_dir)
    ?reuse_build_dir:(flag b.reuse_build_dir)
    ?inplace_build:(flag b.inplace_build) ?show:(flag b.show)
    ?fake:(flag b.fake) ?skip_dev_update:(flag b.skip_update) ()
...
let help_sections =
  [
    ...
    `P
      "$(i,OPAMBESTEFFORTPREFIXCRITERIA) sets the string that must be \
       prepended to the criteria when the `--best-effort` option is set, and \
       is expected to maximise the `opam-query` property in the solution ";
    `P
      "$(i,OPAMCOLOR), when set to $(i,always) or $(i,never), sets a default \
       value for the --color option.";
    `P
      "$(i,OPAMCRITERIA) specifies user $(i,preferences) for dependency \
    ...
    `P
      "$(i,OPAMCUDFFILE file) save the cudf graph to \
       $(i,file)-actions-explicit.dot";
    `P
      "$(i,OPAMCURL) can be used to select a given 'curl' program. See \
       $(i,OPAMFETCH) for more options.";
    `P "$(i,OPAMDEBUG) see options `--debug' and `--debug-level'.";
    `P
      "$(i,OPAMDEBUGSECTIONS) if set, limits debug messages to the \
       space-separated list of sections. Sections can optionally have a \
       specific debug level (for example, $(b,CLIENT:2) or $(b,CLIENT CUDF:2), \
       but otherwise use `--debug-level'.";
    `P
      "$(i,OPAMDOWNLOADJOBS) sets the maximum number of simultaneous downloads.";
    `P "$(i,OPAMDRYRUN) see option `--dry-run`";
    `P
      "$(i,OPAMEDITOR) sets the editor to use for opam file editing, overrides \
       $(i,\\$EDITOR) and $(i,\\$VISUAL)";
    `P
      "$(i,OPAMERRLOGLEN) sets the number of log lines printed when a \
       sub-process fails. 0 to print all.";
    `P "$(i,OPAMEXTERNALSOLVER) see option `--solver'.";
    `P "$(i,OPAMFAKE) see option `--fake`";
    `P
      "$(i,OPAMFETCH) specifies how to download files: either `wget', `curl' \
       or a custom command where variables $(b,%{url}%), $(b,%{out}%), \
       $(b,%{retry}%), $(b,%{compress}%) and $(b,%{checksum}%) will be \
       replaced. Overrides the 'download-command' value from the main config \
       file.";
    `P
      "$(i,OPAMFIXUPCRITERIA) same as $(i,OPAMUPGRADECRITERIA), but specific \
       to fixup";
    ...
    `P
      "$(i,OPAMJSON) log json output to the given file (use character `%' to \
       index the files)";
    `P "$(i,OPAMLOCKED) combination of `--locked` and `--lock-suffix` options";
    `P
      ( "$(i,OPAMLOGS logdir) sets log directory, default is a temporary \
         directory in "
      ^ if Sys.win32 then "%TEMP%" else "/tmp" );
    `P "$(i,OPAMMAKECMD) set the system make command to use";
    `P
      "$(i,OPAMNOAUTOUPGRADE) disables automatic internal upgrade of \
       repositories in an earlier format to the current one, on 'update' or \
       'init'.";
    `P
      "$(i,OPAMKEEPLOGS) tells opam to not remove some temporary command logs \
       and some backups. This skips some finalisers and may also help to get \
       more reliable backtraces";
    `P
      "$(i,OPAMLOCKRETRIES) sets the number of tries after which opam gives up \
    ...
    `P
      "$(i,OPAMDROPWORKINGDIR) overrides packages previously updated with \
       $(b,--working-dir) on update. Without this variable set, opam would \
       keep them unchanged unless explicitely named on the command-line.";
    `P "$(i,OPAMNOSELFUPGRADE) see option `--no-self-upgrade'.";
    `P
      "$(i,OPAMPINKINDAUTO) sets whether version control systems should be \
    ...
    `P
      "$(i,OPAMREQUIRECHECKSUMS) Enables option `--require-checksums' when \
       available (e.g. for `opam install`).";
    `P "$(i,OPAMRETRIES) sets the number of tries before failing downloads.";
    `P
      "$(i,OPAMROOT) see option `--root'. This is automatically set by `opam \
       env --root=DIR --set-root'.";
    ...
    `P
      "$(i,OPAMSKIPVERSIONCHECKS) bypasses some version checks. Unsafe, for \
    ...
    `P
      ( "$(i,OPAMSTATUSLINE) display a dynamic status line showing what's \
         currently going on on the terminal. (one of "
      ^ Arg.doc_alts_enum when_enum
      ^ ")" );
    `P "$(i,OPAMSTATS) display stats at the end of command";
    `P
      "$(i,OPAMSTRICT) fail on inconsistencies (file reading, switch import, \
       etc.)";
    `P
      "$(i,OPAMSWITCH) see option `--switch'. Automatically set by `opam env \
       --switch=SWITCH --set-switch'.";
    `P "$(i,OPAMUNLOCKBASE) see install option `--unlock-base`";
    `P
      "$(i,OPAMUPGRADECRITERIA) specifies user $(i,preferences) for dependency \
    ...
    `P
      ( "$(i,OPAMUTF8) use UTF8 characters in output (one of "
      ^ Arg.doc_alts_enum when_enum
      ^ "). By default `auto', which is determined from the locale)." );
    `P
      "$(i,OPAMUTF8MSGS) use extended UTF8 characters (camels) in opam \
       messages. Implies $(i,OPAMUTF8). This is set by default on OSX only.";
    `P
      "$(i,OPAMVALIDATIONHOOK hook) if set, uses the `%{hook%}` command to \
       validate an opam repository update";
    `P
      "$(i,OPAMVAR_var) overrides the contents of the variable $(i,var)  when \
       substituting `%{var}%` strings in `opam` files.";
    `P
      "$(i,OPAMVAR_package_var) overrides the contents of the variable \
       $(i,package:var) when substituting `%{package:var}%` strings in `opam` \
       files.";
  ...
  @ List.map
      (fun (reason, code) ->
        `I
          ( string_of_int code,
            match reason with
            | `Success -> "Success, or true for boolean queries."
            | `False ->
                "False. Returned when a boolean return value is expected, e.g. \
                 when running with $(b,--check), or for queries like $(b,opam \
                 lint)."
            | `Bad_arguments ->
                "Bad command-line arguments, or command-line arguments \
                 pointing to an invalid context (e.g. file not following the \
                 expected format)."
            | `Not_found ->
                "Not found. You requested something (package, version, \
                 repository, etc.) that couldn't be found."
            | `Aborted ->
                "Aborted. The operation required confirmation, which wasn't \
                 given."
            | `Locked ->
                "Could not acquire the locks required for the operation."
            | `No_solution ->
                "There is no solution to the user request. This can be caused \
                 by asking to install two incompatible packages, for example."
            | `File_error ->
                "Error in package definition, or other metadata files. Using \
                 $(b,--strict) raises this error more often."
            | `Package_operation_error ->
                "Package script error. Some package operations were \
                 unsuccessful. This may be an error in the packages or an \
                 incompatibility with your system. This can be a partial \
                 error."
            | `Sync_error ->
                "Sync error. Could not fetch some remotes from the network. \
                 This can be a partial error."
            | `Configuration_error ->
                "Configuration error. Opam or system configuration doesn't \
                 allow operation, and needs fixing."
            | `Solver_failure ->
                "Solver failure. The solver failed to return a sound answer. \
                 It can be due to a broken external solver, or an error in \
                 solver configuration."
            | `Internal_error ->
                "Internal error. Something went wrong, likely due to a bug in \
                 opam itself."
            | `User_interrupt ->
                "User interrupt. SIGINT was received, generally due to the \
  ...
  @ [
      ...
      `S "AUTHORS";
      `P "Vincent Bernardoff <vb@luminar.eu.org>";
      `Noblank;
      `P "Raja Boujbel       <raja.boujbel@ocamlpro.com>";
      `Noblank;
      ...
      `Noblank;
      `P "Fabrice Le Fessant <Fabrice.Le_fessant@inria.fr>";
      `Noblank;
      `P "Anil Madhavapeddy  <anil@recoil.org>";
      `Noblank;
      `P "Guillem Rieu       <guillem.rieu@ocamlpro.com>";
      `Noblank;
      `P "Ralf Treinen       <ralf.treinen@pps.jussieu.fr>";
      `Noblank;
...
let existing_filename_dirname_or_dash =
  let parse str =
    if str = "-" then `Ok None
      ...
      | None -> (
          match OpamFilename.opt_dir (OpamFilename.Dir.of_string str) with
          | Some d -> `Ok (Some (OpamFilename.D d))
          | None -> `Error (Printf.sprintf "File or directory %s not found" str)
          )
  in
  let print ppf gf =
    pr_str ppf
...
let package_name =
  let parse str =
    try `Ok (OpamPackage.Name.of_string str) with Failure msg -> `Error msg
  in
...
let positive_integer : int Arg.converter =
  ...
  let parser s =
    ...
    | `Ok n as r -> if n <= 0 then `Error "expected a positive integer" else r
  in
  (parser, printer)
...
let atom =
  ...
  in
  let print ppf atom = pr_str ppf (OpamFormula.short_string_of_atom atom) in
  (parse, print)

let atom_or_local =
  let parse str =
    if
      OpamStd.String.contains ~sub:Filename.dir_sep str
    ...
    then
      if OpamFilename.(exists (of_string str)) then
        ...
        `Error
          (Printf.sprintf
             "Not a valid package specification or existing file or directory: \
              %s"
             str)
    else
      match fst atom str with `Ok at -> `Ok (`Atom at) | `Error e -> `Error e
  in
...
let atom_or_dir =
  let parse str =
    match fst atom_or_local str with
    | `Ok (`Filename _) ->
        `Error
          (Printf.sprintf
             "Not a valid package specification or existing directory: %s" str)
...
let variable_bindings =
  ...
  let print ppf x =
    List.map
      (fun (a, b) -> Printf.sprintf "%s=%s" (OpamVariable.to_string a) b)
      x
    |> String.concat "," |> pr_str ppf
  in
  (parse, print)
  ...
  let parse str =
    ...
    let rec aux acc = function
      | `Delim d :: `Text n :: r ->
          ...
          let acc =
            List.fold_left (fun acc n -> (n, enabled) :: acc) acc nums
          in
          aux acc r
      | [] -> acc
      | _ -> raise Not_found
...
let enum_with_default sl : 'a Arg.converter =
  ...
  let parse s = match parse s with `Ok _ as x -> x | _ -> `Ok (`default s) in
  (parse, print)
  ...
  let parse str =
    if OpamStd.String.ends_with ~suffix:":" str then
      ...
      try
        List.find
          (function
            | OpamListCommand.Field _, _ -> false | _, name -> name = str)
          OpamListCommand.field_names
        |> fun (f, _) -> `Ok f
      with Not_found ->
        `Error
          (Printf.sprintf
             "No known printer for column %s. If you meant an opam file field, \
              use '%s:' instead (with a trailing colon)."
             str str)
  in
  let print ppf field =
...
let mk_subdoc ?(defaults = []) commands =
  ...
  :: List.map
       (function
         | "", name ->
             `P (Printf.sprintf "Without argument, defaults to %s." (bold name))
         | arg, default ->
             `I
               ( it arg,
                 Printf.sprintf "With a %s argument, defaults to %s %s."
...
let mk_subcommands_aux my_enum commands =
  let command =
    ...
    let commands =
      List.fold_left (fun acc (c, f, _, _) -> (c, f) :: acc) [] commands
    in
    Arg.(value & pos 0 (some & my_enum commands) None & doc)
  in
  let params =
...
let mk_subcommands commands = mk_subcommands_aux Arg.enum commands
...
let make_command_alias cmd ?(options = "") name =
  ...
  let man =
    [
      `S "DESCRIPTION";
      `P
        (Printf.sprintf "$(mname)$(b, %s) is an alias for $(mname)$(b, %s%s)."
           name orig options);
...
let bad_subcommand subcommands (command, usersubcommand, userparams) =
  ...
  | Some usersubcommand -> (
      ...
      match
        List.find_all (fun (_, cmd, _, _) -> cmd = usersubcommand) subcommands
      with
      | [ (name, _, args, _doc) ] ->
          let usage =
            Printf.sprintf "%s %s [OPTION]... %s %s" exe command name
              (String.concat " " args)
          in
          if List.length userparams < List.length args then
            `Error
              ( false,
                Printf.sprintf "%s: Missing argument.\nUsage: %s\n" exe usage )
          else
            `Error
              ( false,
                Printf.sprintf "%s: Too many arguments.\nUsage: %s\n" exe usage
              )
      | _ -> `Error (true, Printf.sprintf "Invalid %s subcommand" command) )
...
let shell_opt =
  ...
  mk_opt [ "shell" ] "SHELL"
    ...
    (Arg.some (Arg.enum enum))
    None

let dot_profile_flag =
  mk_opt [ "dot-profile" ] "FILENAME"
    ( Printf.sprintf
        "Name of the configuration file to update instead of $(i,~%s.profile) \
         or $(i,~%s.zshrc) based on shell detection."
...
let repo_kind_flag =
  ...
  in
  let aliases_kinds = [ ("wget", `http); ("curl", `http); ("rsync", `rsync) ] in
  mk_opt [ "k"; "kind" ] "KIND"
    (Printf.sprintf "Specify the kind of the repository to be used (%s)."
       (Arg.doc_alts_enum main_kinds))
    Arg.(some (enum (main_kinds @ aliases_kinds)))
    None
...
let jobs_flag =
  mk_opt [ "j"; "jobs" ] "JOBS"
    ...
    Arg.(some positive_integer)
    None

let name_list = arg_list "PACKAGES" "List of package names." package_name
  ...
  arg_list "PACKAGES"
    ( Printf.sprintf
        "List of package names, with an optional version or constraint, e.g \
         ...
         package description, with explicit directory (e.g. `.%ssrcdir' or \
         `.')"
        Filename.dir_sep
    |> Cmdliner.Manpage.escape )
    atom_or_dir
...
let param_list = arg_list "PARAMS" "List of parameters." Arg.string
...
let global_options =
  ...
  let verbose =
    Arg.(
      value & flag_all
      & info ~docs:section [ "v"; "verbose" ]
          ~doc:
            "Be more verbose. One $(b,-v) shows all package commands, repeat \
             to also display commands called internally (e.g. $(i,tar), \
             $(i,curl), $(i,patch) etc.) Repeating $(i,n) times is equivalent \
             to setting $(b,\\$OPAMVERBOSE) to \"$(i,n)\".")
  in
  let quiet = mk_flag ~section [ "q"; "quiet" ] "Disables $(b,--verbose)." in
  let color =
  ...
  in
  let switch =
    mk_opt ~section [ "switch" ] "SWITCH"
      "Use $(docv) as the current compiler switch. This is equivalent to \
       setting $(b,\\$OPAMSWITCH) to $(i,SWITCH)."
      Arg.(some string)
      None
  in
  let yes =
    mk_flag ~section [ "y"; "yes" ]
      "Answer yes to all yes/no questions without prompting. This is \
       equivalent to setting $(b,\\$OPAMYES) to \"true\"."
  in
  let strict =
  ...
  in
  let root =
    mk_opt ~section [ "root" ] "ROOT"
      "Use $(docv) as the current root path. This is equivalent to setting \
       $(b,\\$OPAMROOT) to $(i,ROOT)."
      Arg.(some dirname)
      None
  in
  let d_no_aspcud = mk_flag ~section [ "no-aspcud" ] "Deprecated." in
  let use_internal_solver =
    mk_flag ~section [ "use-internal-solver" ]
      "Disable any external solver, and use the built-in one (this requires \
       ...
       to setting $(b,\\$OPAMNOASPCUD) or $(b,\\$OPAMUSEINTERNALSOLVER)."
  in
  let external_solver =
    mk_opt ~section [ "solver" ] "CMD"
      (Printf.sprintf
         "Specify the CUDF solver to use for resolving package installation \
          ...
          %%{timeout}%%. This is equivalent to setting \
          $(b,\\$OPAMEXTERNALSOLVER)."
         (OpamStd.List.concat_map ", "
  ...
  let solver_preferences =
    mk_opt ~section [ "criteria" ] "CRITERIA"
      "Specify user $(i,preferences) for dependency solving for this run. \
       ...
       For details on the supported language, and the external solvers \
       available, see $(i, http://opam.ocaml.org/doc/External_solvers.html). A \
       general guide to using solver preferences can be found at $(i,  \
  ...
  in
  Term.(
    const create_global_options
    $ git_version $ debug $ debug_level $ verbose $ quiet $ color $ switch $ yes
    $ strict $ root $ external_solver $ use_internal_solver $ cudf_file
    $ solver_preferences $ best_effort $ safe_mode $ json_flag $ no_auto_upgrade
    $ working_dir $ ignore_pin_depends $ d_no_aspcud)
...
let locked section =
  mk_flag ~section [ "locked" ]
    "In commands that use opam files found from pinned sources, if a variant \
     ...
     This is typically useful to offer a more specific set of dependencies and \
     reproduce similar build contexts, hence the name. The $(i, lock)option \
     can be used to generate such files, based on the versions of the \
     dependencies currently installed on the host. This is equivalent to \
     setting the $(b,\\$OPAMLOCKED) environment variable. Note that this \
...
let build_options =
  ...
  let keep_build_dir =
    mk_flag ~section [ "b"; "keep-build-dir" ]
      "Keep the build directories after compiling packages. This is equivalent \
       to setting $(b,\\$OPAMKEEPBUILDDIR) to \"true\"."
  in
  let reuse_build_dir =
  ...
  let inplace_build =
    mk_flag ~section [ "inplace-build" ]
      "When compiling a package which has its source bound to a local \
       ...
       affects packages that are explicitly listed on the command-line. This \
       is equivalent to setting $(b,\\$OPAMINPLACEBUILD) to \"true\"."
  in
  let no_checksums =
    mk_flag ~section [ "no-checksums" ]
      "Do not verify the checksum of downloaded archives.This is equivalent to \
       setting $(b,\\$OPAMNOCHECKSUMS) to \"true\"."
  in
  let req_checksums =
  ...
  let fake =
    mk_flag ~section [ "fake" ]
      "This option registers the actions into the opam database, without \
       actually performing them. WARNING: This option is dangerous and likely \
       to break your opam environment. You probably want `--dry-run'. You've \
       been $(i,warned)."
  in
  ...
  Term.(
    const create_build_options $ keep_build_dir $ reuse_build_dir
    $ inplace_build $ make $ no_checksums $ req_checksums $ build_test
    $ build_doc $ show $ dryrun $ skip_update $ fake $ jobs_flag
    ...
    & info [ "assume-built" ]
        ~doc:
          "For use on locally-pinned packages: assume they have already been \
           ...
           worked on. Implies $(i,--inplace-build). No locally-pinned packages \
           will be skipped.")
...
let package_selection =
  ...
  let conflicts_with =
    ...
    Arg.(
      value
      & opt_all (list package_with_version) []
  ...
  let resolve =
    let doc =
      "Restrict to a solution to install (comma-separated) $(docv), $(i,i.e.) \
       ...
       consistency. Without `--installed`, the answer is self-contained and \
       independent of the current installation. With `--installed', it's \
       computed from the set of currently installed packages. `--no-switch` \
       further makes the solution independent from the currently pinned \
       packages, architecture, and compiler version. The combination with \
  ...
  in
  let post =
    mk_flag [ "post" ] ~section "Include dependencies tagged as $(i,post)."
  in
  let dev =
    mk_flag [ "dev" ] ~section "Include development packages in dependencies."
  in
  let doc_flag =
    mk_flag [ "doc"; "with-doc" ] ~section "Include doc-only dependencies."
  in
  let test =
    mk_flag
      [ "t"; "test"; "with-test" ]
      ~section "Include test-only dependencies."
  in
  let field_match =
  ...
  let has_flag =
    mk_opt_all [ "has-flag" ] "FLAG" ~section
      ...
      ( (fun s ->
          ...
          | Pkgflag_Unknown s ->
              `Error
                ( "Invalid package flag " ^ s ^ ", must be one of "
                ^ OpamStd.List.concat_map " " string_of_pkg_flag
                    all_package_flags )
          | f -> `Ok f),
        fun fmt flag -> Format.pp_print_string fmt (string_of_pkg_flag flag) )
  in
  let has_tag =
    mk_opt_all [ "has-tag" ] "TAG" ~section
      "Only includes packages which have the given tag set" Arg.string
  in
  let filter depends_on required_by conflicts_with coinstallable_with resolve
      recursive depopts nobuild post dev doc_flag test field_match has_flag
      has_tag =
    let dependency_toggles =
      {
        OpamListCommand.recursive;
        depopts;
        build = not nobuild;
        post;
        test;
    ...
    @ List.map
        ...
        field_match
    @ List.map
        (fun deps -> OpamListCommand.Depends_on (dependency_toggles, deps))
        depends_on
    @ List.map
        (fun rdeps -> OpamListCommand.Required_by (dependency_toggles, rdeps))
        required_by
    @ List.map (fun pkgs -> OpamListCommand.Conflicts_with pkgs) conflicts_with
    @ List.map
        (fun deps -> OpamListCommand.Solution (dependency_toggles, deps))
        resolve
    @ List.map
        (fun pkgs ->
  ...
  in
  Term.(
    const filter $ depends_on $ required_by $ conflicts_with
    $ coinstallable_with $ resolve $ recursive $ depopts $ nobuild $ post $ dev
...
let package_listing =
  ...
  let all_versions =
    mk_flag [ "all-versions"; "V" ] ~section
  ...
  let columns =
    mk_opt [ "columns" ] "COLUMNS" ~section
      (Printf.sprintf
         "Select the columns to display among: %s.\n\
          The default is $(b,name) when $(i,--short) is present and %s \
          otherwise."
         (OpamStd.List.concat_map ", "
            (fun (_, f) -> Printf.sprintf "$(b,%s)" f)
            OpamListCommand.field_names)
         (OpamStd.List.concat_map ", "
            (fun f ->
              Printf.sprintf "$(b,%s)" (OpamListCommand.string_of_field f))
            OpamListCommand.default_list_format))
      Arg.(some & opamlist_columns)
      None
  in
  let normalise =
    mk_flag [ "normalise" ] ~section
      "Print the values of opam fields normalised"
  in
  let wrap =
    mk_flag [ "wrap" ] ~section
  ...
  let separator =
    Arg.(
      value & opt string " "
      & info [ "separator" ] ~docv:"STRING" ~docs:package_listing_section
    ...
    let columns =
      ...
      | None ->
          ...
          if all_versions then
            List.map
              (function
                | OpamListCommand.Name -> OpamListCommand.Package | c -> c)
              cols
          else cols
    in
    {
      OpamListCommand.short;
      header = not short;
      columns;
      all_versions;
  ...
  in
  Term.(
    const format $ all_versions $ print_short $ sort $ columns $ normalise
    $ wrap $ separator)
