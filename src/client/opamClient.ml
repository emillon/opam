...
let orphans ?changes ?(transitive = false) t =
  ...
  let orphans =
    ...
    | None -> orphans
    | Some ch ->
        if OpamPackage.Set.is_empty orphans then orphans
        else
          let recompile_cone =
            OpamPackage.Set.of_list
            @@ OpamSolver.reverse_dependencies ~depopts:true ~installed:true
                 ~unavailable:true ~build:true ~post:false universe ch
          in
  ...
  let full_partition orphans =
    let orphan_names =
      (* names for which there is no version left *)
      OpamPackage.Name.Set.diff allnames
        (OpamPackage.names_of_packages (all -- orphans))
    in
  ...
  let full_orphans, orphan_versions =
    if not transitive then (full_orphans, orphan_versions)
    else
      let rec add_trans full_orphans orphan_versions =
        ...
        let new_orphans =
          ...
          @@ OpamSolver.reverse_dependencies ~depopts:false ~installed:false
               ~unavailable:true ~build:true ~post:false universe full_orphans
        in
        let full, versions = full_partition (new_orphans ++ orphan_versions) in
        if OpamPackage.Set.equal full_orphans full then (full, versions)
  ...
  let t =
    if changes = None then t
    else
      let available_packages =
        lazy (Lazy.force t.available_packages ++ (t.installed -- orphans))
      in
      { t with available_packages }
  in
  log "Orphans: (changes: %a, transitive: %b) -> full %a, versions %a"
    (slog @@ OpamStd.Option.to_string OpamPackage.Set.to_string)
    changes transitive
    (slog @@ OpamPackage.Name.Set.to_string @* OpamPackage.names_of_packages)
    full_orphans
    (slog OpamPackage.Set.to_string)
    orphan_versions;
...
let get_installed_atoms t atoms =
  List.fold_left
    (fun (packages, not_installed) atom ->
      try
        let nv = OpamPackage.Set.find (OpamFormula.check atom) t.installed in
        (nv :: packages, not_installed)
      with Not_found -> (packages, atom :: not_installed))
    ([], []) atoms
...
let update_dev_packages_t atoms t =
  ...
  if Unix.time () -. last_update > too_old then
    OpamConsole.note
      "It seems you have not updated your repositories for a while. Consider \
       updating them with:\n\
       %s\n"
      (OpamConsole.colorise `bold "opam update");

  if OpamClientConfig.(!r.skip_dev_update) then t
  else
    let working_dir = OpamClientConfig.(!r.working_dir || !r.inplace_build) in
    let to_update =
      List.fold_left
        (fun to_update (name, _) ->
          try
    ...
    in
    if OpamPackage.Set.is_empty to_update then t
    else (
      ...
      try
        ...
        let _success, t, _pkgs =
          OpamUpdate.dev_packages t ?working_dir to_update
        in
        OpamConsole.msg "\n";
        t
      with e ->
...
let compute_upgrade_t ?(strict_upgrade = true) ?(auto_install = false) ~all
        ...
        | n, None when strict_upgrade -> (
            try
             ...
             a fixed version) *)
              ...
              if
                OpamSwitchState.is_dev_package t nv
              ...
              then (n, None)
                ...
                if
                  OpamPackage.Set.exists (OpamFormula.check atom)
  ...
  let requested_installed, not_installed =
    List.fold_left
      (fun (packages, not_installed) ((n, _) as atom) ->
        try
          let nv = OpamPackage.Set.find (fun nv -> nv.name = n) t.installed in
  ...
  if all then
    ...
    ( names,
      OpamSolution.resolve t Upgrade
        ...
        ~requested:names ~reinstall:t.reinstall
        (OpamSolver.request ~install:to_install
    ...
    let upgrade_atoms =
      ...
      List.map
        (fun nv ->
          let name = nv.name in
          try (name, List.assoc name atoms) with Not_found -> (name, None))
...
let upgrade_t ?strict_upgrade ?auto_install ?ask ?(check = false)
    ?(terse = false) ~all atoms t =
  log "UPGRADE %a"
  ...
  | requested, Conflicts cs ->
      ...
      if cycles <> [] then (
        OpamConsole.error
          "Dependency errors in the upgrade actions. Please update, and report \
           the following to the package maintainers if the error persists:";
        OpamConsole.errmsg "%s\n%s\n"
          (OpamStd.Format.itemize (fun x -> x) cycles)
          "You may try upgrading packages individually to work around this." )
      else (
        OpamConsole.warning
          "Upgrade is not possible because of conflicts or packages that are \
           no longer available:";
        OpamConsole.errmsg "%s" (OpamStd.Format.itemize (fun x -> x) reasons);
        if chains <> [] then
          OpamConsole.errmsg "The following dependencies are the cause:\n%s"
            (OpamStd.Format.itemize (fun x -> x) chains);
        OpamConsole.errmsg
          "\n\
           You may run \"opam upgrade --fixup\" to let opam fix the current \
           state.\n" );
      OpamStd.Sys.exit_because `No_solution
  | requested, Success solution ->
      if check then
        OpamStd.Sys.exit_because
          (if OpamSolver.solution_is_empty solution then `False else `Success)
      else
        ...
        if result = Nothing_to_do then (
          ...
          if OpamPackage.Set.is_empty notuptodate then
                ...
                (fun unopt_pkg map ->
                  let set =
                    OpamSwitchState.conflicts_with t
                      (OpamPackage.Set.singleton unopt_pkg)
                      latest
                  in
                  OpamPackage.Set.fold
                    (fun installed_pkg map ->
                        ...
                        (fun unopt_pkg map ->
                          OpamFormula.fold_left
                            (fun map (n, formula) ->
                              ...
                              then
                                OpamPackage.Map.update unopt_pkg
                                  (OpamStd.List.cons (latest_pkg, formula))
                                  [] map
                              else map)
                            map depends_formula)
                        unopt map)
            ...
            if OpamConsole.verbose () && not (OpamPackage.Set.is_empty unav)
            then (
                ...
                (OpamStd.Format.itemize
                   (fun p ->
                     Printf.sprintf "%s.%s: %s"
                       ...
                       (OpamSwitchState.unavailable_reason t
                          ~default:
                            "unavailable for unknown reasons (this may be a \
                             bug in opam)"
            ...
            if not (OpamPackage.Set.is_empty unopt) then (
              let bullet =
                OpamConsole.(
                  colorise `red (utf8_symbol Symbols.asterisk_operator "--"))
                ^ " "
              in
              let string_dep pkg map reason =
                List.fold_right
                  (fun (p, f) acc ->
                    Printf.sprintf "%s\n%s%s is installed and %s %s" acc bullet
                      (OpamPackage.to_string p) reason
...
let fixup t =
  ...
  let resolve pkgs =
    ( pkgs,
      OpamSolution.resolve t Upgrade ~orphans:all_orphans
  ...
  let requested, solution =
    let s =
      ...
      resolve
        ((t.installed_roots %% t.installed) -- full_orphans ++ orphan_versions)
    in
    if is_success s then s
    else
      let s =
...
let update gt ~repos_only ~dev_only ?(all = false) names =
  ...
  let packages, ignore_packages =
    if repos_only then (OpamPackage.Set.empty, OpamPackage.Set.empty)
      ...
      OpamPackage.Set.iter
        (fun nv ->
          OpamConsole.note
            "%s has previously been updated with --working-dir, not resetting \
  ...
  in
  if remaining <> [] then
    OpamConsole.error "Unknown repositories or installed packages: %s"
      (String.concat ", " remaining);

  (* Do the updates *)
  let rt_before = rt in
  let repo_update_failure, rt =
    if repo_names = [] then ([], rt)
    else
  ...
  let (dev_update_success, dev_changed), st =
    if OpamPackage.Set.is_empty packages then ((true, false), st)
    else
      OpamSwitchState.with_write_lock st @@ fun st ->
      let working_dir =
        if OpamClientConfig.(!r.working_dir) && names <> [] then
          Some
            (OpamPackage.packages_of_names packages
      ...
      let success, st, updates =
        OpamUpdate.dev_packages st ?working_dir packages
      in
...
let init_checks ?(hard_fail_exn = true) init_config =
  ...
  let check_external_dep name = OpamSystem.resolve_command name <> None in
  ...
  OpamConsole.msg "%s.%s\n"
    ...
    ( if unavailable_repos = [] then " Perfect!"
    else
      "\n"
      ^ OpamStd.Format.itemize
          (fun (cmd, msg) ->
            Printf.sprintf
              "you won't be able to use %s repositories unless you install the \
               %s command on your system."
              msg
              (OpamConsole.colorise `bold cmd))
          unavailable_repos );
  let soft_fail =
    if OpamCudfSolver.has_builtin_solver () then false
    else
      ...
      if not (List.exists check_external_dep external_solvers) then (
        OpamConsole.error
          "No external solver found. You should get one of %s, or use a \
           ...
           http://opam.ocaml.org/doc/External_solvers.html for details)"
  ...
  let env v =
    ...
    OpamStd.Option.Op.(
      OpamStd.Option.of_Not_found (List.assoc vs) OpamSysPoll.variables
      >>= Lazy.force)
  in
  let filter_tools =
    OpamStd.List.filter_map (fun (cmd, str, oflt) ->
        ...
        | None -> Some (cmd, str)
        | Some flt ->
            if OpamFilter.eval_to_bool env flt then Some (cmd, str) else None)
  in
  ...
  let hard_fail =
    let msg =
      if hard_fail_exn then OpamConsole.error else OpamConsole.warning
    in
    check_tool
      (fun s ->
        msg
          "Missing dependencies -- the following commands are required for \
           opam to operate:";
        OpamConsole.errmsg "%s" s)
      required_deps
  in

  if hard_fail && hard_fail_exn then
...
let init ~init_config ~interactive ?repo ?(bypass_checks = false) ?dot_profile
    ?update_config ?env_hook ?(completion = true) shell =
  log "INIT %a"
    (slog @@ OpamStd.Option.to_string OpamRepositoryBackend.to_string)
    repo;
  let root = OpamStateConfig.(!r.root_dir) in
  let config_f = OpamPath.config root in
  let root_empty =
    (not (OpamFilename.exists_dir root)) || OpamFilename.dir_is_empty root
  in
  ...
  let gt, rt, default_compiler =
    if OpamFile.exists config_f then (
      ...
      try
        ...
        let custom_scripts =
          ...
          OpamStd.List.filter_map
            (fun ((nam, scr), oflt) ->
              ...
              | None -> Some (nam, scr)
              | Some flt ->
                  if OpamFilter.eval_to_bool env flt then Some (nam, scr)
                  else None)
            scripts
        in
        ...
        let failed, rt =
          OpamRepositoryCommand.update_with_auto_upgrade rt (List.map fst repos)
        in
        if failed <> [] then
          OpamConsole.error_and_exit `Sync_error
            "Initial download of repository failed";
        ( gt,
          OpamRepositoryState.unlock ~cleanup:false rt,
...
let check_conflicts t atoms =
  ...
  let full_orphans, full_orphans_with_local_data =
    OpamPackage.Set.partition has_no_local_data full_orphans
  in
  let orphan_versions, orphan_versions_with_local_data =
    OpamPackage.Set.partition
      (fun nv ->
        has_no_local_data nv || OpamPackage.has_name available_changes nv.name)
      orphan_versions
  in
      ...
      (fun ((name, _) as a) ->
        ...
        not
          (OpamPackage.Set.exists
             (OpamFormula.check a) (* real check *)
             (Lazy.force available)))
      atoms
  in
  if conflict_atoms <> [] then
    OpamConsole.error_and_exit `Not_found
      "Sorry, these packages are no longer available from the repositories: %s"
      (OpamStd.Format.pretty_list
    ...
    ( {
        ...
        available_packages =
          lazy
            ( Lazy.force t.available_packages
            ++ full_orphans_with_local_data ++ orphan_versions_with_local_data
            );
      },
      full_orphans,
      orphan_versions )
...
let assume_built_restrictions t atoms =
  let installed_fixed, not_installed_fixed =
    let rec all_deps set pkgs =
      ...
      let deps =
        OpamPackage.Set.of_list
          (OpamSolver.dependencies ~build:false ~post:true ~depopts:false
             ~installed:false ~unavailable:true universe pkgs)
      in
      let deps = deps -- pkgs in
      if OpamPackage.Set.is_empty deps then set else all_deps (set ++ deps) deps
    in
    let pkg_of_atoms =
      OpamPackage.Set.filter
  ...
  let atoms =
    atoms
    @ OpamSolution.eq_atoms_of_packages
  ...
  let t =
    let avp =
      OpamPackage.Set.filter
        (fun p ->
          not (List.exists (fun a -> OpamFormula.check a p) not_installed_fixed))
        (Lazy.force t.available_packages)
    in
...
let filter_unpinned_locally t atoms f =
  OpamStd.List.filter_map
    (fun at ->
      let n, _ = at in
      if
        OpamSwitchState.is_pinned t n
        && OpamStd.Option.Op.(
             OpamPinned.package_opt t n
      ...
      then Some (f at)
        ...
        log
          "Package %a is not pinned locally and assume built option is set, \
           skipping"
          (slog OpamPackage.Name.to_string)
          n;
        None ))
    atoms
...
let install_t t ?ask atoms add_to_roots ~deps_only ~assume_built =
  ...
  in
  let pkg_skip, pkg_new = get_installed_atoms t atoms in
  let pkg_reinstall =
          ...
          if OpamPackage.Set.mem nv t.installed then
            ...
            | Some true ->
                if OpamPackage.Set.mem nv t.installed_roots then
                  OpamConsole.note "Package %s is already installed as a root."
                    (OpamPackage.Name.to_string nv.name);
                {
                  t with
                  installed_roots = OpamPackage.Set.add nv t.installed_roots;
                }
            | Some false ->
  ...
  if t.installed_roots <> current_roots then (
    ...
    ( if not (OpamPackage.Set.is_empty diff) then
      ...
      OpamConsole.msg "Adding %s to the list of installed roots.\n"
        (OpamStd.Format.pretty_list diff)
    else
      let diff = current_roots -- t.installed_roots in
      let diff = OpamPackage.Set.elements diff in
      let diff = List.rev (List.rev_map OpamPackage.to_string diff) in
      OpamConsole.msg "Removing %s from the list of installed roots.\n"
        (OpamStd.Format.pretty_list diff) );
  ...
  if pkg_new = [] && OpamPackage.Set.is_empty pkg_reinstall then t
  else
    let t, atoms =
      if assume_built then assume_built_restrictions t atoms else (t, atoms)
    in
    ...
    let solution =
      ...
      OpamSolution.resolve t Install
        ~orphans:(full_orphans ++ orphan_versions)
        ~requested:names ?reinstall request
    in
    let t, solution =
      match solution with
      | Conflicts cs ->
          ...
          let solution =
            if deps_only then
              OpamSolver.filter_solution
                (fun nv -> not (OpamPackage.Name.Set.mem nv.name names))
                solution
            else solution
          in
          let add_roots =
            OpamStd.Option.map
              (function true -> names | false -> OpamPackage.Name.Set.empty)
              add_to_roots
          in
          let t, res =
            OpamSolution.apply ?ask t ~requested:names ?add_roots ~assume_built
              solution
          in
          (t, Success res)
    in
...
let install t ?autoupdate ?add_to_roots ?(deps_only = false)
    ?(assume_built = false) names =
  let atoms = OpamSolution.sanitize_atom_list ~permissive:true t names in
  let autoupdate_atoms =
...
let check_installed t atoms =
  ...
  let pkgs =
    OpamPackage.to_map (OpamFormula.packages_of_atoms available atoms)
  in
  ...
  OpamPackage.Name.Map.fold
    (fun name versions map ->
      let compliant, missing_opt =
        OpamPackage.Version.Set.fold
          (fun version (found, missing) ->
            if found then (found, missing)
            else
              let pkg = OpamPackage.create name version in
              let cnf_formula =
                OpamSwitchState.opam t pkg |> OpamFile.OPAM.depends
...
let remove_t ?ask ~autoremove ~force atoms t =
  log "REMOVE autoremove:%b %a" autoremove
    (slog OpamFormula.string_of_atoms)
    atoms;

  let t, full_orphans, orphan_versions =
    let changes =
      if autoremove then None
      else Some (OpamSwitchState.packages_of_atoms t atoms)
    in
    orphans ?changes t
  in
  ...
  if autoremove || packages <> [] then (
    ...
    let to_remove =
      OpamPackage.Set.of_list
        (OpamSolver.reverse_dependencies ~build:true ~post:true ~depopts:false
           ~installed:true universe packages)
    in
    let to_keep =
      (if autoremove then t.installed_roots %% t.installed else t.installed)
      ++ universe.u_base -- to_remove -- full_orphans -- orphan_versions
    in
    let to_keep =
      OpamPackage.Set.of_list
        (OpamSolver.dependencies ~build:true ~post:true ~depopts:true
           ~installed:true universe to_keep)
    in
    ...
    let to_remove =
      if autoremove then
        ...
        if atoms = [] then to_remove
          ...
          %% OpamPackage.Set.of_list
               (OpamSolver.dependencies ~build:true ~post:true ~depopts:true
                  ~installed:true universe to_remove)
      else to_remove
    in
    let t, solution =
...
let reinstall_t t ?ask ?(force = false) ~assume_built atoms =
  ...
  let reinstall, not_installed = get_installed_atoms t atoms in
  let to_install =
    if not_installed <> [] then
      if
        force || assume_built
  ...
  let atoms = to_install @ OpamSolution.eq_atoms_of_packages reinstall in

  let t, full_orphans, orphan_versions = check_conflicts t atoms in

  let requested = OpamPackage.Name.Set.of_list (List.rev_map fst atoms) in
  ...
  let t, solution =
    OpamSolution.resolve_and_apply ?ask t Reinstall
      ...
      ~requested ~assume_built request
  in
...
module PIN = struct
  ...
  let post_pin_action st was_pinned names =
    let names =
      ...
      |> OpamPackage.names_of_packages
      |> (fun s ->
           List.fold_left (fun s p -> OpamPackage.Name.Set.add p s) s names)
      |> OpamPackage.Name.Set.elements
    in
    try
      upgrade_t ~strict_upgrade:false ~auto_install:true ~ask:true ~terse:true
        ~all:false
        (List.map (fun name -> (name, None)) names)
        st
    with e ->
      OpamConsole.note
        "Pinning command successful, but your installed packages may be out of \
         sync.";
      raise e
    ...
    try
      match
        OpamStd.Option.Op.(
          ...
          |> OpamSwitchState.opam_opt t >>= OpamFile.OPAM.dev_repo)
      with
      | None ->
          OpamConsole.error_and_exit `Not_found
      ...
      | Some url -> url
    with Not_found ->
      OpamConsole.error_and_exit `Not_found "No package named %S found"
        (OpamPackage.Name.to_string name)
  ...
  let edit st ?(action = true) ?version name =
    let pinned = st.pinned in
    let st =
      if OpamPackage.has_name st.pinned name then edit st ?version name
      else
        let pin_nv =
          match version with
          | Some v ->
        ...
        | Some nv ->
            ...
            then
              let target =
                OpamStd.Option.Op.(
                  OpamSwitchState.url st nv >>| OpamFile.URL.url)
              in
              let opam = OpamPackage.Map.find_opt nv st.repos_package_index in
              try source_pin st name ~edit:true ?version ?opam target with
              | OpamPinCommand.Aborted -> OpamStd.Sys.exit_because `Aborted
              | OpamPinCommand.Nothing_to_do -> st
            else OpamStd.Sys.exit_because `Aborted
        | None ->
    ...
    in
    if action then post_pin_action st pinned [ name ] else st
