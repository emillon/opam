...
let package_file_changes st packages =
  OpamPackage.Set.fold
    (fun nv acc ->
      let f = OpamPath.Switch.changes st.switch_global.root st.switch nv.name in
      match OpamFile.Changes.read_opt f with
      | None -> acc
      | Some ch -> OpamStd.String.Map.union (fun _ x -> x) acc ch)
    packages OpamStd.String.Map.empty
  ...
  |> List.iter @@ function
     | src, `Unchanged ->
         ...
         if not OpamStateConfig.(!r.dryrun) then
           if OpamFilename.exists src then OpamFilename.copy ~src ~dst
           else
             let as_dir f = OpamFilename.(Dir.of_string (to_string f)) in
             if OpamFilename.exists_dir (as_dir src) then
               OpamFilename.mkdir (as_dir dst)
...
let remove_files_from_destdir st pfx packages =
  package_file_changes st packages
  |> OpamStd.String.Map.bindings |> List.rev
  |> (* Get the longer names first, their parent folders last *)
  ...
  List.fold_left
    (fun acc (n, f) ->
      let name =
        ...
        n >>+ fun () ->
        OpamFile.OPAM.(name_opt (safe_read f)) >>+ fun () ->
        match files with
        | [] | _ :: _ :: _ -> None
...
let name_and_dir_of_opam_file f =
  ...
  let srcdir =
    if
      OpamFilename.dir_ends_with ".opam" srcdir
...
let resolve_locals ?(quiet = false) atom_or_local_list =
  let target_dir dir =
    ...
    OpamUrl.parse ?backend d |> url_with_local_branch
  in
  let to_pin, atoms =
    List.fold_left
      (fun (to_pin, atoms) -> function `Atom a -> (to_pin, a :: atoms)
        | `Dirname d ->
  ...
  if to_pin = [] then
    ...
    let pinning_dirs =
      OpamStd.List.filter_map
        (function `Dirname d -> Some d | _ -> None)
        atom_or_local_list
    in
    log "autopin: %a"
      ( slog
      @@ OpamStd.List.to_string (fun (name, target, _) ->
             Printf.sprintf "%s => %s"
...
let simulate_local_pinnings ?quiet ?(for_view = false) st to_pin =
  assert (not (for_view && OpamSystem.get_lock_flag st.switch_lock = `Lock_write)
  );
  let local_names =
    List.fold_left
      (fun set (name, _, _) -> OpamPackage.Name.Set.add name set)
      OpamPackage.Name.Set.empty to_pin
  in
  let local_opams =
    List.fold_left
      (fun map (name, target, file) ->
        match
      ...
      available_packages =
        lazy
          (OpamPackage.Set.union
             ...
             (OpamSwitchState.compute_available_packages st.switch_global
                st.switch st.switch_config ~pinned:st.pinned ~opams:local_opams));
      pinned;
    }
  in
  (st, local_packages)
...
let simulate_autopin st ?quiet ?(for_view = false) atom_or_local_list =
  ...
  in
  if to_pin = [] then (st, atoms)
  else
    let st =
      OpamPackage.Set.fold
        (fun nv st -> OpamPinCommand.unpin_one st nv)
        obsolete_pins st
    in
    ...
    ( if not for_view then
      ...
      let pin_depends =
        OpamPackage.Set.fold
          (fun nv acc ->
            List.fold_left
              (fun acc (nv, target) -> OpamPackage.Map.add nv target acc)
              acc
      ...
      if not (OpamPackage.Map.is_empty pin_depends) then (
        ...
        OpamConsole.note
          "The following may not reflect the above pinnings (their package \
           definitions are not available at this stage)";
        OpamConsole.msg "\n" ) );
    (st, atoms)
...
let autopin st ?(simulate = false) ?quiet atom_or_local_list =
  if OpamStateConfig.(!r.dryrun) || OpamClientConfig.(!r.show) then
    ...
    if
      to_pin = []
      && OpamPackage.Set.is_empty obsolete_pins
    ...
    then (st, atoms)
      ...
      in
      let st, pins =
        if simulate then simulate_local_pinnings ?quiet st to_pin
        else
          try
            List.fold_left
              (fun (st, pins) (name, target, file) ->
                match
                  OpamPinCommand.read_opam_file_for_pinning ?quiet name file
                    target
                with
                | None -> (st, pins)
                | Some opam ->
              ...
              (st, OpamPackage.Set.empty)
              to_pin
          with OpamPinCommand.Aborted -> OpamStd.Sys.exit_because `Aborted
      in
      let st =
        if OpamClientConfig.(!r.ignore_pin_depends) then st
        else
          OpamPackage.Set.fold
            (fun nv st ->
              OpamPinCommand.handle_pin_depends st nv
                (OpamSwitchState.opam st nv))
            (OpamPackage.Set.union pins already_pinned_set)
            st
      in
      (st, atoms)
...
let get_compatible_compiler ?repos rt dir =
  ...
  let virt_st = OpamSwitchState.load_virtual ?repos_list:repos gt rt in
      ...
      (fun acc (name, f) ->
        ...
        let nv, opam =
          ...
          | None ->
              ...
              (OpamPackage.create name v, OpamFile.OPAM.with_version v opam)
        in
        OpamPackage.Map.add nv opam acc)
      OpamPackage.Map.empty local_files
  in
  let local_packages = OpamPackage.keys local_opams in
  let pin_depends =
    OpamPackage.Map.fold
      (fun _nv opam acc ->
        List.fold_left
          (fun acc (nv, _) -> OpamPackage.Set.add nv acc)
          acc
  ...
  let virt_st =
    ...
    let open OpamPackage.Set.Op in
    {
      ...
      opams =
        OpamPackage.Set.fold
          (fun nv acc -> OpamPackage.Map.add nv (OpamFile.OPAM.create nv) acc)
          pin_depends opams;
      packages = virt_st.packages ++ local_packages ++ pin_depends;
      available_packages =
  ...
  let alldeps =
    OpamSolver.dependencies ~depopts:false ~build:true ~post:true
      ~installed:false univ local_packages
  in
  let compilers =
    OpamPackage.Set.filter
      (fun nv ->
  ...
  in
  if
    (not (OpamPackage.Set.is_empty local_packages))
  ...
  then (
    ...
    | Not_found when not (OpamPackage.Set.is_empty local_packages) ->
        ...
        if
          ...
          || OpamConsole.confirm
               "Create the switch with no specific compiler selected, and \
                attempt to continue?"
        then (None, false)
        else OpamStd.Sys.exit_because `Aborted
    | Failure _ | Not_found -> (
        (* Find a matching compiler from the default selection *)
        let default_compiler = OpamFile.Config.default_compiler gt.config in
        if default_compiler = Empty then (
          ...
          try
              ...
              OpamStd.List.find_map
                (fun atoms ->
                  let has_all compiler_packages =
                    List.for_all
                      (fun at ->
                        OpamPackage.Set.exists (OpamFormula.check at)
                          compiler_packages)
                      atoms
                  in
                  let compiler =
                    OpamFormula.packages_of_atoms
                      ...
                      atoms
                  in
                  if not (has_all compiler) then None
                  else if OpamPackage.Set.is_empty local_packages then
               ...
               coinstallability *)
                    let univ =
                      {
                        univ with
                        u_base = local_packages;
                        u_installed = local_packages;
                      }
                    in
                    let compiler =
                      OpamSolver.installable_subset univ compiler
                    in
            ...
            let candidate =
              ...
              |> fun s ->
                OpamPackage.(
                  max_version s (names_of_packages s |> Name.Set.choose)) )
          ...
          with Not_found ->
            ...
            if
              OpamConsole.confirm
                "You may also proceed, with no specific compiler selected. Do \
                 you want to?"
            then (None, false)
            else OpamStd.Sys.exit_because `Aborted )
