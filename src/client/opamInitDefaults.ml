...
let default_compiler =
  OpamFormula.ors
    [
      ...
      OpamFormula.Atom
        (OpamPackage.Name.of_string "ocaml-base-compiler", OpamFormula.Empty);
    ]
  ...
  [
    ...
    ( OpamVariable.of_string "sys-ocaml-arch",
      [
        "sh";
        "-c";
        "ocamlc -config | tr -d '\\r' | grep '^architecture: ' | sed -e 's/.*: \
         //' -e 's/i386/i686/' -e 's/amd64/x86_64/'";
      ],
    ...
    ( OpamVariable.of_string "sys-ocaml-cc",
      [
        "sh";
        "-c";
        "ocamlc -config | tr -d '\\r' | grep '^ccomp_type: ' | sed -e 's/.*: \
         //'";
      ],
    ...
    ( OpamVariable.of_string "sys-ocaml-libc",
      [
        "sh";
        "-c";
        "ocamlc -config | tr -d '\\r' | grep '^os_type: ' | sed -e 's/.*: //' \
         -e 's/Win32/msvc/' -e '/^msvc$/!s/.*/libc/'";
      ],
...
let not_open_free_bsd_filter = FNot (FOr (openbsd_filter, freebsd_filter))
...
let wrappers ~sandboxing () =
  let cmd t = [ (CString "%{hooks}%/sandbox.sh", None); (CString t, None) ] in
  let w = OpamFile.Wrappers.empty in
  if sandboxing then
    {
      w with
      OpamFile.Wrappers.wrap_build = [ (cmd "build", Some sandbox_filter) ];
...
let bwrap_string () =
  Printf.sprintf
    "Sandboxing tool %s was not found. You should install 'bubblewrap'. See \
...
let req_dl_tools () =
  ...
  let cmd =
    ( OpamStd.Env.getopt "OPAMFETCH" >>= fun s ->
      match OpamStd.String.split s ' ' with c :: _ -> Some c | _ -> None )
...
let dl_tool () =
  let open OpamStd.Option.Op in
  (OpamStd.Env.getopt "OPAMFETCH" >>+ fun () -> OpamStd.Env.getopt "OPAMCURL")
  >>| fun cmd -> [ (CString cmd, None) ]

let recommended_tools () =
  let make = OpamStateConfig.(Lazy.force !r.makecmd) in
  [ ([ make ], None, None); ([ "m4" ], None, None); ([ "cc" ], None, None) ]
