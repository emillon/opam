...
let list gt ~print_short =
  ...
  if print_short then
    ...
    match
      (OpamStateConfig.get_switch_opt (), OpamStateConfig.(!r.switch_from))
    with
    ...
    | Some switch, `Env -> (
        ...
        if not (OpamGlobalState.switch_exists gt switch) then (
          ...
          | Some sw ->
              OpamConsole.msg "\n";
              OpamConsole.note
                "Current switch is set globally and through the OPAMSWITCH \
                 variable.\n\
...
let clear_switch ?(keep_debug = false) gt switch =
  ...
  if keep_debug && (OpamClientConfig.(!r.keep_build_dir) || OpamConsole.debug ())
  then (
    OpamConsole.note
      "Keeping %s despite errors (debug mode), you may want to remove it by \
       hand"
...
let install_compiler_packages t atoms =
  ...
  if atoms = [] then (
    ...
    if not (OpamPackage.Name.Set.is_empty not_found) then
      OpamConsole.error_and_exit `Not_found "No packages %s found."
        (OpamPackage.Name.Set.to_string not_found);
    let solution =
      OpamSolution.resolve t Switch ~orphans:OpamPackage.Set.empty
        ~requested:roots
        {
          wish_install = [];
          wish_remove = [];
    ...
    in
    if
      (not (OpamPackage.Set.is_empty non_comp))
      && not
    ...
    let t, result =
      OpamSolution.apply
        ~ask:OpamClientConfig.(!r.show)
        t ~requested:roots solution
    in
    OpamSolution.check_solution
      ~quiet:OpamClientConfig.(not !r.show)
      t (Success result);
    t

let install gt ~rt ?synopsis ?repos ~update_config ~packages
    ...
    if not (OpamStateConfig.(!r.dryrun) || OpamClientConfig.(!r.show)) then
      ...
      ( gt,
        ...
        let available_packages =
          lazy
            (OpamSwitchState.compute_available_packages gt switch
               (OpamSwitchAction.gen_switch_config gt.root ?repos switch)
               ~pinned:OpamPackage.Set.empty ~opams:st.opams)
        in
        ...
        OpamStd.Option.iter
          (fun switch ->
            OpamSwitchState.drop
              (OpamSwitchAction.set_current_switch `Lock_write
                 ~rt:st.switch_repos gt switch))
          old_switch_opt );
...
let import_t ?ask importfile t =
  ...
  let available =
    OpamSwitchState.compute_available_packages t.switch_global t.switch
      t.switch_config ~pinned ~opams
  in

  let compiler_packages, to_install =
    if OpamPackage.Set.is_empty t.compiler_packages then
      (import_sel.sel_compiler %% available, import_sel.sel_installed)
    else
      (t.compiler_packages, import_sel.sel_installed -- import_sel.sel_compiler)
  in
  ...
  if not (OpamPackage.Set.is_empty unavailable_version) then
    OpamConsole.warning
      "These packages aren't available at the specified versions, version \
  ...
  if not (OpamPackage.Set.is_empty unavailable) then
    OpamConsole.warning
      "These packages are unavailable, they have been ignored from the import \
       file:\n\
       %s"
      (OpamStd.Format.itemize OpamPackage.to_string
  ...
  let t, solution =
    ...
    OpamSolution.resolve_and_apply ?ask t Import
      ...
      ~add_roots ~orphans:OpamPackage.Set.empty
      {
  ...
  if not (OpamStateConfig.(!r.dryrun) || OpamClientConfig.(!r.show)) then (
    ...
    OpamPackage.Set.iter
      (fun nv ->
        ...
        | Some opam ->
            OpamFilename.rmdir
              (OpamPath.Switch.Overlay.package t.switch_global.root t.switch
                 nv.name);
            OpamFile.OPAM.write
              (OpamPath.Switch.Overlay.opam t.switch_global.root t.switch
                 nv.name)
              opam)
      pinned;
    (* Save new pinnings *)
    ...
    (fun nv acc ->
      ...
      | Some opam ->
          if OpamFile.OPAM.extra_files opam <> None then (
            OpamConsole.warning
              "Metadata of package %s uses a files%s subdirectory, it may not \
               be re-imported correctly (skipping definition)"
...
let reinstall init_st =
  ...
  import_t
    {
      OpamFile.SwitchExport.selections = OpamSwitchState.selections init_st;
...
let set_compiler st namesv =
  ...
  in
  if uninstalled <> [] then (
    OpamConsole.warning "These packages are not installed:\n%s"
  ...
  let deps =
    List.map
      (fun nv ->
        ...
        OpamPackageVar.filter_depends_formula ~default:false
          ~env:
  ...
  if not (OpamPackage.Set.is_empty comp_deps) then
    OpamConsole.formatted_msg
      "Package%s %s do%sn't have the 'compiler' flag set, and may not be \
       suitable to set as switch base. You probably meant to choose among the \
       following compiler implementations, which they depend upon:\n\
       %s"
...
let guess_compiler_package ?repos rt name =
  ...
  let no_compiler_error () =
    OpamConsole.error_and_exit `Not_found
      "No compiler matching '%s' found, use 'opam switch list-available' to \
       see what is available, or use '--packages' to select packages \
       explicitly."
      name
  in
  ...
  | _ -> (
      let pkgname =
        try Some (OpamPackage.Name.of_string name) with Failure _ -> None
      in
      match pkgname with
      | Some pkgname when OpamPackage.has_name compiler_packages pkgname ->
          Some (pkgname, None)
      | Some pkgname
        when OpamPackage.Map.exists
               (fun nv _ -> OpamPackage.name nv = pkgname)
               opams ->
          advise_compiler_dependencies rt opams compiler_packages name
            [ (pkgname, None) ];
          no_compiler_error ()
      | _ -> (
          ...
          | Failure _ ->
              OpamConsole.error_and_exit `Bad_arguments
                ...
                name
                (OpamPackage.Set.to_string has_version) ) )
