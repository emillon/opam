...
let names_of_formula flag f =
  OpamPackageVar.filter_depends_formula ~build:true ~post:true ~dev:true
    ~test:flag ~doc:flag ~default:false ~env:OpamStd.Option.none f
  |> OpamFormula.atoms |> List.map fst |> OpamPackage.Name.Set.of_list
...
let filters_of_formula f =
  OpamFormula.fold_left
    (fun acc (_, f) ->
      OpamFormula.fold_left
        (fun acc -> function Constraint (_, f) -> f :: acc
          | Filter f -> f :: acc)
        acc f)
    [] f
...
let map_all_filters f t =
  let mapsnd x = List.map (fun (x, ft) -> (x, f ft)) x in
  let mapsndopt x =
    List.map (function x, Some ft -> (x, Some (f ft)) | nf -> nf) x
  in
  let map_commands =
    List.map (fun (args, filter) ->
        ( List.map (function s, Some ft -> (s, Some (f ft)) | nf -> nf) args,
...
let all_variables ?exclude_post ?command t =
  let commands =
    match command with Some cmd -> cmd | None -> all_commands t
  in
  OpamFilter.commands_variables commands
  @ List.fold_left
      (fun acc f -> OpamFilter.variables f @ acc)
      []
  ...
  let map_filtered_formula =
    OpamFormula.map (fun (name, fc) ->
        let fc =
          OpamFormula.map
            (function
              | Filter flt -> Atom (Filter (OpamFilter.map_variables f flt))
...
let all_expanded_strings t =
  ...
  @ List.fold_left
      ...
      [] (all_commands t)
  @ List.fold_left
      (OpamFilter.fold_down_left (fun acc ->
         function FString s -> s :: acc | _ -> acc))
      [] (all_filters t)
...
let template nv =
  let maintainer =
    let from_git =
      try
        ...
        with
        | [ name ], [ email ] -> Some [ Printf.sprintf "%s <%s>" name email ]
        | _ -> raise Not_found
      with e ->
        OpamStd.Exn.fatal e;
        None
    ...
    | Some u -> u
    | None -> (
        let email = try Some (Sys.getenv "EMAIL") with Not_found -> None in
        try
          let open Unix in
          ...
          let email =
            match email with
            | Some e -> e
            | None -> pw.pw_name ^ "@" ^ gethostname ()
          in
          match OpamStd.String.split pw.pw_gecos ',' with
          | name :: _ -> [ Printf.sprintf "%s <%s>" name email ]
          | _ -> [ email ]
        with Not_found -> ( match email with Some e -> [ e ] | None -> [] ) )
  in
  create nv |> with_name_opt None |> with_maintainer maintainer
  |> with_build
       [
         ( [
             (CString "./configure", None); (CString "--prefix=%{prefix}%", None);
           ],
           None );
  ...
  |> with_depends
       (Atom
          ( OpamPackage.Name.of_string "specify-dependencies-here",
...
let t_lint ?check_extra_files ?(check_upstream = false) ?(all = false) t =
  let format_errors =
    List.map
      (fun (field, (pos, msg)) ->
        ( 3,
          `Error,
          Printf.sprintf "File format error in '%s'%s: %s" field
            ( match pos with
  ...
  let cond num level msg ?detail cd =
    if cd || all then
      let msg =
        match detail with
        | Some d -> Printf.sprintf "%s: \"%s\"" msg (String.concat "\", \"" d)
        | None -> msg
      in
      Some (num, level, msg)
    else None
  ...
  let warnings =
    [
      cond 20 `Warning
        "Field 'opam-version' refers to the patch version of opam, it should \
      ...
      (let empty_fields =
         OpamStd.List.filter_map
           ...
           [
             ...
             ("doc", t.doc);
             ("tags", t.tags);
             ("bug_reports", t.bug_reports);
           ]
       in
       cond 22 `Error "Some fields are present but empty; remove or fill them"
         ~detail:empty_fields (empty_fields <> []));
      cond 23 `Error "Missing field 'maintainer'" (t.maintainer = []);
      cond 24 `Error "Field 'maintainer' has the old default value"
        ( List.mem "contact@ocamlpro.com" t.maintainer
        && not (List.mem "org:ocamlpro" t.tags) );
      cond 25 `Warning "Missing field 'authors'" (t.author = []);
      cond 26 `Warning
...
*)
      (let unk_flags =
         OpamStd.List.filter_map
           ...
           t.flags
       in
       cond 28 `Error "Unknown package flags found" ~detail:unk_flags
         (unk_flags <> []));
      (let filtered_vars =
       ...
       in
       cond 29 `Error "Package dependencies mention package variables"
         ~detail:filtered_vars (filtered_vars <> []));
      (*
    cond 30 `Error
...
*)
      (let dup_depends =
       ...
       cond 31 `Error
         ...
         ~detail:
           OpamPackage.Name.(List.map to_string (Set.elements dup_depends))
         (not (OpamPackage.Name.Set.is_empty dup_depends)));
      cond 32 `Error
        "Field 'ocaml-version:' and variable 'ocaml-version' are deprecated, \
         use a dependency towards the 'ocaml' package instead for \
         availability, and the 'ocaml:version' package variable for scripts"
        ( t.ocaml_version <> None
        || List.mem
             (OpamVariable.Full.of_string "ocaml-version")
             (all_variables t) );
      cond 33 `Error
      ...
      (let pkg_vars =
         List.filter
           (fun v -> not (OpamVariable.Full.is_global v))
       ...
       cond 34 `Error
         ...
         (pkg_vars <> []));
      cond 35 `Warning "Missing field 'homepage'" (t.homepage = []);
      (* cond (t.doc = []) *)
      (*   "Missing field 'doc'"; *)
      cond 36 `Warning "Missing field 'bug-reports'" (t.bug_reports = []);
      cond 37 `Warning "Missing field 'dev-repo'"
...
*)
      cond 39 `Error
        ...
        (List.exists
           (function (CString "make", _) :: _, _ -> true | _ -> false)
           all_commands);
      (*
    cond 40 `Warning
...
*)
      (let undep_pkgs =
         List.fold_left
           (fun acc v ->
             ...
             | Some n
               when t.OpamFile.OPAM.name <> Some n
      ...
      cond 44 `Warning
        "The 'plugin' package flag is set but the package name doesn't begin \
         with 'opam-'"
        ( OpamVersion.compare t.opam_version (OpamVersion.of_string "1.3") >= 0
      ...
      (let unclosed =
         List.fold_left
           (fun acc s -> List.rev_append (OpamFilter.unclosed_expansions s) acc)
           [] all_expanded_strings
       in
       cond 45 `Error "Unclosed variable interpolations in strings"
         ~detail:(List.map snd unclosed) (unclosed <> []));
      cond 46 `Error
           ...
           Re.(
             compile
               (seq
                  [
                    bos;
                    diff any (alt [ blank; lower ]);
                    rep any;
                    diff any (alt [ blank; char '.' ]);
                    eos;
                  ]))
         in
         match t.descr with
         | None -> false
         | Some d -> not (Re.execp valid_re (OpamFile.Descr.synopsis d)));
      cond 48 `Warning
        "The fields 'build-test:' and 'build-doc:' are deprecated, and should \
         be replaced by uses of the 'with-test' and 'with-doc' filter \
      ...
      (let suspicious_urls =
       ...
       cond 49 `Warning
         ...
         (suspicious_urls <> []));
      cond 50 `Warning
        "The 'post' flag doesn't make sense with build or optional dependencies"
        ( List.mem
            (OpamVariable.Full.of_string "post")
            (List.flatten
               (List.map OpamFilter.variables (filters_of_formula t.depopts)))
        || OpamFormula.fold_left
             (fun acc (_, f) ->
               acc
               ||
                    ...
                    | Filter fi ->
                        OpamFilter.fold_down_left
                          (fun acc fi ->
                            ...
                            | _ -> false)
                          false
                          (OpamFilter.distribute_negations fi)
                    | _ -> false)
                  false f)
           false
      ...
      (let mismatching_extra_files =
         ...
         | Some efiles, Some ffiles ->
             OpamStd.List.filter_map
               (fun (n, _) -> if List.mem_assoc n ffiles then None else Some n)
               efiles
             @ OpamStd.List.filter_map
                 (fun (n, check_f) ->
                 ...
                 ffiles
       in
       cond 53 `Error "Mismatching 'extra-files:' field"
      ...
      (let spaced_depexts =
         List.concat
           (List.map
              (fun (dl, _) ->
                List.filter
                  (fun d -> String.contains d ' ' || String.length d = 0)
                  dl)
              t.depexts)
       in
       cond 54 `Warning
         "External dependencies should not contain spaces nor empty string"
         ~detail:spaced_depexts (spaced_depexts <> []));
      (let bad_os_arch_values =
         List.fold_left
           ...
           [] (all_filters t)
       in
       cond 55 `Error "Non-normalised OS or arch string being tested"
         ~detail:
           (List.map
              (fun (used, norm) ->
                Printf.sprintf "%s (use %s instead)" used norm)
              bad_os_arch_values)
         (bad_os_arch_values <> []));
      cond 56 `Warning
        "It is discouraged for non-compiler packages to use 'setenv:'"
        (t.env <> [] && not (has_flag Pkgflag_Compiler t));
      cond 57 `Error "Synopsis and description must not be both empty"
      ...
      (let vars = all_variables ~exclude_post:false ~command:[] t in
       ...
       cond 58 `Warning
         (let var, s_, nvar =
            ...
            | true, true ->
                ("`test` and `doc`", "s", "s are `with-test` and `with-doc`")
               ...
               | chks ->
                   let not_corresponding =
                     List.filter
                       (fun chk ->
                         not
                           (OpamHash.check_file (OpamFilename.to_string f) chk))
                       chks
                   in
                   if not_corresponding = [] then None
                     ...
                     let msg =
                       Printf.sprintf "Cheksum%s %s don't verify archive"
                         ...
                         (OpamStd.List.to_string OpamHash.to_string
                            not_corresponding)
                     in
                     Some msg )
       in
      ...
      (let with_test =
         List.exists
           (( = ) (OpamVariable.Full.of_string "with-test"))
           (OpamFilter.commands_variables t.run_test)
       in
       cond 61 `Warning
         "`with-test` variable in `run-test` is out of scope, it will be \
          ignored"
         with_test);
      (let bad_licenses =
       ...
       in
       cond 62 `Warning
         "License doesn't adhere to the SPDX standard, see \
          https://spdx.org/licenses/"
         ~detail:bad_licenses (bad_licenses <> []));
    ]
  in
  format_errors @ OpamStd.List.filter_map (fun x -> x) warnings
...
let extra_files_default filename =
  let dir =
    OpamFilename.Op.(
      OpamFilename.dirname (OpamFile.filename filename) / "files")
  in
  List.map
    (fun f ->
...
let lint_gen ?check_extra_files ?check_upstream reader filename =
  let warnings, t =
    let warn_of_bad_format (pos, msg) =
      ( 2,
        `Error,
        Printf.sprintf "File format error%s: %s"
          ( match pos with
          | Some (_, li, col) when li >= 0 && col >= 0 ->
              Printf.sprintf " at line %d, column %d" li col
          | _ -> "" )
    ...
    try
      ...
      let _, t =
        OpamPp.parse
          ~pos:(pos_file (OpamFile.filename filename))
          (OpamFormat.I.map_file OpamFile.OPAM.pp_raw_fields)
          f
      in
      let t, warnings =
                    ...
                    ( t,
                      [
                        ( 4,
                          `Warning,
                          Printf.sprintf
                            "Field 'name: %S' while the directory name or \
                             pinning implied %S"
                            (OpamPackage.Name.to_string tname)
                            (OpamPackage.Name.to_string fname) );
                      ] )
              | None -> (OpamFile.OPAM.with_name fname t, [])
            in
            let t, version_warn =
              match t.OpamFile.OPAM.version with
              | Some tversion ->
                  if tversion = fversion then (t, [])
                  else
                    ( t,
                      [
                        ( 4,
                          `Warning,
                          Printf.sprintf
                            "Field 'version: %S' while the directory name or \
                             pinning implied %S"
             ...
             (Some
                ( None,
                  OpamFilename.Dir.to_string
                    (OpamFilename.dirname (OpamFile.filename filename)) ))
             t) )
    with
  ...
  ( ( warnings
    @
    match t with
    | Some t -> lint ~check_extra_files ?check_upstream t
    | None -> [] ),
    t )
...
let all_lint_warnings () = t_lint ~all:true OpamFile.OPAM.empty
...
let warns_to_json ?filename ws =
  let filename = match filename with Some f -> f | None -> "stdout" in
  let warn, err =
    List.fold_left
      (fun (w, e) (n, we, s) ->
        let arr =
          `O [ ("id", `Float (float_of_int n)); ("message", `String s) ]
        in
        match we with `Warning -> (arr :: w, e) | `Error -> (w, arr :: e))
      ([], []) ws
  in
  let result =
    match (warn, err) with
...
let add_aux_files ?dir ~files_subdir_hashes opam =
  ...
  | Some dir ->
      ...
      let opam =
        match
          (OpamFile.OPAM.url opam, try_read OpamFile.URL.read_opt url_file)
        with
      ...
      let opam =
        match
          (OpamFile.OPAM.descr opam, try_read OpamFile.Descr.read_opt descr_file)
        with
      ...
      in
      let opam =
        if not files_subdir_hashes then opam
        else
          let extra_files =
          ...
          | Some ef, None ->
              log "Missing expected extra files %s at %s/files"
                (OpamStd.List.concat_map ", "
                   (fun (f, _) -> OpamFilename.Base.to_string f)
                   ef)
...
let read_opam dir =
  ...
  | _, Some err ->
      OpamConsole.warning "Could not read file %s. skipping:\n%s"
