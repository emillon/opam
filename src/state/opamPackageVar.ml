...
let package_variable_names =
  [
    ...
    ( "enable",
      "Takes the value \"enable\" or \"disable\" depending on whether the \
    ...
    ( "build-id",
      "A hash identifying the precise package version with all its dependencies"
    );
  ]
...
let resolve_global gt full_var =
  ...
  if V.Full.(scope full_var <> Global) then None
  else
...
let resolve_switch ?package st full_var =
  resolve_switch_raw ?package st.switch_global st.switch st.switch_config
    full_var
...
let filter_depends_formula ?(build = true) ?(post = false)
    ?(test = OpamStateConfig.(!r.build_test))
    ?(doc = OpamStateConfig.(!r.build_doc)) ?(dev = false) ?default ~env ff =
  ff
  |> OpamFilter.partial_filter_formula (fun v ->
         if List.mem v predefined_depends_variables then None else env v)
...
let all_depends ?build ?post ?test ?doc ?dev ?(filter_default = false)
  ...
  in
  filter_depends_formula ?build ?post ?test ?doc ~dev ~default:filter_default
    ~env:(resolve_switch ~package:(OpamFile.OPAM.package opam) st)
    deps
...
let all_installed_deps st opam =
  ...
  OpamStd.List.filter_map
    (fun (n, cstr) ->
      try
        let nv = OpamPackage.Set.find (fun nv -> nv.name = n) st.installed in
...
let build_id st opam =
  ...
  let rec aux hash_map nv opam =
    try (hash_map, OpamPackage.Map.find nv hash_map)
    with Not_found -> (
      match OpamFile.OPAM.url opam with
...
let resolve st ?opam:opam_arg ?(local = OpamVariable.Map.empty) v =
  ...
  let read_package_var v =
    ...
    | OpamVariable.Full.Self -> OpamStd.Option.Op.(pkgname >>= get)
  in
  let get_local_var v =
    ...
    | None -> (
        ...
        try
          match OpamVariable.Map.find var local with
  ...
  let get_package_var v =
    if OpamVariable.Full.is_global v then None
      ...
      in
      let opam =
        (* ensure opam, if not None, corresponds to name *)
        match opam_arg with
      ...
      | "installed", None -> Some (bool false)
      | "pinned", _ -> Some (bool (OpamPackage.has_name st.pinned name))
      | "name", _ ->
          if OpamPackage.has_name st.packages name then
            Some (string (OpamPackage.Name.to_string name))
          else None
      | _, None -> None
      | "bin", _ ->
      ...
      | "lib", _ ->
          Some
            (dirname (OpamPath.Switch.lib root st.switch st.switch_config name))
      | "man", _ ->
          Some
            (dirname (OpamPath.Switch.man_dir root st.switch st.switch_config))
      | "doc", _ ->
          Some
            (dirname (OpamPath.Switch.doc root st.switch st.switch_config name))
      | "share", _ ->
          Some
            (dirname
               (OpamPath.Switch.share root st.switch st.switch_config name))
      | "etc", _ ->
          Some
            (dirname (OpamPath.Switch.etc root st.switch st.switch_config name))
      | "build", Some opam ->
          ...
          try
            ...
            if OpamFilename.exists f then
              Some
                (string
                   (OpamHash.to_string
                      (OpamHash.compute ~kind:`MD5 (OpamFilename.to_string f))))
            else Some (string "")
