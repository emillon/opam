  ...
  let check_marshaled_file fd =
    try
      ...
      if file_magic <> this_magic then (
        log "Bad cache: incompatible magic string %S (expected %S)." file_magic
          this_magic;
        None )
      else
        ...
        if expected_size <> current_size then (
          log "Bad cache: wrong length %d (advertised %d)." current_size
            expected_size;
          None )
        else (
  ...
  let rec aux r dir =
    if OpamFilename.exists_dir dir then
      ...
      if Array.fold_left (fun a f -> a || f = "opam") false fnames then (
        ...
        Array.fold_left
          (fun r name -> aux r OpamFilename.Op.(dir / name))
          r fnames
    else r
  in
...
let get_root rt name = get_root_raw rt.repos_global.root rt.repos_tmp name
...
let load lock_kind gt =
  ...
  OpamRepositoryName.Map.iter
    (fun name repo ->
      ...
      if
        (not (OpamFilename.exists_dir uncompressed_root))
        ...
        OpamRepositoryName.Map.fold
          (fun name url (defs, opams) ->
            ...
            ( OpamRepositoryName.Map.add name repo_def defs,
              OpamRepositoryName.Map.add name repo_opams opams ))
          repos_map
          (OpamRepositoryName.Map.empty, OpamRepositoryName.Map.empty)
      in
  ...
  List.fold_left
    (fun acc repo_name ->
      ...
      with Not_found ->
        ...
        acc)
    OpamPackage.Map.empty repo_list
...
let write_config rt =
  OpamFile.Repos_config.write
    (OpamPath.repos_config rt.repos_global.root)
