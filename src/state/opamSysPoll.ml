...
let normalise_arch raw =
  ...
  | a
    when List.exists
           (fun prefix -> OpamStd.String.starts_with ~prefix a)
...
let arch_lazy =
  lazy
    (let raw =
       ...
       | "Win32" ->
           if Sys.word_size = 32 && not (OpamStubs.isWoW64 ()) then Some "i686"
           else Some "x86_64"
       | _ -> None
     in
     match raw with None | Some "" -> None | Some a -> Some (normalise_arch a))
...
let os_lazy =
  lazy
    (let raw =
       match Sys.os_type with "Unix" -> OpamStd.Sys.uname "-s" | s -> norm s
     in
...
let is_android, android_release =
  ...
  ((fun () -> Lazy.force prop <> None), fun () -> Lazy.force prop)

let os_distribution_lazy =
  lazy
    ( match os () with
    | Some "macos" as macos ->
    ...
    | Some "linux" as linux -> (
        if is_android () then Some "android"
        else
...
let os_version_lazy =
  lazy
    ...
    | Some "macos" -> command_output [ "sw_vers"; "-productVersion" ] >>= norm
    | Some "win32" ->
    ...
    | Some "cygwin" -> (
        ...
        with Scanf.Scan_failure _ | End_of_file -> None )
    | Some "freebsd" -> OpamStd.Sys.uname "-U" >>= norm
    | _ -> OpamStd.Sys.uname "-r" >>= norm )
...
let os_family_lazy =
  lazy
    ...
    | Some "linux" ->
        ...
        >>= (fun s -> Scanf.sscanf s " %s" norm) (* first word *)
        >>+ os_distribution
...
let variables =
  List.map
    (fun (n, v) ->
      (OpamVariable.of_string n, lazy (Lazy.force v >>| fun v -> OpamTypes.S v)))
    [
      ("arch", arch_lazy);
      ("os", os_lazy);
