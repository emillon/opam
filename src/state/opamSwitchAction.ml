...
let gen_switch_config root ?(synopsis = "") ?repos _switch =
  let vars =
    List.map
      (fun (s, p) -> (OpamVariable.of_string s, S p))
      [
        ( "user",
  ...
  in
  {
    OpamFile.Switch_config.opam_version = OpamFile.Switch_config.format_version;
    synopsis;
    variables = vars;
    paths = [];
    opam_root = Some root;
...
let create_empty_switch gt ?synopsis ?repos switch =
  ...
  if OpamFilename.exists_dir switch_dir then
    failwith
      (Printf.sprintf "Directory %s already exists"
  ...
  try
    ...
    gt
  with e ->
    if not (OpamConsole.debug ()) then OpamFilename.rmdir switch_dir;
    raise e
...
let add_to_reinstall st ~unpinned_only packages =
  ...
  in
  let reinstall = current_reinstall ++ add_reinst_packages in
  ...
  else OpamFile.PkgList.write reinstall_file reinstall;
...
let install_metadata st nv =
  ...
  OpamFile.OPAM.write
    ...
    opam;
  List.iter
    (fun (f, rel_path, _hash) ->
      let dst =
        OpamFilename.create
          (OpamPath.Switch.installed_opam_files_dir st.switch_global.root
             st.switch nv)
          rel_path
      in
...
let remove_metadata st packages =
  OpamPackage.Set.iter
    (fun nv ->
      OpamFilename.rmdir
        (OpamPath.Switch.installed_package_dir st.switch_global.root st.switch
           nv))
    packages
...
let add_to_installed st ?(root = false) nv =
  ...
  in
  let st =
    { st with conf_files = OpamPackage.Name.Map.add nv.name conf st.conf_files }
  in
  if not OpamStateConfig.(!r.dryrun) then (
    install_metadata st nv;
    if OpamFile.OPAM.env opam <> [] && OpamSwitchState.is_switch_globally_set st
    then OpamEnv.write_dynamic_init_scripts st );
  st

let remove_from_installed ?(keep_as_root = false) st nv =
  let rm = OpamPackage.Set.remove nv in
  let st =
    update_switch_state st ~installed:(rm st.installed)
      ?installed_roots:
        (if keep_as_root then None else Some (rm st.installed_roots))
      ~reinstall:(rm st.reinstall)
  in
  let has_setenv =
    match
      OpamStd.Option.map OpamFile.OPAM.env (OpamSwitchState.opam_opt st nv)
    with
    | Some (_ :: _) -> true
    | _ -> false
  in
  if
    (not OpamStateConfig.(!r.dryrun))
    && has_setenv
