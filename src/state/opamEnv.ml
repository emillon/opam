...
let join_var l = String.concat (String.make 1 OpamStd.Sys.path_sep) l
...
let unzip_to elt =
  let rec aux acc = function
    | [] -> None
    | x :: r -> if x = elt then Some (acc, r) else aux (x :: acc) r
  in
  aux []
...
let rezip_to_string ?insert z = join_var (rezip ?insert z)

let apply_op_zip op arg ((rl1, l2) as zip) =
  let colon_eq ?(eqcol = false) = function
  ...
  | EqPlusEq -> (rl1, arg :: l2)
  | ColonEq ->
      let l, add = colon_eq (rezip zip) in
      ([], add @ l)
  | EqColon ->
...
let reverse_env_update op arg cur_value =
  match op with
  | Eq -> if arg = join_var cur_value then Some ([], []) else None
...
let updates_from_previous_instance =
  lazy
    ( match OpamStd.Env.getopt "OPAM_SWITCH_PREFIX" with
    | None -> None
    | Some pfx -> (
        let env_file =
          OpamPath.Switch.env_relative_to_prefix
            (OpamFilename.Dir.of_string pfx)
        in
...
let expand (updates : env_update list) : env =
  ...
  let rec apply_updates reverts acc = function
    ...
    | [] ->
        ...
        @@ List.rev_append
             (List.rev_map
                (fun (var, z, doc) -> (var, rezip_to_string z, doc))
                acc)
        @@ List.rev_map
             (fun (var, z) ->
...
let add (env : env) (updates : env_update list) =
  let env =
    if Sys.win32 then
      ...
      let updates =
        List.rev_map
          (fun (u, _, _, _) -> (String.uppercase_ascii u, "", "", None))
          updates
      in
      List.filter
        (fun (k, _, _) ->
          let k = String.uppercase_ascii k in
          List.for_all (fun (u, _, _, _) -> u <> k) updates)
        env
    else
      List.filter
        (fun (k, _, _) -> List.for_all (fun (u, _, _, _) -> u <> k) updates)
        env
  in
  env @ expand updates
        ...
        [
          ( "MANPATH",
            ...
            OpamFilename.Dir.to_string
              (OpamPath.Switch.man_dir st.switch_global.root st.switch
                 st.switch_config),
  ...
  let pkg_env =
    ...
    OpamPackage.Set.fold
      (fun nv acc ->
        ...
        | Some opam ->
            List.map (env_expansion ~opam) (OpamFile.OPAM.env opam) @ acc
        | None -> acc)
      st.installed []
  in
...
let get_opam_raw ?(set_opamroot = false) ?(set_opamswitch = false) ~force_path
  ...
  let upd =
    ( "OPAM_SWITCH_PREFIX",
      ...
      Some "Prefix of the current opam switch" )
    :: List.filter
         (function "OPAM_SWITCH_PREFIX", Eq, _, _ -> false | _ -> true)
         upd
  in
  let upd =
    if force_path then
      List.map
        (function
          | "PATH", EqPlusEq, v, doc -> ("PATH", PlusEq, v, doc) | e -> e)
        upd
    else
      List.map
        (function
          | "PATH", PlusEq, v, doc -> ("PATH", EqPlusEq, v, doc) | e -> e)
        upd
  in

  add [] (updates_common ~set_opamroot ~set_opamswitch root switch @ upd)

let get_full ?(set_opamroot = false) ?(set_opamswitch = false) ~force_path
    ?updates:(u = []) st =
...
let is_up_to_date_raw updates =
  ...
  let not_utd =
    List.fold_left
      (fun notutd ((var, op, arg, _doc) as upd) ->
        match OpamStd.Env.getopt var with
        | None -> upd :: notutd
        | Some v ->
  ...
  if not r then
    log "Not up-to-date env variables: [%a]"
      (slog @@ String.concat " " @* List.map (fun (v, _, _, _) -> v))
      not_utd
...
let eval_string gt ?(set_opamswitch = false) switch =
  ...
  | SH_fish -> Printf.sprintf "eval (opam env%s%s%s)" root switch setswitch
  | SH_csh -> Printf.sprintf "eval `opam env%s%s%s`" root switch setswitch
  | _ -> Printf.sprintf "eval $(opam env%s%s%s)" root switch setswitch
  ...
  let fish (k, v, comment) =
    ...
    let to_arr_string v =
      OpamStd.List.concat_map " "
        (fun v ->
          if v = Printf.sprintf "\"$%s\"" k then "$" ^ k (* remove quotes *)
          else v)
        (OpamStd.String.split v ':')
    in
    match k with
    | "PATH" ->
        Printf.sprintf "%sset -gx %s %s;\n" (make_comment comment) k
          (to_arr_string v)
    | "MANPATH" ->
...
let if_interactive_script shell t e =
  let ielse else_opt =
    match else_opt with None -> "" | Some e -> Printf.sprintf "else\n  %s" e
  in
  match shell with
  | SH_sh | SH_zsh | SH_bash ->
      Printf.sprintf "if [ -t 0 ]; then\n  %s%sfi\n" t @@ ielse e
  | SH_csh -> Printf.sprintf "if ( $?prompt ) then\n  %s%sendif\n" t @@ ielse e
  | SH_fish -> Printf.sprintf "if isatty\n  %s%send\n" t @@ ielse e
  ...
  let aux (ident, symbol, string, comment) =
    ...
    let key, value =
      ( ident,
        ...
        | EqColon | EqPlus -> Printf.sprintf "\"$%s\":'%s'" ident string )
    in
    export_in_shell shell (key, value, comment)
  in
...
let write_init_shell_scripts root =
  let scripts =
    List.map
      (fun shell -> (init_file shell, init_script root shell))
      shells_list
  in
...
let write_static_init_scripts root ?completion ?env_hook () =
  ...
  let update_scripts filef scriptf enable =
    ...
    in
    match enable with
    | Some true -> List.iter (write_script (OpamPath.init root)) scripts
    | Some false ->
        List.iter
          (fun (f, _) -> OpamFilename.remove (OpamPath.init root // f))
          scripts
    | None -> ()
  in
...
let write_custom_init_scripts root custom =
  ...
  List.iter
    (fun (name, script) ->
      ...
      if
        (not (OpamFilename.exists hash_file))
        || OpamHash.of_string_opt (OpamFilename.read hash_file)
           ...
           && OpamConsole.confirm ~default:false
                ...
                (OpamFilename.to_string script_file)
      then (
...
let write_dynamic_init_scripts st =
  ...
  try
    ...
    List.iter
      (fun shell ->
        write_script
          (OpamPath.init st.switch_global.root)
...
let dot_profile_needs_update root dot_profile =
  if not (OpamFilename.exists dot_profile) then `yes
  else
    ...
    let uncommented_re patts =
      Re.(
        compile
          (seq [ bol; rep (diff any (set "#:")); alt (List.map str patts) ]))
    in
...
let update_dot_profile root dot_profile shell =
  ...
  let bash_src () =
    if
      (shell = SH_bash || shell = SH_sh)
      && OpamFilename.(Base.to_string (basename dot_profile)) <> ".bashrc"
    then
  ...
  | `no ->
      OpamConsole.msg "  %s is already up-to-date.\n" pretty_dot_profile;
      bash_src ()
  | `otherroot ->
      OpamConsole.msg "  %s is already configured for another opam root.\n"
        pretty_dot_profile
  | `yes ->
      let init_file = init_file shell in
      let body =
        if OpamFilename.exists dot_profile then OpamFilename.read dot_profile
        else ""
      in
      ...
      let body =
        Printf.sprintf "%s\n\n# opam configuration\n%s"
          (OpamStd.String.strip body)
          (source root shell init_file)
      in
  ...
  then
    OpamConsole.formatted_msg
      ...
      (OpamConsole.colorise `bold
         (eval_string st.switch_global (Some st.switch)))

let setup root ~interactive ?dot_profile ?update_config ?env_hook ?completion
    shell =
  let update_dot_profile =
    ...
    | None, Some dot_profile, true -> (
        ...
        if OpamCoreConfig.(!r.answer = Some true) then (
          OpamConsole.warning
            "Shell not updated in non-interactive mode: use --shell-setup";
          None )
        else
          match
            OpamConsole.read
          ...
          | Some ("f" | "F" | "file" | "FILE") -> (
              ...
              | None ->
                  OpamConsole.msg
                    "Alright, assuming you changed your mind, not performing \
                     any changes.\n";
                  None
  ...
  in
  let env_hook =
    match (env_hook, interactive) with
    | Some b, _ -> Some b
    | None, false -> None
    | None, true ->
        Some
