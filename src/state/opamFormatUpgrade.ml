...
let upgrade_depexts_to_2_0_beta5 filename depexts =
  ...
  let rec transform_filter =
    ...
    | _ -> raise Exit
    (* the filter is already in the new format if it
...
let opam_file_from_1_2_to_2_0 ?filename opam =
  ...
  let get_atom ?(op = `Eq) v =
    if v = "system" then (ocaml_system_pkgname, Empty)
    else
      ...
      | FOp (FIdent ([], var, None), op, FString v) -> (
          ...
          | "ocaml-version", _ ->
              ( NMap.singleton ocaml_wrapper_pkgname (mk_constraint op v),
                NMap.empty,
                None )
          | "compiler", `Neq -> (NMap.empty, NMap.of_list [ get_atom v ], None)
          | "compiler", op -> (NMap.of_list [ get_atom ~op v ], NMap.empty, None)
          | _ -> (NMap.empty, NMap.empty, Some avail) )
      | FIdent ([], v, None) when OpamVariable.to_string v = "preinstalled" ->
          (NMap.singleton ocaml_system_pkgname Empty, NMap.empty, None)
      | FNot (FIdent ([], v, None))
        when OpamVariable.to_string v = "preinstalled" ->
          (NMap.empty, NMap.singleton ocaml_system_pkgname Empty, None)
      | FNot f ->
  ...
  let pkg_deps =
    if
      OpamVersion.compare (OpamFile.OPAM.opam_version opam) v2_0 >= 0
  ...
  in
  let available = OpamStd.Option.default (FBool true) available_opt in
  if
    List.exists
      (fun v ->
        match OpamVariable.Full.to_string v with
        | "ocaml-version" | "compiler" | "preinstalled" | "ocaml-native"
        | "ocaml-native-tools" | "ocaml-native-dynlink" ->
            true
        | _ -> false)
      (OpamFilter.variables available)
  then
    OpamConsole.warning
      "Could not translate some 'ocaml-*' variables in the 'available:' field \
       of %s: %s"
      filename
  ...
  let depends =
    ...
    in
    let remain =
      List.map (fun (name, cstr) -> Atom (name, cstr)) (NMap.bindings to_add)
    in
  ...
  let conflicts =
    let to_add, disj =
      List.fold_left
        (fun (to_add, disj) -> function
          | Atom (pkgname, cstr) as atom -> (
              try
                ( NMap.remove pkgname to_add,
                  Atom
                    (pkgname, OpamFormula.ors [ cstr; NMap.find pkgname to_add ])
                  :: disj )
  ...
  let filter_out_flagtags opam =
    OpamFile.OPAM.with_tags
      (List.filter
         (fun tag -> OpamFile.OPAM.flag_of_tag tag = None)
  ...
  let add_filter to_add cmdlist =
    List.map
      ...
      cmdlist
  in
  let test_filter = FIdent ([], OpamVariable.of_string "with-test", None) in
  let doc_filter = FIdent ([], OpamVariable.of_string "with-doc", None) in
  let build =
  ...
  in
  let install =
    OpamFile.OPAM.install opam @ add_filter doc_filter install_doc
  in
  let dev_repo =
    OpamStd.Option.map
  ...
  |> auto_add_flags |> filter_out_flagtags
...
let from_1_0_to_1_1 root _config =
  OpamConsole.error_and_exit `Configuration_error
    "You appear to have an opam setup dating back to opam 1.0, which is no \
     longer supported since opam 2.0. Please remove \"%s\" and run `opam init`"
    ...
    (fun switch _ ->
      ...
      let pinned_version name =
        ...
        with e -> (
          ...
          with Not_found -> OpamPackage.Version.of_string "0" )
      in
      let fix_version nv =
      ...
      List.iter
        (fun f ->
          let name =
            OpamFilename.Base.to_string @@ OpamFilename.basename
            @@ OpamFilename.chop_extension f
          in
...
let from_1_2_to_1_3_dev2 root config =
  ...
  OpamSwitch.Map.iter
    (fun switch c ->
      ...
      let installed = OpamFile.PkgList.safe_read (OpamFile.make installed_f) in
      let installed_roots =
        OpamFile.PkgList.safe_read (OpamFile.make installed_roots_f)
      in
      let pinned = OpamFile.Pinned_legacy.safe_read (OpamFile.make pinned_f) in
      let pinned =
      ...
      let compiler =
        let version =
          match OpamStd.String.cut_at c '+' with Some (v, _) -> v | None -> c
        in
        let comp =
          OpamFile.Comp.read
      ...
      in
      OpamFile.LegacyState.write
        (OpamFile.make (switch_dir // "state"))
        {
    ...
    (fun switch comp_name ->
      ...
      let selections =
        if comp_name <> "empty" then (
          let comp_f =
            OpamFile.make
              ( root / "compilers" / comp_version / comp_name
              // (comp_name ^ ".comp") )
          in
          ...
          in
          let comp_opam = OpamFile.Comp.to_package comp (Some descr) in
          let nv = OpamFile.OPAM.package comp_opam in
          let name = nv.name in
          let switch_config_f =
            OpamFile.make (switch_dir / "config" // "global-config.config")
          in
          ...
          let config =
            if OpamFile.Comp.preinstalled comp then
              let config =
                ...
                @@ [ ("compiler", S comp_name); ("preinstalled", B true) ]
              in
              let ocamlc =
                try
                  let path =
                    OpamStd.Env.get "PATH" |> OpamStd.Sys.split_path_variable
                    |> List.filter (fun s ->
                           not
              ...
              | Some ocamlc ->
                  ...
                  |> OpamFile.Dot_config.with_file_depends
                       [
                         ( ocamlc,
                           OpamHash.compute (OpamFilename.to_string ocamlc) );
                       ]
              ...
              let get_dir d =
                match
                  OpamFile.Dot_config.variable switch_config
                    (OpamVariable.of_string d)
                with
                | Some (S d) -> OpamFilename.Dir.of_string d
                | _ ->
                    OpamPath.Switch.get_stdpath root switch
                      OpamFile.Switch_config.empty (std_path_of_string d)
              in
              OpamFile.Dot_config.create
              @@ List.map (fun (v, c) -> (OpamVariable.of_string v, c))
              @@ [
                   ("ocaml-version", S (OpamFile.Comp.version comp));
                   ...
                   ( "ocaml-stubsdir",
                     S (OpamFilename.Dir.to_string (get_dir "stublibs")) );
                 ]
          in
          let config_f =
            OpamFile.make
...
let from_1_3_dev6_to_1_3_dev7 root conf =
  ...
  List.iter
    (fun switch ->
      ...
      OpamPackage.Set.iter
        (fun nv ->
          let dstdir = meta_dir / "packages" / OpamPackage.to_string nv in
          try
            let srcdir =
              List.find
                (fun d -> OpamFilename.exists (d // "opam"))
                [
...
let from_1_3_dev7_to_2_0_alpha root conf =
  ...
  in
  OpamFile.Repos_config.write
    (OpamPath.repos_config root)
    (OpamRepositoryName.Map.of_list
       (List.map (fun (_, r, u) -> (r, Some (u, None))) prio_repositories));
  let prio_repositories =
    List.stable_sort
      (fun (prio1, _, _) (prio2, _, _) -> prio2 - prio1)
      prio_repositories
  in
  ...
  List.iter
    (fun switch ->
      ...
      (* Cleanup exported variables from the switch config (they are now
         ...
         'ocaml:native-dynlink') *)
      let to_remove_vars =
        List.map OpamVariable.of_string
          [
            "ocaml-version";
            "compiler";
            "preinstalled";
      ...
      let remove_vars config =
        OpamFile.Dot_config.with_vars
          (List.filter
             (fun (var, _) -> not (List.mem var to_remove_vars))
             (OpamFile.Dot_config.bindings config))
          config
      in
      let switch_config_f =
        OpamFile.make (meta_dir / "config" // "global-config.config")
      in
      let switch_config = OpamFile.Dot_config.safe_read switch_config_f in
      ...
      let new_compilers =
        OpamPackage.Set.map
          (fun nv ->
            if nv.name <> OpamPackage.Name.of_string "ocaml" then nv
            else
              let config_f nv =
                OpamFile.make
                  ( meta_dir / "config"
              ...
              if OpamFile.exists (config_f nv) then (
                OpamFile.Dot_config.write (config_f new_nv) (remove_vars config);
      ...
      let selections =
        let open OpamPackage.Set.Op in
        {
          selections with
          sel_installed =
            selections.sel_installed -- selections.sel_compiler ++ new_compilers;
          sel_roots =
            selections.sel_roots -- selections.sel_compiler ++ new_compilers;
  ...
  List.iter
    (fun switch ->
      ...
      | Some oldconf ->
          ...
          let new_config =
            {
              OpamFile.Switch_config.opam_version =
                OpamVersion.nopatch v2_0_alpha3;
              synopsis = "";
              repos = None;
              opam_root;
              paths;
...
let from_2_0_alpha3_to_2_0_beta root conf =
  List.iter
    (fun switch ->
      ...
      in
      let packages_dev_dir = switch_meta_dir / "packages.dev" in
      (* old *)
      let sources_dir = switch_meta_dir / "sources" in
      (* new *)
      let state =
        OpamFile.SwitchSelections.safe_read
         ...
         [
           OpamFormula.Atom
             ( OpamPackage.Name.of_string "ocaml-system",
               OpamFormula.Atom (`Geq, OpamPackage.Version.of_string "4.02.3")
             );
           OpamFormula.Atom
             ( OpamPackage.Name.of_string "ocaml-base-compiler",
               OpamFormula.Empty );
...
let from_2_0_beta_to_2_0_beta5 root conf =
      ...
      let config =
        {
          ...
          C.variables =
            List.filter
              (fun (var, _) -> not (List.mem var rem_variables))
              config.C.variables;
        }
      in
  ...
  OpamFile.Config.with_eval_variables
    (List.filter
       (fun (v, _, _) -> not (List.mem v rem_eval_variables))
       (OpamFile.Config.eval_variables conf))
    conf
...
let as_necessary global_lock root config =
  ...
  if cmp = 0 then ()
    ...
    if OpamFormatConfig.(!r.skip_version_checks) then ()
    else
      OpamConsole.error_and_exit `Configuration_error
        "%s reports a newer opam version, aborting."
        (OpamFilename.Dir.to_string root)
  else if OpamVersion.compare config_version OpamFile.Config.format_version >= 0
  then ()
  else
    let is_dev = OpamVersion.git () <> None in
    OpamConsole.formatted_msg
      "This %sversion of opam requires an update to the layout of %s from \
       version %s to version %s, which can't be reverted.\n\
...
let comp_file ?package ?descr comp =
  OpamFile.Comp.to_package ?package comp descr |> opam_file_from_1_2_to_2_0
