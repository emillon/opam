...
let eval_redirect gt repo repo_root =
  if repo.repo_url.OpamUrl.backend <> `http then None
  else
    let redirect =
      OpamRepositoryPath.repo repo_root
      |> OpamFile.Repo.safe_read |> OpamFile.Repo.redirect
    in
    let redirect =
      List.fold_left
        (fun acc (redirect, filter) ->
          if
    ...
    | (redirect, f) :: _ ->
        let redirect_url =
          if OpamStd.String.contains ~sub:"://" redirect then
            OpamUrl.of_string redirect
          else OpamUrl.Op.(repo.repo_url / redirect)
        in
        if redirect_url = repo.repo_url then None else Some (redirect_url, f)

let repository rt repo =
  let max_loop = 10 in
  let gt = rt.repos_global in
  if repo.repo_url = OpamUrl.empty then Done (fun rt -> rt)
  else
    ...
    let rec job r n =
      if n = 0 then (
        ...
        @@ OpamRepository.update r repo_root
        @@+ fun () ->
        if n <> max_loop && r = repo then (
          ...
          match eval_redirect gt r repo_root with
          | None -> Done r
          | Some (new_url, f) ->
              ...
              let reason =
                match f with
                | None -> ""
                | Some f -> Printf.sprintf " (%s)" (OpamFilter.to_string f)
              in
              OpamConsole.note
    ...
    if not (OpamFile.exists repo_file_path) then
      OpamConsole.warning
        "The repository '%s' at %s doesn't have a 'repo' file, and might not \
         be compatible with this version of opam."
        (OpamRepositoryName.to_string repo.repo_name)
    ...
    if
      ...
      && OpamVersion.compare repo_vers OpamVersion.current > 0
    then
    ...
    | None ->
        ...
        Done
          (fun (* Return an update function to make parallel execution possible *)
                 rt ->
            {
              rt with
              repositories =
...
let pinned_package st ?version ?(working_dir = false) name =
  log "update-pinned-package %s%a"
    (OpamPackage.Name.to_string name)
    (slog @@ function true -> " (working dir)" | false -> "")
    working_dir;
  ...
  | Some (opam, Some urlf) -> (
      ...
      let add_extra_files srcdir file opam =
        if OpamFilename.dirname (OpamFile.filename file) <> srcdir then
          OpamFileTools.add_aux_files ~files_subdir_hashes:true opam
        else opam
      in
      ...
      ( if working_dir then Done ()
        ...
        | true -> (
            ...
            | true ->
                OpamConsole.note
                  "Ignoring uncommitted changes in %s (`--working-dir' not \
                   active)."
                  url.OpamUrl.path ) )
      @@+ fun () ->
      (* Do the update *)
      ...
      let new_source_opam =
        ...
        let warns, opam_opt =
          ...
          | Some opam0 ->
              let opam =
                OpamFormatUpgrade.opam_file ~quiet:true ~filename:f opam0
              in
        ...
        then (
          OpamConsole.warning "%s opam file from upstream of %s:"
      ...
      let save_overlay opam =
        ...
        let opam =
          OpamFile.OPAM.with_url urlf @@ OpamFile.OPAM.with_name name opam
        in
        let opam =
          if OpamFile.OPAM.version_opt opam = None then
            OpamFile.OPAM.with_version version opam
          else opam
        in
        List.iter
          (fun (file, rel_file, hash) ->
            if
              OpamFilename.exists file
              && OpamHash.check_file (OpamFilename.to_string file) hash
            then
      ...
      | Result _, Some new_opam
        when changed_opam old_source_opam new_source_opam
          ...
          let interactive_part st =
            if
              (not (changed_opam old_source_opam overlay_opam))
            ...
            then (
              let bak =
                OpamPath.backup_dir root
                / (OpamPackage.Name.to_string name ^ ".bak")
              in
      ...
      | (Up_to_date _ | Not_available _), _ -> Done ((fun st -> st), false)
      | Result _, Some new_opam
         ...
         confirm, but use it still (e.g. descr may have changed) *)
          ...
          Done
            ( (fun st ->
              ...
              true )
      | Result _, _ -> Done ((fun st -> st), true) )

let dev_package st ?working_dir nv =
  log "update-dev-package %a" (slog OpamPackage.to_string) nv;
  if
    OpamSwitchState.is_pinned st nv.name
    && not (OpamSwitchState.is_version_pinned st nv.name)
  then pinned_package st ~version:nv.version ?working_dir nv.name
  else
...
let pinned_packages st ?(working_dir = OpamPackage.Name.Set.empty) names =
  ...
  let updates =
    OpamPackage.Name.Set.fold
      (fun name acc -> OpamPackage.Set.add (OpamPinned.package st name) acc)
      updates OpamPackage.Set.empty
  in
  (OpamSwitchAction.add_to_reinstall st ~unpinned_only:false updates, updates)
...
let active_caches st nv =
  ...
  let repo_cache =
    match
      OpamRepositoryState.find_package_opt rt (OpamSwitchState.repos_list st) nv
    with
    | None -> []
    | Some (repo, _) ->
        let repo_def = OpamRepositoryName.Map.find repo rt.repos_definitions in
        let root_url =
          match OpamFile.Repo.root_url repo_def with
...
let download_package_source st nv dirname =
  ...
  let fetch_source_job =
    ...
    | None -> Done None
    | Some u ->
        OpamRepository.pull_tree (OpamPackage.to_string nv) ~cache_dir
          ~cache_urls dirname (OpamFile.URL.checksum u)
  ...
  let fetch_extra_source_job (name, u) = function
    ...
    | ret ->
        OpamRepository.pull_file_to_cache
          ...
          ~cache_dir ~cache_urls (OpamFile.URL.checksum u)
          (OpamFile.URL.url u :: OpamFile.URL.mirrors u)
  ...
  | r ->
      OpamProcess.Job.seq
        (List.map fetch_extra_source_job (OpamFile.OPAM.extra_sources opam))
        []
      @@| fun r1 -> (r, r1)
