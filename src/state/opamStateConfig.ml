...
let setk k t ?root_dir ?current_switch ?switch_from ?jobs ?dl_jobs ?build_test
    ?build_doc ?dryrun ?makecmd ?ignore_constraints_on ?unlock_base
    ?no_env_notice ?locked =
...
let initk k =
  ...
  setk
    ...
    ?unlock_base:(env_bool "UNLOCKBASE") ?no_env_notice:(env_bool "NOENVNOTICE")
...
let load opamroot = OpamFile.Config.read_opt (OpamPath.config opamroot)

let local_switch_exists root switch =
  OpamPath.Switch.switch_config root switch |> OpamFile.Switch_config.read_opt
  |> function
  | None -> false
...
let get_current_switch_from_cwd root =
  ...
  OpamFilename.find_in_parents
    ...
    (OpamFilename.cwd ())
  >>| OpamSwitch.of_dirname >>| resolve_local_switch root
...
let load_defaults root_dir =
  ...
  | Some conf ->
      ...
      OpamRepositoryConfig.update
        ?download_tool:
          (OpamFile.Config.dl_tool conf >>| function
           | (CString c, None) :: _ as t
             when OpamStd.String.ends_with ~suffix:"curl" c ->
               lazy (t, `Curl)
           | t -> lazy (t, `Default))
...
let get_switch_opt () =
  ...
  | Some s -> Some (resolve_local_switch !r.root_dir s)
  | None -> None
...
let get_switch () =
  ...
  | None ->
      OpamConsole.error_and_exit `Configuration_error
        "No switch is currently set. Please use 'opam switch' to set or \
         install a switch"
