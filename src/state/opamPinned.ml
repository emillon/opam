...
let check_locked default =
  ...
  | Some ext ->
      ...
      if OpamFilename.exists fl then (
        let base_depends =
          OpamFile.make default |> OpamFile.OPAM.read |> OpamFile.OPAM.depends
        in
        let lock_depends =
          OpamFile.make fl |> OpamFile.OPAM.read |> OpamFile.OPAM.depends
        in
        let ldep_names =
        ...
        in
        let lpkg_f =
          lock_formula |> OpamFormula.atoms |> OpamPackage.Name.Map.of_list
        in
        ...
        if contains <> [] || consistent <> [] then
          OpamConsole.warning
            "Lock file %s is outdated, you may want to re-run opam lock:\n%s"
            (OpamConsole.colorise `underline
               (OpamFilename.Base.to_string (OpamFilename.basename fl)))
            ( ( if contains <> [] then
                Printf.sprintf
            ...
            if consistent <> [] then
              Printf.sprintf
                "Dependencies in lock file not consistent wit opam file filter:\n\
                 %s"
                (OpamStd.Format.itemize
                   (fun (n, lv, (bv : OpamFormula.version_formula)) ->
                     Printf.sprintf "%s: %s in not contained in {%s}"
                       (OpamPackage.Name.to_string n)
                       (OpamPackage.Version.to_string lv)
                       (OpamFormula.string_of_formula
                          (fun (op, vc) ->
                            Printf.sprintf "%s %s" (OpamPrinter.relop op)
...
let find_opam_file_in_source name dir =
  ...
  in
  (match opt with Some base -> Some (check_locked base) | _ -> opt)
...
let name_of_opam_filename dir file =
  ...
  let get_name s =
    if Filename.check_suffix s suffix then
      Some Filename.(chop_suffix (basename s) suffix)
    else None
  in
...
let files_in_source d =
  ...
  let files d =
    ...
    @ OpamStd.List.filter_map
        (fun d ->
          if
            OpamFilename.(basename_dir d = Base.of_string "opam")
...
let orig_opam_file st name opam =
  ...
  >>= fun dir ->
  OpamStd.List.find_opt OpamFilename.exists
    [ dir // (OpamPackage.Name.to_string name ^ ".opam"); dir // "opam" ]
  >>| OpamFile.make
