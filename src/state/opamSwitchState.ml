...
let load_switch_config gt switch =
  ...
  | Some c -> c
  | None ->
      OpamConsole.error "No config file found for switch %s. Switch broken?"
...
let repos_list_raw rt switch_config =
  ...
  in
  let found, notfound =
    List.partition (fun r -> OpamRepositoryName.Map.mem r rt.repositories) repos
  in
  List.iter
    (fun r ->
      log
        "Ignoring %s-selected repository %S, no configured repository by this \
         name found"
...
let repos_list st = repos_list_raw st.switch_repos st.switch_config
...
let load lock_kind gt rt switch =
  ...
  if not (OpamGlobalState.switch_exists gt switch) then (
    log "The switch %a does not appear to be installed according to %a"
      (slog OpamSwitch.to_string)
      switch
      (slog @@ OpamFile.to_string @* OpamPath.config)
      gt.root;
    ...
    if
      OpamVersion.compare
        ...
        (OpamVersion.nopatch
           OpamFormatUpgrade.lastest_compatible_switch_version)
      < 0
    then
    ...
    let {
      ...
      sel_pinned = pinned;
      sel_compiler = compiler_packages;
    } =
          ...
          | Some o ->
              let version =
                ...
                | Some v when v <> nv.version ->
                    log
                      "warn: %s has conflicting pinning versions between \
              ...
              (OpamPackage.Set.add nv pinned, OpamPackage.Map.add nv o opams))
        pinned
        (OpamPackage.Set.empty, OpamPackage.Map.empty)
    in
    let installed_opams =
      OpamPackage.Set.fold
        (fun nv opams ->
    ...
    let available_packages =
      lazy (compute_available_packages gt switch switch_config ~pinned ~opams)
    in
    let opams =
    ...
    let installed_without_def =
      OpamPackage.Set.fold
        (fun nv nodef ->
          if OpamPackage.Map.mem nv installed_opams then nodef
          else
            try
              ...
              if lock_kind = `Lock_write then (
                (* auto-repair *)
                log
                  "Definition missing for installed package %s, copying from \
                   repo"
    ...
    let switch_config =
      if
        ...
        && switch_config.OpamFile.Switch_config.synopsis = ""
      then (
        let synopsis =
          match
            OpamPackage.Set.elements (compiler_packages %% installed_roots)
          with
          | [] -> OpamSwitch.to_string switch
          | [ nv ] ->
    ...
    let ext_files_changed =
      OpamPackage.Name.Map.fold
          ...
          if
            List.exists
              (fun (file, hash) ->
                ...
                let should_exist =
                  let count_not_zero c = function '0' -> c | _ -> succ c in
                  OpamStd.String.fold_left count_not_zero 0
                ...
                if (not exists) && should_exist then
                  OpamConsole.error
                    "System file %s, which package %s depends upon, no longer \
                     exists.\n\
                     The package has been marked as removed, and opam will try \
                     to reinstall it if necessary, but you should reinstall \
                     its system dependencies first."
                    (OpamFilename.to_string file)
                    (OpamPackage.to_string nv)
                else if changed then
                  OpamConsole.warning
                    "File %s, which package %s depends upon, was changed on \
                     your system. %s has been marked as removed, and will be \
        ...
        conf_files OpamPackage.Set.empty
    in
    let installed = installed -- ext_files_changed in
    let reinstall =
...
let load_virtual ?repos_list ?(avail_default = true) gt rt =
  let repos_list =
    match repos_list with
    | Some rl -> rl
    | None -> OpamGlobalState.repos_list gt
  in
  let opams = OpamRepositoryState.build_index rt repos_list in
  let packages = OpamPackage.keys opams in
  let available_packages =
    lazy
      ( OpamPackage.Map.filter
    ...
    switch_config =
      {
        ...
        OpamFile.Switch_config.repos = Some repos_list;
      };
    installed = OpamPackage.Set.empty;
...
let descr_opt st nv = OpamStd.Option.Op.(opam_opt st nv >>= OpamFile.OPAM.descr)

let descr st nv = OpamStd.Option.Op.(descr_opt st nv +! OpamFile.Descr.empty)

let url st nv = OpamStd.Option.Op.(opam_opt st nv >>= OpamFile.OPAM.url)

let primary_url st nv = OpamStd.Option.Op.(url st nv >>| OpamFile.URL.url)
...
let files st nv =
  ...
  | Some opam ->
      List.map
        (fun (file, _base, _hash) -> file)
        (OpamFile.OPAM.get_extra_files
           ~repos_roots:(OpamRepositoryState.get_root st.switch_repos)
           opam)

let package_config st name = OpamPackage.Name.Map.find name st.conf_files

let is_name_installed st name = OpamPackage.has_name st.installed name
...
let get_package st name =
  ...
  with Not_found -> (
    try find_installed_package_by_name st name
    with Not_found -> (
      try OpamPackage.max_version (Lazy.force st.available_packages) name
      with Not_found -> OpamPackage.max_version st.packages name ) )
...
let is_pinned st name = OpamPackage.has_name st.pinned name
      ...
      List.fold_left
        (fun depexts (names, filter) ->
          if OpamFilter.eval_to_bool ~default:false env filter then
            List.fold_left
              (fun depexts n -> OpamStd.String.Set.add n depexts)
              depexts names
          else depexts)
...
let conflicts_with st subset =
  let forward_conflicts, conflict_classes =
    OpamPackage.Set.fold
      ...
      subset
      (OpamFormula.Empty, OpamPackage.Name.Set.empty)
  in
  OpamPackage.Set.filter (fun nv ->
      ...
      && ( OpamFormula.verifies forward_conflicts nv
         ...
         let opam = OpamPackage.Map.find nv st.opams in
         List.exists
           (fun cl -> OpamPackage.Name.Set.mem cl conflict_classes)
           (OpamFile.OPAM.conflict_class opam)
         ||
         ...
         OpamPackage.Set.exists
           (OpamFormula.verifies backwards_conflicts)
           subset ))

let remove_conflicts st subset pkgs = pkgs -- conflicts_with st subset pkgs
  ...
  let conflict_classes =
    OpamPackage.Map.fold
      (fun nv opam acc ->
        List.fold_left
          (fun acc cc ->
            OpamPackage.Name.Map.update cc (OpamPackage.Set.add nv)
              OpamPackage.Set.empty acc)
          acc
          (OpamFile.OPAM.conflict_class opam))
      opams_map OpamPackage.Name.Map.empty
  in
...
let universe st ?(test = OpamStateConfig.(!r.build_test))
    ?(doc = OpamStateConfig.(!r.build_doc)) ?(force_dev_deps = false) ?reinstall
    ~requested user_action =
  let requested_allpkgs = OpamPackage.packages_of_names st.packages requested in
  let env nv v =
    if List.mem v OpamPackageVar.predefined_depends_variables then
      match OpamVariable.Full.to_string v with
      | "dev" -> Some (B (force_dev_deps || is_dev_package st nv))
      | "with-test" ->
          Some (B (test && OpamPackage.Set.mem nv requested_allpkgs))
      | "with-doc" -> Some (B (doc && OpamPackage.Set.mem nv requested_allpkgs))
      ...
      if r = None then
        ...
        else log "ERR: undefined filter variable in dependencies of %s: %s" )
          (OpamPackage.to_string nv)
  ...
  let get_deps f opams =
    OpamPackage.Map.mapi
      (fun nv opam -> OpamFilter.partial_filter_formula (env nv) (f opam))
      opams
  in
  let u_depends =
    let depend =
  ...
  let u_reinstall =
    (* Ignore reinstalls outside of the dependency cone of
       [requested_allpkgs] *)
    let resolve_deps nv =
      OpamPackageVar.filter_depends_formula ~build:true ~post:true ~default:true
        ~env:(env nv)
        (OpamFormula.ands
           [
    ...
    ++ match reinstall with Some set -> set | None -> OpamPackage.Set.empty
  in
  let u =
    {
...
let dump_pef_state st oc =
  ...
  let print_def nv opam =
    ...
    OpamFormula.ors [ Atom (nv.name, Empty); OpamPackage.Map.find nv conflicts ]
  ...
  if OpamPackage.Set.is_empty candidates then
    ...
    let nv =
      ...
      with Not_found -> (
        match vformula with
        | Atom (_, v)
          when OpamPackage.Set.mem (OpamPackage.create name v) candidates ->
    ...
    | Some opam ->
        ...
        then
          Printf.sprintf "unmet availability conditions%s%s"
            (if OpamPackage.Set.cardinal candidates = 1 then ": " else ", e.g. ")
            (OpamFilter.to_string avail)
        else if
          OpamPackage.has_name
            ( Lazy.force st.available_packages
            -- remove_conflicts st st.compiler_packages
                 (Lazy.force st.available_packages) )
            name
        then "conflict with the base packages of this switch"
        else if
          OpamPackage.has_name st.compiler_packages name
          && not OpamStateConfig.(!r.unlock_base)
        then "base of this switch (use `--unlock-base' to force)"
        else default
...
let update_package_metadata nv opam st =
  {
    ...
    reinstall =
      ...
      | Some inst ->
          if OpamFile.OPAM.effectively_equal inst opam then
            OpamPackage.Set.remove nv st.reinstall
...
let update_pin nv opam st =
  ...
  @@ {
       ...
       available_packages =
         lazy
           (OpamPackage.filter_name_out
              (Lazy.force st.available_packages)
              nv.name);
     }

let do_backup lock st =
  match lock with
  | `Lock_write -> (
...
let update_repositories gt update_fun switch =
  ...
  in
  let conf =
    { conf with OpamFile.Switch_config.repos = Some (update_fun repos) }
  in
  OpamFile.Switch_config.write
    (OpamPath.Switch.switch_config gt.root switch)
    conf
