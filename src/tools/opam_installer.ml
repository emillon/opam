...
let do_commands project_root =
  ...
  in
  let cp =
    if Sys.win32 then fun ?exec ~opt ~src ~dst ->
      let src, dst =
        if not (OpamFilename.exists src) then
          ...
          if OpamFilename.exists test then (
            ...
            OpamConsole.warning "Adding .exe for %s"
              (OpamFilename.to_string test);
            (test, dst) )
          else (src, dst)
        else (src, dst)
      in
  ...
  in
  let rm =
    if Sys.win32 then fun ~opt f ->
      let f =
        if OpamFilename.exists f then f
        else
          let test = OpamFilename.add_extension f "exe" in
          if OpamFilename.exists test then (
            OpamConsole.warning "Removing %s instead of %s"
              (OpamFilename.to_string test)
              (OpamFilename.to_string f);
            test )
          else f
  ...
  in
  let confirm s f = if OpamConsole.confirm "%s" s then f () in
...
let iter_install f instfile o =
  ...
  let dest_pkg ?fix instdir_f =
    let fix =
      OpamStd.Option.map
        OpamFilename.Op.(fun d -> d / OpamPackage.Name.to_string o.pkgname)
        fix
    in
    dest ?fix (instdir_f o.prefix (OpamSwitch.of_string "") o.pkgname)
  in
  List.iter f
    [
  ...
  let remove_files (dest, files, _) =
    List.iter
      (fun (base, dst) ->
        ...
        dirs_to_remove :=
          OpamFilename.Dir.Set.add
            (OpamFilename.dirname dst_file)
            !dirs_to_remove)
      files
  in
...
let options =
  ...
  let prefix =
    ...
    in
    Arg.(
      value & opt string "/usr/local" & info ~docv:"PREFIX" ~doc [ "prefix" ])
  in
  let script =
    let doc =
  ...
  let mandir =
    let doc =
      "Manpages dir. Relative to $(b,prefix) or absolute. By default \
       $(i,\\$prefix/man)."
    in
  ...
  let libdir =
    let doc =
      "OCaml lib dir. Relative to $(b,prefix) or absolute. By default \
       $(i,\\$prefix/lib) ; sometimes setting this to $(i,\\$(ocamlc -where)) \
       is preferable."
    in
  ...
  let stubsdir =
    let doc =
      "Stubs installation dir. Relative to $(b,prefix) or absolute. By default \
       $(i,\\$libdir/stublibs)."
    in
  ...
  let topdir =
    let doc =
      "Toplevel install dir. Relative to $(b,prefix) or absolute. By default \
       $(i,\\$libdir/toplevel)."
    in
  ...
  let docdir =
    let doc =
      "Documentation dir. Relative to $(b,prefix) or absolute. By default \
       $(i,\\$prefix/doc)."
    in
  ...
  let make_options file prefix script name mandir libdir stubsdir topdir docdir
    ...
    let file =
      ...
      | Some file ->
          ...
          if OpamFilename.exists f then f
            ...
            if OpamFilename.exists f then f
            else raise (Invalid_argument ("File not found: " ^ file))
      | None -> (
          let candidates = OpamFilename.files (OpamFilename.cwd ()) in
          match
            List.filter
              (fun f -> OpamFilename.check_suffix f ".install")
              candidates
          with
          | [ f ] -> f
    ...
    let pkgname =
      ...
      | None when OpamFilename.check_suffix (OpamFile.filename file) ".install"
        ->
    ...
    let mk_dir = function
      | None -> None
      | Some d when Filename.is_relative d -> Some OpamFilename.Op.(prefix / d)
      | Some d -> Some (OpamFilename.Dir.of_string d)
    in
    let mandir = mk_dir mandir in
    let libdir = mk_dir libdir in
    let stubsdir =
      match (mk_dir stubsdir, libdir) with
      | None, Some d -> Some OpamFilename.Op.(d / "stublibs")
      | d, None | (Some _ as d), _ -> d
    in
    let topdir =
      match (mk_dir topdir, libdir) with
  ...
  in
  Term.(
    const make_options $ file $ prefix $ script $ pkgname $ mandir $ libdir
    $ stubsdir $ topdir $ docdir)
...
let command =
  let remove =
    Arg.(
      ...
      & [
          ( false,
            Arg.info [ "i"; "install" ] ~doc:"Install the package (the default)"
          );
          ( true,
            Arg.info [ "u"; "uninstall"; "remove" ] ~doc:"Remove the package" );
        ])
  in
  Term.(
    const (fun options remove ->
...
let () =
  ...
  OpamStd.Config.init ();
  try
    match Term.eval ~catch:false (command, info) with
    | `Error _ -> exit 2
    | _ -> exit 0
  with
  | Invalid_argument s ->
