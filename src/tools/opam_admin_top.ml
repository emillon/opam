...
let apply f x prefix y = match f with None -> () | Some f -> f x prefix y
...
let to_action f x y =
  match f with
  | None -> `Keep
  | Some f -> ( match y with None -> `Keep | Some y -> `Update (f x y) )

let of_action o = function `Keep -> o | `Update x -> Some x | `Remove -> None
...
let iter_packages_gen ?(quiet = false) f =
  ...
  OpamPackage.Map.iter
    (fun package prefix ->
      ...
      if opam <> opam2 then (
        upd ();
        OpamFile.OPAM.write_with_preserved_format opam_file opam2 );
      if descr <> descr2 then (
        upd ();
        wopt OpamFile.Descr.write descr_file descr2 );
      if url <> url2 then (
        upd ();
        wopt OpamFile.URL.write url_file url2 );
      if dot_install <> dot_install2 then (
        upd ();
        wopt OpamFile.Dot_install.write dot_install_file dot_install2 );
      if !changed then (
        incr changed_pkgs;
        if not quiet then (
          OpamConsole.carriage_delete ();
          OpamConsole.msg "Updated %s\n" (OpamPackage.to_string package) ) )
      else if not quiet then OpamConsole.carriage_delete ())
    packages;
  if not quiet then
    OpamConsole.msg "Done. Updated %d files in %d packages.\n" !changed_files
      !changed_pkgs

let iter_packages ?quiet ?(filter = true_) ?f ?(opam = identity) ?descr ?url
    ?dot_install () =
  iter_packages_gen ?quiet
    (fun p ~prefix ~opam:o ~descr:d ~url:u ~dot_install:i ->
      ...
      else (o, `Keep, `Keep, `Keep))

let regexps_of_patterns patterns =
  let contains_dot str =
    let len = String.length str in
    let rec aux = function -1 -> false | i -> str.[i] = '.' || aux (i - 1) in
    aux (len - 1)
  in
  List.map
    (fun pattern -> if contains_dot pattern then pattern else pattern ^ ".*")
    patterns
