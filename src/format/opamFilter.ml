  ...
  let rec aux ?(context = `Or) t =
    ...
    | FIdent (pkgs, var, converter) -> (
        OpamStd.List.concat_map "+"
          (function None -> "_" | Some p -> OpamPackage.Name.to_string p)
          pkgs
        ^ (if pkgs <> [] then ":" else "")
...
let rec fold_down_left f acc filter =
  match filter with
...
let variables filter =
  fold_down_left
    (fun acc -> function FString s -> string_variables s @ acc
      | FIdent f -> fident_variables f @ acc | _ -> acc)
    [] filter
...
let expand_string_aux ?(partial = false) ?(escape_value = fun x -> x) ?default
    env text =
  let default fident =
    match (default, partial) with
    | None, false -> None
  ...
  let env v =
    if partial then
      match env v with Some (S s) -> Some (S (escape_expansions s)) | x -> x
    else env v
  in
  let f g =
...
let unclosed_expansions text =
  ...
  in
  Re.all re text
  |> OpamStd.List.filter_map @@ fun gr ->
...
let map_variables_in_fident f ((_, _, conv) as fid) =
  ...
  | v :: vars -> (
      ...
      | OpamVariable.Full.Package _ | OpamVariable.Full.Self ->
          if
            List.exists (fun v -> OpamVariable.Full.variable v <> var_name) vars
...
let map_variables_in_string f =
  expand_string ~partial:true
    ~default:(fun fid_string ->
      try
        fid_string |> filter_ident_of_string |> map_variables_in_fident f
...
let rec distribute_negations ?(neg = false) = function
  ...
  | FOp (f1, op, f2) ->
      FOp
        ( distribute_negations ~neg:false f1,
...
let ident_of_var v =
  ( ( match OpamVariable.Full.scope v with
    ...
    OpamVariable.Full.variable v,
    None )

let ident_of_string s = ident_of_var (OpamVariable.Full.of_string s)
...
let expand_interpolations_in_file env file =
  ...
  let is_opam_format =
    try
      let _ = OpamParser.channel ic (OpamFilename.to_string src) in
      true
    with e ->
      OpamStd.Exn.fatal e;
      false
  ...
  let process =
    if is_opam_format then fun () ->
      OpamInterpLexer.main unquoted quoted (Lexing.from_channel ic)
    else
      let rec aux () =
...
let arguments env (a, f) =
  if opt_eval_to_bool env f then
    ...
    | CIdent i -> (
        ...
        | Some (L sl) -> sl
        | None ->
            log "ERR in replacement: undefined ident %S" i;
            [ "" ] )
  else []
...
let simple_arg_variables = function
  ...
  | CIdent i -> (
      try fident_variables (filter_ident_of_string i) with Failure _ -> [] )

let filter_opt_variables = function None -> [] | Some f -> variables f

let argument_variables (a, f) = simple_arg_variables a @ filter_opt_variables f

let command_variables (l, f) =
  List.fold_left
    (fun acc a -> argument_variables a @ acc)
...
let filter_constraints ?default_version ?default env filtered_constraint =
  OpamFormula.partial_eval
    (function
      | Filter flt -> if eval_to_bool ?default env flt then `True else `False
      | Constraint (relop, v) -> (
          try
          ...
          with Failure msg -> (
            ...
            | None ->
                log "Warn: ignoring version constraint %a: %s" (slog to_string)
                  v msg;
                `Formula Empty
  ...
  OpamFormula.partial_eval
    (function
      ...
      | Constraint (relop, flt_v) -> (
          ...
          | FUndef f | f -> `Formula (Atom (Constraint (relop, f))) ))
    filtered_constraint

let gen_filter_formula constraints filtered_formula =
  OpamFormula.map
    (fun (name, fc) ->
      match constraints fc with
...
let filter_formula ?default_version ?default env ff =
  gen_filter_formula (filter_constraints ?default_version ?default env) ff
...
let variables_of_filtered_formula ff =
  OpamFormula.fold_left
    (fun acc (_, f) ->
      OpamFormula.fold_left
        (fun acc -> function Constraint _ -> acc
...
let filter_deps ~build ~post ?test ?doc ?dev ?default_version ?default deps =
  filter_formula ?default_version ?default
    (deps_var_env ~build ~post ?test ?doc ?dev)
    deps
...
let sort_filtered_formula compare ff =
  ...
  let rec vc_sort = function
    ...
    | Atom (n, vf) ->
        Atom
          (n, OpamStd.Option.default vf (simplify_extended_version_formula vf))
