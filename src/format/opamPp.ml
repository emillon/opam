...
let bad_format ?pos fmt =
  Printf.ksprintf (fun str -> raise (Bad_format (pos, str))) fmt

let add_pos pos = function
  | Bad_format (pos_opt, msg) as e ->
      if pos_opt = None || pos_opt = Some pos_null then
        Bad_format (Some pos, msg)
      else e
  | e -> e

let rec string_of_bad_format ?file e =
  match (e, file) with
  | Bad_format (None, msg), Some f | Bad_format (Some (f, -1, -1), msg), _ ->
      Printf.sprintf "In %s:\n%s" f msg
  | Bad_format (Some pos, msg), _ ->
      Printf.sprintf "At %s:\n%s" (string_of_pos pos) msg
  | Bad_format (None, msg), None -> Printf.sprintf "Input error:\n%s" msg
...
let ( -| ) pp1 pp2 =
  {
    ...
    ppname =
      (match pp2.ppname with "" -> pp1.ppname | name -> pp1.name_constr name);
...
let map_fst pp1 =
  pp (fun ~pos (a, b) -> (pp1.parse ~pos a, b)) (fun (a, b) -> (pp1.print a, b))

let map_snd pp1 =
  pp (fun ~pos (a, b) -> (a, pp1.parse ~pos b)) (fun (a, b) -> (a, pp1.print b))

let map_list ?name ?posf pp1 =
  let name = match name with None -> pp1.ppname ^ "*" | Some n -> n in
  pp ~name
    (fun ~pos l ->
...
let of_module (type a) name m =
  ...
  pp ~name (fun ~pos:_ -> X.of_string) X.to_string

(** Build tuples from lists *)
let ( ^+ ) pp1 pp2 =
  pp
    ~name:(Printf.sprintf "%s %s" pp1.ppname pp2.ppname)
    (fun ~pos -> function x :: r -> (parse pp1 ~pos x, parse pp2 ~pos r)
      | [] -> unexpected ())
...
let default d =
  pp (fun ~pos:_ -> function None -> d | Some x -> x) (fun x -> Some x)
...
let fallback pp1 pp2 =
  let parse ~pos x =
    ...
    with e -> (
      ...
      try pp2.parse ~pos x with _ -> Printexc.raise_with_backtrace e bt )
  in
  { pp1 with parse }
...
let ppacc_opt
    ...
    cleanup = fun ~pos:_ _acc x -> x) set get pp1 =
  let parse ~pos = function
    | acc, Some s -> set (cleanup ~pos acc (pp1.parse ~pos s)) acc
    | acc, None -> acc
  in
  let print s = (s, OpamStd.Option.map pp1.print (get s)) in
  { parse; print; ppname = pp1.ppname; name_constr = (fun x -> x) }
