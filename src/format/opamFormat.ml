...
let item_pos = function Section (pos, _) | Variable (pos, _, _) -> pos
...
let values_pos = function [] -> None | x :: _ -> Some (value_pos x)
...
let lines_set ~empty ~add ~fold pp1 =
  pp
    ...
    (fun ~pos:(file, _, _) lines ->
      List.fold_left
        (fun (i, acc) -> function [] -> (i + 1, acc)
          | line -> (i + 1, add (parse pp1 ~pos:(file, i, 0) line) acc))
        (1, empty) lines
      |> snd)
    (fun x -> List.rev (fold (fun v acc -> print pp1 v :: acc) x []))
  ...
  pp
    ...
    (fun ~pos:(file, _, _) lines ->
      List.fold_left
        ...
        (1, empty) lines
      |> snd)
    (fun x -> List.rev (fold (fun k v acc -> print pp1 (k, v) :: acc) x []))
...
module V = struct
  ...
  let simple_arg =
    pp ~name:"ident-or-string"
      (fun ~pos:_ -> function Ident (_, i) -> CIdent i
        | String (_, s) -> CString s | _ -> unexpected ())
      (function
        | CIdent i -> Ident (pos_null, i) | CString s -> String (pos_null, s))

  let variable_contents =
    pp ~name:"string-or-stringlist-or-bool"
      (fun ~pos:_ -> function String (_, s) -> S s | Bool (_, b) -> B b
        | List (_, l) ->
  ...
  let list =
    pp ~name:"list" ~name_constr:(Printf.sprintf "[%s]")
      (fun ~pos:_ -> function List (_, l) -> l | x -> [ x ])
      (fun l -> List (pos_null, l))

  let group =
    pp ~name:"group" ~name_constr:(Printf.sprintf "(%s)")
      (fun ~pos:_ -> function Group (_, l) -> l | x -> [ x ])
      (fun l -> Group (pos_null, l))

  let option =
    pp ~name:"option"
      (fun ~pos:_ -> function Option (_, k, l) -> (k, l) | k -> (k, []))
      (function v, [] -> v | v, l -> Option (pos_null, v, l))

  let option_strict =
    pp ~name:"option"
      (fun ~pos -> function Option (_, k, l) -> (k, l)
  ...
  let list_depth expected_depth =
    ...
    let rec lift n v =
      if n <= 0 then v
      else match v with List (_, [ v ]) -> lift (n - 1) v | v -> v
    in
    pp
  ...
  let option_depth expected_depth =
    let rec depth = function Option (_, v, _) -> 1 + depth v | _ -> 0 in
    let rec wrap n v =
      if n <= 0 then v else wrap (n - 1) (Option (pos_null, v, []))
    in
    let rec lift n v =
      if n <= 0 then v
      else match v with Option (_, v, []) -> lift (n - 1) v | v -> v
    in
    pp
      (fun ~pos:_ v -> wrap (expected_depth - depth v) v)
      (fun v -> lift expected_depth v)
  ...
  let map_list ?(depth = 0) pp1 =
    list_depth depth
    -| pp
         ~name:(Printf.sprintf "[%s]" pp1.ppname)
         (fun ~pos:_ v ->
           match v with
           | List (_, l) ->
               List.rev
  ...
  let map_options_2 pp1 pp2 pp3 =
    option_depth 2 -| option_strict
    -| map_option_contents (option_strict -| map_option_contents pp1 pp2) pp3
    -| pp (fun ~pos:_ ((a, b), c) -> (a, b, c)) (fun (a, b, c) -> ((a, b), c))

  let map_options_3 pp1 pp2 pp3 pp4 =
    option_depth 3 -| option_strict
    -| map_option_contents
         ( option_strict
         -| map_option_contents
              (option_strict -| map_option_contents pp1 pp2)
              pp3 )
         pp4
    -| pp
  ...
  let map_triple pp1 pp2 pp3 =
    pp
      ~name:(Printf.sprintf "[%s %s %s]" pp1.ppname pp2.ppname pp3.ppname)
      (fun ~pos:_ -> function
        | List (_, [ a; b; c ]) ->
            ( parse pp1 ~pos:(value_pos a) a,
              parse pp2 ~pos:(value_pos b) b,
              parse pp3 ~pos:(value_pos c) c ) | _ -> unexpected ())
      (fun (a, b, c) ->
  ...
  let compiler_version =
    ...
    in
    let print v =
      if v = system_compiler then print ident v else print string v
    in
    ...
    let print_filter f =
      let rec aux ?(context = `Or) f =
        let group_if ?(cond = false) f =
          if cond || OpamFormatConfig.(!r.all_parens) then
            Group (pos_null, [ f ])
          else f
        in
        match f with
      ...
      in
      match f with FBool true -> [] | f -> [ aux f ]
    in
  ...
  let constraints version =
    let rec parse_constraints ~pos:_ l =
      let rec aux = function
        | Prefix_relop (pos, op, v) -> Atom (op, parse version ~pos v)
        | Logop (_, `And, l, r) -> And (aux l, aux r)
        | Logop (_, `Or, l, r) -> Or (aux l, aux r)
        | Pfxop (_, `Not, v) ->
            OpamFormula.neg
              (fun (op, s) -> (OpamFormula.neg_relop op, s))
              (aux v)
        | Group (pos, g) -> Block (parse_constraints ~pos g)
    ...
    let rec print_constraints cs =
      let rec aux ?(in_and = false) cs =
        ...
        match cs with
        | Empty -> assert false
        | Atom (r, v) -> group_if (Prefix_relop (pos_null, r, print version v))
        | And (x, y) ->
            group_if
              (Logop (pos_null, `And, aux ~in_and:true x, aux ~in_and:true y))
        | Or (x, y) ->
            group_if ~cond:in_and (Logop (pos_null, `Or, aux x, aux y))
        | Block g -> Group (pos_null, print_constraints g)
      in
      match cs with Empty -> [] | cs -> [ aux cs ]
    in
    pp
      ~name:(version.ppname ^ "-constraints")
      ...
      let rec aux ?(in_and = false) cs =
        ...
        | Atom (Filter flt) -> (
            ...
            | f1 :: fr ->
                group_if
                  (List.fold_left
                     (fun a b -> Logop (pos_null, `And, a, b))
                     f1 fr)
            | [] -> Group (pos_null, []) )
      in
      match cs with Empty -> [] | cs -> [ aux cs ]
  ...
  let version = string -| of_module "version" (module OpamPackage.Version)
    ...
    pp ~name:"version-expr"
      (fun ~pos:_ -> function
        | String (pos, s) ->
            ...
            in
            FString s | Ident (_, s) -> FIdent (filter_ident_of_string s)
        | _ -> unexpected ())
      (function
  ...
  let pkgname = string -| of_module "pkg-name" (module OpamPackage.Name)

  let package_atom constraints = map_option pkgname constraints
  ...
  let rec ands_to_list = function
    | Empty -> []
    | And (e, f) -> List.rev_append (rev_ands_to_list e) (ands_to_list f)
    | x -> [ x ]

  and rev_ands_to_list = function
    | Empty -> []
    | And (e, f) -> List.rev_append (ands_to_list f) (rev_ands_to_list e)
    | x -> [ x ]
  ...
  let package_formula_items kind constraints =
    ...
    let rec print_formula ?(inner = false) f =
      let rec aux ?(in_and = false) f =
        ...
        | Or (e, f) ->
            group_if ~cond:in_and (Logop (pos_null, `Or, aux e, aux f))
...
module I = struct
  let file =
    pp ~name:"opam-file"
      (fun ~pos:_ file ->
        (OpamFilename.of_string file.file_name, file.file_contents))
      (fun (file_name, file_contents) ->
        { file_name = OpamFilename.to_string file_name; file_contents })
  ...
  let anonymous_section pp1 =
    pp ~name:pp1.ppname
      (fun ~pos -> function [ (None, contents) ] -> pp1.parse ~pos contents
        | [ (Some _, _) ] -> bad_format ~pos "Unexpected section title"
        | [] -> bad_format ~pos "Missing section"
        | _ :: _ :: _ -> bad_format ~pos "Duplicate section")
  ...
  let section kind =
    pp ~name:"file-section"
      (fun ~pos:_ -> function
        ...
        | Section (pos, sec) ->
            bad_format ~pos "Unexpected section %s" sec.section_kind
        | Variable (pos, k, _) -> bad_format ~pos "Unexpected field %s" k)
            ...
            | Section
              ...
              ->
                if List.mem_assoc k sections then
                  try (errs, SEM.safe_add (k, n) (pos, v) section_map, field_map)
                  with Failure _ ->
            ...
            | Variable (pos, k, v) ->
                if List.mem_assoc k ppas then
                  try
                    ( errs,
                      section_map,
                      OpamStd.String.Map.safe_add k (pos, v) field_map )
                  with Failure _ ->
                  ...
                  ( (k, (Some pos, "Invalid field " ^ k)) :: errs,
                    section_map,
                    field_map ))
          ([], SEM.empty, OpamStd.String.Map.empty)
          items
      in
      let errs, r =
        List.fold_left
          (fun (errs, acc) (field, ppa) ->
            try
              ...
              try (errs, parse ppa ~pos (acc, Some v))
              with Bad_format (pos, msg) -> ((field, (pos, msg)) :: errs, acc)
            with Not_found ->
              ( ( if List.mem field mandatory_fields then
                  (field, (Some pos, "Missing field " ^ field)) :: errs
                else errs ),
                acc ))
          (errs, empty) ppas
      in
        ...
        List.fold_left
          (fun (errs, acc) (section_kind, ppa) ->
            let secs =
              ...
              |> List.rev
            in
            if secs = [] then (errs, acc)
            else
              try (errs, parse ppa ~pos (acc, Some secs))
              with Bad_format (pos, msg) ->
  ...
  let show_errors ?name ?(strict = OpamFormatConfig.(!r.strict))
    ...
    let parse ~pos:(file, _, _) (t, errs) =
      if errs = [] then t
        ...
        if condition t then
          ...
          OpamConsole.log "FORMAT" "File errors in %s, ignored fields: %s" file
  ...
  let on_errors ?name f =
    let parse ~pos:_ (t, errs) = List.fold_left f t errs in
    let print t = (t, []) in
    pp ?name parse print

  let partition filter =
    pp (fun ~pos:_ -> List.partition filter) (fun (a, b) -> a @ b)

  let partition_fields filter =
    partition @@ function Variable (_, k, _) -> filter k | _ -> false
  ...
  let field name parse =
    pp
      (fun ~pos items ->
        match
          OpamStd.List.filter_map
            (function Variable (_, k, v) when k = name -> Some v | _ -> None)
            items
        with
        | [] -> (None, items)
  ...
  let extract_field name =
    ...
    -| ( map_fst @@ opt
       @@ singleton -| item
          -| pp
               ~name:(Printf.sprintf "'%s:' field" name)
               (fun ~pos:_ (_, v) -> v)
               (fun v -> (name, v)) )

  let check_opam_version ?(optional = false) ~format_version
    ...
    let f v =
      ...
      || match v with Some v -> f v | None -> optional
    in
    let errmsg =
      Printf.sprintf
  ...
  let signature =
    V.list
    -| (V.string ^+ V.string ^+ (last -| V.string))
    -| pp (fun ~pos:_ (a, (b, c)) -> (a, b, c)) (fun (a, b, c) -> (a, (b, c)))
  ...
  let signed ~check =
    ...
    -| pp ~name:"signed-file"
         (fun ~pos -> function
           ...
           | None, _ -> raise (Invalid_signature (pos, None)))
         (fun (sgs, items) ->
           assert (check sgs (OpamPrinter.Normalise.items items));
           (Some (print pp_sig sgs), items))
end
