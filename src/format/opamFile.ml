...
module Pp = struct
  ...
  let warn ?pos ?(strict = OpamFormatConfig.(!r.strict)) ?exn fmt =
    if strict then
      match exn with Some e -> raise e | None -> bad_format ?pos fmt
    else
      Printf.ksprintf
        (fun s ->
          if OpamConsole.verbose () then
            ...
            | Some e -> OpamConsole.warning "%s" (OpamPp.string_of_bad_format e))
        fmt
end
...
module MakeIO (F : IO_Arg) = struct
  ...
  let read_opt f =
    ...
    with
    | OpamSystem.File_not_found _ -> None
    | e ->
        OpamStd.Exn.fatal e;
  ...
  let write_to_string ?(filename = dummy_file) t = F.to_string filename t
end
...
module LinesBase = struct
  ...
  let of_channel (_ : filename) ic = OpamLineLexer.main (Lexing.from_channel ic)
  ...
  let of_string (_ : filename) str = OpamLineLexer.main (Lexing.from_string str)
  ...
  let pp_channel ic oc =
    Pp.pp (fun ~pos:_ () -> of_channel file_none ic) (to_channel file_none oc)
end
...
module Pinned_legacy = struct
  ...
  let pp_pin =
    ...
    let pin_option_of_string ?kind s =
      ...
      | Some (#OpamUrl.backend as backend) -> Source (OpamUrl.parse ~backend s)
      | None -> Source (OpamUrl.parse ~handle_suffix:false s)
    in
    ...
    in
    Pp.pp ~name:"?pin-kind pin-target"
      (fun ~pos -> function [ x ] -> pin_option_of_string x
        | [ k; x ] -> pin_option_of_string ~kind:(pin_kind_of_string k) x
        | _ -> Pp.bad_format ~pos "Invalid number of fields")
      (fun x ->
        [ string_of_pin_kind (kind_of_pin_option x); string_of_pin_option x ])
...
module Environment = LineFile (struct
  ...
  let pp =
    ...
    ^+ Pp.identity ^+ Pp.opt Pp.singleton )
...
module StateTable = struct
  ...
  let pp_state =
    Pp.pp ~name:"pkg-state"
      (fun ~pos:_ -> function "compiler" -> `Compiler | "root" -> `Root
        | "noroot" | "installed" -> `Installed | "uninstalled" -> `Uninstalled
    ...
    -| Pp.pp
             ...
             (fun name (version, (state, pin)) t ->
               let nv = OpamPackage.create name version in
               {
                 sel_installed =
                   ...
                   | `Uninstalled | `Uninstalled_compiler -> t.sel_installed );
                 sel_roots =
                   ( match state with
                   | `Root | `Compiler -> OpamPackage.Set.add nv t.sel_roots
                 ...
                 sel_compiler =
                   ...
                   | `Root | `Installed | `Uninstalled -> t.sel_compiler );
                 sel_pinned =
                   ( match pin with
         ...
         (fun t ->
           ...
           |> OpamPackage.Set.fold
                (fun nv -> M.add nv.name (nv.version, (`Installed, None)))
                t.sel_installed
           |> OpamPackage.Set.fold
                (fun nv -> M.add nv.name (nv.version, (`Root, None)))
                t.sel_roots
           |> OpamPackage.Set.fold
                (fun nv acc ->
                  let name = nv.name in
                  ...
                  with Not_found ->
                    M.add name (nv.version, (`Uninstalled_compiler, None)) acc)
                t.sel_compiler
           |> OpamPackage.Set.fold
                (fun nv map ->
                  let state =
                    try
                      let _, (state, _) = M.find nv.name map in
                      state
                  ...
                  M.add nv.name
                    ( nv.version,
                      (state, Some (Pinned_legacy.Version nv.version)) )
                    map)
                t.sel_pinned)
end
...
module Syntax = struct
  ...
  let to_channel filename oc t = Pp.print (pp_channel filename stdin oc) t

  let of_string (filename : filename) str =
    let lexbuf = Lexing.from_string str in
    parser_main lexbuf filename

  let to_string _file_name t = OpamPrinter.opamfile t

  let to_string_with_preserved_format filename ?(format_from = filename)
      ?format_from_string ~empty ?(sections = []) ~fields pp t =
    let current_str_opt =
                  ...
                  -> (
                    try
                      ...
                      let print_sec ppa t =
                        ...
                        | None -> None
                        | Some v -> (
                            try Some (List.assoc section_name v)
                            with Not_found -> None )
                      in
                      ...
                      then
                        ...
                        try
                          let f =
                            List.filter
                              (fun i ->
                                it_ident i = `Sec (section_kind, section_name))
                              syn_t.file_contents
                          in
                          OpamPrinter.items f :: strs
                        with Not_found -> strs
...
module Wrappers = struct
  ...
  let fields =
    [
      ( "pre-build-commands",
        Pp.ppacc with_pre_build pre_build (Pp.V.map_list ~depth:2 Pp.V.command)
      );
      ( "pre-install-commands",
        Pp.ppacc with_pre_install pre_install
          (Pp.V.map_list ~depth:2 Pp.V.command) );
      ( "pre-remove-commands",
        Pp.ppacc with_pre_remove pre_remove
          (Pp.V.map_list ~depth:2 Pp.V.command) );
      ( "pre-session-commands",
        Pp.ppacc with_pre_session pre_session
          (Pp.V.map_list ~depth:2 Pp.V.command) );
      ( "wrap-build-commands",
        Pp.ppacc with_wrap_build wrap_build
          (Pp.V.map_list ~depth:2 Pp.V.command) );
      ( "wrap-install-commands",
        Pp.ppacc with_wrap_install wrap_install
          (Pp.V.map_list ~depth:2 Pp.V.command) );
      ( "wrap-remove-commands",
        Pp.ppacc with_wrap_remove wrap_remove
          (Pp.V.map_list ~depth:2 Pp.V.command) );
      ( "post-build-commands",
        Pp.ppacc with_post_build post_build
          (Pp.V.map_list ~depth:2 Pp.V.command) );
      ( "post-install-commands",
        Pp.ppacc with_post_install post_install
          (Pp.V.map_list ~depth:2 Pp.V.command) );
      ( "post-remove-commands",
        Pp.ppacc with_post_remove post_remove
          (Pp.V.map_list ~depth:2 Pp.V.command) );
      ( "post-session-commands",
        Pp.ppacc with_post_session post_session
          (Pp.V.map_list ~depth:2 Pp.V.command) );
    ]
...
module ConfigSyntax = struct
  ...
  let criterion kind t =
    try Some (List.assoc kind t.solver_criteria) with Not_found -> None
  ...
  let fields =
    ...
    [
      ( "opam-version",
        Pp.ppacc with_opam_version opam_version
          (Pp.V.string -| Pp.of_module "opam-version" (module OpamVersion)) );
      ( "repositories",
        Pp.ppacc with_repositories repositories
          (Pp.V.map_list ~depth:1
             ( Pp.V.string
             -| Pp.of_module "repository" (module OpamRepositoryName) )) );
      ( "installed-switches",
        Pp.ppacc with_installed_switches installed_switches
          (Pp.V.map_list ~depth:1
             (Pp.V.string -| Pp.of_module "switch" (module OpamSwitch))) );
      ( "switch",
        Pp.ppacc_opt with_switch switch
          (Pp.V.string -| Pp.of_module "switch" (module OpamSwitch)) );
      ("jobs", Pp.ppacc_opt with_jobs jobs Pp.V.pos_int);
      ( "download-command",
        Pp.ppacc_opt with_dl_tool dl_tool (Pp.V.map_list ~depth:1 Pp.V.arg) );
      ("download-jobs", Pp.ppacc with_dl_jobs dl_jobs Pp.V.pos_int);
      ( "archive-mirrors",
        Pp.ppacc_opt with_dl_cache
          (fun t -> t.dl_cache)
          (Pp.V.map_list ~depth:1 Pp.V.url) );
      ( "solver-criteria",
        Pp.ppacc_opt (with_criterion `Default) (criterion `Default) Pp.V.string
      );
      ( "solver-upgrade-criteria",
        Pp.ppacc_opt (with_criterion `Upgrade) (criterion `Upgrade) Pp.V.string
      );
      ( "solver-fixup-criteria",
        Pp.ppacc_opt (with_criterion `Fixup) (criterion `Fixup) Pp.V.string );
      ( "best-effort-prefix-criteria",
        Pp.ppacc_opt with_best_effort_prefix best_effort_prefix Pp.V.string );
      ( "solver",
        Pp.ppacc_opt with_solver solver (Pp.V.map_list ~depth:1 Pp.V.arg) );
      ( "global-variables",
        Pp.ppacc with_global_variables global_variables
          (Pp.V.map_list ~depth:2
             (Pp.V.map_triple
                (Pp.V.ident -| Pp.of_module "variable" (module OpamVariable))
                Pp.V.variable_contents Pp.V.string)) );
      ( "eval-variables",
        Pp.ppacc with_eval_variables eval_variables
          (Pp.V.map_list ~depth:2
             (Pp.V.map_triple
                (Pp.V.ident -| Pp.of_module "variable" (module OpamVariable))
      ...
      ( "repository-validation-command",
        Pp.ppacc_opt with_validation_hook validation_hook
          (Pp.V.map_list ~depth:1 Pp.V.arg) );
      ( "default-compiler",
        Pp.ppacc with_default_compiler default_compiler
          (Pp.V.package_formula `Disj Pp.V.(constraints Pp.V.version)) );
      (* deprecated fields *)
      ( "alias",
        Pp.ppacc_opt with_switch OpamStd.Option.none
          (Pp.V.string -| Pp.of_module "switch-name" (module OpamSwitch)) );
      ( "ocaml-version",
        Pp.ppacc_opt with_switch OpamStd.Option.none
          (Pp.V.string -| Pp.of_module "switch-name" (module OpamSwitch)) );
      ("cores", Pp.ppacc_opt with_jobs OpamStd.Option.none Pp.V.pos_int);
...
module InitConfigSyntax = struct
  ...
  let pp_repository_def =
    Pp.V.map_options_3
      (Pp.V.string -| Pp.of_module "repository" (module OpamRepositoryName))
  ...
  let fields =
    [
      ...
      ( "default-compiler",
        Pp.ppacc with_default_compiler default_compiler
          (Pp.V.package_formula `Disj Pp.V.(constraints Pp.V.version)) );
      ("jobs", Pp.ppacc_opt (with_jobs @* OpamStd.Option.some) jobs Pp.V.pos_int);
      ( "download-command",
        Pp.ppacc_opt
          (with_dl_tool @* OpamStd.Option.some)
          dl_tool
          (Pp.V.map_list ~depth:1 Pp.V.arg) );
      ( "download-jobs",
        Pp.ppacc_opt (with_dl_jobs @* OpamStd.Option.some) dl_jobs Pp.V.pos_int
      );
      ( "archive-mirrors",
        Pp.ppacc with_dl_cache dl_cache (Pp.V.map_list ~depth:1 Pp.V.url) );
      ( "solver-criteria",
        Pp.ppacc_opt (with_criterion `Default) (criterion `Default) Pp.V.string
      );
      ( "solver-upgrade-criteria",
      ...
      ( "solver",
        Pp.ppacc_opt
          (with_solver @* OpamStd.Option.some)
          solver
          (Pp.V.map_list ~depth:1 Pp.V.arg) );
      ( "global-variables",
        Pp.ppacc with_global_variables global_variables
        ...
        Pp.ppacc with_eval_variables eval_variables
          (Pp.V.map_list ~depth:2
             (Pp.V.map_triple
                ...
                Pp.V.string)) );
      ( "recommended-tools",
        Pp.ppacc with_recommended_tools recommended_tools
          (Pp.V.map_list
             (Pp.V.map_options_2
                ...
                (Pp.opt Pp.V.filter))) );
      ( "required-tools",
        Pp.ppacc with_required_tools required_tools
          (Pp.V.map_list
             (Pp.V.map_options_2
                (Pp.V.map_list ~depth:1 Pp.V.string)
                (Pp.opt @@ (Pp.singleton -| Pp.V.string))
                (Pp.opt Pp.V.filter))) );
      ( "init-scripts",
        Pp.ppacc with_init_scripts init_scripts
          (Pp.V.map_list ~depth:2
             (Pp.V.map_option
                (Pp.V.map_pair Pp.V.string Pp.V.string_tr)
                (Pp.opt Pp.V.filter))) );
    ]
    @ List.map
        (fun (fld, ppacc) -> (fld, Pp.embed with_wrappers wrappers ppacc))
  ...
  let add t1 t2 =
    ...
    let list = function [] -> fun l -> l | l -> fun _ -> l in
    {
      ...
      default_compiler =
        ( if t2.default_compiler <> Empty then t2.default_compiler
        else t1.default_compiler );
      ...
      solver_criteria =
        List.fold_left
          (fun acc c ->
            try (c, List.assoc c t2.solver_criteria) :: acc
            with Not_found -> (
              try (c, List.assoc c t1.solver_criteria) :: acc
              with Not_found -> acc ))
          []
...
module Switch_configSyntax = struct
  ...
  let sections =
    [
      ( "paths",
        Pp.ppacc
          (fun paths t -> { t with paths })
          (fun t -> t.paths)
          ( Pp.I.anonymous_section Pp.I.items
          -| Pp.map_list
               (Pp.map_pair
                  (Pp.of_pair "std-path"
                     (std_path_of_string, string_of_std_path))
                  Pp.V.string) ) );
      ( "variables",
        Pp.ppacc
          (fun variables t -> { t with variables })
          (fun t -> t.variables)
          ( Pp.I.anonymous_section Pp.I.items
          -| Pp.map_list
               (Pp.map_pair
  ...
  let fields =
    [
      ( "opam-version",
        Pp.ppacc
          (fun opam_version t -> { t with opam_version })
          (fun t -> t.opam_version)
          (Pp.V.string -| Pp.of_module "opam-version" (module OpamVersion)) );
      ( "synopsis",
        Pp.ppacc
          (fun synopsis t -> { t with synopsis })
          (fun t -> t.synopsis)
          Pp.V.string );
      ( "repositories",
        Pp.ppacc_opt
          (fun r t -> { t with repos = Some r })
          (fun t -> t.repos)
          ( Pp.V.map_list ~depth:1
  ...
  let variable t s =
    try Some (List.assoc s t.variables) with Not_found -> None

  let path t p = try Some (List.assoc p t.paths) with Not_found -> None
...
module SwitchSelectionsSyntax = struct
  ...
  let pp_package = Pp.of_module "package" (module OpamPackage)
  ...
  let fields =
    [
      ( "opam-version",
        Pp.ppacc
          (fun _ t -> t)
          (fun _ -> format_version)
          (Pp.V.string -| Pp.of_module "opam-version" (module OpamVersion)) );
      ( "compiler",
        Pp.ppacc
          (fun sel_compiler t -> { t with sel_compiler })
          (fun t -> t.sel_compiler)
          pp_pkglist );
      ( "roots",
        Pp.ppacc
      ...
      ( "installed",
        Pp.ppacc
          (fun installed t -> { t with sel_installed = installed })
          (fun t -> t.sel_installed)
          pp_pkglist );
      ( "pinned",
        Pp.ppacc
          (fun sel_pinned t -> { t with sel_pinned })
          (fun t -> t.sel_pinned)
          ( Pp.V.map_list ~depth:1
              ( Pp.V.option
...
module Repo_config_legacySyntax = struct
    ...
    [
      ...
      ( "root",
        Pp.ppacc
          ...
          (fun r -> r.repo_root)
          (Pp.V.string -| Pp.of_module "directory" (module OpamFilename.Dir)) );
    ]
  ...
  let pp =
    ...
    @@ Pp.I.fields ~name ~empty
         ~mandatory_fields:[ "root"; "address"; "name" ]
         fields
       -| Pp.I.show_errors ~name ~strict:true ()
end
...
module Dot_configSyntax = struct
  ...
  let pp_contents =
    Pp.I.fields ~name:"config-file" ~empty
      ~sections:
        [
          ( "variables",
            Pp.ppacc with_vars vars (Pp.I.anonymous_section pp_variables) );
        ]
      [
        ( "opam-version",
          Pp.ppacc
            ...
            (Pp.V.string -| Pp.of_module "opam-version" (module OpamVersion)) );
        ( "file-depends",
          Pp.ppacc with_file_depends file_depends
            ( Pp.V.map_list ~depth:2
            @@ Pp.V.map_pair
    ...
    @@ Pp.I.check_opam_version ~format_version ~optional:true ()
       ...
       -| Pp.pp
            (fun ~pos (opam_version_opt, s) ->
              ...
              | Some v
                when OpamVersion.compare v (OpamVersion.of_string "1.3~dev3")
                     > 0 ->
  ...
  let variable t s = try Some (List.assoc s t.vars) with Not_found -> None

  let set k v t =
    let vars = List.remove_assoc k t.vars in
    let vars = match v with Some v -> (k, v) :: vars | None -> vars in
    { t with vars }
end
...
module RepoSyntax = struct
  ...
  let create ?browse ?upstream ?opam_version ?(redirect = []) ?root_url
      ?dl_cache ?(announce = []) ?stamp () =
    {
      opam_version;
      browse;
      upstream;
  ...
  let fields =
    [
      ( "opam-version",
        Pp.ppacc_opt with_opam_version opam_version
          (Pp.V.string -| Pp.of_module "opam-version" (module OpamVersion)) );
      ("browse", Pp.ppacc_opt with_browse browse Pp.V.string);
      ("upstream", Pp.ppacc_opt with_upstream upstream Pp.V.string);
      ( "redirect",
        Pp.ppacc with_redirect redirect
          (Pp.V.map_list ~depth:1
             (Pp.V.map_option Pp.V.string (Pp.opt Pp.V.filter))) );
      ( "archive-mirrors",
        Pp.ppacc with_dl_cache dl_cache (Pp.V.map_list ~depth:1 Pp.V.string) );
      ( "announce",
        Pp.ppacc with_announce announce
          (Pp.V.map_list ~depth:1
             (Pp.V.map_option Pp.V.string (Pp.opt Pp.V.filter))) );
      ("stamp", Pp.ppacc_opt with_stamp stamp Pp.V.string);
    ]
...
module URLSyntax = struct
  ...
  let empty = { url = OpamUrl.empty; mirrors = []; checksum = []; errors = [] }
  ...
  let fields =
    ...
    [
      ...
      ( "archive",
        Pp.ppacc_opt with_url OpamStd.Option.none (Pp.V.url_with_backend `http)
      );
      ( "http",
        Pp.ppacc_opt with_url OpamStd.Option.none (Pp.V.url_with_backend `http)
      );
      ( "git",
        Pp.ppacc_opt with_url OpamStd.Option.none (Pp.V.url_with_backend `git)
      );
      ( "darcs",
        Pp.ppacc_opt with_url OpamStd.Option.none (Pp.V.url_with_backend `darcs)
      );
      ( "hg",
        Pp.ppacc_opt with_url OpamStd.Option.none (Pp.V.url_with_backend `hg) );
      ( "local",
        Pp.ppacc_opt with_url OpamStd.Option.none (Pp.V.url_with_backend `rsync)
      );
      ( "checksum",
        Pp.ppacc with_checksum checksum
          (Pp.V.map_list ~depth:1
             (Pp.V.string -| Pp.of_module "checksum" (module OpamHash))) );
      ( "mirrors",
        Pp.ppacc with_mirrors mirrors (Pp.V.map_list ~depth:1 Pp.V.url) );
    ]
...
module OPAMSyntax = struct
  ...
  let extended t fld parse =
    ...
    try
      ...
      try Some (parse s) with Pp.Bad_format _ as e -> raise (Pp.add_pos pos e)
    with Not_found -> None
  ...
  let with_url url t =
    ...
    in
    { t with url = Some url; format_errors = format_errors @ t.format_errors }
  ...
  let with_url_opt url t =
    ...
    in
    { t with url; format_errors = format_errors @ t.format_errors }
  ...
  let with_synopsis synopsis t =
    { t with descr = Some (synopsis, OpamStd.Option.default "" (descr_body t)) }

  let with_descr_body text t =
    { t with descr = Some (OpamStd.Option.default "" (synopsis t), text) }
  ...
  let cleanup_name _opam_version ~pos:((file, _, _) as pos) name =
    ...
    | Some nv when nv.OpamPackage.name <> name ->
        Pp.warn ~pos
          "This file is for package '%s' but its 'name:' field advertises '%s'."
  ...
  let cleanup_version _opam_version ~pos:((file, _, _) as pos) version =
    ...
    | Some nv when nv.OpamPackage.version <> version ->
        Pp.warn ~pos
          "This file is for version '%s' but its 'version:' field advertises \
           '%s'."
  ...
  let cleanup_depopts opam_version ~pos depopts =
    ...
    then depopts
      ...
      let rec aux acc disjunction =
        List.fold_left
          (fun acc -> function OpamFormula.Atom _ as atom -> atom :: acc
            | f ->
                Pp.warn ~pos
                  "Optional dependencies must be a disjunction. Treated as \
                   such.";
                aux acc
                  (OpamFormula.fold_left
                     (fun acc a -> OpamFormula.Atom a :: acc)
                     [] f))
          acc disjunction
      in
      OpamFormula.ors_to_list depopts |> aux [] |> List.rev |> OpamFormula.ors
  ...
  let cleanup_conflicts opam_version ~pos conflicts =
    ...
    let is_disjunction f =
      List.for_all
        (function Atom _ -> true | _ -> false)
        OpamFormula.(ors_to_list f)
    in
    if is_disjunction conflicts then conflicts
    else
      let force_disjunction f =
        OpamFormula.map_formula (function And (a, b) -> Or (a, b) | f -> f) f
      in
  ...
  let cleanup_flags _opam_version ~pos flags =
    let known_flags =
      List.filter (function Pkgflag_Unknown _ -> false | _ -> true) flags
    in
    if known_flags <> flags then
      Pp.warn ~pos "Unknown package flags %s ignored"
        (OpamStd.Format.pretty_list
           (OpamStd.List.filter_map
              (function Pkgflag_Unknown s -> Some s | _ -> None)
              flags));
    known_flags
  ...
  let cleanup_dev_repo opam_version ~pos:_ dev_repo =
    if OpamVersion.(compare opam_version (of_string "1.3") >= 0) then dev_repo
    else OpamUrl.parse ~handle_suffix:true (OpamUrl.to_string dev_repo)

  let pp_basename = Pp.V.string -| Pp.of_module "file" (module OpamFilename.Base)
  ...
  let fields_gen =
    let no_cleanup (ppacc : ?cleanup:(pos:_ -> _) -> _) set get pp =
      let p = ppacc set get pp in
      (p, p)
    in
    let with_cleanup cleanup (ppacc : ?cleanup:(pos:_ -> _) -> _) set get pp =
      let cleanup ~pos acc x = cleanup acc.opam_version ~pos x in
      (ppacc set get pp, ppacc set get ~cleanup pp)
    in
    [
      ( "opam-version",
        no_cleanup Pp.ppacc with_opam_version opam_version
          (Pp.V.string -| Pp.of_module "opam-version" (module OpamVersion)) );
      ( "name",
        with_cleanup cleanup_name Pp.ppacc_opt with_name name_opt Pp.V.pkgname
      );
      ( "version",
        with_cleanup cleanup_version Pp.ppacc_opt with_version version_opt
          (Pp.V.string_tr -| Pp.of_module "version" (module OpamPackage.Version))
      );
      ("synopsis", no_cleanup Pp.ppacc_opt with_synopsis synopsis Pp.V.string_tr);
      ( "description",
        no_cleanup Pp.ppacc_opt with_descr_body descr_body Pp.V.string_tr );
      ( "maintainer",
        no_cleanup Pp.ppacc with_maintainer maintainer
          (Pp.V.map_list ~depth:1 Pp.V.string) );
      ( "authors",
        no_cleanup Pp.ppacc with_author author
          (Pp.V.map_list ~depth:1 Pp.V.string) );
      ( "author",
        no_cleanup Pp.ppacc
          (fun a t ->
            if t.author = [] then with_author a t
            else Pp.bad_format "multiple \"authors:\" fields" author)
          (fun _ -> [])
          (Pp.V.map_list ~depth:1 Pp.V.string) );
      ( "license",
        no_cleanup Pp.ppacc with_license license
          (Pp.V.map_list ~depth:1 Pp.V.string) );
      ( "tags",
        with_cleanup cleanup_tags Pp.ppacc with_tags tags
          (Pp.V.map_list ~depth:1 Pp.V.string) );
      ( "homepage",
        no_cleanup Pp.ppacc with_homepage homepage
          (Pp.V.map_list ~depth:1 Pp.V.string) );
      ( "doc",
        no_cleanup Pp.ppacc with_doc doc (Pp.V.map_list ~depth:1 Pp.V.string) );
      ( "bug-reports",
        no_cleanup Pp.ppacc with_bug_reports bug_reports
          (Pp.V.map_list ~depth:1 Pp.V.string) );
      ( "depends",
        no_cleanup Pp.ppacc with_depends depends
          (Pp.V.package_formula `Conj Pp.V.(filtered_constraints ext_version))
      );
      ( "depopts",
        with_cleanup cleanup_depopts Pp.ppacc with_depopts depopts
          (Pp.V.package_formula `Disj Pp.V.(filtered_constraints ext_version))
      );
      ( "conflicts",
        with_cleanup cleanup_conflicts Pp.ppacc with_conflicts conflicts
          (Pp.V.package_formula `Disj Pp.V.(filtered_constraints ext_version))
      );
      ( "conflict-class",
        no_cleanup Pp.ppacc with_conflict_class conflict_class
          (Pp.V.map_list ~depth:1 Pp.V.pkgname) );
      ( "available",
        no_cleanup Pp.ppacc with_available available
          (Pp.V.list_depth 1 -| Pp.V.list -| Pp.V.filter) );
      ( "flags",
        with_cleanup cleanup_flags Pp.ppacc add_flags flags
          ( Pp.V.map_list ~depth:1
          @@ Pp.V.ident
             -| Pp.of_pair "package-flag"
                  (pkg_flag_of_string, string_of_pkg_flag) ) );
      ( "setenv",
        no_cleanup Pp.ppacc with_env env
          (Pp.V.map_list ~depth:2 Pp.V.env_binding) );
      ( "build",
        no_cleanup Pp.ppacc with_build build
          (Pp.V.map_list ~depth:2 Pp.V.command) );
      ( "run-test",
        no_cleanup Pp.ppacc with_run_test run_test
          (Pp.V.map_list ~depth:2 Pp.V.command) );
      ( "install",
        no_cleanup Pp.ppacc with_install install
          (Pp.V.map_list ~depth:2 Pp.V.command) );
      ( "remove",
        no_cleanup Pp.ppacc with_remove remove
          (Pp.V.map_list ~depth:2 Pp.V.command) );
      ( "substs",
        no_cleanup Pp.ppacc with_substs substs
          (Pp.V.map_list ~depth:1 pp_basename) );
      ( "patches",
        no_cleanup Pp.ppacc with_patches patches
          ( Pp.V.map_list ~depth:1
          @@ Pp.V.map_option pp_basename (Pp.opt Pp.V.filter) ) );
      ( "build-env",
        no_cleanup Pp.ppacc with_build_env build_env
          (Pp.V.map_list ~depth:2 Pp.V.env_binding) );
      ( "features",
        no_cleanup Pp.ppacc with_features features
          ( Pp.V.map_list ~depth:2
          @@ Pp.V.map_options_2
               (Pp.V.ident -| Pp.of_module "variable" (module OpamVariable))
               (Pp.V.package_formula_items `Conj
                  Pp.V.(filtered_constraints ext_version))
               (Pp.singleton -| Pp.V.string) ) );
      ( "messages",
        no_cleanup Pp.ppacc with_messages messages
          ( Pp.V.map_list ~depth:1
          @@ Pp.V.map_option Pp.V.string_tr (Pp.opt Pp.V.filter) ) );
      ( "post-messages",
        no_cleanup Pp.ppacc with_post_messages post_messages
          ( Pp.V.map_list ~depth:1
          @@ Pp.V.map_option Pp.V.string_tr (Pp.opt Pp.V.filter) ) );
      ( "depexts",
        no_cleanup Pp.ppacc with_depexts depexts
          (Pp.fallback
             ( Pp.V.map_list ~depth:2
      ...
      ( "libraries",
        no_cleanup Pp.ppacc with_libraries libraries
          ( Pp.V.map_list ~depth:1
          @@ Pp.V.map_option Pp.V.string (Pp.opt Pp.V.filter) ) );
      ( "syntax",
        no_cleanup Pp.ppacc with_syntax syntax
          ( Pp.V.map_list ~depth:1
          @@ Pp.V.map_option Pp.V.string (Pp.opt Pp.V.filter) ) );
      ( "dev-repo",
        with_cleanup cleanup_dev_repo Pp.ppacc_opt with_dev_repo dev_repo
          ( Pp.V.string
          -| Pp.of_pair "vc-url"
               OpamUrl.(parse ?backend:None ~handle_suffix:false, to_string) )
      );
      ( "pin-depends",
        no_cleanup Pp.ppacc with_pin_depends pin_depends
          (OpamFormat.V.map_list ~depth:2
             (OpamFormat.V.map_pair
                ( OpamFormat.V.string
                -| OpamPp.of_module "versioned package" (module OpamPackage) )
                (OpamFormat.V.string -| OpamPp.of_module "URL" (module OpamUrl))))
      );
      ( "extra-files",
        no_cleanup Pp.ppacc_opt with_extra_files extra_files
          ( Pp.V.map_list ~depth:2
          @@ Pp.V.map_pair pp_basename
               (Pp.V.string -| Pp.of_module "checksum" (module OpamHash)) ) );
      (* deprecated fields, here for compat *)
      ("configure-style", (Pp.ppacc_ignore, Pp.ppacc_ignore));
      ( "ocaml-version",
        no_cleanup Pp.ppacc_opt with_ocaml_version OpamStd.Option.none
          ( Pp.V.list_depth 1 -| Pp.V.list
          -| Pp.V.constraints Pp.V.compiler_version ) );
      ( "os",
        no_cleanup Pp.ppacc_opt with_os OpamStd.Option.none Pp.V.os_constraint
      );
      ( "descr",
        no_cleanup Pp.ppacc_opt with_descr OpamStd.Option.none
          ( Pp.V.string_tr
          -| Pp.of_pair "descr" Descr.(of_string (), to_string ()) ) );
      ( "extra-sources",
        no_cleanup Pp.ppacc_opt with_extra_sources OpamStd.Option.none
          ( Pp.V.map_list ~depth:2
          @@ Pp.V.map_pair
               (Pp.V.map_option Pp.V.url
                  (Pp.opt @@ (Pp.singleton -| pp_basename)))
               (Pp.V.string -| Pp.of_module "checksum" (module OpamHash))
             -| Pp.pp
                  (fun ~pos:_ ((u, b), md5) ->
                    ( OpamStd.Option.default
                        (OpamFilename.Base.of_string (OpamUrl.basename u))
                        b,
                      URL.create ~checksum:[ md5 ] u ))
                  (fun (f, urlf) ->
                    URL.((url urlf, Some f), List.hd (checksum urlf))) ) );
      ( "build-test",
        no_cleanup Pp.ppacc_opt with_deprecated_build_test OpamStd.Option.none
          (Pp.V.map_list ~depth:2 Pp.V.command) );
      ( "build-doc",
        no_cleanup Pp.ppacc_opt with_deprecated_build_doc
          (fun x -> Some (deprecated_build_doc x))
          (Pp.V.map_list ~depth:2 Pp.V.command) );
    ]

  (* These don't have a printer and their info is stored in new fields *)
  let alias_fields = [ ("author", "authors"); ("descr", "description") ]
  ...
  let fields =
    List.map
      (fun (name, (_, cleaned_up_pp)) -> (name, cleaned_up_pp))
      fields_gen
  ...
  let sections =
    [
      ...
      ( "extra-source",
        Pp.ppacc with_extra_sources extra_sources
          (Pp.map_list
             (Pp.map_pair
                (Pp.pp
                   (fun ~pos -> function
  ...
  let raw_fields =
    List.map (fun (name, (raw_pp, _)) -> (name, raw_pp)) fields_gen
  ...
  let handle_deprecated_available =
    let add_available available filter =
      match available with FBool true -> filter | f -> FAnd (filter, f)
    in
    let parse ~pos:_ t =
      ...
      let available =
        ...
        | Some ocaml_version ->
            ...
            let mk_atom (op, v) = FOp (FIdent ([], var, None), op, FString v) in
  ...
  let pp_raw_fields =
    ...
    -| Pp.map_pair
         ...
         -| Pp.I.on_errors (fun t e ->
                { t with format_errors = e :: t.format_errors })
         -| handle_flags_in_tags -| handle_deprecated_available )
    -| Pp.pp
    ...
    -| Pp.pp
         ...
         (fun (filename, t) ->
           ( filename,
             ...
             | Some nv, _, _ ->
                 if
                   (t.name <> None && t.name <> Some nv.OpamPackage.name)
                   || t.version <> None
                      && t.version <> Some nv.OpamPackage.version
                 then
                   OpamConsole.warning
                     ...
                     @@ OpamPackage.create
                          ...
                          (OpamStd.Option.default nv.OpamPackage.version
                             t.version) )
                     (OpamFilename.prettify filename);
                 { t with name = None; version = None } ))

  let to_string_with_preserved_format ?format_from ?format_from_string filename
      t =
    Syntax.to_string_with_preserved_format ?format_from ?format_from_string
      filename ~empty ~sections ~fields:raw_fields pp t

  let write_with_preserved_format ?format_from ?format_from_string filename t =
    let s =
      to_string_with_preserved_format ?format_from ?format_from_string filename
        t
    in
    OpamFilename.write filename s

  let contents ?(filename = dummy_file) t = Pp.print pp (filename, t)
    ...
    let rec aux acc pfx = function
      ...
      | Section (_, { section_kind; section_name = Some n; section_items }) :: r
        ->
          aux
            (aux acc
               (Printf.sprintf "%s(%s)" section_kind n :: pfx)
               section_items)
            pfx r
  ...
  let effective_part (t : t) =
    {
      ...
      url =
        ...
        | Some u -> (
            ...
            | cksum :: _ -> Some (URL.with_checksum [ cksum ] URL.empty) ) );
      ...
      os = empty.os;
      deprecated_build_test = [];
      (* merged into run_test *)
      deprecated_build_doc = t.deprecated_build_doc;
    }

  let effectively_equal o1 o2 = effective_part o1 = effective_part o2

  let equal o1 o2 = with_metadata_dir None o1 = with_metadata_dir None o2

  let get_metadata_dir ~repos_roots o =
    match metadata_dir o with
    | None -> None
    | Some (None, abs) -> Some (OpamFilename.Dir.of_string abs)
    | Some (Some r, rel) -> Some OpamFilename.Op.(repos_roots r / rel)
    ...
    OpamStd.Option.Op.(
      ( get_metadata_dir ~repos_roots o >>= fun mdir ->
        ...
        >>| List.map @@ fun (basename, hash) ->
            (OpamFilename.create files_dir basename, basename, hash) )
      +! [])
  ...
  let print_errors ?file o =
    if o.format_errors <> [] then
      OpamConsole.error "In the opam file%s:\n%s%s %s been %s."
        ...
        | _, _, Some f, _ -> Printf.sprintf " at %s" (to_string f)
        | _, _, _, Some (None, dir) -> Printf.sprintf " in %s" dir
...
module Dot_installSyntax = struct
  ...
  let add_man_section_dir src =
    ...
    let section =
      let base =
        if Filename.check_suffix file ".gz" then Filename.chop_suffix file ".gz"
        else file
      in
  ...
  let man t =
    List.map
      (fun (src, dst) ->
        (src, match dst with Some _ -> dst | None -> add_man_section_dir src))
      t.man
    ...
    Pp.pp ~name:"file-name"
      (fun ~pos:_ str ->
        ...
        if String.length str > 0 && str.[0] = '?' then
          { optional = true; c = mk (String.sub str 1 (String.length str - 1)) }
        else { optional = false; c = mk str })
      (fun op ->
  ...
  let fields =
    ...
    let pp_misc =
      ...
      @@ Pp.V.map_option
           (Pp.V.string -| pp_optional)
           ( Pp.singleton -| Pp.V.string
           -| Pp.pp ~name:"abs-filename"
                (fun ~pos s ->
                  if not (Filename.is_relative s) then OpamFilename.of_string s
                  else Pp.bad_format ~pos "%s is not an absolute filename." s)
    ...
    @@ Pp.I.check_opam_version ~optional:true ~format_version ()
       ...
       -| Pp.check ~errmsg:"man file without destination or recognised suffix"
            (fun t ->
              List.for_all
                (function
                  | m, None -> add_man_section_dir m <> None | _, Some _ -> true)
                t.man)
end
  ...
  let pp =
    ...
    @@ Pp.I.check_opam_version ~format_version ()
       -| Pp.I.partition (function
            ...
            | _ -> true)
       -| Pp.map_pair
            ( Pp.I.fields ~name ~empty:SwitchSelectionsSyntax.empty fields
            -| Pp.I.show_errors ~name () )
            ( Pp.map_list
...
module CompSyntax = struct
  ...
  let env (t : t) =
    List.map
      (function
        | var, op, value, None ->
            (var, op, value, Some ("Updated by compiler " ^ t.name))
        | b -> b)
      t.env
    ...
    [
      ( "opam-version",
        Pp.ppacc with_opam_version opam_version
          (Pp.V.string -| Pp.of_module "opam-version" (module OpamVersion)) );
      ( "name",
        Pp.ppacc_opt with_name
          (fun t -> if t.name = empty.name then None else Some t.name)
          Pp.V.string );
      ( "version",
        Pp.ppacc_opt with_version
          (fun t -> if t.version = empty.version then None else Some t.version)
          Pp.V.string );
      ("src", Pp.ppacc_opt with_src src Pp.V.url);
      ( "http",
        Pp.ppacc_opt with_src OpamStd.Option.none (Pp.V.url_with_backend `http)
      );
      ( "archive",
        Pp.ppacc_opt with_src OpamStd.Option.none (Pp.V.url_with_backend `http)
      );
      ( "git",
        Pp.ppacc_opt with_src OpamStd.Option.none (Pp.V.url_with_backend `git)
      );
      ( "darcs",
        Pp.ppacc_opt with_src OpamStd.Option.none (Pp.V.url_with_backend `darcs)
      );
      ( "hg",
        Pp.ppacc_opt with_src OpamStd.Option.none (Pp.V.url_with_backend `hg) );
      ( "local",
        Pp.ppacc_opt with_src OpamStd.Option.none (Pp.V.url_with_backend `rsync)
      );
      ( "patches",
        Pp.ppacc with_patches patches (Pp.V.map_list ~depth:1 @@ Pp.V.url) );
      ( "configure",
        Pp.ppacc with_configure configure (Pp.V.map_list ~depth:1 Pp.V.string)
      );
      ("make", Pp.ppacc with_make make (Pp.V.map_list ~depth:1 Pp.V.string));
      ("build", Pp.ppacc with_build build (Pp.V.map_list ~depth:1 Pp.V.command));
      ( "packages",
        Pp.ppacc with_packages packages
          (Pp.V.package_formula `Conj (Pp.V.constraints Pp.V.version)) );
      ("env", Pp.ppacc with_env env (Pp.V.map_list ~depth:2 Pp.V.env_binding));
      ( "preinstalled",
        Pp.ppacc_opt with_preinstalled
          (fun t -> if t.preinstalled then Some true else None)
          Pp.V.bool );
      ("tags", Pp.ppacc with_tags tags (Pp.V.map_list ~depth:1 Pp.V.string));
    ]

  let system_compiler = "system"

  let version_of_name name =
    match OpamStd.String.cut_at name '+' with Some (v, _) -> v | None -> name
  ...
  let pp_raw =
    ...
    @@ Pp.I.check_opam_version ~format_version ()
       ...
       -| Pp.check
            ~errmsg:
              "fields 'build:' and 'configure:'+'make:' are mutually exclusive "
            (fun t -> t.build = [] || (t.configure = [] && t.make = []))

  let of_filename f =
    if OpamFilename.check_suffix f ".comp" then
      f |> OpamFilename.chop_extension |> OpamFilename.basename
         ...
         (fun ~pos (filename, (t : t)) ->
           ( filename,
             ...
             | None ->
                 ...
                 then
                   Pp.bad_format ~pos
                     "File name not in the form <name>.<version>, and missing \
                      'name:' or 'version:' fields"
                 else
                   Pp.warn ~pos
             ...
             | Some name ->
                 let version =
                   if name = "system" then t.version else version_of_name name
                 in
                 if t.name <> empty.name && t.name <> name then
                   Pp.warn ~pos "Mismatching file name and 'name:' field";
                 if
                   name <> system_compiler && t.version <> empty.version
                   && t.version <> version
                 then Pp.warn ~pos "Mismatching file name and 'version:' field";
                 { t with name; version } ))
         (fun (filename, t) ->
           ( filename,
             match of_filename filename with
             ...
             | Some name ->
                 ...
                 in
                 if
                   (t.name <> empty.name && t.name <> name)
                   || name <> system_compiler && t.version <> empty.version
                      && t.version <> version
                 then
                   OpamConsole.warning
                     "Skipping inconsistent 'name:' or 'version:' fields \
                      (%s.%s) while saving %s"
                     t.name version
                     (OpamFilename.to_string filename);
  ...
  let to_package ?package comp descr_opt =
    ...
    let depends =
      OpamFormula.map
        (fun (n, formula) ->
          ...
          Atom
            ( n,
              OpamFormula.ands [ OpamFormula.map cstr formula; Atom post_flag ]
            ))
        (OpamFormula.ands
           [
             Atom
    ...
    in
    let url =
      OpamStd.Option.map (fun url -> URL.with_url url URL.empty) comp.src
    in
    let build, install =
      match comp.build with
      | [] ->
          ( List.map
              (fun l -> nofilter (List.map nofilter l))
              [
    ...
    let extra_sources =
      List.map
        (fun url ->
          (OpamFilename.Base.of_string (OpamUrl.basename url), URL.create url))
        comp.patches
    in
    let patches =
      List.map
