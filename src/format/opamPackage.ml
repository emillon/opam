  ...
  let of_string x =
    ...
    String.iter
      (function
        ...
        | c ->
            failwith
              (Printf.sprintf "Invalid character '%c' in package version %S" c
                 x))
      x;
    x
...
module Name = struct
  ...
  let of_string x =
    ...
    with
    | Some false ->
        failwith (Printf.sprintf "Invalid character in package name %S" x)
    | None ->
        failwith
          (Printf.sprintf "Package name %S should contain at least one letter"
             x)
    | Some true -> x
...
type t = { name : Name.t; version : Version.t }
...
let of_string_opt s =
  if OpamStd.String.contains_char s ' ' || OpamStd.String.contains_char s '\n'
  then None
  else
    match OpamStd.String.cut_at s sep with
    | None -> None
    | Some (n, v) -> (
...
let to_string t =
  ...
  | _ ->
      Printf.sprintf "%s%c%s" (Name.to_string t.name) sep
...
let equal nv1 nv2 = compare nv1 nv2 = 0

let to_json nv =
  `O
    [
      ("name", Name.to_json (name nv)); ("version", Version.to_json (version nv));
    ]
...
let to_map nv =
  Set.fold
    (fun nv map ->
      ...
      try
        Name.Map.add name (Version.Set.add version (Name.Map.find name map)) map
      with Not_found -> Name.Map.add name (Version.Set.singleton version) map)
    nv Name.Map.empty

let of_map nvm =
  Name.Map.fold
    (fun n -> Version.Set.fold (fun v -> Set.add (create n v)))
    nvm Set.empty

let keys map = Map.fold (fun nv _ set -> Set.add nv set) map Set.empty

(* $DIR/$NAME.$VERSION/ *)
let of_dirname f =
  f |> OpamFilename.basename_dir |> OpamFilename.Base.to_string |> of_string_opt
...
let versions_of_packages nvset =
  Set.fold
    ...
    nvset Version.Set.empty

let has_name nvset n = Set.exists (fun nv -> name nv = n) nvset

let names_of_packages nvset =
  Set.fold (fun nv vset -> Name.Set.add (name nv) vset) nvset Name.Set.empty
...
let package_of_name nvset n = Set.choose (packages_of_name nvset n)
...
let packages_of_names nvset nameset =
  Name.Set.fold
    (fun name acc -> Set.union acc (packages_of_name nvset name))
    nameset Set.empty

let versions_of_name packages n =
  versions_of_packages (packages_of_name packages n)
...
module Graph : OpamParallel.GRAPH with type V.t = t = OpamParallel.MakeGraph (O)
