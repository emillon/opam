...
let make_and a b =
  match (a, b) with Empty, r | r, Empty -> r | a, b -> And (a, b)
...
let string_of_formula string_of_a f =
  let rec aux ?(in_and = false) f =
    let paren_if ?(cond = false) s =
      if cond || OpamFormatConfig.(!r.all_parens) then Printf.sprintf "(%s)" s
      else s
    in
    match f with
    | Empty -> "0"
    ...
    | And (x, y) ->
        paren_if
          (Printf.sprintf "%s & %s" (aux ~in_and:true x) (aux ~in_and:true y))
    | Or (x, y) ->
        paren_if ~cond:in_and (Printf.sprintf "%s | %s" (aux x) (aux y))
  in
  aux f
...
let rec map f = function
  ...
  | Block x -> ( match map f x with Empty -> Empty | x -> Block x )
...
let compare_version_formula =
  compare_formula (fun (op1, v1) (op2, v2) ->
      ...
      if c <> 0 then c else compare_relop op1 op2)
...
let verifies f nv =
  let name_formula =
    map
      (fun ((n, _) as a) -> if n = OpamPackage.name nv then Atom a else Empty)
      (dnf_of_formula f)
  in
  name_formula <> Empty
  && eval
    ...
    (fun name acc ->
      ...
      @@ OpamPackage.Set.filter
           (fun nv ->
             ...
             eval
               (fun (_name, cstr) -> check_version_formula cstr v)
               name_formula)
           (OpamPackage.packages_of_name pkgset name))
...
let of_atom_formula (a : atom formula) : t =
  let atom (n, v) =
    match v with None -> Atom (n, Empty) | Some (r, v) -> Atom (n, Atom (r, v))
  in
  map atom a
...
let of_conjunction c = of_atom_formula (ands (List.rev_map (fun x -> Atom x) c))

let ors l = List.fold_left make_or Empty l

let rec ors_to_list = function
  | Empty -> []
  | Or (e, f) -> List.rev_append (rev_ors_to_list e) (ors_to_list f)
  | Block f -> ors_to_list f
  | x -> [ x ]

and rev_ors_to_list = function
  | Empty -> []
  | Or (e, f) -> List.rev_append (ors_to_list f) (rev_ors_to_list e)
...
let atoms t = fold_right (fun accu x -> x :: accu) [] (to_atom_formula t)

let to_cnf t =
  let atf = to_atom_formula t in
  let atoms = fold_right (fun acc a -> a :: acc) [] in
  let conj = rev_ands_to_list atf in
  if List.for_all is_disjunction conj then List.rev_map atoms conj
    (* this gives a nice speedup *)
  else List.rev_map atoms @@ rev_ands_to_list @@ cnf_of_formula atf

let to_dnf t =
  let atf = to_atom_formula t in
  let atoms = fold_right (fun acc a -> a :: acc) [] in
  let disj = rev_ors_to_list atf in
  if List.for_all is_conjunction disj then List.rev_map atoms disj
  else List.rev_map atoms @@ rev_ors_to_list @@ dnf_of_formula atf
...
let of_disjunction d = of_atom_formula (ors (List.rev_map (fun x -> Atom x) d))
...
let set_to_disjunction set t =
  List.map
    (function
      | And _ ->
          failwith
            (Printf.sprintf "%s is not a valid disjunction" (to_string t))
      | Or _ | Block _ | Empty -> assert false
      | Atom (name, Empty) -> [ (name, None) ]
      | Atom (name, Atom a) -> [ (name, Some a) ]
      | Atom (name, cstr) ->
          get_disjunction_formula (OpamPackage.versions_of_name set name) cstr
          |> List.map (function
...
let simplify_ineq_formula vcomp f =
  ...
  let rec mk_ranges acc n =
    if n < 0 then acc else mk_ranges (n :: acc) (n - 1)
  in
  ...
  let vals =
    List.map
      (fun i ->
        (eval (fun (relop, iref) -> check_relop relop (i - iref)) int_formula, i))
      ranges
  in
  if List.for_all (fun (t, _) -> not t) vals then None
  else
    let rec aux = function
    ...
    let rec aux2 = function
      | (((`Geq | `Gt | `Neq) as op), i) :: r ->
          let rec find_upper acc = function
            ...
            | (`Neq, i) :: r -> find_upper (Atom (`Neq, val_of_int i) :: acc) r
...
let gen_formula l f =
  ...
  let rec aux ((t, x) as bound) l =
    match (t, l) with
    | true, (false, y) :: (true, _) :: r | false, (true, y) :: (false, _) :: r
      -> (
        let a = ((if t then `Neq else `Eq), y) in
        match aux bound r with b :: r -> b :: a :: r | r -> a :: r )
    | true, (true, _) :: r | false, (false, _) :: r -> aux bound r
    | true, ((false, _) as bound') :: r | false, ((true, _) as bound') :: r ->
  ...
  let rec aux2 = function
    | (((`Geq | `Neq), _) as a) :: r ->
        let rec find_upper acc = function
          ...
          | ((`Neq, _) as a) :: r -> find_upper (Atom a :: acc) r
          | r -> ands (List.rev acc) :: aux2 r
        in
        find_upper [ Atom a ] r
    | a :: r -> Atom a :: aux2 r
...
let formula_of_version_set set subset =
  ...
  match
    gen_formula (OpamPackage.Version.Set.elements set) (fun x ->
        OpamPackage.Version.Set.mem x subset)
  with
  | Some f -> f
