...
let command_not_found cmd = raise (Command_not_found cmd)

let permission_denied cmd = raise (Permission_denied cmd)
...
let rec mk_temp_dir ?(prefix = "opam") () =
  ...
  if Sys.file_exists s then mk_temp_dir () else s
...
let rm_command = if Sys.win32 then "cmd /d /v:off /c rd /s /q" else "rm -rf"

let remove_dir dir =
  log "rmdir %s" dir;
  if Sys.file_exists dir then
    let err = Sys.command (Printf.sprintf "%s %s" rm_command dir) in
    if err <> 0 then internal_error "Cannot remove %s (error %d)." dir err
...
let logs_cleaner =
  ...
  fun tmp_dir ->
    ...
    else to_clean := OpamStd.String.Set.add tmp_dir !to_clean

let rec temp_file ?(auto_clean = true) ?dir prefix =
  let temp_dir =
    match dir with None -> OpamCoreConfig.(!r.log_dir) | Some d -> d
  in
  mkdir temp_dir;
  let file = temp_dir / temp_basename prefix in
  if Hashtbl.mem temp_files file then temp_file ~auto_clean ?dir prefix
  else (
...
let read file =
  let ic =
    try open_in_bin file with Sys_error _ -> raise (File_not_found file)
  in
...
let write file contents =
  ...
  let oc =
    try open_out_bin file with Sys_error _ -> raise (File_not_found file)
  in
...
let chdir dir =
  try Unix.chdir dir with Unix.Unix_error _ -> raise (File_not_found dir)
...
let in_dir dir fn =
  let reset_cwd =
    ...
    fun () ->
      match cwd with
      | None -> ()
      | Some cwd -> ( try chdir cwd with File_not_found _ -> () )
  in
  chdir dir;
  try
    let r = fn () in
    reset_cwd ();
    r
  with e -> OpamStd.Exn.finalise e reset_cwd
...
let rec_files dir =
  let rec aux accu dir =
    ...
    List.fold_left aux (f @ accu) d
  in
  aux [] dir

let files dir = files_with_links dir

let rec_dirs dir =
  let rec aux accu dir =
    let d = directories_with_links dir in
    List.fold_left aux (d @ accu) d
  in
  aux [] dir

let dirs dir = directories_with_links dir
...
let with_tmp_dir fn =
  ...
  try
    ...
    remove_dir dir;
    e
  with e -> OpamStd.Exn.finalise e @@ fun () -> remove_dir dir
...
let remove file =
  if try Sys2.is_directory file with Sys_error _ -> false then remove_dir file
  else remove_file file
...
let normalize s = try getchdir (getchdir s) with File_not_found _ -> s
...
let real_path p =
  ...
  | None ->
      let rec resolve dir =
        if Sys.file_exists dir then normalize dir
        else
...
let default_env = Unix.environment ()
...
let forward_to_back =
  if Sys.win32 then String.map (function '/' -> '\\' | c -> c) else fun x -> x
...
let t_resolve_command =
    ...
    if Sys.win32 then ( fun f ->
      ...
      try
        ...
        let mask =
          ...
          lor if List.mem st_gid groups then 0o010 else 0
        in
        st_perm land mask <> 0
      with e ->
        OpamStd.Exn.fatal e;
  ...
  let resolve ?dir env name =
    if not (Filename.is_relative name) then
    ...
    else if is_external_cmd name then
      (* relative *)
      let cmd =
        match dir with None -> name | Some d -> Filename.concat d name
      in
      if check_perms cmd then `Cmd cmd else `Denied
    else if (* bare command, lookup in PATH *)
            Sys.win32 then
      ...
      let cmdname =
        OpamStd.(
          List.find_opt
            (fun path -> check_perms (Filename.concat path name))
            path
          |> Option.map (fun path -> Filename.concat path name))
      in
      match cmdname with Some cmd -> `Cmd cmd | None -> `Denied
    else
      let cmd, args =
        ("/bin/sh", [ "-c"; Printf.sprintf "command -v %s" name ])
      in
      let r =
        OpamProcess.run
  ...
  in
  fun ?(env = default_env) ?dir name -> resolve env ?dir name
...
let apply_cygpath name =
  let r =
    OpamProcess.run
      (OpamProcess.command ~name:(temp_file "command") ~verbose:false "cygpath"
         [ name ])
  in
  OpamProcess.cleanup ~force:true r;
  if OpamProcess.is_success r then List.hd r.OpamProcess.r_stdout
  else
    OpamConsole.error_and_exit `Internal_error "Could not apply cygpath to %s"
      name

let get_cygpath_function =
  if Sys.win32 then fun ~command ->
    lazy
      ( if
        OpamStd.(
          Option.map_default Sys.is_cygwin_variant `Native
...
let print_stats () =
  ...
  | l ->
      OpamConsole.msg "%d external processes called:\n%s" (List.length l)
        (OpamStd.Format.itemize ~bullet:"  " (String.concat " ") l)

let log_file ?dir name = temp_file ?dir (OpamStd.Option.default "log" name)

let make_command ?verbose ?(env = default_env) ?name ?text ?metadata
    ?allow_stdin ?stdout ?dir ?(resolve_path = true) cmd args =
  ...
  let full_cmd =
    if resolve_path then t_resolve_command ~env ?dir cmd else `Cmd cmd
  in
  match full_cmd with
  | `Cmd cmd ->
      OpamProcess.command ~env ~name ?text ~verbose ?metadata ?allow_stdin
        ?stdout ?dir cmd args
  | `Not_found -> command_not_found cmd
  | `Denied -> permission_denied cmd

let run_process ?verbose ?(env = default_env) ~name ?metadata ?stdout
    ?allow_stdin command =
...
let verbose_for_base_commands () = OpamCoreConfig.(!r.verbose_level) >= 3

let cygify f = if Sys.win32 then List.map (Lazy.force f) else fun x -> x

let copy_file_aux f src dst =
  if try Sys.is_directory src with Sys_error _ -> raise (File_not_found src)
...
let copy_dir src dst =
  if Sys.file_exists dst then
    if Sys.is_directory dst then
      ...
      | srcfiles ->
          command
            ~verbose:(verbose_for_base_commands ())
            ([ "cp"; "-PRp" ] @ srcfiles @ [ dst ])
    else
      internal_error "Can not copy dir %s to %s, which is not a directory" src
        dst
  else (
    mkdir (Filename.dirname dst);
    command ~verbose:(verbose_for_base_commands ()) [ "cp"; "-PRp"; src; dst ] )
        ...
        if is_pe then (
          try
            ...
            if size_of_opt_header <= 0 || characteristics land 0x2 = 0 then
              raise End_of_file;
            close_in c;
            if characteristics land 0x2000 <> 0 then `Dll arch else `Exe arch
          with End_of_file ->
            close_in c;
            `Unknown )
        else `Exe `i386
...
let default_install_warning dst = function
  | `Add_exe -> OpamConsole.warning "Automatically adding .exe to %s" dst
  | `Install_dll ->
  ...
  | `Install_unknown ->
      ...
      ()
  | `Cygwin -> OpamConsole.warning "%s is a Cygwin-linked executable" dst
  | `Cygwin_libraries ->
      OpamConsole.warning
        "%s links with a Cygwin-compiled DLL (almost certainly a packaging or \
         environment error)"
        dst
...
let install ?(warning = default_install_warning) ?exec src dst =
  ...
  mkdir (Filename.dirname dst);
  let exec = match exec with Some e -> e | None -> is_exec src in
  if Sys.win32 then
    if exec then (
      let dst, cygcheck =
        ...
        | `Exe _ ->
            if
              (not (Filename.check_suffix dst ".exe"))
              && not (Filename.check_suffix dst ".dll")
            then (
              warning dst `Add_exe;
              (dst ^ ".exe", true) )
            else (dst, true)
      ...
      if cygcheck then
        ...
        | `Native -> ()
        | `Cygwin -> warning dst `Cygwin
        | `CygLinked -> warning dst `Cygwin_libraries )
    else copy_file src dst
  else command [ "install"; "-m"; (if exec then "0755" else "0644"); src; dst ]
...
module Tar = struct
  type extract = Bzip2 | Gzip | Lzma | Xz
  ...
  let match_ext file ext = List.exists (Filename.check_suffix file) ext
    ...
    let ext =
      List.fold_left
        (fun acc (ext, t) ->
          match acc with
          | Some t -> Some t
          | None -> if match_ext file ext then Some t else None)
        None extensions
    in
    match ext with
    | Some t -> Some t
    | None -> ( match guess_type file with Some t -> Some t | _ -> None )

  let is_archive file = get_type file <> None
  ...
  let tar_cmd =
    match OpamStd.Sys.os () with OpamStd.Sys.OpenBSD -> "gtar" | _ -> "tar"
...
let is_archive file = Tar.is_archive file || Zip.is_archive file
...
let extract_job ~dir file =
  if not (Sys.file_exists file) then Done (Some (File_not_found file))
  else
    with_tmp_dir_job @@ fun tmp_dir ->
    match extract_command file with
    | None -> Done (Some (Failure ("Unknown archive type: " ^ file)))
    | Some cmd -> (
        cmd tmp_dir @@> fun r ->
        if not (OpamProcess.is_success r) then
          if Zip.is_archive file then Done (Some (Process_error r))
          else
            match Tar.check_extract file with
          ...
          let flist =
            OpamStd.Op.(
              ...
              |> List.filter
                   (not @* OpamStd.String.contains ~sub:"pax_global_header"))
          in
          match flist with
          | [] -> (
              ...
              | _ ->
                  internal_error
                    "The archive %S contains multiple root directories." file )
          | _ -> (
              mkdir (Filename.dirname dir);
              try
                copy_dir tmp_dir dir;
                Done None
              with e ->
                OpamStd.Exn.fatal e;
                Done (Some e) ) )
...
let link src dst =
  ...
  with Unix.Unix_error (Unix.EXDEV, _, _) ->
    ...
    let src =
      if Filename.is_relative src then Filename.dirname dst / src else src
    in
    if Sys.is_directory src then copy_dir src dst else copy_file src dst
...
let unix_lock_op ~dontblock = function
  ...
  | `Lock_write ->
      ...
      else if dontblock then Unix.F_TLOCK
      else Unix.F_LOCK
...
let rec flock_update :
 ...
 fun flag ?(dontblock = OpamCoreConfig.(!r.safe_mode)) lock ->
  log "LOCK %s (%a => %a)" ~level:2 lock.file (slog string_of_lock_kind)
    lock.kind (slog string_of_lock_kind) flag;
  if lock.kind = (flag :> lock_flag) then ()
    ...
    | `Lock_none, { fd = Some fd; kind = `Lock_read | `Lock_write; _ } ->
        ...
        Unix.close fd;
        (* implies Unix.lockf fd Unix.F_ULOCK 0 *)
    ...
    | ((`Lock_read | `Lock_write) as flag), { fd = Some fd; file; kind } ->
        ...
        if kind <> flag then (
          try
            ...
            (if Sys.win32 && kind <> `Lock_none then Unix.(lockf fd F_ULOCK 0));
          ...
          with Unix.Unix_error (Unix.EAGAIN, _, _) ->
            ...
            OpamConsole.formatted_msg
              "Another process has locked %s, waiting (%s to abort)... " file
              (if Sys.win32 then "CTRL+C" else "C-c");
            let rec lock_w_ignore_sig () =
              try Unix.lockf fd (unix_lock_op ~dontblock:false flag) 0 with
              | Sys.Break as e ->
                  OpamConsole.msg "\n";
                  raise e
...
and flock : 'a. ([< lock_flag ] as 'a) -> ?dontblock:bool -> string -> lock =
 fun flag ?dontblock file ->
  ...
  | flag ->
      mkdir (Filename.dirname file);
      let rdflag =
        if (flag :> lock_flag) = `Lock_write then Unix.O_RDWR else Unix.O_RDONLY
      in
...
let get_lock_fd lock =
  match lock.fd with Some fd -> fd | None -> raise Not_found

let lock_max flag1 flag2 =
  match (flag1, flag2) with
  | `Lock_write, _ | _, `Lock_write -> `Lock_write
  | `Lock_read, _ | _, `Lock_read -> `Lock_read
  | `Lock_none, `Lock_none -> `Lock_none

let lock_none = { fd = None; file = ""; kind = `Lock_none }

let lock_isatleast flag lock = lock_max flag lock.kind = lock.kind
...
let get_eol_encoding file =
  ...
  let rec read_lines cr line =
    ...
    | line ->
        if has_cr = cr then read_lines cr line
        else (
          close_in ch;
          None )
    | exception End_of_file ->
        let result =
          if cr = has_cr then Some cr
          else if cr && last_char ch <> '\n' then Some true
          else None
        in
        close_in ch;
        result
  ...
  | line_one -> (
      ...
      match input_line ch with
      | line_two -> read_lines has_cr line_two
      | exception End_of_file ->
          let result = if last_char ch = '\n' then Some has_cr else None in
          close_in ch;
          result )
  | exception End_of_file ->
      close_in ch;
...
let translate_patch ~dir orig corrected =
  ...
  let ch =
    try open_in_bin orig with Sys_error _ -> raise (File_not_found orig)
  in
    ...
    | "@@" :: a :: b :: "@@" :: _ ->
        ...
        if l_a > 1 && l_b > 1 && a.[0] = '-' && b.[0] = '+' then (
          ...
          with e ->
            ...
            `Header )
        else (* TODO Should display some kind of re-sync warning *)
          `Header
    | _ ->
  ...
  let process_state_transition next_state state transforms =
    ...
    | `Processing _, `Processing _ -> transforms
  ...
  let rec fold_lines state n transforms =
    ...
    | line ->
        let line =
          if strip_cr then String.sub line 0 (String.length line - 1) else line
        in
        let length = String.length line in
        let next_state =
          match state with
          | `Header -> (
              ...
              | "--- " ->
                  ...
                  if file = "/dev/null" then `NewHeader
                  else
                    let target =
              ...
              | "*** " ->
                  OpamConsole.warning
                    "File %s uses context diffs which are less portable; \
                     consider using unified diffs"
                    orig;
                  `SkipFile
              | _ ->
          ...
          | `NewHeader ->
              if (if length > 4 then String.sub line 0 4 else "") = "+++ " then
                ...
                `Header
          | `New ->
              process_chunk_header (fun neg pos -> `NewChunk (neg, pos)) line
          | `NewChunk (neg, pos) ->
              (* Weakness: new files should only have + lines *)
              let neg =
                if line = "" || line.[0] = ' ' || line.[0] = '-' then neg - 1
                else neg
              in
              let pos =
                if line = "" || line.[0] = ' ' || line.[0] = '+' then pos - 1
                else pos
              in
              if neg = 0 && pos = 0 then `New
              else
          ...
          | `Processing
              ...
              let patch_crlf =
                ...
                | None -> Some (Some has_cr)
                | Some (Some think_cr) when think_cr <> has_cr ->
                    log ~level:2
                      "Patch adaptation disabled for %s: mixed endings or \
                       binary file"
                      target;
                    Some None
                | _ -> patch_crlf
  ...
  if transforms = [] then copy_file orig corrected
  else (
    seek_in ch 0;
    let ch_out =
    ...
    let normal, add_cr, strip_cr =
      ...
      let id x = x in
      if strip_cr then (strip 1, id, strip 2)
      else (id, (fun s -> s ^ "\r"), strip 1)
    in
    if OpamConsole.debug () then (
      ...
      let rec fold_lines n transforms =
        ...
        | line ->
            let f, transforms =
              ...
              | (first_line, last_line, add_cr_to_chunks) :: next_transforms ->
                  let transforms =
                    if n = last_line then next_transforms else transforms
                  in
                  let f =
                    if n >= first_line then
                      if add_cr_to_chunks then add_cr else strip_cr
                    else normal
                  in
                  (f, transforms)
              | [] -> (normal, [])
        ...
        | exception End_of_file -> close_out ch_out
      in
...
let patch ?(preprocess = true) ~dir p =
  ...
  let patch_cmd =
    ...
    | OpamStd.Sys.OpenBSD | OpamStd.Sys.FreeBSD -> "gpatch"
    | _ -> "patch"
  in
  make_command ~name:"patch" ~dir patch_cmd [ "-p1"; "-i"; p' ] @@> fun r ->
  if not (OpamConsole.debug ()) then Sys.remove p';
  if OpamProcess.is_success r then Done None else Done (Some (Process_error r))
...
let register_printer () =
  Printexc.register_printer (function
    ...
    | Unix.Unix_error (e, fn, msg) ->
        ...
        let error =
          Printf.sprintf "%s: %S failed%s: %s" Sys.argv.(0) fn msg
...
let () = OpamProcess.set_resolve_command resolve_command
