  ...
  OpamStd.Format.itemize
    (fun (f, change) ->
      Printf.sprintf "%s of %s"
        (String.capitalize_ascii (string_of_change change))
        f)
    (SM.bindings t)
...
let get_digest ?(precise = OpamCoreConfig.(!r.precise_tracking)) f size mtime =
  if precise then cached_digest f size mtime else quick_digest f size mtime
...
let is_precise_digest d = not (OpamStd.String.starts_with ~prefix:"F:S" d)
  ...
  let rec make_index acc prefix dir =
    let files =
      ...
      with Sys_error _ as e ->
        log "Error at dir %s: %a"
          (Filename.concat prefix dir)
    ...
    Array.fold_left
      (fun acc f ->
        ...
        if OpamFilename.Base.(Set.mem (of_string rel) except) then acc
        else
          let f = Filename.concat prefix rel in
          try
            let item = item_of_filename f in
            let acc = SM.add rel item acc in
            match item with _, Dir -> make_index acc prefix rel | _ -> acc
...
let revert ?title ?(verbose = OpamConsole.verbose ()) ?(force = false)
  ...
  let already, modified, nonempty, cannot =
    List.fold_left
      (fun ((already, modified, nonempty, cannot) as acc) (fname, op) ->
            ...
            if cur_dg = None then (fname :: already, modified, nonempty, cannot)
              ...
              if OpamFilename.dir_is_empty d then (
                ...
                let nonempty =
                  if
                    List.exists
                      (OpamStd.String.starts_with ~prefix:fname)
                      nonempty
                  then nonempty
                  else fname :: nonempty
                in
        ...
        | Contents_changed dg ->
            if check_digest f dg = `Changed then
              ...
              (* File has changed, assume the removal script reverted it *)
