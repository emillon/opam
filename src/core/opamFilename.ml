...
module Base = struct
  ...
  let check_suffix filename s = Filename.check_suffix filename s

  let add_extension filename suffix = filename ^ "." ^ suffix
end
...
let mk_tmp_dir () = Dir.of_string @@ OpamSystem.mk_temp_dir ()
...
let rmdir dirname = OpamSystem.remove_dir (Dir.to_string dirname)
...
let cwd () = Dir.of_string (Unix.getcwd ())

let mkdir dirname = OpamSystem.mkdir (Dir.to_string dirname)
...
let dir_is_empty d = OpamSystem.dir_is_empty (Dir.to_string d)
...
let exec dirname ?env ?name ?metadata ?keep_going cmds =
  let env = match env with None -> None | Some l -> Some (env_of_list l) in
  in_dir dirname (fun () ->
      OpamSystem.commands ?env ?name ?metadata ?keep_going cmds)

let move_dir ~src ~dst =
  OpamSystem.command
    ~verbose:(OpamSystem.verbose_for_base_commands ())
...
let opt_dir dirname = if exists_dir dirname then Some dirname else None
...
let link_dir ~target ~link =
  if exists_dir link then
    OpamSystem.internal_error "Cannot link: %s already exists."
      (Dir.to_string link)
  else OpamSystem.link (Dir.to_string target) (Dir.to_string link)

let to_list_dir dir =
  let base d = Dir.of_string (Filename.basename (Dir.to_string d)) in
  let rec aux acc dir =
    let d = dirname_dir dir in
    if d <> dir then aux (base dir :: acc) d else base dir :: acc
  in
  aux [] dir
...
type t = { dirname : Dir.t; basename : Base.t }

let create dirname basename =
  let b1 =
    OpamSystem.forward_to_back (Filename.dirname (Base.to_string basename))
  in
  let b2 = Base.of_string (Filename.basename (Base.to_string basename)) in
  let dirname = OpamSystem.forward_to_back dirname in
  if basename = b2 then { dirname; basename }
  else { dirname = dirname / b1; basename = b2 }
...
let touch t = OpamSystem.write (to_string t) ""

let chmod t p = Unix.chmod (to_string t) p

let of_string s =
  let dirname = Filename.dirname s in
  let basename = Filename.basename s in
  { dirname = Dir.of_string dirname; basename = Base.of_string basename }

let dirname t = t.dirname

let basename t = t.basename

let read filename = OpamSystem.read (to_string filename)
...
let write filename raw = OpamSystem.write (to_string filename) raw

let remove filename = OpamSystem.remove_file (to_string filename)

let exists filename =
  try (Unix.stat (to_string filename)).Unix.st_kind = Unix.S_REG
  with Unix.Unix_error _ -> false

let opt_file filename = if exists filename then Some filename else None

let with_contents fn filename = fn (read filename)

let check_suffix filename s = Filename.check_suffix (to_string filename) s

let add_extension filename suffix = of_string (to_string filename ^ "." ^ suffix)
...
let install ?warning ?exec ~src ~dst () =
  if src <> dst then
    OpamSystem.install ?warning ?exec (to_string src) (to_string dst)
...
let readlink src =
  ...
  else OpamSystem.internal_error "%s does not exist." (to_string src)
...
let starts_with dirname filename =
  OpamStd.String.starts_with ~prefix:(Dir.to_string dirname)
    (to_string filename)
...
let remove_prefix prefix filename =
  let prefix =
    ...
    if str = "" then "" else Filename.concat str ""
  in
  ...
  let basename =
    ...
    | Some r ->
        if starts_with r src then remove_prefix r src
          ...
          OpamSystem.internal_error "%s is not a prefix of %s" (Dir.to_string r)
            (to_string src)
  in
...
let is_archive filename = OpamSystem.is_archive (to_string filename)
...
let extract_generic_file filename dirname =
  match filename with
  | F f ->
      log "extracting %a to %a" (slog to_string) f (slog Dir.to_string) dirname;
      extract f dirname
  | D d ->
      if d <> dirname then (
        log "copying %a to %a" (slog Dir.to_string) d (slog Dir.to_string)
          dirname;
...
let rec find_in_parents f dir =
  if f dir then Some dir
  else
    let parent = dirname_dir dir in
    if parent = dir then None else find_in_parents f parent

let link ?(relative = false) ~target ~link =
  if target = link then ()
  else
    let target =
      if not relative then to_string target
      else
        match
          find_in_parents
            (fun d -> d <> "/" && starts_with d link)
            (dirname target)
        with
...
let flock flag ?dontblock file =
  OpamSystem.flock flag ?dontblock (to_string file)
...
let with_flock flag ?dontblock file f =
  ...
  try
    let fd, ch =
      ...
      | exception Not_found ->
          let null =
            if OpamStd.Sys.(os () = Win32) then "nul" else "/dev/null"
          in
    ...
    r
  with e -> OpamStd.Exn.finalise e @@ fun () -> OpamSystem.funlock lock
...
let with_flock_upgrade flag ?dontblock lock f =
  if OpamSystem.lock_isatleast flag lock then f (OpamSystem.get_lock_fd lock)
    ...
    try
      ...
      r
    with e ->
      OpamStd.Exn.finalise e @@ fun () -> OpamSystem.flock_update old_flag lock
...
let prettify_dir d = prettify_path (Dir.to_string d)

let prettify s = prettify_path (to_string s)
...
module Op = struct
  ...
  let ( // ) d1 s2 =
    ...
    if d <> "." then create (d1 / d) (Base.of_string b)
    else create d1 (Base.of_string s2)
end

module Attribute = struct
  type t = { base : Base.t; md5 : OpamHash.t; perm : int option }
  ...
  let create base md5 perm = { base; md5; perm }

  let to_string_list t =
    let perm =
      match t.perm with None -> [] | Some p -> [ Printf.sprintf "0o%o" p ]
    in
  ...
  let of_string_list = function
    ...
    | k ->
        OpamSystem.internal_error "remote_file: '%s' is not a valid line."
          (String.concat " " k)
        ...
        try
          ...
          let perm =
            if not (List.mem_assoc "perm" dict) then None
              ...
              match List.assoc "perm" dict with
              | `String hash -> (
