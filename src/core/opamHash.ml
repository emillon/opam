...
let len = function `MD5 -> 32 | `SHA256 -> 64 | `SHA512 -> 128
...
let of_string_opt s =
  try
    ...
    in
    if valid kind s then Some (kind, String.lowercase_ascii s) else None
...
let to_string (kind, s) = String.concat pfx_sep_str [ string_of_kind kind; s ]

let to_json s = `String (to_string s)

let of_json = function `String s -> of_string_opt s | _ -> None

let to_path (kind, s) = [ string_of_kind kind; String.sub s 0 2; s ]
...
let compute ?(kind = default_kind) file =
  ...
  | (`SHA256 | `SHA512) as kind ->
      let sha =
        if not OpamCoreConfig.(!r.use_openssl) then OpamSHA.hash kind file
        else
          try
            match
              OpamSystem.read_command_output
                [ "openssl"; string_of_kind kind; file ]
            with
            | [ l ] ->
                let len = len kind in
...
let compute_from_string ?(kind = default_kind) str =
  match kind with
