...
type console_font_infoex = {
  font : int;  (** Index in the system's console font table *)
...
type _ registry_value = REG_SZ : string registry_value

(** SHGetFolderPath flags *)
type shGFP_type =
  | SHGFP_TYPE_CURRENT  (** Retrieve the current path *)
  | SHGFP_TYPE_DEFAULT  (** Retrieve the default path *)
