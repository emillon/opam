...
let empty = { backend = `http; transport = "https"; path = ""; hash = None }
           ...
           [
             ...
             @@ seq
                  [
                    ...
                    opt
                    @@ seq [ char '.'; group @@ rep1 @@ diff any (set "\\/.#") ];
                  ];
...
let string_of_vc = function `git -> "git" | `darcs -> "darcs" | `hg -> "hg"
    ...
    | None -> (
        ...
        | None -> (
            let of_suffix ~default =
              if not handle_suffix then default
              else
                match suffix with
            ...
            | Some tr -> (
                try vc_of_string tr
                with Failure _ -> of_suffix ~default:(backend_of_string tr) ) )
        )
  in
  let transport, path =
    match (backend, transport, looks_like_ssh_path path) with
    | `http, None, _ -> ("http", path)
    | _, (None | Some ("git" | "hg" | "darcs")), Some path -> ("ssh", path)
    | _, (None | Some ("hg" | "darcs")), None ->
        ("file", OpamSystem.real_path path |> OpamSystem.back_to_forward)
    | _, Some tr, _ -> (tr, path)
  in
  { transport; path; hash; backend }
...
let to_string url =
  ...
  | #version_control as vc ->
      ...
      if url.transport = vc then
        (* Don't be redundant on e.g git:// protocols *)
        Printf.sprintf "%s://%s%s" vc url.path hash
      else Printf.sprintf "%s+%s://%s%s" vc url.transport url.path hash
  | `rsync | `http -> Printf.sprintf "%s://%s%s" url.transport url.path hash
...
let local_path = function
  | {
      transport = "file" | "path" | "local" | "rsync";
      path;
      hash = _;
      backend = #version_control | `rsync;
    }
...
let local_dir url =
  ...
  local_path url >>| OpamFilename.Dir.of_string >>= fun d ->
  if OpamFilename.exists_dir d then Some d else None

let local_file url =
  let open OpamStd.Option.Op in
  local_path url >>| OpamFilename.of_string >>= fun f ->
...
let guess_version_control s =
  ...
  if vc = None && transport = None && looks_like_ssh_path path = None then
    ...
    if exists_dir (dir / ".git") || exists (dir // ".git") then Some `git
    else if exists_dir (dir / ".hg") then Some `hg
    else if exists_dir (dir / "_darcs") then Some `darcs
    else None
  else None
...
let basename =
  ...
  in
  fun t -> try Re.Group.get (Re.exec re t.path) 1 with Not_found -> ""
...
let root =
  ...
  fun t ->
    let path =
      ...
      if t.transport = "file" then "" else Re.replace_string re ~by:"" t.path
    in
    { t with path }

let has_trailing_slash url = OpamStd.String.ends_with ~suffix:"/" url.path
...
let map_file_url f url =
  if url.transport = "file" then { url with path = f url.path } else url
...
module Op = struct
  ...
  let ( / ) url dir =
    let url = if Filename.is_relative dir then url else root url in
