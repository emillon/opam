...
module Make (G : G) = struct
  ...
  let aux_map ~jobs ~command ?(dry_run = false) ?(pools = []) g =
    log "Iterate over %a task(s) with %d process(es)"
      (slog @@ G.nb_vertex @> string_of_int)
      g jobs;
    ...
    let pools =
      let defined =
        List.map (fun (elts, jobs) -> (S.of_list elts, jobs)) pools
      in
      let default =
        ( List.fold_left (fun acc (pool, _) -> acc -- pool) all_jobs defined,
          jobs )
      in
      default :: defined
    in
    ...
    ( if G.has_cycle g then
      ...
      let sccs =
        List.filter (function _ :: _ :: _ -> true | _ -> false) sccs
      in
      raise (Cyclic sccs) );

    let print_status (finished : int)
        (running : (OpamProcess.t * 'a * string option) M.t) =
      let texts =
        OpamStd.List.filter_map (fun (_, _, t) -> t) (M.values running)
      in
      let rec limit_width acc rem_cols = function
        ...
        | t :: ts ->
            ...
            if ts = [] && len < rem_cols then List.rev (t :: acc)
              ...
              List.rev
                ( Printf.sprintf "%s+%2d"
                    (String.make (rem_cols - 4) ' ')
                    (List.length ts + 1)
                :: acc )
            else limit_width (t :: acc) (rem_cols - len - 1) ts
      in
      let title =
        Printf.sprintf "Processing %2d/%d:"
          (finished + M.cardinal running)
          njobs
      in
      let texts =
    ...
    let rec loop (nslots : (S.t * int) list) (* number of free slots *)
        (results : 'b M.t) (running : (OpamProcess.t * 'a * string option) M.t)
        (ready : S.t) =
      ...
      let release_slot nslots n =
        List.map
          (fun (pool, slots) ->
            if S.mem n pool then (pool, slots + 1) else (pool, slots))
          nslots
      in
      let run_seq_command nslots ready n = function
        ...
        | Run (cmd, cont) ->
            log "Next task in job %a: %a"
              (slog (string_of_int @* V.hash))
              n
              (slog OpamProcess.string_of_command)
              cmd;
            let p =
      ...
      let fail node error =
        ...
        ( try List.iter (fun _ -> ignore (OpamProcess.wait_one pend)) pend
          with e ->
            log "%a in sub-process cleanup" (slog Printexc.to_string) e );
      ...
      if M.is_empty running && S.is_empty ready then results
      else if
      ...
      then (
        ...
        let ready =
          List.fold_left
            (fun acc (pool, slots) -> if slots = 0 then acc -- pool else acc)
            (S.remove n ready) (get_slots nslots n)
        in
        ...
        in
        let process, result =
          if dry_run then OpamProcess.dry_wait_one (List.map fst processes)
          else
            try
...
module MakeGraph (X : VERTEX) = struct
  ...
  let to_json (graph : t) : OpamJson.t =
    ...
    let vertices =
      ...
      `O (List.map vertex_to_json vertex_map)
    in
    let edges =
    ...
    in
    `O [ ("vertices", vertices); ("edges", edges) ]

  let of_json : t OpamJson.decoder = function
    | `O dict -> (
        try
          let vertices_json =
            match List.assoc "vertices" dict with
            | `O vertices -> vertices
            | _ -> raise Not_found
          in
          let edges_json =
            match List.assoc "edges" dict with
            | `A edges -> edges
            | _ -> raise Not_found
          in
          let vertex_map =
            let vertex_of_json (ij, vj) =
              ...
              let v =
                match X.of_json vj with None -> raise Not_found | Some v -> v
              in
              (i, v)
            in
          ...
          let edges =
            ...
            let edge_of_json = function
              ...
              | _ -> raise Not_found
            in
            List.map edge_of_json edges_json
          in
...
let map ~jobs ~command ?dry_run l =
  ...
  let rec mklist acc n =
    if n < 0 then acc else mklist (IntGraph.Parallel.M.find n r :: acc) (n - 1)
  in
