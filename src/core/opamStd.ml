...
module OpamList = struct
    ...
    | l ->
        ...
        let strs, len =
          List.fold_left
            (fun (strs, len) x ->
              let s = f x in
              (s :: strs, String.length s + seplen + len))
        ...
        let pos =
          List.fold_left
            (fun (pos, cur_sep) s -> (prepend (prepend pos cur_sep) s, sep))
            (pos, last_sep) (List.tl strs)
          |> fst
        in
  ...
  let to_string f = concat_map ~left:"{ " ~right:" }" ~nil:"{}" ", " f
  ...
  let filter_map f l =
    let rec loop accu = function
      | [] -> List.rev accu
      | h :: t -> (
          match f h with None -> loop accu t | Some x -> loop (x :: accu) t )
    in
    loop [] l
  ...
  let rec find_map f = function
    | [] -> raise Not_found
    | x :: r -> ( match f x with Some r -> r | None -> find_map f r )
  ...
  let pick_assoc x l =
    let rec aux acc = function
      | [] -> (None, l)
      | ((k, v) as b) :: r ->
          if k = x then (Some v, List.rev_append acc r) else aux (b :: acc) r
    in
    aux [] l
  ...
  let update_assoc k v l =
    let rec aux acc = function
      | [] -> List.rev ((k, v) :: acc)
      | ((k1, _) as b) :: r ->
          if k1 = k then List.rev_append acc ((k, v) :: r) else aux (b :: acc) r
    in
    aux [] l
end
...
module Set = struct
  module Make (O : OrderedType) = struct
    ...
    let fold f set i =
      let r = ref i in
      S.iter (fun elt -> r := f elt !r) set;
      !r

    let is_singleton s = (not (is_empty s)) && min_elt s == max_elt s
    ...
    let map f t = S.fold (fun e set -> S.add (f e) set) t S.empty

    exception Found of elt

    let find_opt fn t =
      try
        iter (fun x -> if fn x then raise (Found x)) t;
        None
      with Found x -> Some x

    let find fn t =
      match find_opt fn t with Some x -> x | None -> raise Not_found
    ...
    let safe_add elt t =
      if mem elt t then
        failwith (Printf.sprintf "duplicate entry %s" (O.to_string elt))
      else add elt t
...
module Map = struct
  module Make (O : OrderedType) = struct
    ...
    let fold f map i =
      let r = ref i in
      M.iter (fun key value -> r := f key value !r) map;
      !r

    let map f map = fold (fun key value map -> add key (f value) map) map empty

    let mapi f map =
      fold (fun key value map -> add key (f key value) map) map empty

    let values map = List.rev (M.fold (fun _ v acc -> v :: acc) map [])

    let keys map = List.rev (M.fold (fun k _ acc -> k :: acc) map [])
    ...
    let is_singleton s =
      (not (is_empty s)) && fst (min_binding s) == fst (max_binding s)
    ...
    let to_string string_of_value m =
      if M.cardinal m > max_print then
        ...
        let s (k, v) =
          Printf.sprintf "%s:%s" (O.to_string k) (string_of_value v)
        in
        let l = fold (fun k v l -> s (k, v) :: l) m [] in
        OpamList.to_string (fun x -> x) l

    let of_list l = List.fold_left (fun map (k, v) -> add k v map) empty l
    ...
    let to_json json_of_value t =
      ...
      let jsons =
        List.map
          (fun (k, v) ->
            `O [ ("key", O.to_json k); ("value", json_of_value v) ])
          bindings
      in
      `A jsons
    ...
    let safe_add k v map =
      if mem k map then
        failwith (Printf.sprintf "duplicate entry %s" (O.to_string k))
      else add k v map
...
module AbstractString = struct
  ...
  let of_json = function `String x -> Some x | _ -> None
...
module Option = struct
  ...
  let default dft = function None -> dft | Some x -> x

  let default_map dft = function None -> dft | some -> some

  let replace f = function None -> None | Some x -> f x

  let map_default f dft = function None -> dft | Some x -> f x
  ...
  let to_string ?(none = "") f = function Some x -> f x | None -> none
  ...
  let of_Not_found f x = try Some (f x) with Not_found -> None

  module Op = struct
    let ( >>= ) = function None -> fun _ -> None | Some x -> fun f -> f x

    let ( >>| ) opt f = map f opt

    let ( >>+ ) opt f = match opt with None -> f () | some -> some

    let ( +! ) opt dft = default dft opt

    let ( ++ ) = function None -> fun opt -> opt | some -> fun _ -> some
  end
end
...
module OpamString = struct
  module OString = struct
    ...
    let of_json = function `String s -> Some s | _ -> None
  end
  ...
  let contains ~sub = Re.(execp (compile (str sub)))
  ...
  let find_from f s i =
    ...
    if i < 0 || i > l then invalid_arg "find_from"
    else
      let rec g i =
        if i < l then if f s.[i] then i else g (succ i) else raise Not_found
      in
      g i
  ...
  let strip_right str =
    ...
    let l = String.length str in
    let i = aux (l - 1) in
    if i = l - 1 then str else String.sub str 0 (i + 1)

  let sub_at n s = if String.length s <= n then s else String.sub s 0 n
  ...
  let split_delim s c =
    ...
    in
    let acc0 = match tokens with `Delim _ :: _ -> [ "" ] | _ -> [] in
    List.rev (aux acc0 tokens)
  ...
  *)
  let cut_value ~prefix c v =
    ...
    let rec aux before = function
      | [] -> ([], List.rev before)
      | curr :: after when OpamString.starts_with ~prefix curr -> (before, after)
  ...
  let escape_single_quotes ?(using_backslashes = false) =
    ...
    else Re.(replace_string (compile (char '\'')) ~by:"'\"'\"'")
end
...
module OpamSys = struct
    ...
    if Sys.win32 then fun path ->
      ...
      let rec f acc index current last normal =
        if index = length then
          ...
          let c = path.[index] and next = succ index in
          if (c = ';' && normal) || c = '"' then
            let current = current ^ String.sub path last (index - last) in
            if c = '"' then f acc next current next (not normal)
            else
              let acc = if current = "" then acc else current :: acc in
              f acc next "" next true
          else f acc next current last normal
      in
      f [] 0 "" 0 true
    else fun path ->
  ...
  let with_process_in cmd args f =
    if Sys.win32 then assert false;
  ...
  let default_columns =
    ...
    let cols =
      try int_of_string (Env.get "COLUMNS")
    ...
    let () =
      try
        Sys.set_signal 28 (* SIGWINCH *)
          (Sys.Signal_handle (fun _ -> v := lazy (get_terminal_columns ())))
      with Invalid_argument _ -> ()
    in
    if Sys.win32 then fun () ->
      if tty_out then win32_get_console_width () else default_columns
    else fun () -> if tty_out then Lazy.force !v else default_columns
  ...
  let uname =
    ...
    fun arg ->
      try Hashtbl.find memo arg
      with Not_found ->
        let r =
          try
            with_process_in "uname" arg (fun ic ->
                Some (OpamString.strip (input_line ic)))
  ...
  let guess_shell_compat () =
    let parent_guess =
      if Sys.win32 then None
        ...
        try Some (Unix.readlink (Filename.concat dir "exe"))
        with e -> (
          fatal e;
          try
            with_process_in "ps"
              (Printf.sprintf "-p %d -o comm= 2>/dev/null" ppid) (fun ic ->
                Some (input_line ic))
          with
          | Unix.Unix_error _ | Sys_error _ | Failure _ | End_of_file
          | Not_found
          -> (
            try
              let c =
                open_in_bin ("/proc/" ^ string_of_int ppid ^ "/cmdline")
              in
              try
    ...
    let shell =
      ...
      | None -> Option.of_Not_found Env.get "SHELL" |> Option.replace test
      | some -> some
    in
    Option.default SH_sh shell

  let guess_dot_profile shell =
    let home f = try Filename.concat (home ()) f with Not_found -> f in
    match shell with
    | SH_fish ->
        List.fold_left Filename.concat (home ".config")
          [ "fish"; "config.fish" ]
    | SH_zsh -> home ".zshrc"
    | SH_bash -> (
        try
  ...
  let exec_at_exit () =
    List.iter (fun f -> try f () with _ -> ()) !registered_at_exit
  ...
  let is_cygwin_variant =
    if Sys.win32 then (
        ...
        let rec f a =
          ...
          | x ->
              if OpamString.ends_with ~suffix:"cygwin1.dll" (String.trim x) then
                if OpamString.starts_with ~prefix:"  " x then f `Cygwin
                else if a <> `Cygwin then f `CygLinked
                else f a
              else f a
          | exception _ ->
      ...
      in
      fun name ->
        if Filename.is_relative name then requires_cygwin name
        else
          try Hashtbl.find results name
          with Not_found ->
            let result = requires_cygwin name in
...
module Win32 = struct
  module RegistryHive = struct
    ...
    let of_string = function
      | "HKCR" | "HKEY_CLASSES_ROOT" -> OpamStubs.HKEY_CLASSES_ROOT
      | "HKCC" | "HKEY_CURRENT_CONFIG" -> OpamStubs.HKEY_CURRENT_CONFIG
      | "HKCU" | "HKEY_CURRENT_USER" -> OpamStubs.HKEY_CURRENT_USER
      | "HKLM" | "HKEY_LOCAL_MACHINE" -> OpamStubs.HKEY_LOCAL_MACHINE
      | "HKU" | "HKEY_USERS" -> OpamStubs.HKEY_USERS
      | _ -> failwith "RegistryHive.of_string"
  end

  let set_parent_pid, parent_putenv =
    let ppid =
      ref (lazy OpamStubs.(getCurrentProcessID () |> getParentProcessID))
    in
    let parent_putenv =
      lazy
        (let ppid = Lazy.force !ppid in
         if OpamStubs.isWoW64 () <> OpamStubs.isWoW64Process ppid then (
           ...
           in
           at_exit quit_putenv;
           if Sys.file_exists putenv_exe then ( fun key value ->
             if !ctrl = stdout then (
             ...
             let warning =
               lazy
                 ( !console.warning
                     "opam-putenv was not found - OPAM is unable to alter \
    ...
    in
    ( (fun pid ->
        if Lazy.is_val parent_putenv then failwith "Target parent already known";
  ...
  let persistHomeDirectory dir =
    ...
    OpamStubs.(
      sendMessageTimeout hWND_BROADCAST 5000 sMTO_ABORTIFHUNG WM_SETTINGCHANGE 0
        "Environment")
    |> ignore
end
...
module OpamFormat = struct
      ...
      if i >= len then acc
        ...
        | '\027' -> (
            try
              ...
              if j > len then acc - (len - i)
              else aux (acc - (j - i + 1)) (j + 1)
            with Not_found | Invalid_argument _ -> acc - (len - i) )
        | _ -> aux acc (i + 1)
    in
    aux len 0
  ...
  let cut_at_visual s width =
    let rec aux extra i =
      ...
      with
      | Not_found -> min (String.length s) (width + extra)
      | Invalid_argument _ -> String.length s
    in
    let cut_at = aux 0 0 in
    if cut_at = String.length s then s
    else
  ...
  let indent_left s ?(visual = s) nb =
    ...
    if nb <= 0 then s else s ^ String.make nb ' '

  let indent_right s ?(visual = s) nb =
    let nb = nb - String.length visual in
    if nb <= 0 then s else String.make nb ' ' ^ s
  ...
  let align_table ll =
    let rec transpose ll =
      if List.for_all (( = ) []) ll then []
        ...
        let col, rest =
          List.fold_left
            (fun (col, rest) -> function hd :: tl -> (hd :: col, tl :: rest)
              | [] -> ("" :: col, [] :: rest))
            ([], []) ll
        in
    ...
    let pad n s =
      ...
      if sn >= n then s else s ^ String.make (n - sn) ' '
    in
    let pad_multi n s =
  ...
  let reformat ?(start_column = 0) ?(indent = 0)
      ?(width = OpamSys.terminal_columns ()) s =
    let slen = String.length s in
    let buf = Buffer.create 1024 in
    let rec find_nonsp i =
      if i >= slen then i
      else match s.[i] with ' ' -> find_nonsp (i + 1) | _ -> i
    in
...
module Config = struct
  ...
  let env conv var =
    ...
    with Failure _ ->
      flush stdout;
      !console.warning "Invalid value for environment variable OPAM%s, ignored."
        var;
      None
      ...
      (fun s ->
        let f map elt =
          let parse_value (section, value) =
            try (section, Some (int_of_string value))
            with Failure _ -> (section, None)
          in
          let section, level =
            Option.map_default parse_value (elt, None)
              (OpamString.cut_at elt ':')
          in
          OpamCoreConfig.StringMap.add section level map
        in
        List.fold_left f OpamCoreConfig.StringMap.empty (OpamString.split s ' '))
      var

  let env_string var = env (fun s -> s) var

  let env_float var = env float_of_string var
  ...
  let initk k =
    ...
    in
    let answer =
      match (env_bool "YES", env_bool "NO") with
    ...
    OpamCoreConfig.(
      ...
      ?verbose_level:(env_level "VERBOSE") ?color:(env_when "COLOR") ?utf8
      ?disp_status_line:(env_when "STATUSLINE") ?answer
      ?safe_mode:(env_bool "SAFE") ?log_dir:(env_string "LOGS")
      ?keep_log_dir:(env_bool "KEEPLOGS") ?errlog_length:(env_int "ERRLOGLEN")
      ?merged_output:(env_bool "MERGEOUT") ?use_openssl:(env_bool "USEOPENSSL")
