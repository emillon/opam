...
let dumb_term =
  lazy (try OpamStd.Env.get "TERM" = "dumb" with Not_found -> not Sys.win32)

let color =
  let auto = lazy (OpamStd.Sys.tty_out && not (Lazy.force dumb_term)) in
  fun () ->
    match OpamCoreConfig.(!r.color) with
    | `Always -> true
    | `Never -> false
    ...
    lazy
      ( if Sys.win32 then
        let attempt handle =
           ...
           *)
          info.fontFamily land 0x4 <> 0
          && OpamStubs.getConsoleOutputCP () = 65001
        in
        try attempt OpamStubs.STD_OUTPUT_HANDLE
        with Not_found -> (
          try attempt OpamStubs.STD_INPUT_HANDLE with Not_found -> false )
      else
        let checkv v =
          try
            Some (OpamStd.String.ends_with ~suffix:"UTF-8" (OpamStd.Env.get v))
          with Not_found -> None
        in
        OpamStd.Option.Op.(checkv "LC_ALL" ++ checkv "LANG" +! false) )
  in
  ( (fun () ->
      match OpamCoreConfig.(!r.utf8) with
        ...
        if Sys.win32 then
          ...
          let checker =
            ...
            | { contents = None } ->
                ...
                win32_glyph_checker := Some checker;
                checker
            | { contents = Some checker } -> checker
          in
          let check_glyph scalar =
            try Hashtbl.find checker.glyphs scalar
            with Not_found ->
...
let global_start_time = Unix.gettimeofday ()
...
let colorise style s =
  if not (color ()) then s
  else Printf.sprintf "\027[%sm%s\027[0m" (style_code style) s
...
let enable_win32_vt100 ch =
  ...
  try
    ...
    let vt100_on = 0x4 in
    if mode land vt100_on <> 0 then (hConsoleOutput, VT100)
    else if OpamStubs.setConsoleMode hConsoleOutput (mode lor vt100_on) then (
      let restore_console () =
        let mode = OpamStubs.getConsoleMode hConsoleOutput land lnot vt100_on in
        OpamStubs.setConsoleMode hConsoleOutput mode |> ignore
      in
      at_exit restore_console;
      (hConsoleOutput, VT100) )
    else (hConsoleOutput, Shim)
  with Not_found -> (hConsoleOutput, VT100)
...
let get_win32_console_shim : type s. [ `stdout | `stderr ] -> s shim_return -> s
    =
 fun ch ->
  let ch = if ch = `stdout then stdout_state else stderr_state in
  function
  | Handle -> Lazy.force ch
  | Mode -> Lazy.force ch |> snd
  | Peek ->
      fun mode -> if Lazy.is_val ch then snd (Lazy.force ch) = mode else false
...
let is_windows_10 =
  lazy
    (let v, _, _, _ = OpamStubs.getWindowsVersion () in
     v >= 10)

let win32_print_message ch msg =
  let ocaml_ch = match ch with `stdout -> stdout | `stderr -> stderr in
    ...
    if mode = VT100 then (
      ...
      let execute_code =
        ...
        let blend ?(inheritbold = true) bits =
          let bits =
            if inheritbold then !color land 0b1000 lor (bits land 0b111)
            else bits
          in
          let result =
            attributes land lnot 0b1111 lor (bits land 0b1000)
        ...
        fun code ->
          ...
          let f attributes attribute =
            ...
            | "1" | "01" -> blend ~inheritbold:false (!color lor 0b1000)
            | "4" | "04" ->
            ...
            | "32" -> blend 0b010
            | "33" -> blend 0b110
            | "34" -> blend ~inheritbold:false 0b001
            | "35" -> blend 0b101
            | "36" -> blend 0b011
            | "37" -> blend 0b111
            | "0" | "" -> blend ~inheritbold:false 0b0111
            | _ -> assert false
          in
      ...
      let rec f index start in_code =
        if index < length then
          ...
          if c = '\027' then (
            ...
            if fragment <> "" then Printf.fprintf ocaml_ch "%s%!" fragment;
            f index index true )
          else if in_code && c = 'm' then (
            let fragment = String.sub msg start (index - start) in
            let index = succ index in
            execute_code fragment;
            f index index false )
          else f (succ index) start in_code
        else
          let fragment = String.sub msg start (index - start) in
          if fragment <> "" then
            if in_code then execute_code fragment
            else Printf.fprintf ocaml_ch "%s%!" fragment
          else flush ocaml_ch
      in
      flush ocaml_ch;
      f 0 0 false

let carriage_delete_unix _ = print_string "\r\027[K"
      ...
      lazy
        ...
        | Shim ->
            fun () ->
              ...
              displaying_status := false
        | VT100 -> clear_status_unix )
    in
    fun () -> Lazy.force clear_status ()
  else clear_status_unix

let print_message =
  if Sys.win32 then ( fun ch fmt ->
...
let log section ?(level = 1) fmt =
  let debug_level =
    ...
    if OpamCoreConfig.StringMap.is_empty sections then debug_level
    else
  ...
  if level <= abs debug_level then
    ...
    if Sys.win32 then (
       ...
       *)
      win32_print_message `stderr
        (Printf.sprintf "%s%a  " timestamp
           (acolor_with_width (Some 30) `yellow)
           section);
...
let win32_print_functions =
  lazy
    ( match get_win32_console_shim `stdout Mode with
    | Shim -> (true, fun s -> win32_print_message `stdout (s ^ "\n"))
    | VT100 -> (false, print_endline) )

let status_line fmt =
  let batch = debug () || not (disp_status_line ()) in
  let use_shim, print_msg =
    if Sys.win32 then Lazy.force win32_print_functions
    ...
    Printf.ksprintf
      (fun s ->
        if s <> !last_status then (
          ...
          print_msg s ))
      fmt
  else if use_shim then write_status_windows fmt
  else write_status_unix fmt

let header_width () = min 80 (OpamStd.Sys.terminal_columns ())

let header_msg fmt =
  let utf8camel = "\xF0\x9F\x90\xAB " in
  (* UTF-8 <U+1F42B, U+0020> *)
  let padding =
    "<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>"
  in
  Printf.ksprintf
...
let confirm ?(default = true) fmt =
  Printf.ksprintf
    (fun s ->
      try
        if OpamCoreConfig.(!r.safe_mode) then false
        else
          let prompt () =
          ...
          if OpamCoreConfig.(!r.answer) = Some true then (
            ...
            OpamCoreConfig.(!r.answer) = Some false || OpamStd.Sys.(not tty_in)
          then (
            prompt ();
            msg "n\n";
            false )
          else if OpamStd.Sys.((not tty_out) || os () = Win32 || os () = Cygwin)
          then
            let rec loop () =
              prompt ();
            ...
            let rec loop () =
              ...
              | Some 'n' ->
                  ...
                  false
              | Some '\n' ->
                  print_endline (if default then "y" else "n");
                  default
              | _ -> loop ()
            in
  ...
  Printf.ksprintf
    (fun s ->
      ...
      if OpamCoreConfig.(!r.answer = None && not !r.safe_mode) then (
        try match read_line () with "" -> None | s -> Some s with
        | End_of_file ->
            msg "\n";
            None
        | Sys.Break as e ->
...
let print_table ?cut oc ~sep table =
  ...
  in
  let output_string s =
    if oc = stdout then msg "%s\n" s
    else if oc = stderr then errmsg "%s\n" s
    else (
      output_string oc s;
      output_char oc '\n' )
  in
  let replace_newlines by = Re.(replace_string (compile (char '\n')) ~by) in
    ...
    | `Wrap wrap_sep ->
        ...
        let rec split_at_overflows start_col acc cur =
          let append = function
            | [] -> acc
            | last :: r ->
                List.rev (OpamStd.String.strip_right last :: r) :: acc
          in
          function
          | [] -> List.rev (append cur)
          | cell :: rest ->
              ...
              let indent ~sep n cell =
                let spc =
                  if sep then
                    String.make
                      (max 0 (if sep then n - wrap_sep_len else n))
                      ' '
                    ^ wrap_sep
                  else String.make n ' '
                in
                OpamStd.List.concat_map ("\n" ^ spc) OpamStd.String.strip_right
              ...
              if start_col + sep_len + text_width <= width then
                ...
                else split_at_overflows end_col acc (cell :: cur) rest
              else if
                rest = [] && acc = [] && (not multiline)
              ...
              then
                ...
                split_at_overflows (margin + cell_width) (append cur) [ cell ]
                  rest
        in
...
let () = OpamStd.Sys.(set_warning_printer { warning })
