...
module Conv32 = struct
  ...
  let toggle_big_endian = if Sys.big_endian then fun x -> x else swap
end
...
module Input_file (Buf : BufSig with type t = bigstring) = struct
  ...
  type t = { fd : Unix.file_descr; blocksize : int; buf : Buf.t }
  ...
  let init ~blocksize src =
    ...
    let buf =
      B.(
        array1_of_genarray
          (OpamCompat.Unix.map_file fd B.char c_layout false [| -1 |]))
    in
    { fd; blocksize; buf }
  ...
  let get_chunk { blocksize; buf; _ } i =
    ...
    if (i + 1) * block_bytes <= len then A.sub buf (i * block_bytes) block_bytes
    else
...
module Input_string (Buf : BufSig with type t = Bytes.t) = struct
  ...
  type t = { blocksize : int; buf : Bytes.t }

  type chunk = { offset : int; b : Bytes.t }
  ...
  let get_chunk { blocksize; buf } i =
    ...
    let block_bytes = blocksize * Buf.Conv.bytes in
    if (i + 1) * block_bytes <= len then { offset = i * block_bytes; b = buf }
    else
...
let hash_file = function `SHA256 -> sha256_file | `SHA512 -> sha512_file

let sha256_bytes = SHA256_string.hash

let sha512_bytes = SHA512_string.hash

let hash_bytes = function `SHA256 -> sha256_bytes | `SHA512 -> sha512_bytes
