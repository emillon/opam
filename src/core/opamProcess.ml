...
let log ?level fmt = OpamConsole.log "PROC" ?level fmt
...
let cygwin_create_process_env prog args env fd1 fd2 fd3 =
  ...
  let make_args argv =
    ...
    let gen_quote ~quote ~pre ?(post = pre) s =
      ...
      let rec f i =
        let j =
          try
            OpamStd.String.find_from
              (fun c -> try String.index quote c >= 0 with Not_found -> false)
              s (succ i)
          with Not_found -> l
        in
        ...
        if j < l then (
          ...
          let j =
            try
              OpamStd.String.find_from
                (fun c -> try String.index quote c < 0 with Not_found -> true)
                s (succ i)
            with Not_found -> l
          in
          Buffer.add_string b (String.sub s i (j - i));
          Buffer.add_string b post;
          if j < l then f j else Buffer.contents b )
        else Buffer.contents b
      in
      let r = if s = "" then "\"\"" else f 0 in
    ...
    if
      true
      || List.exists
           (fun s -> try String.index s '"' >= 0 with Not_found -> false)
           argv
    then
      ( "\""
        ^ String.concat "\" \""
            (List.map (gen_quote ~quote:"\"" ~pre:"\"'" ~post:"'\"") argv)
        ^ "\"",
        false )
    else
  ...
  let f item =
    ...
    | "cygwin" ->
        let () = if key = "CYGWIN" then set := true in
        ...
        if settings = [] then (
          ...
          None )
        else Some (key ^ "=" ^ String.concat " " settings)
    | "path" ->
        let path_dirs = OpamStd.Sys.split_path_variable item in
        let winsys =
          Filename.concat (OpamStd.Sys.system ()) "." |> String.lowercase_ascii
        in
        let rec f prefix suffix = function
          | dir :: dirs ->
              let contains_cygpath =
                Sys.file_exists (Filename.concat dir "cygpath.exe")
              in
              if suffix = [] then
                if String.lowercase_ascii (Filename.concat dir ".") = winsys
                then f prefix [ dir ] dirs
                else if contains_cygpath then path_dirs
                else f (dir :: prefix) [] dirs
              else if contains_cygpath then (
                log ~level:2 "Moving %s to after %s in PATH" dir
                  (List.hd prefix);
                List.rev_append prefix (dir :: List.rev_append suffix dirs) )
              else f prefix (dir :: suffix) dirs
          | [] -> assert false
        in
...
let command ?env ?verbose ?name ?metadata ?dir ?allow_stdin ?stdout ?text cmd
    args =
  {
    cmd;
    args;
...
let make_info ?code ?signal ~cmd ~args ~cwd ~env_file ~stdout_file ~stderr_file
    ~metadata () =
  let b = ref [] in
  let home = OpamStd.Sys.home () in
  let print name str =
    let str =
      if OpamStd.String.starts_with ~prefix:home str then
        "~" ^ OpamStd.String.remove_prefix ~prefix:home str
      else str
    in
    b := (name, str) :: !b
  in
  let print_opt name = function None -> () | Some s -> print name s in
  ...
  if stderr_file = stdout_file then print_opt "output-file" stdout_file
  else (
...
let string_of_info ?(color = `yellow) info =
  ...
  List.iter
    (fun (k, v) ->
      Printf.bprintf b "%s %-20s %s\n"
        (OpamConsole.colorise color "#")
        (OpamConsole.colorise color k)
        v)
    info;
  Buffer.contents b
...
(** [create cmd args] create a new process to execute the command
    ...
    set). *)
let create ?info_file ?env_file ?(allow_stdin = true) ?stdout_file ?stderr_file
    ?env ?(metadata = []) ?dir ~verbose ~tmp_files cmd args =
  let nothing () = () in
  let tee f =
  ...
  let stdin_fd, close_stdin =
    if allow_stdin then (Unix.stdin, nothing)
    else
      ...
      (fd, close_stdin)
  in
  let stdout_fd, close_stdout =
    match stdout_file with None -> (Unix.stdout, nothing) | Some f -> tee f
  in
  let stderr_fd, close_stderr =
    match stderr_file with
    | None -> (Unix.stderr, nothing)
    | Some f -> if stdout_file = Some f then (stdout_fd, nothing) else tee f
  in
  ...
  let () =
    ...
    | Some f ->
        ...
        let env =
          List.filter
            (fun line -> not (OpamStd.String.contains_char line '$'))
            env
        in
  ...
  let pid =
    let cmd, args =
      if Sys.win32 then
        try
          let actual_command =
            if Sys.file_exists cmd then cmd
            else if Sys.file_exists (cmd ^ ".exe") then cmd ^ ".exe"
            else raise Exit
          in
          let actual_image, args =
            ...
            try
              if really_input_string c 2 = "#!" then (
                ...
                let cmd, arg =
                  try
                    ...
                    let cmd =
                      Filename.basename (String.trim (String.sub l 0 i))
                    in
                    let arg =
                      String.trim (String.sub l i (String.length l - i))
                    in
                    if cmd = "env" then (arg, None) else (cmd, Some arg)
                  with Not_found -> (Filename.basename l, None)
                in
                close_in c;
                try
                  ...
                  ( cmd,
                    OpamStd.Option.map_default (fun arg -> arg :: args) args arg
                  )
                with Not_found ->
    ...
    let create_process, cmd, args =
      if Sys.win32 then
        ...
        else (Unix.create_process_env, cmd, args)
      else (Unix.create_process_env, cmd, args)
    in
    try
      create_process cmd
        (Array.of_list (cmd :: args))
        env stdin_fd stdout_fd stderr_fd
    with e ->
      close_stdin ();
      close_stdout ();
      close_stderr ();
    ...
    | Some n ->
        let d =
          if Filename.is_relative n then
            match dir with Some d -> d | None -> OpamCoreConfig.(!r.log_dir)
          else ""
        in
        Some (Filename.concat d (Printf.sprintf "%s.%s" n ext))
  in
  let stdout_file = OpamStd.Option.Op.(cmd_stdout >>+ fun () -> file "out") in
  let stderr_file =
...
let verbose_print_cmd p =
  OpamConsole.msg "%s %s %s%s\n"
    ...
    (if p.p_cwd = Sys.getcwd () then "" else Printf.sprintf " (CWD=%s)" p.p_cwd)

let verbose_print_out =
  let pfx = OpamConsole.colorise `yellow "- " in
  fun s -> OpamConsole.msg "%s%s\n" pfx s
...
let set_verbose_f, print_verbose_f, isset_verbose_f, stop_verbose_f =
  ...
  let set files =
    ...
    if Sys.win32 then ()
      ...
      let ics =
        List.map
          (open_in_gen
             [ Open_nonblock; Open_rdonly; Open_text; Open_creat ]
             0o600)
          files
      in
      let f () =
        List.iter
          (fun ic ->
            try
              while true do
                verbose_print_out (input_line ic)
              done
  ...
  in
  let print () = match !verbose_f with Some (_, f) -> f () | None -> () in
...
let exit_status p return =
  ...
  let stdout =
    OpamStd.Option.default [] (OpamStd.Option.map read_lines p.p_stdout)
  in
  let stderr =
    OpamStd.Option.default [] (OpamStd.Option.map read_lines p.p_stderr)
  in
  ...
  in
  if isset_verbose_f () then stop_verbose_f ()
  ...
  let info =
    make_info ?code ?signal ~cmd:p.p_name ~args:p.p_args ~cwd:p.p_cwd
      ~metadata:p.p_metadata ~env_file:p.p_env ~stdout_file:p.p_stdout
      ~stderr_file:p.p_stderr ()
  in
  {
...
let wait_one processes =
  ...
  try
    let p = List.find (fun p -> Hashtbl.mem dead_childs p.p_pid) processes in
  ...
  with Not_found ->
    let rec aux () =
      let pid, return =
        ...
        else safe_wait (List.hd processes).p_pid Unix.wait ()
      in
      try
...
let safe_unlink f =
  try
    ...
    Unix.unlink f
  with Unix.Unix_error _ -> log ~level:2 "safe_unlink: %s (FAILED)" f
...
let truncate_line str =
  if String.length str <= log_line_limit then str
  else
...
let truncate l =
  let unindented s = String.length s > 0 && s.[0] <> ' ' && s.[0] <> '\t' in
  let rec cut n acc = function
    | [] -> acc
    | [ x ] when n = 0 -> truncate_line x :: acc
    | _ when n = 0 -> truncate_str :: acc
    | x :: l when n = 1 -> (
        if unindented x then truncate_str :: truncate_line x :: acc
        else
  ...
  if len <= 0 then l else cut len [] (List.rev l)
              ...
              (List.find
                 Re.(
                   execp
                     (compile
                        (seq
                           [ bos; rep (diff any alpha); no_case (str "error") ])))
                 (List.rev es))
          with
          | Not_found -> ""
          | _ -> "" ) )
...
module Job = struct
  ...
  let rec dry_run = function
    | Done x -> x
    | Run (_command, cont) -> dry_run (cont empty_result)

  let rec catch handler fjob =
    try
      match fjob () with
      | Done x -> Done x
      | Run (cmd, cont) -> Run (cmd, fun r -> catch handler (fun () -> cont r))
    with e -> handler e
  ...
  let rec finally fin fjob =
    try
      ...
      | Done x ->
          fin ();
          Done x
      | Run (cmd, cont) -> Run (cmd, fun r -> finally fin (fun () -> cont r))
    with e ->
      fin ();
      raise e
