...
module Job : sig
  ...
  module Op : sig
    type 'a job = Done of 'a | Run of command * (result -> 'a job)
  ...
  val ignore_errors :
    default:'a -> ?message:string -> (unit -> 'a Op.job) -> 'a Op.job
  ...
  val of_list :
    ?keep_going:bool -> command list -> (command * result) option Op.job
