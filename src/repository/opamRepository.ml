...
let cache_url root_cache_url checksum =
  List.fold_left OpamUrl.Op.( / ) root_cache_url (OpamHash.to_path checksum)
...
let fetch_from_cache =
  ...
  let rec no_concurrent_dls key f x =
    if List.mem key !currently_downloading then
      Run
        (OpamProcess.command "sleep" [ "1" ], fun _ -> no_concurrent_dls key f x)
    else (
  ...
  fun cache_dir cache_urls checksums ->
      ...
      OpamConsole.error
        ...
        (OpamStd.Format.itemize
           (fun ck ->
             ...
             ^
             if OpamHash.check_file (OpamFilename.to_string file) ck then
               OpamConsole.colorise `green " (match)"
    ...
    let dl_from_cache_job root_cache_url checksum file =
      ...
      | `http ->
          OpamDownload.download_as ~quiet:true ~validate:false ~overwrite:true
            ~checksum url file
      | `rsync -> (
          ...
          let rec try_cache_dl = function
            ...
            | root_cache_url :: other_caches ->
                OpamProcess.Job.catch (function
                  | Failure _ | OpamDownload.Download_fail _ ->
                      try_cache_dl other_caches
                  | e -> raise e)
                @@ fun () ->
                dl_from_cache_job root_cache_url checksum tmpfile @@+ fun () ->
                if
                  List.for_all
                    (OpamHash.check_file (OpamFilename.to_string tmpfile))
                    checksums
                then (
...
let validate_and_add_to_cache label url cache_dir file checksums =
  ...
  with Not_found ->
    ...
    | Some dir, ck :: _ -> OpamFilename.copy ~src:file ~dst:(cache_file dir ck)
    (* idea: hardlink to the other checksums? *)
    | _ -> () );
    true

(* [cache_dir] used to add to cache only *)
let pull_from_upstream label ?(working_dir = false) cache_dir destdir checksums
    url =
  ...
  @@ ( if working_dir then B.sync_dirty destdir url
       ...
       let url, pull =
         if
           OpamUrl.(
             match url.backend with #version_control -> false | _ -> true)
           && OpamFilename.exists_dir pin_cache_dir
         then (
           ...
           in
           let pull =
             let module BR = (val url_backend rsync : OpamRepositoryBackend.S)
             in
             BR.pull_url
           in
           (rsync, pull) )
  ...
  | (Result (Some file) | Up_to_date (Some file)) as ret ->
      if
        OpamRepositoryConfig.(!r.force_checksums) = Some false
...
let pull_from_mirrors label ?working_dir cache_dir destdir checksums urls =
  ...
  | url, (Result None | Up_to_date None) when checksums <> [] ->
      OpamConsole.error
        "%s: file checksum specified, but a directory was retrieved from %s"
...
let pull_tree label ?cache_dir ?(cache_urls = []) ?working_dir local_dirname
  ...
  | None ->
      ...
      Done (Not_available (Some m, m)) )
  @@+ function
  | Up_to_date (archive, _) -> extract_archive archive "cached"
  | Result (archive, url) ->
      let msg =
        match url.OpamUrl.backend with
...
let pull_file label ?cache_dir ?(cache_urls = []) ?(silent_hits = false) file
    checksums remote_urls =
  ( match cache_dir with
  | Some cache_dir ->
      let text = OpamProcess.make_command_text label "dl" in
  ...
  | Not_available _ ->
      ...
      then
        ...
        OpamFilename.with_tmp_dir_job (fun tmpdir ->
            ...
            | _, Result (Some f) ->
                OpamFilename.move ~src:f ~dst:file;
                Result ()
            | _, Result None ->
...
let pull_file_to_cache label ~cache_dir ?(cache_urls = []) checksums remote_urls
  ...
  | Result (_, url) -> Done (Result (OpamUrl.to_string url))
  | Not_available _ ->
      OpamFilename.with_tmp_dir_job (fun tmpdir ->
...
let validate_repo_update repo repo_root update =
  ...
  | None, Some _, Some true ->
      OpamConsole.error
        "No trust anchors for repository %s, and security was enforced: not \
         updating"
        (OpamRepositoryName.to_string repo.repo_name);
      Done false
  | None, _, _ | _, None, _ | _, _, Some false -> Done true
  | Some ta, Some hook, _ ->
      let cmd =
        let open OpamRepositoryBackend in
        let env v =
          match (OpamVariable.Full.to_string v, update) with
        ...
        | cmd :: args ->
            OpamSystem.make_command ~name:"validation-hook"
...
let apply_repo_update repo repo_root = function
  | Update_full d ->
      log "%a: applying update from scratch at %a"
        (slog OpamRepositoryName.to_string)
        repo.repo_name
      ...
      else OpamFilename.move_dir ~src:d ~dst:repo_root;
...
let on_local_version_control url ~default f =
  ...
  | #OpamUrl.version_control as backend -> (
      ...
      | None -> default
      | Some dir -> f dir (find_vcs_backend backend) )
  | #OpamUrl.backend -> default

let current_branch url =
  on_local_version_control url ~default:(Done None) @@ fun dir (module VCS) ->
  VCS.current_branch dir

let is_dirty url =
  on_local_version_control url ~default:(Done false) @@ fun dir (module VCS) ->
  VCS.is_dirty dir

let report_fetch_result pkg = function
  | Result msg ->
      OpamConsole.msg "[%s] synchronised (%s)\n"
  ...
  | Not_available (s, l) ->
      ...
      OpamConsole.msg "[%s] fetching sources failed: %s\n"
        (OpamConsole.colorise `red (OpamPackage.to_string pkg))
        msg;
      Not_available (s, l)
