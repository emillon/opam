...
module VCS = struct
  ...
  let exists repo_root = OpamFilename.exists_dir (repo_root / "_darcs")
    ...
    darcs repo_root
      (with_tag repo_url
         [
           ...
           repo_str;
           "--repodir";
           OpamFilename.Dir.to_string repodir;
           "--quiet";
           "--lazy";
         ])
  ...
  let reset_tree repo_root _repo_url =
    darcs repo_root [ "obliterate"; "--all"; "-t"; opam_local_tag ] @@> fun r ->
    OpamSystem.raise_on_process_error r;
  ...
  let versioned_files repo_root =
    darcs repo_root [ "show"; "files" ] @@> fun r ->
    OpamSystem.raise_on_process_error r;
  ...
  let is_dirty dir =
    darcs dir [ "whatsnew"; "--quiet"; "--summary" ] @@> fun r ->
  ...
  let get_remote_url ?hash:_ repo_root =
    darcs repo_root [ "show"; "repo" ] @@> function
    | { OpamProcess.r_code = 0; _ } as r ->
        let res =
          ...
          | [ line ] -> (
              ...
              in
              match repo with [ repo ] -> Some repo | _ -> None )
          | _ -> None
        in
        Done (OpamStd.Option.map (fun u -> OpamUrl.parse ~backend:`darcs u) res)
