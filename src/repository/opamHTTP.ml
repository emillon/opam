...
let sync_state name destdir url =
  ...
  OpamDownload.download_as ~quiet:true ~overwrite:true
    (remote_index_archive url) local_index_archive
  @@+ fun () ->
  ...
  @@ OpamFilename.extract_in_job local_index_archive destdir
  @@+ function None -> Done () | Some err -> raise err
...
module B = struct
  ...
  let fetch_repo_update repo_name ?cache_dir:_ repo_root url =
    ...
    OpamRepositoryBackend.job_text repo_name "sync"
      (sync_state repo_name quarantine url)
    @@+ fun () ->
    if
  ...
  let pull_url ?cache_dir:_ dirname checksum remote_url =
    log "pull-file into %a: %a"
      ...
      dirname (slog OpamUrl.to_string) remote_url;
    OpamProcess.Job.catch (fun e ->
        OpamStd.Exn.fatal e;
        let s, l =
    ...
    @@ fun () ->
    OpamDownload.download ~quiet:true ~overwrite:true ?checksum remote_url
      dirname
...
let make_index_tar_gz repo_root =
  OpamFilename.in_dir repo_root (fun () ->
      ...
      | d ->
          OpamSystem.command
            ("tar" :: "czhf" :: "index.tar.gz" :: "--exclude=.git*" :: d))
