...
let rsync ?(args = []) ?(exclude_vcdirs = true) src dst =
  ...
  let exclude_args =
    ( if not exclude_vcdirs then []
    else [ "--exclude"; ".git"; "--exclude"; "_darcs"; "--exclude"; ".hg" ] )
    @ [ "--exclude"; ".#*"; "--exclude"; OpamSwitch.external_dirname ^ "*" ]
  in
  if not (remote || Sys.file_exists src) then Done (Not_available (None, src))
  else if src = dst then Done (Up_to_date [])
    ...
    call_rsync
      (fun () -> not (OpamSystem.dir_is_empty dst))
...
let rsync_dirs ?args ?exclude_vcdirs url dst =
  ...
  then
    Done
      (Not_available (None, Printf.sprintf "Directory %s does not exist" src_s))
  else
...
let rsync_file ?(args = []) url dst =
  ...
  if not (is_remote url || OpamFilename.(exists (of_string src_s))) then
  ...
  else if src_s = dst_s then Done (Up_to_date dst)
  else (
    OpamFilename.mkdir (OpamFilename.dirname dst);
    let convert_path = Lazy.force convert_path in
    call_rsync
      (fun () -> Sys.file_exists dst_s)
    ...
    | Some l ->
        OpamSystem.internal_error "unknown rsync output: {%s}"
          (String.concat ", " l) )

module B = struct
  let name = `rsync

  let pull_dir_quiet local_dirname url = rsync_dirs url local_dirname
  ...
  let fetch_repo_update repo_name ?cache_dir:_ repo_root url =
    ...
    OpamRepositoryBackend.job_text repo_name "sync"
      ...
      | None ->
          ...
          else OpamFilename.mkdir quarantine;
          pull_dir_quiet quarantine url )
    @@+ function
    | Not_available _ ->
        finalise ();
    ...
    | Result _ -> (
        if
          (not (OpamFilename.exists_dir repo_root))
          || OpamFilename.dir_is_empty repo_root
        then Done (OpamRepositoryBackend.Update_full quarantine)
        else
  ...
  let pull_url ?cache_dir:_ local_dirname _checksum remote_url =
    ...
    let remote_url =
      ...
      | None -> remote_url
    in
    rsync remote_url.OpamUrl.path dir @@| function
    | Not_available _ as na -> na
    | (Result _ | Up_to_date _) as r ->
        let res x =
          ...
          | _ -> assert false
        in
        if OpamUrl.has_trailing_slash remote_url then res None
        else
          let filename =
          ...
          if OpamFilename.exists filename then res (Some filename)
            ...
            Not_available
              ( None,
                Printf.sprintf
                  "Could not find target file %s after rsync with %s. Perhaps \
                   you meant %s/ ?"
                  (OpamUrl.basename remote_url)
                  (OpamUrl.to_string remote_url)
                  (OpamUrl.to_string remote_url) )

  let revision _ = Done None

  let sync_dirty dir url = pull_url dir None url

  let get_remote_url ?hash:_ _ = Done None
end
