      ...
      lazy
        (let os = OpamStd.Sys.os () in
         try
           ...
           let cmd, kind =
             List.find
               (fun (c, _) -> OpamSystem.resolve_command c <> None)
               tools
           in
...
let setk k t ?download_tool ?validation_hook ?retries ?force_checksums =
...
let initk k =
    ...
    ( env_string "FETCH" >>= fun s ->
      ...
      | cmd :: a ->
          let cmd, kind =
            ...
            else if cmd = "wget" then ((CIdent "wget", None), `Default)
            else ((CString cmd, None), `Default)
          in
          let c = cmd :: List.map (fun a -> (OpamTypes.CString a, None)) a in
          Some (lazy (c, kind))
      | [] -> None )
    >>+ fun () ->
    env_string "CURL" >>| fun s -> lazy ([ (CString s, None) ], `Curl)
  in
  let validation_hook =
  ...
  setk
    (setk (fun c ->
         r := c;
         k))
    !r ?download_tool ?validation_hook ?retries:(env_int "RETRIES")
    ?force_checksums
