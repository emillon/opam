...
let curl_args =
  [
    ...
    (CString "--retry", None);
    (CIdent "retry", None);
    (CString "--retry-delay", None);
    (CString "2", None);
    ( CString "--compressed",
      Some (FIdent (OpamFilter.ident_of_string "compress")) );
    (CString "--user-agent", None);
    (user_agent, None);
    (CString "-L", None);
    (CString "-o", None);
    (CIdent "out", None);
  ...
  OpamFilter.single_command
    (fun v ->
      if not (OpamVariable.Full.is_global v) then None
        ...
        | "hashalgo" ->
            OpamStd.Option.map
              (fun c -> S OpamHash.(string_of_kind (kind c)))
              checksum
        | "hashpath" ->
            OpamStd.Option.map
...
let tool_return url ret =
  ...
  | _, `Curl -> (
      if OpamProcess.is_failure ret then
        fail
          ( Some "Curl failed",
            Printf.sprintf "Curl failed: %s" (OpamProcess.result_summary ret) );
      match ret.OpamProcess.r_stdout with
      | [] ->
          fail
            ( Some "curl empty response",
      ...
      | l ->
          ...
          if num >= 400 then
            fail
              ( Some ("curl error code " ^ code),
                Printf.sprintf "curl: code %s while downloading %s" code
                  (OpamUrl.to_string url) )
          else Done () )
...
let really_download ?(quiet = false) ~overwrite ?(compress = false) ?checksum
    ?(validate = true) ~url ~dst =
  ...
  OpamProcess.Job.catch (function
    ...
    | e ->
        ...
        raise e)
  @@ fun () ->
  download_command ~compress ?checksum ~url ~dst:tmp_dst @@+ fun () ->
  if not (Sys.file_exists tmp_dst) then
    fail
      ( Some "Downloaded file not found",
  ...
  if validate && OpamRepositoryConfig.(!r.force_checksums <> Some false) then
    OpamStd.Option.iter
      (fun cksum ->
...
let download_as ?quiet ?validate ~overwrite ?compress ?checksum url dst =
  ...
  | Some src ->
      if src = dst then Done ()
      else (
        if OpamFilename.exists dst then
          if overwrite then OpamFilename.remove dst
          else
  ...
  | None ->
      ...
      really_download ?quiet ~overwrite ?compress ?checksum ?validate ~url
