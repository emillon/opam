...
let cudfnv2opam ?version_map ?cudf_universe (name, v) =
  ...
  | None -> (
      ...
      | Some vmap ->
          let nvset =
            OpamPackage.Map.filter (fun nv cv -> nv.name = name && cv = v) vmap
          in
...
let string_of_package p =
  ...
  Printf.sprintf "%s.%d(%s)" p.Cudf.package p.Cudf.version installed

let string_of_packages l = OpamStd.List.to_string string_of_package l
...
module Json = struct
  ...
  let int_of_json = function `Float x -> Some (int_of_float x) | _ -> None
  ...
  let list_to_json elem_to_json li : OpamJson.t = `A (List.map elem_to_json li)

  let list_of_json elem_of_json = function
    | `A jsons -> (
        try
          let get = function None -> raise Not_found | Some v -> v in
  ...
  let option_of_json elem_of_json = function
    | `Null -> Some None
    | other -> elem_of_json other >>= fun elem -> Some (Some elem)

  let pair_to_json fst_field fst_to_json snd_field snd_to_json (fst, snd) =
    `O [ (fst_field, fst_to_json fst); (snd_field, snd_to_json snd) ]

  let pair_of_json fst_field fst_of_json snd_field snd_of_json : OpamJson.t -> _
      = function
    | `O dict -> (
        try
          fst_of_json (List.assoc fst_field dict) >>= fun fst ->
          snd_of_json (List.assoc snd_field dict) >>= fun snd -> Some (fst, snd)
        with Not_found -> None )
    | _ -> None
  ...
  let vpkgformula_of_json json = list_of_json (list_of_json vpkg_of_json) json
  ...
  let type_schema_to_json tag value_to_json value =
    pair_to_json "type" string_to_json "default"
      (option_to_json value_to_json)
      (tag, value)

  let rec typedecl1_to_json = function
    | `Int n -> type_schema_to_json "int" int_to_json n
    | `Posint n -> type_schema_to_json "posint" int_to_json n
    | `Nat n -> type_schema_to_json "nat" int_to_json n
    | `Bool b -> type_schema_to_json "bool" bool_to_json b
    | `String s -> type_schema_to_json "string" string_to_json s
    | `Pkgname s -> type_schema_to_json "pkgname" pkgname_to_json s
    | `Ident s -> type_schema_to_json "ident" string_to_json s
    | `Enum (enums, v) ->
        pair_to_json "type" string_to_json "default"
          (pair_to_json "set"
             (list_to_json string_to_json)
             "default"
             (option_to_json string_to_json))
          ("enum", (enums, v))
    | `Vpkg v -> type_schema_to_json "vpkg" vpkg_to_json v
    | `Vpkgformula v -> type_schema_to_json "vpkgformula" vpkgformula_to_json v
    | `Vpkglist v -> type_schema_to_json "vpkglist" vpkglist_to_json v
    | `Veqpkg v -> type_schema_to_json "veqpkg" veqpkg_to_json v
    | `Veqpkglist v -> type_schema_to_json "veqpkglist" veqpkglist_to_json v
    | `Typedecl td -> type_schema_to_json "typedecl" typedecl_to_json td

  and typedecl_to_json td = stanza_to_json typedecl1_to_json td
  ...
  let rec typedecl1_of_json json =
    ...
    | "string" ->
        option_of_json string_of_json json >>= fun x -> Some (`String x)
    | "pkgname" ->
        option_of_json string_of_json json >>= fun x -> Some (`Pkgname x)
    ...
    | "enum" ->
        pair_of_json "set"
          ...
          json
        >>= fun x -> Some (`Enum x)
    | "vpkg" -> option_of_json vpkg_of_json json >>= fun x -> Some (`Vpkg x)
    | "vpkgformula" ->
        option_of_json vpkgformula_of_json json >>= fun x ->
        Some (`Vpkgformula x)
    | "vpkglist" ->
  ...
  and typedecl_of_json json = stanza_of_json typedecl1_of_json json
  ...
  let typed_value_to_json : Cudf_types.typed_value -> _ = function
    | `Int n -> type_tagged_to_json "int" int_to_json n
    | `Posint n -> type_tagged_to_json "posint" int_to_json n
    | `Nat n -> type_tagged_to_json "nat" int_to_json n
    | `Bool b -> type_tagged_to_json "bool" bool_to_json b
    | `String s -> type_tagged_to_json "string" string_to_json s
    | `Pkgname name -> type_tagged_to_json "pkgname" pkgname_to_json name
    | `Ident id -> type_tagged_to_json "ident" string_to_json id
    | `Enum (enums, value) ->
        type_tagged_to_json "enum"
          (pair_to_json "set"
             (list_to_json string_to_json)
             "choice" string_to_json)
          (enums, value)
    | `Vpkg vpkg -> type_tagged_to_json "vpkg" vpkg_to_json vpkg
    ...
    | `Veqpkg veqpkg -> type_tagged_to_json "veqpkg" veqpkg_to_json veqpkg
  ...
  let typed_value_of_json json : Cudf_types.typed_value option =
    ...
    | "enum" ->
        pair_of_json "set"
          (list_of_json string_of_json)
          "choice" string_of_json json
        >>= fun p -> Some (`Enum p)
    | "vpkg" -> vpkg_of_json json >>= fun x -> Some (`Vpkg x)
    | "vpkgformula" ->
        vpkgformula_of_json json >>= fun x -> Some (`Vpkgformula x)
  ...
  let package_to_json p =
    `O
      [
        ("name", pkgname_to_json p.Cudf.package);
  ...
  let package_of_json = function
    | `O dict -> (
        try
          ...
          bool_of_json (List.assoc "was_installed" dict)
          >>= fun was_installed ->
          enum_keep_of_json (List.assoc "keep" dict) >>= fun keep ->
          stanza_of_json typed_value_of_json (List.assoc "pkg_extra" dict)
          >>= fun pkg_extra ->
          Some
            {
              Cudf.package;
...
type conflict = Cudf.universe * int package_map * conflict_case
...
module Graph = struct
  ...
  let of_universe u =
    ...
    let iter_deps f deps =
      ...
      let strong_deps, weak_deps =
        ...
        List.fold_left
          (fun (strong_deps, weak_deps) l ->
            let names =
              List.fold_left
                (fun acc (n, _) -> OpamStd.String.Map.add n Set.empty acc)
              ...
              List.fold_left
                (fun acc (n, cstr) ->
                  List.fold_left
                    (fun s x -> Set.add x s)
                    acc
                    (Cudf.lookup_packages ~filter:cstr u n))
                Set.empty l
            in
            let by_name =
              Set.fold
      ...
      OpamStd.String.Map.iter
        (fun name p ->
          if not (OpamStd.String.Map.mem name strong_deps) then Set.iter f p)
        weak_deps
    in
    Cudf.iter_packages
      (fun p ->
  ...
  let transitive_closure g = PO.O.add_transitive_closure g

  let close_and_linearize g pkgs =
    let _, l =
      Topo.fold
        (fun pkg (closure, topo) ->
          if Set.mem pkg closure then (closure, pkg :: topo)
...
let string_of_atom (p, c) =
  let const = function
    | None -> ""
    | Some (r, v) -> Printf.sprintf " (%s %d)" (OpamPrinter.relop r) v
  in
...
let vpkg2atom cudfnv2opam (name, cstr) =
  match cstr with
  | None -> (OpamPackage.Name.of_string (Common.CudfAdd.decode name), None)
  | Some (relop, v) ->
  ...
  let rec aux = function
    ...
    | Conflict (i, j, jc) :: rs ->
        if is_dose_request i || is_dose_request j then
          ...
          if is_dose_request a then aux rs
          else if is_base a then
            let str =
              Printf.sprintf
                "Package %s is part of the base for this compiler and can't be \
                 changed"
                (OpamPackage.name_to_string (cudf2opam a))
            in
            str :: aux rs
          else
            let str =
              Printf.sprintf "Conflicting query for package %s"
                (OpamPackage.to_string (cudf2opam a))
            in
            str :: aux rs
          ...
          if is_base i || is_base j then
            let str =
              Printf.sprintf
                "Package %s is part of the base for this compiler and can't be \
                 changed"
                (OpamPackage.name_to_string (cudf2opam i))
            in
            str :: aux rs
          else
            let str =
              Printf.sprintf
                "No available version of %s satisfies the constraints"
                (OpamPackage.name_to_string (cudf2opam i))
            in
            str :: aux rs
        else
          ...
          let versions, rs =
            List.fold_left
              (fun (versions, rs) -> function
                | Conflict (i1, _, jc1)
                  when (cudf2opam i1).name = nva.name && jc1 = jc ->
                    ( OpamPackage.Version.Set.add (cudf2opam i1).version versions,
                      rs ) | r -> (versions, r :: rs))
              (OpamPackage.Version.Set.singleton nva.version, [])
              rs
          in
          let rs = List.rev rs in
          let formula =
            OpamFormula.formula_of_version_set
              (OpamPackage.versions_of_name packages nva.name)
              versions
          in
          let str =
            Printf.sprintf "%s is in conflict with %s"
          ...
          (fun name ->
            let formula =
              OpamFormula.ors
                (List.map
                   (function
                     | n, Some atom when n = name -> Atom atom | _ -> Empty)
                   atoms)
            in
...
let make_chains packages cudfnv2opam depends =
  ...
  let map_addlist k v map =
    try Map.add k (v @ Map.find k map) map with Not_found -> Map.add k v map
  in
  let roots, notroots, deps, vpkgs =
    List.fold_left
      (fun (roots, notroots, deps, vpkgs) -> function
        ...
        | Missing (i, vpkgl) when not (is_dose_request i) ->
            let jl =
              List.map
                (fun (package, _) -> { Cudf.default_package with Cudf.package })
                vpkgl
            in
            ( Set.add i roots,
              notroots,
              map_addlist i jl deps,
              map_addlist i vpkgl vpkgs ) | _ -> (roots, notroots, deps, vpkgs))
      (Set.empty, Set.empty, Map.empty, Map.empty)
      depends
  in
  let roots = Set.diff roots notroots in
  if Set.is_empty roots then []
  else
    let children cpkgs =
      Set.fold
        (fun c acc ->
    ...
    let rec aux constrs direct_deps =
      if Set.is_empty direct_deps then [ [] ]
      else
        let depnames =
          Set.fold
            (fun p set -> OpamStd.String.Set.add p.Cudf.package set)
            direct_deps OpamStd.String.Set.empty
        in
        OpamStd.String.Set.fold
          (fun name acc ->
            let name_deps =
              (* Gather all deps with the given name *)
              Set.filter (fun p -> p.Cudf.package = name) direct_deps
            in
            let name_constrs =
              List.map (List.filter (fun (n, _) -> n = name)) constrs
            in
            let to_opam_constr p = snd (vpkg2opam cudfnv2opam p) in
            let formula =
              OpamFormula.ors
                (List.map
                   (fun conj -> OpamFormula.ands (List.map to_opam_constr conj))
                   name_constrs)
            in
            let opam_name =
              OpamPackage.Name.of_string (Common.CudfAdd.decode name)
            in
            let all_versions =
              OpamPackage.versions_of_name packages opam_name
            in
            let formula =
              OpamFormula.simplify_version_set all_versions formula
            in
            ...
            let children_constrs =
              List.map
                (fun p -> try Map.find p vpkgs with Not_found -> [])
                (Set.elements name_deps)
            in
    ...
    let start_constrs =
      let set =
        Set.fold
          (fun p acc -> OpamStd.String.Set.add p.Cudf.package acc)
          roots OpamStd.String.Set.empty
      in
      List.map (fun name -> [ (name, None) ]) (OpamStd.String.Set.elements set)
    in
    aux start_constrs roots

let strings_of_final_reasons packages cudfnv2opam unav_reasons reasons =
  let reasons = strings_of_reasons packages cudfnv2opam unav_reasons reasons in
...
let strings_of_cycles cycles = List.map arrow_concat cycles
...
let string_of_conflict packages unav_reasons conflict =
  ...
  in
  if cycles <> [] then
    Printf.bprintf b "The actions to process have cyclic dependencies:\n%a"
      pr_items cycles;
  if chains <> [] then
    Printf.bprintf b "The following dependencies couldn't be met:\n%a" pr_items
      chains;
  if final <> [] then
...
let remove universe name constr =
  let filter p =
    p.Cudf.package <> name || not (Cudf.version_matches p.Cudf.version constr)
  in
...
let uninstall_all universe =
  ...
  let packages =
    List.rev_map (fun p -> { p with Cudf.installed = false }) packages
  in
  Cudf.load_universe packages

let install universe package =
  let p =
    Cudf.lookup_package universe (package.Cudf.package, package.Cudf.version)
  in
  let p = { p with Cudf.installed = true } in
  let packages =
    let filter p =
      p.Cudf.package <> package.Cudf.package
      || p.Cudf.version <> package.Cudf.version
    in
...
let remove_all_uninstalled_versions_but universe name constr =
  let filter p =
    p.Cudf.installed || p.Cudf.package <> name
    || Cudf.version_matches p.Cudf.version constr
  in
  let packages = Cudf.get_packages ~filter universe in
  Cudf.load_universe packages
...
let dump_universe oc univ =
  Cudf_printer.pp_cudf oc (default_preamble, univ, Cudf.default_request)

let dump_cudf_request ~version_map ((_, univ, _) as cudf) criteria = function
  | None -> None
  | Some f ->
      ...
      OpamPackage.Map.iter
        (fun (pkg : OpamPackage.t) (vnum : int) ->
...
let dump_cudf_error ~version_map univ req =
  let cudf_file =
    ...
    | None ->
        ...
        / ("solver-error-" ^ string_of_int (OpamStubs.getpid ()))
  in
  match
...
let call_external_solver ~version_map univ req =
  ...
  if Cudf.universe_size univ > 0 then (
    ...
    ignore
      (dump_cudf_request ~version_map cudf_request criteria
         OpamSolverConfig.(!r.cudf_file));
    try
      let r =
        ...
        let msg =
          Printf.sprintf
            "Sorry, resolution of the request timed out.\n\
             ...
             increase the allowed time by setting OPAMSOLVERTIMEOUT to a \
             bigger value (currently, it is set to %.1f seconds)."
    ...
    | Failure msg ->
        let msg =
          Printf.sprintf
            "Solver failure: %s\n\
             This may be due to bad settings (solver or solver criteria) or a \
             broken solver solver installation. Check $OPAMROOT/config, and \
             the --solver and --criteria options."
            msg
        in
        raise (Solver_failure msg)
    | e ->
        OpamStd.Exn.fatal e;
        let msg = Printf.sprintf "Solver failed: %s" (Printexc.to_string e) in
        raise (Solver_failure msg) )
  else Algo.Depsolver.Sat (None, Cudf.load_universe [])
...
let check_request ?(explain = true) ~version_map univ req =
  ...
  | Algo.Depsolver.Error msg ->
      ...
      let msg =
        Printf.sprintf "Internal solver failed with %s Request saved to %S" msg
          f
      in
      raise (Solver_failure msg)
  | Algo.Depsolver.Unsat _ ->
      (* normally when [explain] = false *)
...
let get_final_universe ~version_map univ req =
  let fail msg =
    let f = dump_cudf_error ~version_map univ req in
    let msg =
      Printf.sprintf "External solver failed with %s Request saved to %S" msg f
    in
    raise (Solver_failure msg)
  in
...
let diff univ sol =
  ...
  in
  let after =
    Set.of_list (Cudf.get_packages ~filter:(fun p -> p.Cudf.installed) sol)
  in
  let open Set.Op in
...
let find_cycles g =
  ...
  let roots =
    fold_vertex (fun v acc -> if in_degree g v = 0 then v :: acc else acc) g []
  in
  let roots =
    if roots = [] then fold_vertex (fun v acc -> v :: acc) g [] else roots
  in
  ...
  let rec follow v path =
    ...
    | None ->
        if Hashtbl.mem seen v then []
          ...
          let path = v :: path in
          Hashtbl.add seen v ();
          List.fold_left (fun acc s -> follow s path @ acc) [] (succ g v)
  in
  List.fold_left (fun cycles root -> follow root [] @ cycles) [] roots
...
let compute_root_causes g requested reinstall =
        ...
        let propagate causes actions direction =
          List.fold_left
            (fun causes act ->
              ...
              if Set.mem p seen then causes
              else
                let cause = direct_cause act direction action in
                if cause = Unknown then causes
                else
                  try
                    Map.add p
                      (merge_causes (cause, depth) (Map.find p causes))
                      causes
                  with Not_found ->
  ...
  let make_roots causes base_cause f =
    ActionGraph.fold_vertex
      (fun act acc ->
        if Map.mem (action_contents act) causes then acc
        else if f act then Map.add (action_contents act) (base_cause, 0) acc
        else acc)
      g Map.empty
  in
  ...
  let causes =
    let roots =
      if Map.is_empty requested_actions then
        (* Assume a global upgrade *)
        make_roots causes Requested (function
          | `Change (`Up, _, _) -> true
          | _ -> false)
      else Map.map (fun _ -> (Requested, 0)) requested_actions
    in
    get_causes causes roots
  in
  let causes =
    ...
    let roots =
      make_roots causes Unknown (function
        | `Change _ as act ->
            List.for_all
...
let atomic_actions ~simple_universe ~complete_universe root_actions =
  log "graph_of_actions root_actions=%a" (slog string_of_actions) root_actions;

  let to_remove, to_install =
    List.fold_left
      (fun (rm, inst) a ->
        match a with
        | `Change (_, p1, p2) -> (Set.add p1 rm, Set.add p2 inst)
        | `Install p -> (rm, Set.add p inst)
        | `Reinstall p -> (Set.add p rm, Set.add p inst)
        | `Remove p -> (Set.add p rm, inst))
      (Set.empty, Set.empty) root_actions
  in
  ...
  (* uninstall order *)
  Graph.iter_edges
    (fun p1 p2 -> ActionGraph.add_edge g (`Remove p1) (`Remove p2))
    (pkggraph to_remove);
  (* install order *)
  Graph.iter_edges
    (fun p1 p2 ->
      if Set.mem p1 to_install then
        let cause = if Set.mem p2 to_install then `Install p2 else `Remove p2 in
  ...
  in
  Algo.Defaultgraphs.PackageGraph.UG.iter_edges
    (fun p1 p2 ->
    ...
    conflicts_graph;
  (* check for cycles *)
  match find_cycles g with [] -> g | cycles -> raise (Cyclic_actions cycles)
