...
let call_external_solver command ~criteria ?timeout ((_, universe, _) as cudf) =
  let solver_in = OpamFilename.of_string (OpamSystem.temp_file "solver-in") in
  let solver_out = OpamFilename.of_string (OpamSystem.temp_file "solver-out") in
  try
    let _ =
    ...
    let () =
      let cmd =
        OpamFilter.single_command
          (fun v ->
            if not (OpamVariable.Full.is_global v) then None
            else
          ...
          command
      in
      OpamSystem.command ~verbose:OpamCoreConfig.(abs !r.debug_level >= 2) cmd
    in
    ...
    if not (OpamFilename.exists solver_out) then
      ...
      with End_of_file ->
        close_in ic;
        false
    then raise Common.CudfSolver.Unsat
    else
      let r =
        Cudf_parser.load_solution_from_file
          (OpamFilename.to_string solver_out)
          universe
      in
      ...
      | Some cmd -> (
          try
            ...
            | s :: _ -> (
                ...
                | "aspcud" :: _ :: v :: _
                  when OpamVersionCompare.compare v "1.9" >= 0 ->
  ...
  let default_criteria =
    {
      ...
      crit_upgrade =
        "-count(down),-count(removed),-sum(solution,version-lag),-count(new)";
      crit_fixup =
        "-count(changed),-notuptodate(solution),-sum(solution,version-lag)";
...
module Mccs_def = struct
  ...
  let default_criteria =
    {
      crit_default =
        "-removed,-count[version-lag:,true],-changed,-count[version-lag:,false],-new";
      crit_upgrade = "-removed,-count[version-lag:,false],-new";
  ...
  | [ (CIdent name, _) ] | [ (CString name, _) ] -> (
      ...
      with Not_found ->
        OpamConsole.error_and_exit `Configuration_error
          "No installed solver matching the selected '%s' found" name )
      ...
      let criteria =
        try
          let corresponding_module =
            List.find
              (fun (module S : S) ->
                S.command_name = Some (Filename.basename name)
                && S.is_present ())
              default_solver_selection
          in
...
let solver_of_string s =
  ...
  custom_solver (List.map (fun a -> (OpamTypes.CString a, None)) args)
  ...
  with Not_found ->
    OpamConsole.error_and_exit `Configuration_error
      ...
      ( if has_builtin_solver () then
        ...
        "This opam has been compiled without a built-in solver, so you need to \
         install and configure an external one. See \
