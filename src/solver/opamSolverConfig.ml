...
let default =
  let solver =
    lazy (OpamCudfSolver.get_solver OpamCudfSolver.default_solver_selection)
  in
  {
    cudf_file = None;
    solver;
...
let setk k t ?cudf_file ?solver ?best_effort ?solver_preferences_default
    ?solver_preferences_upgrade ?solver_preferences_fixup
    ?solver_preferences_best_effort_prefix ?solver_timeout =
...
let with_auto_criteria config =
  ...
  set config
    ~solver_preferences_default:
      ( lazy
        ( match config.solver_preferences_default with
        | (lazy None) -> Some (Lazy.force criteria).OpamCudfSolver.crit_default
        | (lazy some) -> some ) )
    ~solver_preferences_upgrade:
      ( lazy
        ( match config.solver_preferences_upgrade with
        | (lazy None) -> Some (Lazy.force criteria).OpamCudfSolver.crit_upgrade
        | (lazy some) -> some ) )
    ~solver_preferences_fixup:
      ( lazy
        ( match config.solver_preferences_fixup with
        | (lazy None) -> Some (Lazy.force criteria).OpamCudfSolver.crit_fixup
        | (lazy some) -> some ) )
    ~solver_preferences_best_effort_prefix:
      ( lazy
        ( match config.solver_preferences_best_effort_prefix with
        | (lazy None) ->
...
let initk k =
  ...
  in
  let best_effort = env_bool "BESTEFFORT" in
  let criteria = env_string "CRITERIA" >>| fun c -> lazy (Some c) in
  let upgrade_criteria =
    (env_string "UPGRADECRITERIA" >>| fun c -> lazy (Some c)) ++ criteria
  in
  let fixup_criteria = env_string "FIXUPCRITERIA" >>| fun c -> lazy (Some c) in
  let best_effort_prefix_criteria =
    env_string "BESTEFFORTPREFIXCRITERIA" >>| fun c -> lazy (Some c)
  in
  let solver_timeout =
    env_float "SOLVERTIMEOUT" >>| fun f -> if f <= 0. then None else Some f
  in
  setk
    (setk (fun c ->
         r := with_auto_criteria c;
         k))
    !r ~cudf_file:(env_string "CUDFFILE") ~solver ?best_effort
...
let best_effort =
  let r =
    lazy
      ...
      &&
      let crit =
        match Lazy.force !r.solver_preferences_default with
        | Some c -> c
      ...
      ||
      ( OpamConsole.warning
          "Your solver configuration does not support --best-effort, the \
           option was ignored (you need to specify variable \
           OPAMBESTEFFORTCRITERIA, or set your criteria to maximise the count \
           for cudf attribute 'opam-query')";
        false ) )
  in
  fun () -> Lazy.force r
...
let call_solver ~criteria cudf =
  ...
  OpamConsole.log "SOLVER" "Calling solver %s with criteria %s"
    (OpamCudfSolver.get_name (module S))
    criteria;
