...
let symbol_of_action =
  ...
  | `Remove _ ->
      utf8_symbol Symbols.circled_division_slash
        ~alternates:[ Symbols.greek_small_letter_lambda ]
        "X"
  | `Install _ ->
      utf8_symbol Symbols.asterisk_operator
        ...
        "*"
  | `Change (`Up, _, _) ->
      utf8_symbol Symbols.north_east_arrow ~alternates:[ Symbols.upwards_arrow ]
        "U"
  | `Change (`Down, _, _) ->
      utf8_symbol Symbols.south_east_arrow
        ~alternates:[ Symbols.downwards_arrow ]
        "D"
  | `Reinstall _ ->
  ...
  | `Fetch _ ->
      utf8_symbol Symbols.downwards_black_arrow
        ~alternates:
          [
            Symbols.downwards_double_arrow; Symbols.black_down_pointing_triangle;
          ]
        "F"
...
module MakeAction (P : GenericPackage) : ACTION with type package = P.t = struct
  module Pkg = P
  ...
  let to_string a =
    ...
    | `Change (_, p0, p) ->
        Printf.sprintf "%s.%s %s %s" (P.name_to_string p0)
          (P.version_to_string p0) (action_strings a) (P.version_to_string p)

  let to_aligned_strings ?(append = fun _ -> "") l =
    List.map
      (fun a ->
        let a = (a :> package action) in
        ( if OpamConsole.utf8 () then action_color a (symbol_of_action a)
        else "-" )
        :: name_of_action a
  ...
  let of_json =
    ...
    | `O [ ("change", `A [ dj; oj; pj ]) ] ->
        ...
        P.of_json oj >>= fun o ->
        P.of_json pj >>= fun p -> Some (`Change (d, o, p))
...
module Make (A : ACTION) : SIG with type package = A.package = struct
  ...
  let reduce g =
    ...
    let removals =
      fold_vertex
        (fun v acc ->
          match v with
          | `Remove p -> OpamStd.String.Map.add (A.Pkg.name_to_string p) p acc
          | _ -> acc)
    ...
    iter_vertex
      (function
        | `Build p as build -> (
            match
              fold_succ
                (fun v _ -> if v = `Install p then Some v else None)
                g build None
            with
            | None -> ()
            | Some inst ->
      ...
      map_vertex
        (function
          | `Install p as act -> (
              try
                let p0 =
                  OpamStd.String.Map.find (A.Pkg.name_to_string p) removals
                in
                let act =
  ...
  let compute_closed_predecessors noop_remove g =
        ...
        (fun a acc ->
          ...
          | `Build p ->
              let pred =
                ...
                List.filter
                  (function `Remove nv -> not (noop_remove nv) | _ -> true)
                  (pred closed_g a)
              in
        ...
        (fun a acc ->
          ...
          | `Install p | `Reinstall p | `Change (_, _, p) ->
              let preds =
                List.filter
                  (function
                    | `Build p -> Set.mem p closed_packages | _ -> false)
                  (pred closed_g a)
              in
    ...
    | p -> (
        match
          OpamStd.String.Map.find_opt (A.Pkg.name_to_string p)
  ...
  let explicit ?(noop_remove = fun _ -> false) ~sources_needed g0 =
    ...
    iter_vertex
      (function
        | `Remove p as a ->
            List.iter (fun b -> add_edge g b a) (closed_predecessors p)
    ...
    let acc_add_action (acc : vertex list Map.t) (p : A.package) (a : vertex) :
        vertex list Map.t =
  ...
  let fold_descendants f acc t v =
    let rec aux seen f acc t v =
      if A.Set.mem v seen then (seen, acc)
      else
        fold_succ
          (fun v (seen, acc) -> aux seen f acc t v)
          t v
          (A.Set.add v seen, f v acc)
    in
