...
let is_installed universe (name, _) =
  OpamPackage.Set.exists
    (fun pkg -> OpamPackage.name pkg = name)
...
let solution_to_json solution = OpamCudf.ActionGraph.to_json solution

let solution_of_json json = OpamCudf.ActionGraph.of_json json
      ...
      (fun _ deps acc ->
        List.fold_left
          (fun acc -> function
            ...
            | _, None -> acc)
          acc
          (OpamFormula.atoms (filt deps)))
      refmap acc
  in
  let filt f = OpamFilter.filter_deps ~build:true ~post:true ~default:false f in
  let id x = x in
...
let name_to_cudf name = Common.CudfAdd.encode (OpamPackage.Name.to_string name)
...
let constraint_to_cudf version_map name (op, v) =
  ...
  with Not_found -> (
    (* The version for comparison doesn't exist: match to the closest
       ...
       constraints) *)
    let all_versions =
      OpamPackage.Map.filter (fun nv _ -> nv.name = name) version_map
    in
    match op with
    ...
    | (`Geq | `Gt | `Leq | `Lt) as op ->
        ...
        let rev_version_map =
          OpamPackage.Map.fold
            (fun nv cv acc -> OpamStd.IntMap.add (sign cv) nv.version acc)
            all_versions OpamStd.IntMap.empty
        in
        let map =
...
let opam2cudf universe version_map packages =
  let set_to_bool_map set =
    OpamPackage.Set.fold
      (fun nv -> OpamPackage.Map.add nv true)
            ...
            pkg_extra =
              [
                ...
                ( OpamCudf.s_source_number,
                  `String (OpamPackage.version_to_string nv) );
              ];
          })
  ...
  let keep_map =
    OpamPackage.Set.fold
      (fun nv acc ->
        if OpamPackage.Set.mem nv universe.u_available then
          OpamPackage.Map.add nv `Keep_version acc
        else if OpamPackage.has_name universe.u_available nv.name then
          OpamPackage.Map.add nv `Keep_package acc
        else acc)
      (packages %% universe.u_base)
  ...
  let version_lag_map =
    OpamPackage.Name.Map.fold
      (fun name version_set acc ->
        let nvers, vs =
          OpamPackage.Version.Set.fold
            (fun v (i, acc) -> (i + 1, OpamPackage.Version.Map.add v i acc))
            version_set
            (0, OpamPackage.Version.Map.empty)
        in
        OpamPackage.Version.Map.fold
          (fun v i ->
            let lag = lag_function (nvers - i - 1) in
            if lag > 0 then OpamPackage.Map.add (OpamPackage.create name v) lag
            else fun acc -> acc)
          vs acc)
  ...
  let extras_maps =
    List.map
      (fun (label, set) ->
        OpamPackage.Set.fold
          (fun nv -> OpamPackage.Map.add nv (label, `Int 1))
          (packages %% set) OpamPackage.Map.empty)
      universe.u_attrs
  in
  let add elts f map =
  ...
  let univ0 =
    ...
    |> add pinned_to_current_version_map (fun _ x cp ->
           {
             cp with
             Cudf.pkg_extra = (OpamCudf.s_pinned, `Bool x) :: cp.Cudf.pkg_extra;
           })
    ...
    |> List.fold_right
         (fun m ->
           add m (fun _ x cp ->
               { cp with Cudf.pkg_extra = x :: cp.Cudf.pkg_extra }))
         extras_maps
  in
  ...
  in
  let depends_map =
    OpamPackage.Map.map preresolve_deps (only_packages universe.u_depends)
  in
  let depopts_map =
    OpamPackage.Map.map preresolve_deps (only_packages universe.u_depopts)
  in
  let conflicts_map =
  ...
  let conflicts_map_resolved =
    OpamPackage.Map.map
      (List.rev_map (atom2cudf universe version_map))
      conflicts_map
  in
  fun ~depopts ~build ~post ->
    let all_depends_map =
      if depopts then
        OpamPackage.Map.union
          (fun d dopts -> OpamFormula.(ands [ d; dopts ]))
          ...
          |> OpamFormula.map (fun (name, (filter, cstr)) ->
                 if
                   OpamFilter.eval_to_bool ~default:false
                     (OpamFilter.deps_var_env ~build ~post)
                     filter
                 then Atom (name, cstr)
                 else Empty)
    ...
    |> add conflicts_map_resolved (fun _ conflicts cp ->
           { cp with Cudf.conflicts })
...
let load_cudf_universe opam_universe ?version_map opam_packages =
  ...
  let version_map =
    ...
    | Some vm -> vm
    | None -> cudf_versions_map opam_universe opam_packages
  in
  ...
  in
  let univ_gen = opam2cudf opam_universe version_map opam_packages in
  ...
  fun ?(depopts = false) ~build ~post () ->
    log "Load cudf universe (depopts:%a, build:%b, post:%b)"
      (slog string_of_bool) depopts build post;
    let chrono = OpamConsole.timer () in
    let cudf_universe =
...
let string_of_request r =
  let to_string =
    OpamFormula.string_of_conjunction OpamFormula.string_of_atom
  in
  Printf.sprintf "install:%s remove:%s upgrade:%s" (to_string r.wish_install)
    (to_string r.wish_remove) (to_string r.wish_upgrade)
...
let cudf_to_opam_graph cudf2opam cudf_graph =
  ...
  OpamCudf.ActionGraph.iter_vertex
    ...
    cudf_graph;
  OpamCudf.ActionGraph.iter_edges
    (fun p1 p2 ->
      ActionGraph.add_edge opam_graph (map_action cudf2opam p1)
...
let cleanup_request universe (req : atom request) =
  if OpamSolverConfig.best_effort () then
    ...
    let wish_install =
      List.filter
        (fun (n, _) -> not (List.mem_assoc n req.wish_upgrade))
        req.wish_install
    in
    let wish_upgrade =
      List.rev_map
        (fun ((n, c) as pkg) ->
          if
            c = None && is_installed universe pkg
            && is_available universe req.wish_remove pkg
          then (n, Some (`Geq, find_installed universe pkg))
          else pkg)
        req.wish_upgrade
    in
    { req with wish_install; wish_upgrade }
...
let cycle_conflict ~version_map univ cycles =
  OpamCudf.cycle_conflict ~version_map univ
    (List.map
       (List.map (fun a -> Action.to_string (map_action OpamCudf.cudf2opam a)))
       cycles)
...
let resolve universe ~orphans request =
  ...
  | Success actions -> (
      ...
      let complete_universe =
        univ_gen ~depopts:true ~build:true ~post:false ()
      in
      try
        let atomic_actions =
...
let get_atomic_action_graph t = cudf_to_opam_graph OpamCudf.cudf2opam t
...
let installable universe =
  ...
  Cudf.fold_packages
    ...
    OpamPackage.Set.empty trimmed_universe
...
let dependency_graph ~depopts ~build ~post ~installed ?(unavailable = false)
    universe =
  let u_packages =
  ...
  OpamCudf.Graph.iter_vertex
    (fun v -> PkgGraph.add_vertex g (OpamCudf.cudf2opam v))
    cudf_graph;
  OpamCudf.Graph.iter_edges
    (fun v1 v2 ->
      PkgGraph.add_edge g (OpamCudf.cudf2opam v1) (OpamCudf.cudf2opam v2))
    cudf_graph;
  g

let filter_dependencies f_direction ~depopts ~build ~post ~installed
  ...
  if OpamPackage.Set.is_empty packages then []
    ...
    let cudf_universe =
      load_cudf_universe ~depopts ~build ~post universe ~version_map u_packages
        ()
    in
    let cudf_packages =
    ...
    log "filter_dependencies result=%a"
      (slog (OpamStd.List.to_string OpamPackage.to_string))
      result;
    result
...
let coinstallability_check universe packages =
  ...
  let cudf_universe =
    load_cudf_universe ~build:true ~post:true ~version_map universe
      universe.u_packages ()
  in
  let cudf_packages =
    opam2cudf universe ~depopts:false ~build:true ~post:true version_map
      packages
  in
  match Algo.Depsolver.edos_coinstall cudf_universe cudf_packages with
  | { Algo.Diagnostic.result = Algo.Diagnostic.Success _; _ } -> None
...
let new_packages sol =
  OpamCudf.ActionGraph.fold_vertex
    (fun action packages ->
      match action with
...
let stats sol =
  OpamCudf.ActionGraph.fold_vertex
    (fun action stats ->
      ...
      | `Change (`Down, _, _) ->
          { stats with s_downgrade = stats.s_downgrade + 1 }
    ...
    OpamStd.List.concat_map " | "
      (fun (n, t) ->
        Printf.sprintf "%s to %s"
          (OpamConsole.colorise `yellow (string_of_int n))
          t)
      msgs

let solution_is_empty t = OpamCudf.ActionGraph.is_empty t
...
let print_solution ~messages ~append ~requested ~reinstall t =
  let dump_cudf sfx t =
    ...
    | Some f ->
        ...
        ActionGraph.Dot.output_graph oc
  ...
  dump_cudf "" t;
  let causes = OpamCudf.compute_root_causes t requested reinstall in
  let actions, details =
    OpamCudf.ActionGraph.Topological.fold
      (fun a (actions, details) ->
        let cause =
  ...
  |> List.map2
       (fun (cause, messages) line ->
         (" " :: line)
         @ [ (if cause = "" then "" else Printf.sprintf "[%s]" cause) ]
         @ if messages = [] then [] else [ String.concat "\n" messages ])
       details
...
let request ?(criteria = `Default) ?(install = []) ?(upgrade = [])
    ?(remove = []) () =
  {
    wish_install = install;
    wish_upgrade = upgrade;
    wish_remove = remove;
    criteria;
    extra_attributes = [];
  }
