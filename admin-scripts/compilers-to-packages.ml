  ...
  (fun c comp_file ->
    ...
    let comp =
      ...
      OpamFile.Comp.with_packages
        (OpamFormula.map
           (fun ((name, _) as atom) ->
             if List.mem name drop_names then OpamFormula.Empty else Atom atom)
           (OpamFile.Comp.packages comp))
        comp
    in
    let opam =
      OpamFile.Comp.to_package (OpamPackage.Name.of_string "ocaml") comp descr
    in
    ...
    if List.mem None extra_sources then ()
      ...
      OpamFile.Dot_config.write
        (OpamFile.make
           OpamFilename.Op.(
             OpamRepositoryPath.files repo (Some "ocaml") nv
             // "ocaml.config.in"))
        config;
