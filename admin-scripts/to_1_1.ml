...
let filter_string =
  ...
  Re_pcre.substitute ~rex ~subst:(fun s ->
      ...
      | s when String.contains s '?' -> (
          try
            (* new if/else printers: use default *)
...
let rec filter_vars = function
  ...
  | FNot f -> (
      match filter_vars f with Some f -> Some (FNot f) | None -> None )
  | FUndef -> None

let filter_vars_optlist ol =
  List.map (fun (x, filter) -> (x, filter >>= filter_vars)) ol
...
let filter_vars_commands ol =
  List.map
    (fun (args, filter) ->
      (filter_args (filter_vars_optlist args), filter >>= filter_vars))
    ol

let to_1_1 _ opam =
  let module OF = OpamFile.OPAM in
  if
    OpamVersion.compare (OF.opam_version opam) (OpamVersion.of_string "1.2") < 0
  then opam
  else
    let opam =
      OF.with_build opam
        (filter_vars_commands (OF.build opam @ OF.install opam))
    in
    ...
    in
    let opam =
      OF.with_available opam (filter_vars (OF.available opam) +! FBool true)
    in
    ...
    let opam =
      OF.with_post_messages opam (filter_vars_optlist (OF.post_messages opam))
    in
    opam
