    ...
    let depopts =
      ...
      let atoms =
        OpamFormula.fold_left
          ...
          [] formula
      in
      OpamFormula.ors @@ OpamStd.List.remove_duplicates @@ List.rev atoms
    in
    let conflicts =
      (* add complement of the depopts as conflicts *)
      let module NM = OpamPackage.Name.Map in
      let depopts =
        (* get back a map (name => version_constraint) *)
      ...
      let neg_depopts =
        NM.fold
          (fun name f acc ->
            if f = OpamFormula.Empty then acc
            else
              ...
              | cnf ->
                 ...
                 but to enumerate *)
                  let f =
                    ...
                    @@ List.map OpamFormula.of_disjunction cnf
                  in
                  let conflict_packages =
                    OpamPackage.Set.filter
                      (fun pkg ->
                        OpamFormula.eval
                          (fun atom -> OpamFormula.check atom pkg)
                          f)
      ...
      let add_conflicts =
        ...
        List.filter (fun f -> not (List.mem f c)) neg_depopts
      in
