...
let rec couverture acc t pkgs =
  ...
  if P.Name.Set.is_empty added then
    ...
    let pkgs =
      P.Set.filter (fun nv -> not (P.has_name step (P.name nv))) pkgs
    in
...
let () =
        ...
        List.fold_left
          (fun wanted name ->
            let nvs =
              ...
              else P.packages_of_name avail (P.Name.of_string name)
            in
  ...
  List.iter
    (fun s ->
      print_endline
        (OpamStd.List.concat_map " " OpamPackage.to_string (P.Set.elements s)))
    couv;
